...
export type FundingStreamRevenueAccount = {
    id: number;
    code: string;
    name: string;
};
...
const funderTypeLabel = (v: string) =>
    FUNDER_TYPES.find((t) => t.value === v)?.label ?? v;
...
export function FundingStreamDialog({
        ...
        fundingStream
            ? {
                  ...
                  default_revenue_account_id:
                      fundingStream.default_revenue_account_id != null
    ...
    const revenueLabel =
        revenueOptions.find((a) => a.value === data.default_revenue_account_id)
            ?.label ?? 'None';
    ...
    const submit = () => {
        form.transform((d) => ({
            ...
            default_revenue_account_id:
                d.default_revenue_account_id === 'none'
                    ? null
                    : d.default_revenue_account_id,
    ...
    return (
        <WizardShell
            ...
            description={
                isEdit
                    ? 'Update this funding stream'
                    : 'Add a funding stream to track revenue sources'
            }
            railIcon={Sprout}
            ...
            success={
                succeeded ? (
                    <WizardSuccessPane
                        title={
                            isEdit
                                ? 'Funding stream updated'
                                : `${data.name || 'Funding stream'} created`
                        }
                        blurb={
                            isEdit
        ...
        >
            {index === 0 && (
                <div>
                    <StepHead
                        icon={Sprout}
                        title="Funding stream details"
                        blurb="Identify the revenue source and its funder."
                    />
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <Field
                            ...
                            span
                            hint="optional"
                            error={errors.default_revenue_account_id}
                        >
                            <SelectInput
                                value={data.default_revenue_account_id}
                                onChange={(v) =>
                                    setData('default_revenue_account_id', v)
                                }
                                placeholder="None"
                        ...
                        <div className="flex items-center justify-between gap-3 sm:col-span-2">
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground">
                                    Inactive streams are hidden from allocations
                                </p>
                            </div>
                            <Switch
            ...
            {index === 1 && (
                <div>
                    ...
                    <ReviewCard icon={Sprout} title="Funding stream">
                        ...
                        <ReviewRow
                            label="Funder type"
                            value={funderTypeLabel(data.funder_type)}
                        />
                        {data.contact_name && (
                            <ReviewRow
                                label="Contact"
