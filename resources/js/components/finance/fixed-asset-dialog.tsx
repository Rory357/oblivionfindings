...
const STEPS: readonly WizardStep[] = [
    {
        key: 'details',
        label: 'Details',
        blurb: 'Name, category & cost',
        icon: FileText,
    },
    {
        key: 'depreciation',
        label: 'Depreciation & GL',
        blurb: 'Method & accounts',
        icon: Calculator,
    },
    {
        key: 'review',
...
export function FixedAssetDialog({
    ...
    const form = useForm<{
        ...
        asset
            ? {
                  ...
                  gl_asset_account_id:
                      asset.gl_asset_account_id != null
                          ? String(asset.gl_asset_account_id)
                          : '',
                  gl_depreciation_account_id:
                      asset.gl_depreciation_account_id != null
                          ? String(asset.gl_depreciation_account_id)
                          : '',
                  gl_expense_account_id:
                      asset.gl_expense_account_id != null
                          ? String(asset.gl_expense_account_id)
                          : '',
                  notes: asset.notes ?? '',
              }
            : {
    ...
    const assetOptions = assetAccounts.map((a) => ({
        value: String(a.id),
        label: `${a.code} · ${a.name}`,
    }));
    ...
    const assetAccountLabel =
        assetOptions.find((a) => a.value === data.gl_asset_account_id)?.label ??
        '—';
    ...
    const acquisitionLines: PostingLine[] = useMemo(() => {
        ...
        const acct = assetAccounts.find(
            (a) => String(a.id) === data.gl_asset_account_id,
        );
        return [
            {
    ...
    return (
        <WizardShell
            ...
            description={
                isEdit
                    ? 'Update this fixed asset'
                    : 'Register a fixed asset in the register'
            }
            railIcon={Boxes}
        ...
        >
            {index === 0 && (
                <div>
                    <StepHead
                        icon={FileText}
                        title="Asset details"
                        blurb="What the asset is and what it cost."
                    />
                    {generalError && (
                        <div className="mb-4">
                    ...
                    {locked && (
                        <div className="mb-4">
                            <InfoCard icon={AlertTriangle} tone="warn">
                                Depreciation has been recorded for this asset,
                                so purchase cost and purchase date are locked.
                            </InfoCard>
                        </div>
                    )}
            ...
            {index === 1 && (
                <div>
                    <StepHead
                        icon={Calculator}
                        title="Depreciation & GL accounts"
                        blurb="How it depreciates, and where it posts in the ledger."
                    />
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <Field
                            ...
                            monthlyDepreciation > 0 && (
                                <div className="sm:col-span-2">
                                    <InfoCard icon={Calculator}>
                                        ...
                                        {data.depreciation_method ===
                                            'diminishing_value' &&
                                            ' (first month — diminishing value tapers over time)'}
                                    </InfoCard>
                                </div>
                            )}
                        <Field
                            ...
                            span
                            hint="posts an acquisition journal on create"
                            error={errors.gl_asset_account_id}
                        >
                            <SelectInput
                                value={data.gl_asset_account_id}
                                onChange={(v) =>
                                    setData('gl_asset_account_id', v)
                                }
                                placeholder="None"
                        ...
                        <Field
                            ...
                            error={errors.gl_depreciation_account_id}
                        >
                            <SelectInput
                                value={data.gl_depreciation_account_id}
                                onChange={(v) =>
                                    setData('gl_depreciation_account_id', v)
                                }
                        ...
                        <Field
                            ...
                            hint="defaults to 8000"
                            error={errors.gl_expense_account_id}
                        >
                            <SelectInput
                                value={data.gl_expense_account_id}
                                onChange={(v) =>
                                    setData('gl_expense_account_id', v)
                                }
            ...
            {index === 2 && (
                <div>
                    <StepHead
                        ...
                        blurb={
                            isEdit
                                ? 'Updates this fixed asset.'
                                : 'Registers the asset — an acquisition journal posts if a GL asset account is set.'
                        }
                    />
                    <ReviewCard icon={FileText} title="Fixed asset">
                        ...
                        />
                        {data.asset_tag && (
                            <ReviewRow label="Tag" value={data.asset_tag} />
                        )}
                        <ReviewRow
                            label="Category"
                        ...
                        <ReviewRow
                            label="Purchase cost"
                            value={formatMoney(data.purchase_cost)}
                        />
                        {data.residual_value !== '' && (
                            <ReviewRow
                                label="Residual value"
                                value={formatMoney(data.residual_value)}
                            />
                        )}
                        <ReviewRow
                            label="Depreciation"
                            value={`${methodLabel(data.depreciation_method)} · ${data.useful_life_months || '—'} months`}
                        />
                        <ReviewRow
                            label="Asset account"
                            value={assetAccountLabel}
                        />
                    </ReviewCard>
                    {acquisitionLines.length > 0 && (
                        <div className="mt-4">
                            <PostingPreview
                                ...
                                title="Acquisition journal preview"
                            />
                        </div>
                    )}
                    {processing && (
