...
const SECTIONS: {
    ...
    {
        key: 'include_journals',
        label: 'Journals & Journal Lines',
        description: 'All general ledger journals with line-level detail',
        short: 'Journals',
    },
    {
        key: 'include_bank_reconciliations',
        label: 'Bank Reconciliations',
        description: 'Bank statement reconciliation records',
        short: 'Bank Recon',
    },
    {
        key: 'include_ap',
        label: 'Accounts Payable',
        description: 'Bills, payment runs, and vendor transactions',
        short: 'AP',
    },
    {
        key: 'include_ar',
        label: 'Accounts Receivable',
        description: 'Payment allocations and receivable transactions',
        short: 'AR',
    },
    {
        key: 'include_gst',
        label: 'GST Returns',
        description: 'GST/tax return filings and calculations',
        short: 'GST',
    },
    {
        key: 'include_fixed_assets',
...
export function AuditExportDialog({
    ...
    const rangeValid =
        !data.period_from ||
        !data.period_to ||
        data.period_to >= data.period_from;
    const setupValid =
        !!data.export_name.trim() &&
        !!data.period_from &&
        !!data.period_to &&
        rangeValid;
            ...
            {index === 0 && (
                <div>
                    <StepHead
                        icon={History}
                        title="Export setup"
                        blurb="Name the export, choose the period and the data sections."
                    />
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <Field
                            ...
                            span
                            required
                            error={errors.export_name}
                        >
                            <Input
                                value={data.export_name}
                                onChange={(e) =>
                                    setData('export_name', e.target.value)
                                }
                        ...
                        <Field
                            ...
                            span
                            hint="each generates a separate CSV file"
                        >
            ...
            {index === 1 && (
                <div>
                    <StepHead
                        ...
                        blurb="Generation runs in the background — download from the list when ready."
                    />
