...
import { Ban, Check, Clock, Download, Fingerprint, Loader2, Mail, Send, ShieldAlert, Trash2 } from 'lucide-react';
...
const ACTIONS: Record<PrivacyActionKind, ActionConfig> = {
    verify: {
        ...
        fields: [{ type: 'select', name: 'verification_method', label: 'Verification method', required: true, options: ['RealMe verified', 'Drivers licence', 'Passport', 'In person', 'Known to organisation'] }],
    },
    extend: {
        ...
        fields: [
            { type: 'date', name: 'extended_due_date', label: 'New due date', required: true },
            { type: 'textarea', name: 'extension_reason', label: 'Reason for extension', required: true, placeholder: 'Why the statutory deadline is being extended…' },
        ],
    },
    complete: {
        title: 'Mark complete',
        ...
        fields: [{ type: 'textarea', name: 'completion_notes', label: 'Completion notes', placeholder: 'How the request was fulfilled…' }],
    },
    refuse: {
        ...
        fields: [
            { type: 'textarea', name: 'rejection_reason', label: 'Reason', required: true, placeholder: 'Why the request is being refused…' },
            { type: 'text', name: 'rejection_legal_basis', label: 'Legal basis', required: true, placeholder: 'e.g. Privacy Act 2020 s 49 — evaluative material' },
        ],
    },
    export: {
    ...
    'notify-opc': {
        ...
        fields: [{ type: 'text', name: 'authority_reference', label: 'OPC / NotifyUs reference', hint: 'optional', placeholder: 'Reference from NotifyUs' }],
    },
    'notify-subjects': {
        ...
        fields: [{ type: 'select', name: 'notification_method', label: 'Notification method', required: true, options: ['Email', 'Letter', 'Phone', 'In person', 'Public notice'] }],
    },
    resolve: {
        ...
        fields: [{ type: 'textarea', name: 'resolution_notes', label: 'Resolution notes', required: true, placeholder: 'How the breach was contained and resolved…' }],
    },
    release: {
        title: 'Release hold',
        ...
        fields: [{ type: 'textarea', name: 'release_reason', label: 'Release reason', required: true, placeholder: 'Why the hold is no longer required…' }],
    },
    approve: {
        title: 'Approve DPIA',
    ...
    review: {
        ...
        fields: [{ type: 'textarea', name: 'review_notes', label: 'Review notes', required: true, placeholder: 'What needs the Privacy Officer’s attention…' }],
    },
    execute: {
        ...
        fields: [{ type: 'checkbox', name: 'confirm', label: 'I understand this permanently anonymises matching records and cannot be undone.', required: true }],
    },
};
...
export function PrivacyActionModal({
    ...
    const initial: Record<string, unknown> = { ...(action.extraInitial?.(recordId) ?? {}) };
    for (const f of action.fields) initial[f.name] = f.type === 'checkbox' ? false : '';
    ...
    const err = (name: string) => errors[name] ?? (form.errors as Record<string, string>)[name];

    const submit = () => {
        const e: Record<string, string> = {};
        for (const f of action.fields) {
            if (f.required && !isFilled(data[f.name])) e[f.name] = `${f.label} is required`;
        }
        setErrors(e);
    ...
    return (
        <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
            <DialogContent className="max-w-md gap-0 overflow-hidden p-0">
                ...
                <DialogDescription className="sr-only">{action.blurb}</DialogDescription>

                <div className="flex items-start gap-3 border-b border-border p-5">
                    <span className={cn('grid h-11 w-11 shrink-0 place-items-center rounded-xl', critical ? 'bg-status-critical-bg text-status-critical' : 'bg-primary/10 text-primary')}>
                        <Icon className="h-5 w-5" />
                    </span>
                    <div>
                        <h2 className="text-base font-bold">{action.title}</h2>
                        <p className="mt-0.5 text-[13px] leading-relaxed text-muted-foreground">{action.blurb}</p>
                    </div>
                </div>
                ...
                {action.fields.length ? (
                    <div className="flex flex-col gap-4 p-5">
                        {action.fields.map((f) => (
                            <ActionFieldRow key={f.name} field={f} value={data[f.name]} error={err(f.name)} onChange={(v) => setData(f.name, v)} />
                        ))}
                    </div>
                ) : (
                ...
                <div className="flex items-center justify-end gap-2.5 border-t border-border bg-muted/30 px-5 py-3.5">
                    ...
                    <Button type="button" variant={critical ? 'destructive' : 'default'} onClick={submit} disabled={processing}>
                        {processing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Icon className="h-4 w-4" />}
                        {action.confirm}
                    </Button>
                </div>
            </DialogContent>
...
function ActionFieldRow({ field: f, value, error, onChange }: { field: ActionField; value: unknown; error?: string; onChange: (v: unknown) => void }) {
    if (f.type === 'checkbox') {
        return (
            <label className="flex cursor-pointer items-start gap-2.5 rounded-lg border border-status-critical/30 bg-status-critical-bg/40 p-3 text-[13px]">
                <input type="checkbox" checked={Boolean(value)} onChange={(e) => onChange(e.target.checked)} className="mt-0.5 h-4 w-4 shrink-0 rounded border-border" />
    ...
    if (f.type === 'select') {
        return (
            <Field label={f.label} required={f.required} hint={f.hint} error={error}>
                <SelectInput value={String(value ?? '')} onChange={onChange} placeholder="Select…" options={(f.options ?? []).map((o) => ({ value: o, label: o }))} />
            </Field>
        );
    }
    if (f.type === 'textarea') {
    ...
    return (
        <Field label={f.label} required={f.required} hint={f.hint} error={error}>
            <Input type={f.type === 'date' ? 'date' : 'text'} value={String(value ?? '')} onChange={(e) => onChange(e.target.value)} placeholder={f.placeholder} aria-invalid={!!error} />
        </Field>
    );
}
