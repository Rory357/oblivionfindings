...
export function SiteProfileHero({
    ...
    const style = {
        ...
        background: `linear-gradient(135deg, color-mix(in srgb, ${colour} 92%, black), ${colour} 58%, color-mix(in srgb, ${colour} 82%, white))`,
        color: foreground,
    } as CSSProperties;
    ...
    return (
        ...
        >
            <div
                ...
                aria-hidden="true"
            >
                <div className="absolute -top-20 -right-16 h-64 w-64 rounded-full bg-white/8" />
                <div className="absolute -bottom-24 left-1/4 h-52 w-52 rounded-full bg-white/8" />
            </div>
                ...
                <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0">
                        ...
                        <div className="mt-3 flex flex-wrap gap-1.5 text-xs font-medium">
                            <span className="rounded-full border border-current/20 bg-white/10 px-2.5 py-1">
                                {typeLabel}
                            </span>
                            {region ? (
                                <span className="rounded-full border border-current/20 bg-white/10 px-2.5 py-1">
                                ...
                                >
                                    {avatars.map((avatar) => (
                                        ...
                                        >
                                            ...
                                            <AvatarFallback className="bg-white/15 text-xs text-current">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            {actions.length ? (
                                <DropdownMenu>
                                    <DropdownMenuTrigger asChild>
                                        <button
                                            type="button"
                                            className="inline-flex h-9 items-center gap-2 rounded-lg bg-white px-3.5 text-sm font-semibold text-slate-900 shadow-sm"
                                        >
                            ...
                            {onEdit ? (
                                <button
                                    ...
                                    onClick={onEdit}
                                    title="Edit Site"
                                    className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-current/20 bg-white/10"
                                >
                        ...
                        <div className="grid w-full grid-cols-3 gap-2 md:w-[360px]">
                            {stats.map((stat) => {
                                ...
                                return (
                                    <div
                                        key={stat.id}
                                        className="rounded-xl border border-current/20 bg-white/10 px-3 py-2 text-center"
                                    >
