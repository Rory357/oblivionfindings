...
export default function TimesheetsHero({
    ...
    return (
        ...
        <PageHero
                ...
                <span>
                    ...
                    <span className="block">
                        <span className="font-normal text-primary-foreground/80">
                            Kia ora {summary.firstName}, your week of timesheets —{' '}
                        </span>
            ...
            description={
                <span>
                    <span className="font-semibold tabular-nums text-primary-foreground">{summary.unapproved}</span> timesheet
                    {summary.unapproved === 1 ? '' : 's'} need a decision,{' '}
                    <span className="font-semibold tabular-nums text-primary-foreground">{summary.timesheets_returned}</span> have
                    been returned for changes, and{' '}
                    <span className="font-semibold tabular-nums text-primary-foreground">{summary.hours_this_week}</span> of{' '}
                    <span className="tabular-nums">{summary.hours_target}</span> rostered hours have been logged so far. Payroll
                    closes <span className="font-semibold text-primary-foreground">{summary.next_payroll_date}</span>.
                </span>
            }
            meta={[
                { icon: CalendarDays, label: `Week ${summary.week_number} · Mon–Sun` },
                {
                    icon: MapPin,
            ...
            stats={[
                { label: 'Total', value: summary.timesheets_total },
                {
                    ...
                    tone: summary.timesheets_submitted > 0 ? 'warning' : undefined,
                },
                {
                    label: 'Approved',
                ...
                },
                {
                    ...
                    tone: summary.timesheets_returned > 0 ? 'critical' : undefined,
                },
            ]}
            actions={
                <>
            ...
            children={
                <div className="max-w-xl rounded-xl border border-primary-foreground/15 bg-primary-foreground/5 p-3">
                    <div className="mb-1.5 flex items-center justify-between text-[11.5px] text-primary-foreground/80">
                        <span className="font-medium text-primary-foreground">Hours logged vs. rostered</span>
