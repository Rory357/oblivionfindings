 ...
 */
export function GovernanceTimeline({ timeline, defaultLimit = 8 }: GovernanceTimelineProps) {
    const [expanded, setExpanded] = useState(false);
    const events = timeline.events;

    const grouped = useMemo(() => {
        const map = new Map<string, TimelineEvent[]>();
        for (const e of events.slice(0, expanded ? events.length : defaultLimit)) {
        ...
        <Card data-dusk="cockpit-timeline">
            <CardHeader className="pb-3">
                <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                        <CardTitle className="text-lg">Governance Timeline</CardTitle>
                        <CardDescription>
                            {timeline.since
                ...
                {events.length === 0 ? (
                    <div className="rounded-lg border border-dashed border-border p-8 text-center">
                        ...
                        <p className="mt-2 text-sm font-medium text-foreground">
                            Nothing has changed{timeline.since ? ` since ${timeline.since.title}` : ' recently'}.
                        </p>
                        <p className="mt-1 text-xs text-muted-foreground">
                            New governance activity will appear here automatically.
                        </p>
                    </div>
                ) : (
                                    ...
                                    {dayEvents.map((event) => {
                                        ...
                                        return (
                                            <li key={event.id} className="relative">
                                                <span
                                                    className="absolute -left-[1.41rem] top-2 h-2 w-2 rounded-full bg-primary"
                                                    aria-hidden="true"
                                                />
                                                {event.href ? (
                                                    <Link
                                                        href={event.href}
                                                        className="block rounded-md p-1 -m-1 transition hover:bg-muted"
                                                    >
                                                        {Content}
                                                    </Link>
                                                ) : (
                                                    <div className="rounded-md p-1 -m-1">{Content}</div>
                                                )}
                                            </li>
                                        );
                                    })}
                        ...
                        {events.length > defaultLimit ? (
                            ...
                            >
                                {expanded ? (
                                    ...
                                    <>
                                        Show all {events.length} events <ChevronDown className="ml-1 h-4 w-4" />
                                    </>
                                )}
                            </Button>
                        ) : null}
