...
import { ShieldAlert, FileCheck, AlertOctagon, ArrowRight } from 'lucide-react';
...
function StatusBadge({ status }: { status: string }) {
    return (
        <Badge className={cn('border text-[10px] uppercase', STATUS_BADGE[status] ?? STATUS_BADGE.unknown)}>
            {status}
        </Badge>
    );
}
    ...
    return (
        <Card data-dusk="cockpit-risk-watchlist">
            <CardHeader className="pb-3">
                <CardTitle className="text-lg">Risk &amp; Compliance Watchlist</CardTitle>
                <CardDescription>
                    Critical risks, overdue obligations and privacy gaps the board needs to be aware of.
                </CardDescription>
            </CardHeader>
            <CardContent className="grid gap-3 md:grid-cols-2">
                <MiniCard
                    ...
                    icon={ShieldAlert}
                    primaryVerb={canManageRisks ? 'Review risks' : 'Open risk register'}
                />
                <MiniCard
                    card={cardsByKey['compliance_calendar']}
                    icon={FileCheck}
                    primaryVerb={canManageCompliance ? 'Upload evidence' : 'Open obligations'}
                />
                <MiniCard
                ...
                />
                <MiniCard card={cardsByKey['incidents']} icon={AlertOctagon} primaryVerb="Open incidents" />
            </CardContent>
        </Card>
    );
}
