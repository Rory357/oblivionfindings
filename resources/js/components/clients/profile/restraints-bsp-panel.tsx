...
export function RestraintsBspPanel({ clientId }: { clientId: number }) {
        ...
        (async () => {
            try {
                const res = await fetch(
                    `/health-safety/restraints/clients/${clientId}/summary`,
    ...
    if (
        !loaded ||
        !summary ||
        (!summary.active_plan && summary.recent_events.length === 0)
    )
        return null;

    const plan = summary.active_plan;
    const reviewState = plan
        ? (REVIEW_STATE_META[plan.review_state] ?? REVIEW_STATE_META.ok)
        : null;
        ...
        <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
                ...
                <div className="flex items-center gap-2.5">
                    ...
                    <div>
                        ...
                        <div className="text-xs text-muted-foreground">
                            {summary.total_events} restraint event
                            {summary.total_events === 1 ? '' : 's'} on record
                        </div>
                    </div>
                </div>
                <GuardrailButton
                    ...
                    onClick={() =>
                        router.visit(
                            `/health-safety/restraints?client_id=${clientId}`,
                        )
                    }
            ...
            {plan ? (
                <GuardrailButton
                    ...
                    onClick={() =>
                        router.visit(
                            `/health-safety/restraints?lens=plans&plan=${plan.id}`,
                        )
                    }
                ...
                >
                    ...
                    {reviewState && plan.review_state !== 'ok' ? (
                        <span
                            className={`inline-flex shrink-0 items-center rounded-full px-2 py-0.5 text-[11px] font-medium ${CHIP[reviewState.tone]}`}
                        >
                            {reviewState.label}
                        </span>
                    ) : null}
                ...
                <div className="mb-3 flex items-center gap-2.5 rounded-xl border border-dashed border-status-warning/40 bg-status-warning-bg/40 p-3 text-[13px] text-status-warning">
                    ...
                    No active behaviour support plan — restrictive practice
                    should be governed by a current plan.
                </div>
            )}
                    ...
                    {summary.recent_events.map((e) => {
                        ...
                        return (
                            <GuardrailButton
                                ...
                                onClick={() =>
                                    router.visit(
                                        `/health-safety/restraints?event=${e.id}`,
                                    )
                                }
                            ...
                            >
                                ...
                                <div className="flex shrink-0 items-center gap-1">
                                    {e.injury_occurred ? (
                                        <HeartPulse className="h-3.5 w-3.5 text-status-critical" />
                                    ) : null}
                                    {!e.within_support_plan ? (
                                        <AlertTriangle className="h-3.5 w-3.5 text-status-critical" />
                                    ) : null}
                                    <span
                                        className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium ${CHIP[sev.tone as ChipTone]}`}
                                    >
                                        {sev.label}
                                    </span>
                                </div>
