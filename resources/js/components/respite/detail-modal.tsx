...
function buildRows(detail: RespiteDetail): Row[] {
    if (detail.kind === 'referral') {
        ...
        return [
            ...
            ['Age', r.age ?? '—'],
            ['Referrer', r.referrer ? `${r.referrer}${r.referrerType ? ` · ${r.referrerType}` : ''}` : '—'],
            ['Contact', r.contact ?? '—'],
            ['Reason', r.reason ?? '—'],
    ...
    if (detail.kind === 'request') {
        ...
        return [
            ...
            ['Dates', `${fmtRange(r.start, r.end)}${r.nights != null ? ` · ${r.nights} nights` : ''}`],
            ['Funding ref', r.funding ?? '—'],
    ...
    if (detail.kind === 'booking') {
        const b = detail.row;
        return [
            ['Dates', `${fmtRange(b.start, b.end)}${b.nights != null ? ` · ${b.nights} nights` : ''}`],
            ['Home', b.site ?? '—'],
...
export function RespiteDetailModal({ detail, onClose }: { detail: RespiteDetail | null; onClose: () => void }) {
                    ...
                    <>
                        <DialogHeader>
                            <div className="flex items-center gap-3">
                                <Avatar name={row.client} className="h-11 w-11 text-sm" />
                                <div className="min-w-0">
                                    <DialogTitle className="truncate text-left text-lg">{row.client}</DialogTitle>
                        ...
                        <dl className="divide-y divide-border">
                            {rows.map(([k, v], i) => (
                                <div key={i} className="flex justify-between gap-6 py-2 text-sm">
