...
const STEPS: WizardStep[] = [
    {
        key: 'outcome',
        label: 'Outcome',
        blurb: 'Did the dose help?',
        icon: Stethoscope,
    },
    {
        key: 'observations',
        label: 'Observations',
        blurb: 'Resident response',
        icon: FileText,
    },
    {
        key: 'signoff',
        label: 'Escalation & sign-off',
        blurb: 'Confirm the review',
        icon: Check,
    },
];
...
export function PrnEffectivenessDialog({
    ...
    const railSub =
        [
            followUp.medication_name,
            followUp.given_time ? `given ${followUp.given_time}` : null,
            ago,
        ]
            .filter(Boolean)
    ...
    const validate = (i: number): Record<string, string> => {
        ...
        if (i === 2 && escNeeded && !form.data.escalation_action.trim())
            e.escalation_action = 'Describe what was done';
        return e;
    };
    const next = () => {
    ...
    const submit = () => {
        ...
        form.transform(() => ({
            ...
            escalation_action: escNeeded
                ? form.data.escalation_action.trim() || null
                : null,
        }));
        ...
        >
            {step === 0 ? (
                <WizardStepPane>
                    <StepHead
                        icon={Stethoscope}
                        title="Did the dose help?"
                        blurb="Record the outcome and when you reviewed it."
                    />
            ...
            {step === 1 ? (
                <WizardStepPane>
                    <StepHead
                        ...
                        blurb="Resident response and any side effects."
                    />
                    <div className="grid gap-4">
                        ...
                        <InfoCard icon={FileText}>
                            Note the resident&rsquo;s response and any side
                            effects here. Structured side-effect, pain-score and
                            post-dose-vitals fields are a planned enhancement
                            (see the PRN gap analysis).
                        </InfoCard>
                    </div>
                </WizardStepPane>
            ) : null}
            ...
            {step === 2 ? (
                <WizardStepPane>
                    <StepHead
                        ...
                        blurb="Flag if anyone needs to know, then save."
                    />
                    <div className="grid gap-4">
                        <div
                            className={cn(
                        ...
                        >
                            ...
                            <Switch
                                checked={escNeeded}
                                onCheckedChange={setEscNeeded}
                                aria-label="Escalation needed"
                            />
                        </div>
                        {escNeeded ? (
                            <Field
                                label="What was done?"
                                required
                                error={errors.escalation_action}
                            >
                                <Textarea
                                    rows={2}
                        ...
                        <div className="rounded-lg border border-border p-4">
                            <ReviewRow
                                label="Outcome"
                                value={outcome ? OUTCOME_LABEL[outcome] : null}
                            />
                            <ReviewRow
                                label="Reviewed after"
                                value={minutes ? `${minutes} min` : null}
                            />
                            <ReviewRow
                                label="Observations"
                                value={form.data.observations.trim() || null}
                            />
                            <ReviewRow
                                label="Escalation"
                                value={
