...
function StatusPill({ label }: { label: string }) {
    return (
        <span
            className={`rounded-full px-2 py-0.5 text-[11px] font-semibold capitalize ${statusTone(label)}`}
        >
            {label.replace(/_/g, ' ')}
        </span>
    );
...
function rail(s: CdDetailSubject): {
    icon: typeof Lock;
    title: string;
    sub: string;
} {
    switch (s.kind) {
        ...
        case 'entry':
            return {
                icon: Lock,
                title: s.entry.client_name,
                sub: s.entry.medication_name ?? 'CD movement',
            };
        case 'discrepancy':
            return {
                icon: AlertTriangle,
                title: s.disc.client
                    ? `${s.disc.client.first_name} ${s.disc.client.last_name}`
                    : 'CD discrepancy',
                sub: s.disc.medication?.name ?? 'Discrepancy',
            };
        case 'destruction':
            return {
                icon: Trash2,
                title: s.destruction.client_name,
                sub: s.destruction.medication_name ?? 'Destruction',
            };
        case 'loss':
            return {
                icon: FileWarning,
                title: s.loss.client
                    ? `${s.loss.client.first_name} ${s.loss.client.last_name}`
                    : 'CD loss report',
                sub: s.loss.medication_name ?? 'Loss report',
            };
    }
}
...
function stepsFor(s: CdDetailSubject): WizardStep[] {
        ...
        return [
            {
                key: 'movement',
                label: 'Movement',
                blurb: 'Drug, quantity & balance',
                icon: Package,
            },
            {
                key: 'audit',
                label: 'Audit trail',
                blurb: 'Witness & timestamp',
                icon: FileText,
            },
        ];
    }
    const one: Record<Exclude<CdDetailSubject['kind'], 'entry'>, WizardStep> = {
        medication: {
            ...
            blurb: 'On-hand & last check',
            icon: ShieldCheck,
        },
        discrepancy: {
            key: 'disc',
            label: 'Discrepancy',
            blurb: 'Counts & resolution',
            icon: AlertTriangle,
        },
        destruction: {
            key: 'dest',
            label: 'Destruction',
            blurb: 'Item, method & witnesses',
            icon: Trash2,
        },
        loss: {
            key: 'loss',
            label: 'Loss report',
            blurb: 'Loss & investigation',
            icon: FileWarning,
        },
    };
    return [one[s.kind]];
...
export function CdDetailDialog({
    ...
    if (subject.kind === 'medication') {
        if (onCheckBalance)
            footerEnd.push(
                <Button
                    key="bal"
                    type="button"
                    onClick={() => onCheckBalance(subject.med.id)}
                >
                    <ClipboardCheck className="h-4 w-4" /> Check balance
                </Button>,
            );
        if (onRecordMovement)
            footerEnd.push(
                ...
                >
                    <Package className="h-4 w-4" /> Record movement
                </Button>,
            );
    } else if (subject.kind === 'entry') {
        if (onRecordMovement)
            footerEnd.push(
                <Button key="mv" type="button" onClick={onRecordMovement}>
                    <Package className="h-4 w-4" /> Record movement
                </Button>,
            );
        if (onCheckBalance && subject.med)
            footerEnd.push(
                <Button
                    key="bal"
                    type="button"
        ...
        if (
            onResolveDiscrepancy &&
            subject.disc.status !== 'closed' &&
            subject.disc.status !== 'resolved'
        ) {
            footerEnd.push(
                <Button
                    key="res"
                    type="button"
                    onClick={() => onResolveDiscrepancy(subject.disc)}
                >
        ...
        if (onLossAction && subject.loss.investigation_status === 'reported')
            footerEnd.push(
                <Button
                    key="inv"
                    type="button"
                    variant="outline"
                    onClick={() => onLossAction(subject.loss, 'investigate')}
                >
                    <FileWarning className="h-4 w-4" /> Investigate
                </Button>,
            );
        if (onLossAction && subject.loss.investigation_status !== 'resolved')
            footerEnd.push(
                <Button
                    key="resl"
                    type="button"
                    onClick={() => onLossAction(subject.loss, 'resolve')}
                >
                    <ShieldCheck className="h-4 w-4" /> Resolve
                </Button>,
            );
    ...
    if (clientId)
        footerEnd.push(
            <Button
                ...
                variant="ghost"
                onClick={() =>
                    router.visit(`/operations/clients/${clientId}?tab=mar`)
                }
            >
    ...
    footerEnd.push(
        <Button
            ...
            type="button"
            variant="ghost"
            onClick={() => window.open(CD_REGISTER_PDF, '_blank')}
        >
            <Printer className="h-4 w-4" /> Export register
        </Button>,
    );
    ...
    return (
        ...
        >
            {subject.kind === 'medication' && (
                <MedicationBody med={subject.med} />
            )}
            {subject.kind === 'entry' && (
                <EntryBody entry={subject.entry} section={section} />
            )}
            {subject.kind === 'discrepancy' && (
                <DiscrepancyBody disc={subject.disc} />
            )}
            {subject.kind === 'destruction' && (
                <DestructionBody destruction={subject.destruction} />
            )}
...
function IncidentLink({
    ...
    return (
        ...
        <button
            type="button"
            onClick={() => router.visit(`/clients/${clientId}/incidents`)}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
            <ReviewCard icon={Lock} title="Controlled drug">
                <ReviewRow
                    ...
                    value={
                        <DrugName
                            name={med.name}
                            controlled={med.controlled_drug}
                        />
                    }
                />
                <ReviewRow
                    label="CD schedule"
                    value={med.schedule ? `Schedule ${med.schedule}` : null}
                />
            ...
            <ReviewCard icon={ShieldCheck} title="Reconciliation" span>
                ...
                <ReviewRow
                    ...
                    value={fmtTs(med.last_balance_check_at) ?? 'Never'}
                />
                <ReviewRow
                    label="Days since check"
                    value={
                        med.days_since_check != null
                            ? `${med.days_since_check}d`
                            : null
                    }
                />
            </ReviewCard>
        ...
        return (
            <div className="grid gap-4">
                <ReviewCard icon={FileText} title="Audit trail">
                    <ReviewRow
                        label="Recorded by"
                        value={entry.recorded_by_name}
                    />
                    <ReviewRow
                        label="Witnessed by"
                        value={entry.witnessed_by_name}
                    />
                    <ReviewRow
                        label="Recorded at"
                        value={fmtTs(entry.recorded_at)}
                    />
                </ReviewCard>
                {entry.notes ? (
    ...
    return (
        <div className="grid gap-4 sm:grid-cols-2">
                ...
                <ReviewRow
                    ...
                    value={
                        <DrugName
                            ...
                            controlled={entry.controlled_drug ?? true}
                        />
                    }
                />
            ...
            <ReviewCard icon={Package} title="Movement">
                ...
                <ReviewRow
                    label="Quantity"
                    value={`${entry.quantity ?? '—'} ${entry.unit ?? ''}`.trim()}
                />
                <ReviewRow
                    label="Balance"
                    value={`${entry.on_hand_before ?? '—'} → ${entry.on_hand_after ?? '—'}`}
                />
        ...
        <div className="grid gap-4">
            {!resolved ? (
                <InfoCard icon={AlertTriangle} tone="crit">
                    Open discrepancy — investigate the count and resolve against
                    the linked incident.
                </InfoCard>
            ) : null}
            <div className="grid gap-4 sm:grid-cols-2">
                <ReviewCard icon={AlertTriangle} title="Discrepancy">
                    <ReviewRow
                        label="Drug"
                        value={
                            <DrugName name={disc.medication?.name} controlled />
                        }
                    />
                    <ReviewRow
                        label="Resident"
                    ...
                    <ReviewRow
                        label="Status"
                        value={<StatusPill label={disc.status} />}
                    />
                ...
                <ReviewCard icon={ShieldCheck} title="Counts">
                    <ReviewRow
                        label="Expected"
                        value={
                            disc.on_hand_before != null
                                ? String(disc.on_hand_before)
                                : null
                        }
                    />
                    <ReviewRow
                        label="Actual"
                        value={
                            disc.on_hand_after != null
                                ? String(disc.on_hand_after)
                                : null
                        }
                    />
                    <ReviewRow
                ...
                <ReviewCard icon={FileText} title="Reporting & resolution" span>
                    {disc.incident_id ? (
                        <ReviewRow
                            ...
                            value={
                                <IncidentLink
                                    id={disc.incident_id}
                                    title={disc.incident_title}
                                    clientId={disc.client?.id ?? null}
                                />
                            }
                        />
                    ) : null}
                    <ReviewRow
                        label="Reported by"
                        value={disc.reported_by_name}
                    />
                    <ReviewRow
                        label="Witnessed by"
                        value={disc.witnessed_by_name}
                    />
                    <ReviewRow
                        label="Reported at"
                        value={fmtTs(disc.reported_at)}
                    />
                    {disc.notes ? (
                        <ReviewRow label="Notes" value={disc.notes} />
                    ) : null}
                    {disc.immediate_action_taken ? (
                        <ReviewRow
                            label="Immediate action"
                            value={disc.immediate_action_taken}
                        />
                    ) : null}
                    {resolved ? (
                        <ReviewRow
                            label="Resolved by"
                            value={disc.resolved_by_name}
                        />
                    ) : null}
                    {resolved ? (
                        <ReviewRow
                            label="Resolved at"
                            value={fmtTs(disc.resolved_at)}
                        />
                    ) : null}
                    {disc.resolution_notes ? (
                        <ReviewRow
                            label="Resolution"
                            value={disc.resolution_notes}
                        />
                    ) : null}
                </ReviewCard>
            </div>
    ...
    return (
        <div className="grid gap-4 sm:grid-cols-2">
            <ReviewCard icon={Trash2} title="Item destroyed">
                <ReviewRow
                    label="Drug"
                    value={<DrugName name={d.medication_name} controlled />}
                />
                <ReviewRow label="Resident" value={d.client_name} />
                <ReviewRow
                    label="Quantity"
                    value={`${d.quantity ?? '—'} ${d.unit ?? ''}`.trim()}
                />
            ...
            <ReviewCard icon={ShieldCheck} title="Method & authorisation">
                <ReviewRow
                    ...
                    value={humanize(d.disposal_method)}
                />
...
function LossBody({ loss: l }: { loss: CdLossReport }) {
    ...
    return (
        <div className="grid gap-4">
            {open ? (
                <InfoCard icon={FileWarning} tone="crit">
                    Open loss investigation — capture findings and escalate to
                    the CD Accountable Officer / regulator as required.
                </InfoCard>
            ) : null}
            <div className="grid gap-4 sm:grid-cols-2">
                <ReviewCard icon={FileWarning} title="Loss">
                    <ReviewRow
                        label="Drug"
                        value={<DrugName name={l.medication_name} controlled />}
                    />
                    <ReviewRow
                        label="Resident"
                        value={
                    ...
                    <ReviewRow
                        label="Quantity lost"
                        value={`${l.quantity_lost ?? '—'} ${l.unit ?? ''}`.trim()}
                    />
                    <ReviewRow
                        label="Status"
                        value={<StatusPill label={l.investigation_status} />}
                    />
                    <ReviewRow label="Circumstances" value={l.circumstances} />
                    <ReviewRow
                        label="Immediate action"
                        value={l.immediate_action_taken}
                    />
                </ReviewCard>
                <ReviewCard
                    icon={ShieldCheck}
                    title="Escalation & investigation"
                >
                    {l.incident_id ? (
                        <ReviewRow
                            label="Linked incident"
                    ...
                    <ReviewRow
                        ...
                        value={l.accountable_officer_name}
                    />
                    <ReviewRow
                        label="Discovered by"
                        value={l.discovered_by_name}
                    />
                    <ReviewRow
                        label="Discovered at"
                        value={fmtTs(l.discovered_at)}
                    />
                    <ReviewRow
                        label="Police"
                        value={
                            l.reported_to_police
                                ? l.police_reference || 'Reported'
                                : 'No'
                        }
                    />
                    <ReviewRow
                        label="Pharmacy"
                        value={
                            l.reported_to_pharmacy
                                ? l.pharmacy_name || 'Reported'
                                : 'No'
                        }
                    />
                    <ReviewRow
                        label="Regulator"
                        value={
                            l.reported_to_regulator
                                ? [l.regulator_name, l.regulator_reference]
                                      .filter(Boolean)
                                      .join(' · ') || 'Notified'
                                : 'No'
                        }
                    />
                    {l.investigation_notes ? (
                        <ReviewRow
                            label="Findings"
                            value={l.investigation_notes}
                        />
                    ) : null}
                    {l.resolution_outcome ? (
                        <ReviewRow
                            label="Outcome"
                            value={l.resolution_outcome}
                        />
                    ) : null}
                </ReviewCard>
            </div>
