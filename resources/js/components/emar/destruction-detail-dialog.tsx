...
const SECTIONS: WizardStep[] = [
    {
        key: 'disposal',
        label: 'Disposal',
        blurb: 'Item, quantity & method',
        icon: Package,
    },
    {
        key: 'witness',
        label: 'Witness & audit',
        blurb: 'Signatories & trail',
        icon: ShieldCheck,
    },
];
...
export function DestructionDetailDialog({
    ...
    return (
        <WizardShell
            ...
            railSub={
                [d.client_name, d.site_name].filter(Boolean).join(' · ') ||
                'Disposal register'
            }
            steps={SECTIONS}
            stepIndex={section}
            ...
            {d.is_voided ? (
                <div className="mb-4">
                    <InfoCard icon={Ban} tone="warn">
                        Voided
                        {d.voided_by_name ? ` by ${d.voided_by_name}` : ''}
                        {d.voided_at ? ` · ${fmtDateTime(d.voided_at)}` : ''}
                        {d.void_reason ? ` — ${d.void_reason}` : ''}. The
                        original entry is retained (struck through) and excluded
                        from live counts.
                    </InfoCard>
                </div>
            ) : null}
            ...
            {section === 0 ? (
                <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <ReviewCard icon={Package} title="Medication">
                        ...
                        <ReviewRow
                            label="Expiry"
                            value={fmtDate(d.expiry_date)}
                        />
                    </ReviewCard>
                    <ReviewCard icon={Trash2} title="Disposal" span>
                        <ReviewRow
                            label="Reason"
                            value={d.reason_label ?? d.reason}
                        />
                        <ReviewRow
                            label="Method"
                            value={d.disposal_method_label ?? d.disposal_method}
                        />
                        <ReviewRow
                            label="Denaturing kit"
                            value={
                                denaturing
                                    ? 'Used — rendered irretrievable'
                                    : 'Not applicable'
                            }
                        />
                        <ReviewRow
                            label="Destroyed"
                            value={fmtDateTime(d.destroyed_at)}
                        />
                    </ReviewCard>
                    {d.notes ? (
                ...
                <div className="grid gap-4">
                    <ReviewCard
                        icon={ShieldCheck}
                        title="Witnesses & authorisation"
                    >
                        <ReviewRow
                            label="Destroyed by"
                            value={d.destroyed_by_name}
                        />
                    ...
                    <ReviewCard icon={FileText} title="Audit trail">
                        <ReviewRow
                            label="Recorded"
                            value={`${d.destroyed_by_name ?? '—'}${d.destroyed_at ? ` · ${fmtDateTime(d.destroyed_at)}` : ''}`}
                        />
                        <ReviewRow
                            label="Voided"
                            value={
                                d.is_voided
                                    ? `${d.voided_by_name ?? '—'}${d.voided_at ? ` · ${fmtDateTime(d.voided_at)}` : ''}`
                                    : null
                            }
                        />
                    </ReviewCard>
                    {d.is_controlled_drug ? (
                        <InfoCard icon={ShieldCheck} tone="info">
                            Controlled-drug destruction — recorded with two
                            distinct witnesses and authorisation, per MoD Regs
                            1977.
                        </InfoCard>
                    ) : null}
                </div>
