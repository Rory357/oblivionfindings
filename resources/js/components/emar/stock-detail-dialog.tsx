...
const SECTIONS: WizardStep[] = [
    { key: 'overview', label: 'Overview', blurb: 'Stock level, batch & flags', icon: Package },
    { key: 'activity', label: 'Activity', blurb: 'Counts, orders & movements', icon: History },
];
...
export function stockStatusPill(s: StockRow): { label: string; cls: string } {
    if (s.is_expired) return { label: 'Expired', cls: 'bg-status-critical-bg text-status-critical' };
    if (s.is_low) return { label: 'Reorder now', cls: 'bg-status-warning-bg text-status-warning' };
    if (s.is_expiring_soon) return { label: 'Expiring', cls: 'bg-status-warning-bg text-status-warning' };
    return { label: 'In stock', cls: 'bg-status-success-bg text-status-success' };
}
...
function MovementRow({ m }: { m: StockMovement }) {
    ...
    const deltaTone = m.delta == null ? '' : m.delta > 0 ? 'text-status-success' : m.delta < 0 ? 'text-status-critical' : 'text-muted-foreground';
    return (
        <li className="flex items-start gap-3 px-4 py-2.5">
            ...
            <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between gap-2">
                    <span className="truncate text-[13px] font-medium capitalize">{m.type}</span>
                    {m.delta != null && m.delta !== 0 ? (
                        <span className={`shrink-0 font-mono text-xs font-semibold tabular-nums ${deltaTone}`}>
                            {m.delta > 0 ? '+' : ''}{m.delta} {m.unit ?? ''}
                        </span>
                    ) : null}
                </div>
                <div className="truncate text-xs text-muted-foreground">{m.summary}</div>
                <div className="text-[10.5px] text-muted-foreground">{[fmtDateTime(m.at), m.actor].filter(Boolean).join(' · ') || '—'}</div>
            </div>
        </li>
    );
}
...
export function StockDetailDialog({
    ...
    const ratio = reorder > 0 ? Math.min(100, (item.on_hand / (reorder * 2)) * 100) : 100;
    const barTone = item.is_low ? 'bg-status-critical' : item.on_hand <= reorder * 1.4 ? 'bg-status-warning' : 'bg-status-success';
    ...
    return (
        <WizardShell
            ...
            railSub={[item.client_name, item.client_room, item.site_name].filter(Boolean).join(' · ') || 'Stock item'}
            steps={SECTIONS}
            stepIndex={section}
            ...
            footerEnd={
                <>
                    ...
                    {item.mar_url ? (
                        <Button type="button" variant="ghost" onClick={() => router.visit(item.mar_url!)}>
            ...
            {section === 0 ? (
                <div className="grid gap-4 sm:grid-cols-2">
                        ...
                        <ReviewRow
                            ...
                            value={
                                <span className="inline-flex items-center gap-1.5">
                                    ...
                                    {item.requires_cold_chain ? <Snowflake className="h-3.5 w-3.5 text-status-info" aria-label="Cold chain" /> : null}
                                </span>
                            }
                        />
                        <ReviewRow label="Dose" value={item.medication_dose} />
                        <ReviewRow label="Storage" value={STORAGE_LABEL[item.storage_condition] ?? item.storage_condition} />
                    </ReviewCard>
                    <ReviewCard icon={Package} title="Stock level">
                        <ReviewRow
                            label="On hand"
                            value={
                                <span className={`font-mono font-semibold tabular-nums ${item.is_low ? 'text-status-critical' : ''}`}>
                                    {item.on_hand} {item.unit}
                                </span>
                            }
                        />
                        <ReviewRow label="Reorder level" value={item.reorder_level ?? '—'} />
                        <ReviewRow label="Reorder quantity" value={item.reorder_quantity ?? '—'} />
                        <ReviewRow
                            label="Status"
                            value={<span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${pill.cls}`}>{pill.label}</span>}
                        />
                    ...
                    <ReviewCard icon={Truck} title="Batch & supply">
                        <ReviewRow label="Batch number" value={item.batch_number} />
                        <ReviewRow
                            label="Expiry"
                            value={
                                item.expiry_date ? (
                                    <span className={item.is_expired ? 'text-status-critical' : item.is_expiring_soon ? 'text-status-warning' : ''}>
                                        {fmtDate(item.expiry_date)}
                                    </span>
                                ) : null
                            }
                        />
                        <ReviewRow label="Supplier" value={item.supplier_name} />
                    </ReviewCard>
                </div>
            ) : (
                <div className="grid gap-4">
                    <div className="grid gap-4 sm:grid-cols-2">
                        <ReviewCard icon={ClipboardCheck} title="Last count">
                            <ReviewRow label="Last counted" value={item.last_counted_at ? fmtDateTime(item.last_counted_at) : 'Never'} />
                            <ReviewRow label="On hand now" value={`${item.on_hand} ${item.unit}`} />
                        </ReviewCard>
                        {openOrder ? (
                            <ReviewCard icon={ShoppingCart} title="Open pharmacy order">
                                <ReviewRow label="Pharmacy" value={openOrder.pharmacy_name} />
                                <ReviewRow label="Status" value={<span className="capitalize">{openOrder.status}</span>} />
                                <ReviewRow label="Quantity" value={openOrder.quantity_ordered != null ? `${openOrder.quantity_ordered} units` : null} />
                                <ReviewRow label="Ordered" value={fmtDate(openOrder.ordered_at)} />
                            </ReviewCard>
                        ) : (
                            <InfoCard icon={ShoppingCart} tone="info">
                                No open pharmacy order for this item. Use “Order more” to resupply.
                            </InfoCard>
                        )}
                    </div>
                    <div className="overflow-hidden rounded-xl border bg-card">
                        ...
                        {movements.length === 0 ? (
                            <p className="px-4 py-6 text-center text-sm text-muted-foreground">No recorded movements yet.</p>
                        ) : (
