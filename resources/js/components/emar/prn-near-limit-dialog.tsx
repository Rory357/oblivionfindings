...
import { AlertTriangle, Clock, FileText, Flag, Gauge, Pill, Printer, Stethoscope, User } from 'lucide-react';
import { useState } from 'react';

const SECTIONS: WizardStep[] = [
    { key: 'limit', label: 'Limit & status', blurb: 'Daily cap & next dose', icon: Gauge },
    { key: 'doses', label: "Today's doses", blurb: 'Last 24 hours', icon: Clock },
];
...
export function PrnNearLimitDialog({
    ...
    const pct = med.max_per_day ? Math.min(100, Math.round((med.given_last_24h / med.max_per_day) * 100)) : 0;
    ...
    return (
        <WizardShell
            ...
            railTitle={med.name}
            railSub={[client?.name ?? med.client_name, client?.site_name].filter(Boolean).join(' · ') || 'Near daily limit'}
            steps={SECTIONS}
            stepIndex={section}
            ...
            footerEnd={
                <>
                    ...
                    <Button type="button" variant="ghost" onClick={() => router.visit(`/clients/${med.client_id}/incidents`)}>
            ...
            {section === 0 ? (
                <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <ReviewCard icon={User} title="Resident">
                        <ReviewRow label="Name" value={client?.name ?? med.client_name} />
                        <ReviewRow label="Site" value={client?.site_name} />
                    </ReviewCard>
                    <ReviewCard icon={Gauge} title="Daily limit" span>
                        <div className="mb-2.5 flex items-center justify-between">
                            <span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${med.over_limit ? 'bg-status-critical-bg text-status-critical' : 'bg-status-warning-bg text-status-warning'}`}>
                                {med.over_limit ? 'At limit' : `${med.remaining_today ?? 0} left`}
                            </span>
                            <span className="text-xs text-muted-foreground">
                                {med.given_last_24h} of {med.max_per_day ?? '—'} in 24h
                            </span>
                        </div>
                        <div className="mb-3 h-1.5 overflow-hidden rounded-full bg-muted">
                            <div className={`h-full rounded-full ${med.over_limit ? 'bg-status-critical' : 'bg-status-warning'}`} style={{ width: `${pct}%` }} />
                        </div>
                        <ReviewRow label="Max per day" value={med.max_per_day != null ? `${med.max_per_day}×` : null} />
                        <ReviewRow label="Min hours between" value={med.min_hours_between != null ? `${med.min_hours_between} h` : null} />
                        <ReviewRow label="Remaining today" value={med.remaining_today != null ? `${med.remaining_today}` : null} />
                        <ReviewRow label="Last given" value={med.last_given_label} />
                        <ReviewRow label="Next dose allowed" value={med.interval_blocked ? med.next_allowed_label : 'Now'} />
                    </ReviewCard>
                    {med.over_limit ? (
                        <InfoCard icon={AlertTriangle} tone="crit">
                            This PRN has reached its daily limit. Don&rsquo;t give another dose without senior sign-off — a further
                            attempt raises an over-limit incident.
                        </InfoCard>
                    ) : (
                        <InfoCard icon={AlertTriangle} tone="warn">
                            Approaching the daily limit ({med.remaining_today ?? 0} dose{med.remaining_today === 1 ? '' : 's'} left).
                            Check the indication and minimum interval before giving another.
                        </InfoCard>
                    )}
                    {incident ? (
                        <ReviewCard icon={Flag} title="Over-limit incident" span>
                            <ReviewRow label="Status" value={incident.status} />
                            <ReviewRow label="Raised" value={incident.occurred_label} />
                            <div className="pt-2">
                                <Button type="button" size="sm" variant="outline" onClick={() => router.visit(incident.url)}>
                                    <Flag className="h-3.5 w-3.5" /> Open incident
                                </Button>
                            </div>
                        </ReviewCard>
                    ) : null}
                ...
                <div className="grid gap-4">
                    {doses.length === 0 ? (
                        ...
                        <ol className="overflow-hidden rounded-xl border border-border">
                            {doses.map((d, i) => (
                                <li key={d.id} className={`flex flex-wrap items-center gap-x-3 gap-y-1 px-4 py-3 ${i > 0 ? 'border-t border-border' : ''}`}>
                                    <span className="w-14 shrink-0 font-semibold tabular-nums">{d.time ?? '—'}</span>
                                    <span className="text-xs text-muted-foreground">{d.date_label}</span>
                                    <span className="font-medium">{d.dose ?? 'Dose recorded'}</span>
                                    <span className="text-xs text-muted-foreground">by {d.given_by ?? '—'}</span>
                                    <span className={`ml-auto rounded-full px-2 py-0.5 text-[11px] font-semibold ${effPillClass(d.effectiveness)}`}>
