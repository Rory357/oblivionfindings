...
export default function RoundActivity({ activity, rounds, siteFilter, residentFilter, onView }: Props) {
        ...
        () =>
            activity.filter(
                (a) =>
                    ...
                    (residentFilter == null || a.resident_id === residentFilter) &&
        ...
        <div className="overflow-hidden rounded-2xl border bg-card shadow-sm">
            <div className="flex flex-col gap-3 border-b px-4 py-3.5">
                <div>
                    ...
                    <div className="mt-0.5 text-xs text-muted-foreground">Every administration flows through the audit trail. Click an entry for the full record.</div>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                    <div className="inline-flex gap-0.5 rounded-lg border bg-card p-0.5">
                        {STATUS_FILTERS.map((s) => (
                            <Button
                                ...
                                size="sm"
                                variant="ghost"
                                className={cn('h-7', statusFilter === s.id ? 'bg-accent text-primary hover:bg-accent' : 'text-muted-foreground')}
                    ...
                    <Select value={roundFilter ? String(roundFilter) : 'all'} onValueChange={(v) => setRoundFilter(v === 'all' ? null : Number(v))}>
            ...
            {filtered.length === 0 ? (
                <div className="px-5 py-10 text-center text-sm text-muted-foreground">
                    {activity.length === 0 ? 'No round activity yet today.' : 'No activity matches the current filters.'}
                </div>
            ) : (
                <div className="flex flex-col gap-1 p-2">
                    {filtered.map((a) => {
                        ...
                        return (
                            ...
                            >
                                <span className={cn('mt-0.5 grid h-7 w-7 shrink-0 place-items-center rounded-full', TONE[meta.tone] ?? TONE.muted)}>
                                    <Icon className="h-4 w-4" />
                                </span>
                                <div className="min-w-0 flex-1">
                                    <div className="text-[13px] leading-snug">
                                        <span className="font-semibold">{a.resident_name ?? 'Resident'}</span>
                                        <span className="text-muted-foreground">
                                            {' — '}
                                            {a.medication_name ?? 'Medication'}
                                            {a.dose ? ` ${a.dose}` : ''} · {meta.label.toLowerCase()}
                                        </span>
                                    </div>
                                    <div className="mt-0.5 truncate text-[11.5px] text-muted-foreground">
                                        {[a.time, a.staff, a.round_name].filter(Boolean).join(' · ')}
                                    </div>
                                </div>
                            </button>
                        );
