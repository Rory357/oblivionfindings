...
import RoundAuditTimeline, { auditMetaFromRound, cellsToAuditEntries } from './round-audit-timeline';
...
export default function RoundAuditDialog({ round, canExport, onClose, onOpenGuided, onPrint }: Props) {
                ...
                <DialogHeader className="space-y-0 border-b p-5 pr-12">
                    <div className="flex items-start justify-between gap-3">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <DialogTitle className="text-base font-bold">{round.name}</DialogTitle>
                                <p className="text-xs text-muted-foreground">
                                    Audit &amp; timeline · {round.scheduled_time} · ±{round.window_minutes} min
                                </p>
                            </div>
                        </div>
                        <RoundStatusBadge status={round.status} className="mt-1" />
                    </div>
                </DialogHeader>

                <div className="grid grid-cols-4 gap-2 px-5 pt-4">
                    <CountTile label="Given" value={counts.given} tone="text-status-success" />
                    <CountTile label="Refused" value={counts.refused} tone="text-status-warning" />
                    <CountTile label="Held" value={counts.held} tone="text-status-warning" />
                    <CountTile label="Due/Missed" value={counts.due + counts.missed} tone="text-status-critical" />
                </div>

                <div className="max-h-[46vh] overflow-y-auto px-5 py-5">
                    <div className="mb-3 text-[11.5px] font-bold tracking-wide text-muted-foreground uppercase">Audit trail</div>
                    <RoundAuditTimeline meta={auditMetaFromRound(round)} entries={cellsToAuditEntries(round.cells)} />
                </div>
