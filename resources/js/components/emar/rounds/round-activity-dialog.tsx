...
export default function RoundActivityDialog({
    item,
    onClose,
}: {
    item: ActivityItem;
            ...
            <DialogContent className="max-w-[520px] gap-0 p-0">
                ...
                <div className="px-5 py-4">
                    ...
                    <div className="mt-3">
                        <Row
                            label="Outcome"
                            value={doseStatusMeta(item.status).label}
                        />
                        <Row label="Round" value={item.round_name} />
                        <Row label="Recorded by" value={item.staff} />
                        <Row
                            label="Recorded at"
                            value={fmt(item.administered_at) ?? item.time}
                        />
                        <Row
                            label="Scheduled for"
                        ...
                        <Row
                            ...
                            value={
                                item.blood_glucose_level != null
                                    ? `${item.blood_glucose_level} mmol/L`
                                    : null
                            }
                        />
                        <Row
                    ...
                    {chips.length > 0 ? (
                        <div className="mt-3 flex flex-wrap gap-1.5">
                            {chips.map((c) => (
                                <span
                                    key={c}
                                    className="rounded-md border bg-muted px-2 py-0.5 text-[11.5px]"
                                >
                                    {c}
                                </span>
                            ))}
                    ...
                    {item.reason ? (
                        <div className="mt-3 rounded-lg bg-muted px-3 py-2 text-[12.5px] text-foreground italic">
                            “{item.reason}”
                        </div>
                    ) : null}
                </div>
