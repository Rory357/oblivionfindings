...
function TooltipPortal({
    ...
    const diffColor =
        diff >= 0 ? 'var(--status-success)' : 'var(--status-warning)';
...
function TopSiteRow({
    ...
    const popover: HoverPopoverContent = {
        icon: 'building-2',
        tone: site.region?.toLowerCase().includes('whang')
            ? 'critical'
            : 'info',
        title: site.name,
        ...
        rows: [
            {
                time: 'Region',
                site: site.region ?? site.city ?? '—',
                detail: `${site.client_count} clients`,
            },
            {
                time: 'Hours',
                site: `${site.hours}h`,
                detail: 'This week · approved',
            },
            {
                time: 'Share',
                site: `${site.pct}%`,
                detail: 'Of total delivered',
            },
        ],
...
export function ShiftsBarChart({
    ...
    const max = Math.max(
        250,
        ...shiftsPerDay.map((d) => Math.max(d.scheduled, d.target)),
    );
    const [hover, setHover] = useState<{
        rect: DOMRect;
        day: ShiftsPerDay;
    } | null>(null);
    const totalScheduled = shiftsPerDay.reduce(
        ...
        >
            ...
            <div className="p-4">
                <div className="relative ml-8 h-44">
                    <div className="absolute inset-0 flex flex-col justify-between text-[9px] text-muted-foreground tabular-nums">
                        {[250, 200, 150, 100, 50, 0].map((v) => (
                            <div
                                key={v}
                                className="flex w-7 -translate-x-7 items-center justify-end"
                            >
                                <span>{v}</span>
                            </div>
                        ))}
                            ...
                            return (
                                <div
                                    ...
                                    onMouseEnter={(e) => {
                                        ...
                                        if (barEl)
                                            setHover({
                                                rect: barEl.getBoundingClientRect(),
                                                day: d,
                                            });
                                    }}
                ...
                <div className="mt-2 ml-8 flex text-[10.5px] text-muted-foreground tabular-nums">
                    {shiftsPerDay.map((d) => (
            ...
            {hover ? (
                <TooltipPortal rect={hover.rect} payload={hover.day} />
            ) : null}
        </GuardrailCard>
    );
}
