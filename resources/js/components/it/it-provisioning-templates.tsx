...
export function ItProvisioningTemplates({
    ...
    const commandPending =
        !editingId && (createCommand.pending || createCommand.busy);
    const editorBlocked =
        form.processing || (!editingId && !createCommand.canEdit);
    const concealEditor =
        concealed ||
        actorId !== form.data.actor_user_id ||
        (!editingId && ['expired', 'denied'].includes(createCommand.state));
    useEffect(() => {
        if (concealEditor) setConcealed(true);
    }, [concealEditor]);
    const dirty = open && !saved && baseline !== JSON.stringify(form.data);
    useEffect(() => {
        if (!open || saved) return;
        const unload = (event: BeforeUnloadEvent) => {
            if (dirty || form.processing || commandPending)
                event.preventDefault();
        };
        const remove = router.on('before', (event) => {
            if (allowNavigation.current || submitting.current) return;
            if (form.processing || commandPending) {
                event.preventDefault();
                return;
            }
            if (!dirty) return;
    ...
    }, [open, saved, dirty, form.processing, commandPending]);
    useEffect(() => {
        if (open && !saved) pane.current?.focus();
    }, [open, saved, step]);
    const returnToServiceDesk = () => {
        allowNavigation.current = true;
        router.visit('/it');
    };
    const close = () => {
    ...
    const next = () => {
        if (!editorBlocked && !concealEditor && validate(step))
            setStep(step + 1);
    };
    const reloadSaved = () => {
        setLeaveAction(() => () => {
    ...
    const save = async (event: FormEvent) => {
        event.preventDefault();
        if (editorBlocked || concealEditor || saved) return;
        if (step < 2) {
            next();
            return;
        }
        ...
        if (!editingId) {
            handleCreateOutcome(await createCommand.submit(form.data));
            ...
            if (result.state === 'validation') {
                ...
                setStep(
                    Object.keys(result.errors).some(
                        (field) =>
                            field === 'tasks' || field.startsWith('tasks.'),
                    )
                        ? 1
                        : 0,
                );
        ...
        form.patch(`/it/setup/provisioning-templates/${editingId}`, options);
    };
    ...
    return (
        <>
                ...
                <section className="rounded-2xl border border-border bg-card p-4 sm:p-5">
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                            ...
                            <p className="mt-1 max-w-3xl text-xs text-muted-foreground">
                                Define ordered or parallel IT steps for HR
                                joiners, role/site movers, and leavers. The most
                                specific active role, site, and employment match
                                wins.
                            </p>
                        </div>
                    ...
                    {templates.length > 0 ? (
                        ...
                        <div className="mt-4 rounded-xl border border-dashed border-border px-5 py-10 text-center">
                            ...
                            <p className="mt-1 text-xs text-muted-foreground">
                                Create one before HR lifecycle events can
                                generate coordinated IT work.
                            </p>
                        </div>
                    )}
                </section>
            ...
            <WizardShell
                open={open && actorId === form.data.actor_user_id}
                onClose={close}
                title={
                    editingId
                ...
                stepIndex={step}
                onStepClick={(index) => {
                    if (!editorBlocked && !concealEditor) setStep(index);
                }}
                pct={
                    concealEditor
                        ? undefined
                ...
                footerStart={
                    <Button
                        ...
                        disabled={
                            !concealEditor &&
                            (form.processing || commandPending)
                        }
                        onClick={
                            concealEditor
                                ? returnToServiceDesk
                ...
                footerEnd={
                    concealEditor ? undefined : step < 2 ? (
                        <Button
                            key="continue"
                            type="button"
                            disabled={editorBlocked || concealEditor}
                            onClick={next}
                        >
                            Continue
                        </Button>
                        ...
                        <Button
                            ...
                            disabled={editorBlocked || concealEditor}
                        >
                            {form.processing || createCommand.busy ? (
                                <Loader2
                ...
                <WizardStepPane key={step}>
                    ...
                    {concealEditor ? (
                        <p role="alert">
                            ...
                            account and check the saved result, or reopen
                            templates with your current access.
                        </p>
                    ) : (
                        <form
            ...
            <ConfirmDialog
                ...
                description={
                    editingId
                        ? 'The saved template is unchanged. Discard your edits to continue.'
                        : 'Discard this unsaved template? No template will be created.'
                }
