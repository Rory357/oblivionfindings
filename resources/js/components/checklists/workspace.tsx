...
function weekInfo(today: string, offset: number): WeekInfo {
    ...
    return { label: lbl(offset), range, prevLabel: lbl(offset - 1), nextLabel: lbl(offset + 1) };
}

export function ChecklistsWorkspace({ scope, data }: { scope: ChecklistScope; data: ChecklistsData }) {
        ...
        return [
            {
                ...
                overdue_runs: data.activeRuns.filter((r) => r.is_overdue).length,
                scheduled_runs: data.activeRuns.filter((r) => r.status === 'scheduled').length,
    ...
    const tabs: RosterTabItem[] = [
        ...
        },
        {
            ...
            icon: PlayCircle,
            tone: 'info',
            badge: data.activeRuns.length + data.recentRuns.length || undefined,
        },
        { id: 'schedule', label: 'Schedule', icon: CalendarDays, tone: 'primary' },
        { id: 'library', label: 'Library', icon: Layers, tone: 'primary', badge: data.templates.length || undefined },
        {
            id: 'assignments',
            label: 'Assignments',
            icon: ClipboardList,
    ...
    return (
        ...
        >
            <div className="space-y-5">
                <ChecklistHero
                    stats={data.stats}
                    siteCount={scope.mode === 'org' ? data.sitesOverview.length : 1}
                ...
                <div>
                    {tab === 'overview' ? <OverviewPane ctx={ctx} stats={data.stats} goTab={setTab} /> : null}
