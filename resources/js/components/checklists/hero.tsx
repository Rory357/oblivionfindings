...
export function ChecklistHero({
    ...
    const page = usePage();
    const userName = (page.props as { auth?: { user?: { name?: string } } })
        ?.auth?.user?.name;
    ...
    const eyebrow = (
        <span className="mb-2 flex items-center justify-center gap-2 text-[10.5px] font-semibold tracking-wider text-primary-foreground/80 uppercase md:justify-start">
            ...
            {site
                ? `${site.name} · live checklist status`
                : 'Live across your network · refreshed just now'}
        </span>
    );
    ...
    if (stats.failures > 0) {
        badges.push({
            icon: TriangleAlert,
            label: `${stats.failures} failures → hazards`,
            tone: 'info',
        });
    }
    ...
    return (
        <PageHero
            ...
            meta={[
                ...
                {
                    icon: Layers,
                    label: `${templateCount} templates · ${categoryCount} categories`,
                },
                {
                    icon: CalendarClock,
                    label: `${stats.scheduled} scheduled this week`,
                },
            ]}
            badges={badges}
            stats={heroStats}
