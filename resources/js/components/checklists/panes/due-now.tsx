import { AlertTriangle, CalendarClock, CalendarDays, type LucideIcon, Search } from 'lucide-react';
...
export function DueNowPane({ ctx }: { ctx: PaneCtx }) {
    ...
    const groups: { key: string; label: string; tone: string; Icon: LucideIcon; items: ChecklistRun[] }[] = [
        {
            key: 'overdue',
            label: 'Overdue',
            tone: 'critical',
            Icon: AlertTriangle,
            items: ctx.runs.filter((r) => r.scheduled_date && r.scheduled_date < today).filter(match),
        },
        {
            key: 'today',
            label: 'Due today',
            tone: 'warning',
            Icon: CalendarClock,
            items: ctx.runs.filter((r) => r.scheduled_date === today).filter(match),
        },
        {
            key: 'soon',
            label: 'Coming up',
        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between gap-3">
                <p className="text-sm text-muted-foreground">
                    Your worklist — start or finish the checklists waiting on you
                </p>
                ...
                g.items.length === 0 ? null : (
                    <div key={g.key}>
                        <div className="mb-2 flex items-center gap-2">
                            <span className={cn('flex h-6 w-6 items-center justify-center rounded-md', GROUP_BG[g.tone])}>
