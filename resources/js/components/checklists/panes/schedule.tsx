...
export function SchedulePane({
    ...
    const rangeLabel = `${new Date(`${weekStart}T00:00:00`).toLocaleDateString(
        'en-NZ',
        {
            day: 'numeric',
            month: 'short',
            ...
            <div className="flex items-center justify-between border-b border-border px-5 py-3.5">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                        {rangeLabel} · scheduled &amp; overdue runs
                    </p>
                </div>
                <p className="text-xs text-muted-foreground">
                    Use the week stepper in the banner to change weeks
                </p>
            </div>
            <div className="grid grid-cols-1 divide-y divide-border md:grid-cols-7 md:divide-x md:divide-y-0">
                {days.map((ds, i) => {
                    const isToday = ds === today;
                    const items = ctx.runs.filter(
                        (r) => r.scheduled_date === ds,
                    );
                    const show: ChecklistRun[] =
