...
export function OverviewPane({ ctx, stats, goTab }: { ctx: PaneCtx; stats: HeroStats; goTab: GoTab }) {
    ...
    const scheduled = ctx.runs.filter((r) => r.status === 'scheduled' && r.scheduled_date && r.scheduled_date > today);
    const onTrack = stats.onTrack;

    const workSegs = [
        { key: 'completed', label: 'Completed', value: ctx.history.length, color: 'var(--status-success)' },
        { key: 'scheduled', label: 'Scheduled', value: scheduled.length, color: 'var(--primary)' },
        { key: 'inprog', label: 'In progress', value: inProg.length, color: 'var(--status-warning)' },
        { key: 'overdue', label: 'Overdue', value: overdue.length, color: 'var(--status-critical)' },
    ];
    ...
    const siteAttention = ctx.sites
        .map((s) => ({
            site: s,
            overdue: ctx.runs.filter((r) => r.site?.id === s.id && r.scheduled_date && r.scheduled_date < today).length,
            due: ctx.runs.filter((r) => r.site?.id === s.id && r.scheduled_date === today).length,
        }))
        .filter((x) => x.overdue > 0 || x.due > 0)
        .sort((a, b) => b.overdue - a.overdue);

    const categoriesHealthy = ctx.reports.complianceByCategory.filter((c) => c.rate >= 95).length;
            ...
            <div className="grid gap-4 lg:grid-cols-3">
                <GuardrailCard unstyled className="flex flex-col items-center rounded-xl border border-border bg-card p-5 shadow-sm">
                    ...
                    <Donut
                        value={onTrack}
                        size={150}
                        color={onTrack >= 90 ? 'var(--status-success)' : 'var(--primary)'}
                        label="on track"
                ...
                <GuardrailCard unstyled className="flex flex-col rounded-xl border border-border bg-card p-5 shadow-sm">
                    <div className="mb-1 flex items-start justify-between">
                        <div>
                            ...
                            <p className="text-xs text-muted-foreground">Runs completed · last 8 weeks</p>
                        </div>
                        <div className="text-right">
                            <div className="text-2xl font-bold leading-none tabular-nums">{lastDone}</div>
                            <div
                                className={cn(
                                    'mt-0.5 flex items-center justify-end gap-0.5 text-[11px] font-medium',
                                    delta >= 0 ? 'text-status-success' : 'text-status-critical',
                                )}
                            >
                    ...
                    <div className="mt-2 flex-1">
                        <Sparkline series={doneSeries.length ? doneSeries : [0]} color="var(--primary)" />
                    </div>
                    <div className="mt-3 flex items-center justify-between border-t border-border pt-3 text-xs">
                        ...
                        <Button variant="ghost" size="sm" className="h-7" onClick={() => goTab('reports')}>
                            Analytics
                ...
                <GuardrailCard unstyled className="rounded-xl border border-border bg-card shadow-sm lg:col-span-2">
                    <div className="flex items-center justify-between border-b border-border px-5 py-3.5">
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                                {ctx.reports.complianceByCategory.length} categories across the library
                            </p>
                        </div>
                        <Button variant="ghost" size="sm" onClick={() => goTab('library')}>
                            Library
                            <ArrowRight className="h-3.5 w-3.5" />
                        </Button>
                    </div>
                    <div className="grid grid-cols-1 gap-x-4 gap-y-2 p-4 sm:grid-cols-2 xl:grid-cols-3">
                        {ctx.reports.complianceByCategory.map((c) => (
                            ...
                            >
                                <MiniRing value={c.rate} size={42} color={catColorVar(c.tone)} />
                                <div className="min-w-0 flex-1">
                                    <div className="truncate text-sm font-medium">{c.label}</div>
                                    {c.overdue > 0 ? (
                                        <div className="text-[11px] font-medium text-status-critical">{c.overdue} overdue</div>
                                    ) : (
                ...
                <GuardrailCard unstyled className="rounded-xl border border-border bg-card shadow-sm">
                    <div className="flex items-center justify-between border-b border-border px-5 py-3.5">
                        ...
                        {siteAttention.length ? (
                            <StatusBadge tone="critical">{siteAttention.length} sites</StatusBadge>
                        ) : (
                    ...
                    <div className="p-2">
                        {siteAttention.length === 0 ? (
                            <Empty Icon={CheckCircle2} title="All caught up" sub="Nothing overdue or due." />
                        ) : (
                                    ...
                                    >
                                        <div className="flex min-w-0 items-center gap-2.5">
                                            ...
                                            <div className="min-w-0">
                                                <div className="truncate text-sm font-medium">{x.site.name}</div>
                                        ...
                                        <div className="flex shrink-0 items-center gap-1">
                                            {x.overdue ? (
                                                <span className="rounded-md bg-status-critical-bg px-1.5 py-0.5 text-[11px] font-semibold tabular-nums text-status-critical">
                                                    {x.overdue}
                                                </span>
                                            ) : null}
                                            {x.due ? (
                                                <span className="rounded-md bg-status-warning-bg px-1.5 py-0.5 text-[11px] font-semibold tabular-nums text-status-warning">
                                                    {x.due}
                                                </span>
                                            ) : null}
                                        </div>
            ...
            <div className="grid gap-4 lg:grid-cols-3">
                ...
                <div className={cn('rounded-xl border border-border bg-card shadow-sm', scope.mode === 'org' ? '' : 'lg:col-span-3')}>
                    <div className="border-b border-border px-5 py-3.5">
                        ...
                        <p className="text-sm text-muted-foreground">Latest completed runs</p>
                    </div>
                    <div className="p-2">
                        {ctx.history.length === 0 ? (
                            <Empty Icon={CheckCircle2} title="No completed runs yet" />
                        ) : (
                                ...
                                {ctx.history.slice(0, 6).map((r) => {
                                    ...
                                    return (
                                        <div key={r.id} className="flex items-center gap-2.5 rounded-lg px-3 py-2">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <div className="truncate text-sm font-medium">{r.template?.name}</div>
                                                <div className="truncate text-[11px] text-muted-foreground">
                                                    {r.site?.name} · {relDay(r.scheduled_date, today)}
                                                </div>
                                            </div>
                                            {failed ? (
                                                ...
                                                <span className="shrink-0 text-[11px] font-medium text-status-success">100%</span>
                                            )}
                                        </div>
                                    );
                                })}
