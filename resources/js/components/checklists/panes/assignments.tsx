...
export function AssignmentsPane({ ctx }: { ctx: PaneCtx }) {
    ...
    const match = (a: ChecklistAssignment) => {
        ...
        const matchSite =
            siteFilter === 'all' || String(a.site?.id) === siteFilter;
        return matchQ && matchCat && matchSite;
    };

    const groups = new Map<
        number,
        { site: SiteRef; items: ChecklistAssignment[] }
    >();
    ctx.assignments.filter(match).forEach((a) => {
        if (!a.site) return;
        if (!groups.has(a.site.id))
            groups.set(a.site.id, { site: a.site, items: [] });
        groups.get(a.site.id)!.items.push(a);
    });
    const grouped = [...groups.values()].sort((x, y) =>
        x.site.name.localeCompare(y.site.name),
    );
    ...
    return (
        <div className="space-y-4">
            <SectionHead
                title="Assignments"
                desc="Which templates run at which site, and how often"
            >
                {cfg.scope.mode === 'org' ? (
                    <Dropdown
                        value={siteFilter}
                    ...
                    >
                        <div className="flex items-center justify-between gap-2 border-b border-border bg-muted/40 px-4 py-2.5">
                            <div className="flex flex-wrap items-center gap-2">
                                ...
                                <StatusBadge tone="neutral">
                                    {typeLabel(cfg, g.site.type)}
                                </StatusBadge>
                                <CountBadge>
                                    {g.items.length} assigned
                                </CountBadge>
                            </div>
                            <OpenSiteLink
                                href={`/sites/${g.site.id}/checklists`}
                            />
                        </div>
                        <div className="divide-y divide-border">
                            {g.items.map((a) => (
                                ...
                                >
                                    <CategoryIcon
                                        category={a.template?.category ?? null}
                                        box={32}
                                        size={16}
                                    />
