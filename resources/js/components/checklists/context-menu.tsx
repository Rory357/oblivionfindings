...
function MenuItem({
    ...
    return (
        <GuardrailButton unstyled
            ...
            onClick={onClick}
            className={cn(
                'flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-left text-sm transition-colors outline-none focus:bg-accent',
                tone === 'critical'
...
function RunContextMenu({
    ...
    const reschedule = () => {
        ...
        setBusy(true);
        router.patch(
            `/checklists/runs/${run.id}/schedule`,
            { scheduled_date: date },
            opts,
        );
    };
    ...
    const restore = () => {
        setBusy(true);
        router.post(`/checklists/runs/${run.id}/restore`, {}, opts);
    };
    ...
    const showSchedule = can.schedule && !completed && !skipped;
    const showSkip = can.run && !completed && !skipped;
    const showRestore = can.schedule && skipped;

    return (
        <FloatingMenu x={x} y={y} onClose={onClose}>
            {mode === 'menu' ? (
                <>
                    <div className="truncate px-2.5 pt-1 pb-1 text-xs font-semibold text-muted-foreground">
                    ...
                    <MenuItem
                        ...
                        autoFocus
                    />
                    {showSchedule || showSkip || showRestore ? (
                ...
                <div className="p-1.5">
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="h-9 w-full rounded-md border border-input bg-background pr-2 pl-8 text-sm outline-none focus:border-ring focus:ring-2 focus:ring-ring/30"
                        />
                    </div>
