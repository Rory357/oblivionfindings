    ...
    <div className="rounded-xl border p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <div className="text-xs text-muted-foreground">{range === 'today' ? 'Today' : 'Next 7 days'}</div>
        </div>
      ...
      <div className="mt-4 max-h-[60vh] space-y-4 overflow-y-auto pr-1">
        {grouped.length === 0 ? (
          <div className="text-sm text-muted-foreground">{emptyText ?? 'No activity scheduled.'}</div>
        ) : (
            ...
            <div key={dateLabel} className="space-y-2">
              ...
              <div className="space-y-2">
                {items.map((e) => {
                  ...
                  const clientName = e.client ? `${e.client.first_name} ${e.client.last_name}` : null;
                  ...
                  return (
                    ...
                    >
                      <div className="flex min-w-0 items-start gap-3">
                        <div className="mt-0.5 w-24 shrink-0 text-xs font-medium">{fmtTime(e.occurred_at)}</div>

                        <div className="min-w-0">
                          <div className="truncate text-sm font-medium">{e.subject}</div>
                          <div className="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
                            <span className="rounded-md border px-2 py-0.5">{typeLabel(e.type)}</span>
                            {clientName ? <span>{clientName}</span> : null}
                            {e.site?.name ? <span>{e.site.name}</span> : null}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        {shiftId ? (
                          <>
                            <Button asChild size="sm" variant="outline">
                              <Link href={`/shifts/${shiftId}/edit`}>View</Link>
                            </Button>
                            <Button asChild size="sm">
                              <Link href={`/timesheets/create?shift_id=${shiftId}`}>Timesheet</Link>
                            </Button>
                          </>
                        ) : null}
                        ...
                        {!shiftId && e.client?.id ? (
                          <Button asChild size="sm" variant="outline">
                            <Link href={`/clients/${e.client.id}/timeline`}>Open</Link>
                          </Button>
                        ) : null}
