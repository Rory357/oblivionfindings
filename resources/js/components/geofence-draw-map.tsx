...
import { Check, Circle, Pentagon, Square, Trash2, Undo2 } from 'lucide-react';
...
export default function GeofenceDrawMap({
    ...
    const [mode, setMode] = useState<'circle' | 'rectangle' | 'polygon'>(
    ...
    const [radius, setRadius] = useState(
        initialShape?.type === 'circle'
            ? String(initialShape.radius_m ?? 200)
            : '200',
    );
    const [circleCenter, setCircleCenter] = useState<{
        ...
        initialShape?.type === 'circle' ? (initialShape.center ?? null) : null,
    );
    ...
    >(() => {
        if (
            initialShape?.type === 'rectangle' &&
            initialShape.coordinates &&
            initialShape.coordinates.length >= 4
        ) {
            return [
                initialShape.coordinates[0],
                initialShape.coordinates[1],
                initialShape.coordinates[2],
                initialShape.coordinates[3],
            ];
        }
        return [];
    });
    const [polyPoints, setPolyPoints] = useState<
        { lat: number; lng: number }[]
    >(initialShape?.type === 'polygon' ? (initialShape.coordinates ?? []) : []);
    const [polyDone, setPolyDone] = useState(
        initialShape?.type === 'polygon' &&
            (initialShape.coordinates?.length ?? 0) >= 3,
    );
    ...
    useEffect(() => {
        ...
        map.on('click', (e: L.LeafletMouseEvent) => {
            window.dispatchEvent(
                new CustomEvent('gf-click', { detail: coord(e.latlng) }),
            );
        });
        return () => {
            map.remove();
    ...
    const addDragMarker = useCallback(
        (
            lat: number,
            lng: number,
            icon: L.DivIcon,
            onDragEnd: (ll: { lat: number; lng: number }) => void,
        ) => {
            const map = mapRef.current;
            if (!map) return;
    ...
    useEffect(() => {
        ...
        addDragMarker(circleCenter.lat, circleCenter.lng, dotIcon(), (p) =>
            setCircleCenter(p),
        );
    ...
    useEffect(() => {
        ...
        if (rectCorners.length === 4) {
            const poly = L.polygon(
                rectCorners.map((c) => [c.lat, c.lng] as L.LatLngTuple),
                FILL,
            ).addTo(map);
    ...
    useEffect(() => {
        ...
        if (polyDone && polyPoints.length >= 3) {
            const poly = L.polygon(
                polyPoints.map((p) => [p.lat, p.lng] as L.LatLngTuple),
                FILL,
            ).addTo(map);
            shapeRef.current = poly;
            ...
            const line = L.polyline(
                polyPoints.map((p) => [p.lat, p.lng] as L.LatLngTuple),
                { color: PURPLE, weight: 2, dashArray: '6 4' },
            ).addTo(map);
    ...
    const status = (() => {
        if (mode === 'circle') {
            if (!circleCenter)
                return {
                    text: 'Click on the map to place the center point',
                    ok: false,
                };
            return {
                text: `Center: ${circleCenter.lat}, ${circleCenter.lng} · Radius: ${radius}m`,
                ok: true,
            };
        }
        if (mode === 'rectangle') {
            if (rectCorners.length === 0)
                return { text: 'Click to place the first corner', ok: false };
            if (rectCorners.length === 1)
                return {
                    text: 'Click to place the opposite corner',
                    ok: false,
                };
            return {
                text: 'Rectangle ready — drag any corner to adjust',
                ok: true,
            };
        }
        if (polyPoints.length === 0)
            return { text: 'Click to start adding vertices', ok: false };
        if (!polyDone)
            return {
                text: `${polyPoints.length} point${polyPoints.length > 1 ? 's' : ''}${polyPoints.length < 3 ? ` — need ${3 - polyPoints.length} more` : ' — click Complete or drag to adjust'}`,
                ok: false,
            };
        return {
            text: `Polygon with ${polyPoints.length} vertices — drag any point to adjust`,
            ok: true,
        };
    })();
    ...
    return (
        ...
        >
                ...
                <div className="flex gap-0.5 rounded-lg border p-0.5">
                    {[
                        ...
                        <Button
                            ...
                            className={cn(
                                'h-auto gap-1.5 rounded-md px-3 py-1.5 text-sm font-medium transition-all',
                                mode === k
            ...
            <div
                ref={containerRef}
                style={{ height }}
                className="relative z-0 cursor-crosshair overflow-hidden rounded-lg border"
            />
            ...
            <div className="rounded-b-lg border border-t-0 bg-muted/30 px-4 py-3">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            {Number(radius) >= 1000 && (
                                <span className="text-xs text-muted-foreground">
                                    {(Number(radius) / 1000).toFixed(1)} km
                                </span>
                            )}
                        </div>
                        <input
                            ...
                            className="h-2 w-full cursor-pointer appearance-none rounded-full bg-primary/10 accent-purple-600"
                        />
                        <div className="flex gap-1.5">
                            {[50, 100, 200, 500, 1000, 2000].map((r) => (
                                <Button
                                    ...
                                    className={cn(
                                        'h-auto rounded-full px-2.5 py-1 text-[10px] font-medium transition-colors',
                                        String(r) === radius
                ...
                {mode === 'rectangle' && (
                    <p className="text-sm text-muted-foreground">
                        ...
                        {rectCorners.length === 1 && (
                            <span className="font-medium text-primary">
                                Corner 1 placed — click the opposite corner.
                            </span>
                        )}
                        {rectCorners.length === 4 && (
                            <span className="font-medium text-primary">
                                ✓ Drag any numbered corner to resize. Click map
                                to restart.
                            </span>
                        )}
                    </p>
            ...
            <div
                className={cn(
                    'mt-2 flex items-center justify-between rounded-lg border px-3 py-2 text-xs',
                    status.ok
