...
import { Building2, CalendarClock, ClipboardCheck, ListChecks, Minus, Plus, Target, User as UserIcon, Users } from 'lucide-react';
...
import { rollup, type Cycle, type KrType, type Objective, type ObjectiveTemplate } from './okr-shared';
...
export function ObjectiveWizard({
        ...
        () =>
            isEdit
                ? [
                      ...
                      { key: 'timing', label: 'Timing', blurb: 'Window & confidence', icon: CalendarClock },
                      { key: 'review', label: 'Review', blurb: 'Confirm & save', icon: ClipboardCheck },
                  ]
                : [
                      { key: 'objective', label: 'Objective', blurb: 'What & who', icon: Target },
                      { key: 'krs', label: 'Key results', blurb: 'Measure & weight', icon: ListChecks },
                      { key: 'timing', label: 'Timing', blurb: 'Window & confidence', icon: CalendarClock },
                      { key: 'review', label: 'Review', blurb: 'Confirm & save', icon: ClipboardCheck },
                  ],
        [isEdit],
    ...
    const form = useForm<{
        ...
        parent_goal_id: goal?.parent_goal_id
            ...
            : prefillParentId
              ...
              : NONE,
        cycle_id: goal?.cycle_id ? String(goal.cycle_id) : defaultCycleId ? String(defaultCycleId) : NONE,
    ...
    const applyTemplate = (id: string) => {
        ...
        setKrs(
            (tpl.key_results.length ? tpl.key_results : [{ title: '', kr_type: 'percent', start_value: 0, target_value: 100, unit: '%', weight: 1 }]).map((k) => ({
                key: `kr-${krSeq++}`,
                title: k.title,
                kr_type: k.kr_type,
    ...
    const ownerName = owners.find((o) => String(o.id) === form.data.user_id)?.name ?? '—';
    const parentOptions = useMemo(
        () => [
            { value: NONE, label: 'Top-level (no parent)' },
            ...parentGoals.filter((p) => !goal || p.id !== goal.id).map((p) => ({ value: String(p.id), label: p.title })),
        ],
        [parentGoals, goal],
    );
    const cycleOptions = useMemo(
        () => [{ value: NONE, label: 'No cycle' }, ...cycles.map((c) => ({ value: String(c.id), label: c.name }))],
        [cycles],
    );
    ...
    const step0Valid = form.data.user_id !== '' && form.data.title.trim() !== '' && form.data.goal_type !== '';
    const timingValid = form.data.start_date !== '' && form.data.due_date !== '';
    ...
    const updateKr = (key: string, patch: Partial<KrRow>) =>
        setKrs((prev) => prev.map((k) => (k.key === key ? { ...k, ...patch } : k)));
    ...
    const submit = (stay = false) => {
        form.transform((data) => ({
            ...
            tags,
            parent_goal_id: data.parent_goal_id === NONE ? null : data.parent_goal_id,
        ...
        const opts = {
            ...
            onError: () => {
                if (form.errors.user_id || form.errors.title || form.errors.goal_type) wizard.goTo(0);
            },
        };
    ...
    return (
                ...
                <>
                    ...
                    {wizard.isLast ? (
                        <>
                            ...
                            >
                                {form.processing ? 'Saving…' : isEdit ? 'Save changes' : 'Create objective'}
                            </button>
                        </>
                    ) : (
                        <button
        ...
        >
            {stepKey === 'objective' && (
                <WizardStepPane>
                    <StepHead icon={Target} title="Objective" blurb="The owner, the objective, and how it cascades." />
                        ...
                        <div className="mb-4">
                            <Field label="Start from template" hint="optional">
                                <SelectInput
                                    ...
                                    options={[{ value: NONE, label: 'Blank objective' }, ...templates.map((t) => ({ value: String(t.id), label: t.name }))]}
                                />
                            </Field>
                        </div>
                    )}
                    <div className="grid gap-4 sm:grid-cols-2">
                        <Field label="Owner" required span error={form.errors.user_id}>
                            <SelectInput
                        ...
                        <Field label="Description" hint="optional" span error={form.errors.description}>
                            <Textarea
                                rows={2}
                        ...
                        <Field label="Type" required span error={form.errors.goal_type}>
                            <TilePicker
                        ...
                        <Field label="Category" hint="optional" error={form.errors.category}>
                            <Input
                        ...
                        <Field label="Parent objective" hint="optional" error={form.errors.parent_goal_id}>
                            <SelectInput
                                value={form.data.parent_goal_id}
                                onChange={(v) => form.setData('parent_goal_id', v)}
                        ...
                        <Field label="Tags" hint="optional" span>
                            <div className="flex flex-wrap items-center gap-1.5 rounded-lg border border-border bg-card px-2 py-1.5">
                                {tags.map((t) => (
                                    <span key={t} className="inline-flex items-center gap-1 rounded-md bg-primary/10 px-1.5 py-0.5 text-[11px] font-semibold text-primary">
                                        {t}
                                        <button type="button" onClick={() => setTags((p) => p.filter((x) => x !== t))} aria-label={`Remove ${t}`} className="text-primary/70 hover:text-primary">
                                            ×
                                        </button>
                                    </span>
                                ))}
            ...
            {stepKey === 'krs' && (
                <WizardStepPane>
                    <StepHead icon={ListChecks} title="Key results" blurb="Add the measurable results — baseline → target. Weights drive the roll-up." />
                    <div className="flex flex-col gap-4 lg:flex-row">
                        <div className="min-w-0 flex-1">
                            <div className="flex flex-col gap-3">
                                {krs.map((k, i) => (
                                    <div key={k.key} className="rounded-xl border border-border bg-sidebar p-3.5">
                                        <div className="mb-2.5 flex items-center gap-2">
                                            ...
                                            <Input
                                                value={k.title}
                                                onChange={(e) => updateKr(k.key, { title: e.target.value })}
                                            ...
                                            {krs.length > 1 && (
                                                <button
                                                    ...
                                                    onClick={() => setKrs((prev) => prev.filter((x) => x.key !== k.key))}
                                        ...
                                        <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">
                                            <label className="block">
                                                <span className="mb-1 block text-[10px] font-semibold text-muted-foreground">Type</span>
                                                <SelectInput
                                                    value={k.kr_type}
                                                    onChange={(v) => updateKr(k.key, { kr_type: v as KrType })}
                                                    placeholder="Type"
                                                    options={KR_TYPE_OPTIONS}
                                                />
                                            </label>
                                            <label className="block">
                                                <span className="mb-1 block text-[10px] font-semibold text-muted-foreground">Baseline</span>
                                                <Input
                                                    value={k.start_value}
                                                    onChange={(e) => updateKr(k.key, { start_value: e.target.value })}
                                                    className="h-8"
                                                />
                                            </label>
                                            <label className="block">
                                                <span className="mb-1 block text-[10px] font-semibold text-muted-foreground">Target</span>
                                                <Input
                                                    value={k.target_value}
                                                    onChange={(e) => updateKr(k.key, { target_value: e.target.value })}
                                                    className="h-8"
                                                />
                                            </label>
                                            <label className="block">
                                                <span className="mb-1 block text-[10px] font-semibold text-muted-foreground">Unit</span>
                                                <Input
                                                    value={k.unit}
                                                    onChange={(e) => updateKr(k.key, { unit: e.target.value })}
                                                    className="h-8"
                                                />
                                            </label>
                                            <label className="block">
                                                <span className="mb-1 block text-[10px] font-semibold text-muted-foreground">Weight</span>
                                                <Input
                                                    value={k.weight}
                                                    onChange={(e) => updateKr(k.key, { weight: e.target.value })}
                                                    className="h-8"
                                                />
                                            </label>
                                        </div>
                                ...
                                >
                                    <Plus className="h-4 w-4" /> Add another key result
                                </button>
                            </div>
                        </div>
                        <div className="w-full shrink-0 lg:w-[188px]">
                            <div className="rounded-xl border border-border bg-gradient-to-b from-primary/5 to-card p-4 text-center">
                                <div className="mb-3 text-[10.5px] font-bold uppercase tracking-wide text-muted-foreground">Roll-up preview</div>
                                <div
                                    className="relative mx-auto h-24 w-24 rounded-full"
                                    style={{ background: `conic-gradient(var(--primary) ${previewRollup}%, var(--muted) 0)` }}
                                >
                                    <div className="absolute inset-[9px] grid place-items-center rounded-full bg-card text-2xl font-extrabold tabular-nums">
                                        {previewRollup}%
                                    </div>
                                </div>
                                <p className="mt-3 text-[11.5px] leading-snug text-muted-foreground">Weighted from your key results at their baseline.</p>
                            </div>
                        </div>
                    </div>
                </WizardStepPane>
            ...
            {stepKey === 'timing' && (
                <WizardStepPane>
                    <StepHead icon={CalendarClock} title="Timing & confidence" blurb="The objective window and your starting confidence." />
                    <div className="grid max-w-xl gap-4 sm:grid-cols-2">
                        ...
                        <Field label="Check-in cadence" span>
                            <Segmented
                                ...
                                onChange={(v) => form.setData('checkin_frequency', v)}
                                options={[
            ...
            {stepKey === 'review' && (
                <WizardStepPane>
                    <StepHead icon={ClipboardCheck} title="Review & create" blurb="Confirm the objective — its key results attach in the same step." />
                    <div className="flex flex-wrap gap-4">
                        <div className="min-w-[280px] flex-1">
                            <ReviewCard icon={Target} title="Objective" onEdit={() => wizard.goTo(0)}>
                                <ReviewRow label="Owner" value={ownerName} />
                                <ReviewRow label="Title" value={form.data.title} />
                                <ReviewRow label="Type" value={form.data.goal_type} />
                                <ReviewRow label="Priority" value={form.data.priority} />
                                <ReviewRow label="Category" value={form.data.category || undefined} />
                                <ReviewRow
                                    label="Cycle"
                                    value={cycles.find((c) => String(c.id) === form.data.cycle_id)?.name}
                                />
                                <ReviewRow
                                    label="Window"
                                    value={
                                ...
                                />
                                {!isEdit && <ReviewRow label="Key results" value={krs.filter((k) => k.title.trim()).length || undefined} />}
                            </ReviewCard>
                        </div>
                        {!isEdit && (
                            <div className="w-[170px] shrink-0 rounded-xl border border-border bg-sidebar p-4 text-center">
                                <div className="mb-2.5 text-[10.5px] font-bold uppercase tracking-wide text-muted-foreground">Roll-up</div>
                                <div className="text-3xl font-extrabold tabular-nums">{previewRollup}%</div>
                            </div>
                        )}
                    </div>
                </WizardStepPane>
