 ...
 * shadcn <Button>/<Card> case. Generalised from leave-context-menu.tsx so any HR
 * hub (assets, …) can attach a row/tab context menu. */
...
/**
 * Right-click / ⋯ context menu for any HR hub row, card or tab. `open(items)`
 * returns an event handler usable for both `onContextMenu` and a button
 * `onClick`, opening the menu at the cursor. Render `element` once in the host.
 */
export function useRowContextMenu() {
    ...
    const open = (items: RowCtxItem[]) => (e: React.MouseEvent) => {
        e.preventDefault();
        e.stopPropagation();
