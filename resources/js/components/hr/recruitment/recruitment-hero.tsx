 ...
 * injected as a CSS var) plus the recruitment stage hue scale, so application
 * brand theming still propagates. */
import {
    CalendarPlus,
    Download,
    FilePlus2,
    ...
    return (
        ...
        >
            ...
            <div className="relative flex flex-wrap items-stretch">
                    ...
                    <div className="mt-[18px] flex flex-wrap gap-2">
                        ...
                        {canManage && handlers?.onNewRequisition ? (
                            <QuickAction
                                ...
                                onClick={handlers.onNewRequisition}
                            />
                        ) : null}
                        ...
                        {handlers?.onReviewOffers ? (
                            <QuickAction
                                icon={Send}
                                label="Review offers"
                                onClick={handlers.onReviewOffers}
                            />
                        ) : null}
                        {handlers?.onExport ? (
                            <QuickAction
                ...
                <div className="flex w-full flex-none flex-col border-t border-primary-foreground/15 bg-black/[0.08] p-[24px] sm:w-[340px] sm:border-t-0 sm:border-l">
                    <div className="mb-2 flex items-center justify-between">
                        ...
                        <div className="inline-flex gap-0.5 rounded-lg bg-primary-foreground/[0.12] p-0.5">
                            <RailTab
                                label="Funnel"
                                active={right === 'funnel'}
                                onClick={() => setHero('funnel')}
                            />
                            <RailTab
                                label="Speed"
                                active={right === 'speed'}
                                onClick={() => setHero('speed')}
                            />
                        </div>
                    </div>
        ...
        <div className="mt-1.5 flex flex-col gap-2">
            {funnel.map((f) => (
                <div key={f.key} className="flex items-center gap-2.5">
                    <span className="w-[62px] flex-none text-[11px] font-semibold whitespace-nowrap text-primary-foreground/85">
                        {f.label}
                    </span>
                    ...
                    <span className="w-5 flex-none text-right text-xs font-bold text-primary-foreground tabular-nums">
                        {f.count}
                    </span>
                </div>
            ))}
        ...
        <div className="mt-2 flex flex-col gap-3.5">
            {stats.map((s) => (
                <div key={s.label}>
                    ...
                    <div className="mt-0.5 text-[11px] text-primary-foreground/65">
                        {s.label}
                    </div>
                </div>
            ))}
