...
export function RepresentativeDetailDialog({
    ...
    const [action, setAction] = useState<WpDetailAction | null>(
        initialAction === 'edit' || initialAction === 'training'
            ? initialAction
            : null,
    );
    ...
    const SECTIONS: {
    ...
    }[] = [
        {
            key: 'overview',
            label: 'Overview',
            blurb: 'Mandate & training',
            icon: UserRound,
        },
        {
            key: 'history',
            label: 'History',
            blurb: 'Record & training',
            icon: History,
        },
    ];
    ...
    const footerStart = (
        <div className="flex items-center gap-2 text-xs">
            <span className="inline-flex items-center gap-1 rounded-full bg-muted px-2 py-0.5 font-medium">
                <span
                    className={`h-1.5 w-1.5 rounded-full ${DOT[statusTone]}`}
                />
...
const STATUS_OPTS: {
    value: 'active' | 'inactive' | 'resigned';
    label: string;
}[] = [
...
function EditPane({ d, onDone }: { d: RepDetail; onDone: () => void }) {
    const form = useForm<{
        ...
        term_expires_at: d.term_expires_at
            ? d.term_expires_at.slice(0, 10)
            : '',
        status: (['active', 'inactive', 'resigned'].includes(d.status)
            ? d.status
            : 'active') as 'active' | 'inactive' | 'resigned',
        notes: d.notes ?? '',
    });
    const submit = (e: FormEvent) => {
        ...
        if (form.data.training_days_completed.trim() === '') {
            form.setError(
                ...
                'Enter the number of paid training days completed.',
            );
            return;
        }
    ...
    return (
        <>
            <StepHead
                ...
                blurb="Update the rep's mandate, training record and standing. The person and site can't be reassigned here."
            />
            <form onSubmit={submit} className="flex flex-col gap-4">
                <div className="grid gap-3 sm:grid-cols-2">
                    <Field
                        label="Work group"
                        hint="Optional"
                        error={form.errors.work_group}
                    >
                        <Input
                            value={form.data.work_group}
                            onChange={(e) =>
                                form.setData('work_group', e.target.value)
                            }
                    ...
                    <Field
                        ...
                        error={form.errors.training_days_completed}
                    >
                        <Input
                            type="number"
                            min={0}
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                    <Field
                        label="Term expires"
                        hint="Optional"
                        error={form.errors.term_expires_at}
                    >
                        <Input
                            type="date"
                            value={form.data.term_expires_at}
                            onChange={(e) =>
                                form.setData('term_expires_at', e.target.value)
                            }
                        />
                    </Field>
...
function TrainingPane({ d, onDone }: { d: RepDetail; onDone: () => void }) {
    const form = useForm<{
        ...
        training_days_completed: String(d.training_days_completed ?? 0),
        initial_training_completed_at: d.initial_training_completed_at
            ? d.initial_training_completed_at.slice(0, 10)
            : '',
    });
    const submit = (e: FormEvent) => {
        e.preventDefault();
    ...
    return (
        <>
            <StepHead
                icon={GraduationCap}
                title="Record training days"
                blurb="Update the cumulative paid H&S training days this rep has completed."
            />
            <form onSubmit={submit} className="flex flex-col gap-4">
                <Field
                    label="Training days completed"
                ...
                <Field
                    ...
                    error={form.errors.initial_training_completed_at}
                >
                    <Input
                        type="date"
                ...
                <InfoCard icon={GraduationCap} tone="info">
                    Under HSWA 2015 an elected H&amp;S representative is
                    entitled to <b>two days&apos; paid training each year</b>.
                    Approved training maps to <b>NZQA Unit Standard 29315</b>{' '}
                    (HSR training). Recording the completion date adds a tracked
        ...
        <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="min-w-0">
                    ...
                    <p className="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-xs text-muted-foreground">
                        ...
                        <span className="inline-flex items-center gap-1">
                            ...
                            {d.work_group ?? 'All kaimahi'}
                        </span>
                    </p>
                </div>
                <span
                    className={`ml-auto inline-flex shrink-0 items-center gap-1.5 rounded-full bg-muted px-2.5 py-1 text-[11px] font-semibold`}
                >
                    <span
                        className={`h-1.5 w-1.5 rounded-full ${DOT[statusTone]}`}
                    />
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2">
                        <span>
                            <span className="font-semibold">
                                Below the training minimum.
                            </span>{' '}
                            This rep has {trainingDays}{' '}
                            {trainingDays === 1 ? 'day' : 'days'} — HSWA 2015
                            entitles them to two paid days a year.
                        </span>
                        {onTraining ? (
            ...
            <ReviewCard icon={ShieldCheck} title="Mandate">
                <ReviewRow
                    ...
                    value={
                        d.election_method
                            ? titleCase(d.election_method)
                            : undefined
                    }
                />
                <ReviewRow
                    label="Elected / appointed"
                    value={d.elected_at ? fmtDate(d.elected_at) : undefined}
                />
                <ReviewRow
                    label="Term ends"
                    value={
                ...
                <ReviewRow
                    label="Status"
                    value={STATUS_LABEL[d.status] ?? titleCase(d.status)}
                />
            </ReviewCard>
            ...
            <ReviewCard icon={GraduationCap} title="Training">
                ...
                <ReviewRow
                    label="Standard met"
                    value={belowMin ? 'Below minimum' : 'Minimum met'}
                />
                <ReviewRow
                    label="First trained"
                    value={
                        d.initial_training_completed_at
                            ? fmtDate(d.initial_training_completed_at)
                            : undefined
                    }
                />
            </ReviewCard>
...
function HistorySection({ d }: { d: RepDetail }) {
    ...
    if (d.elected_at) {
        const verb = d.election_method
            ? titleCase(d.election_method)
            : 'Mandated';
        events.push({
            at: d.elected_at,
            label: `${verb} as H&S representative`,
            tone: 'success',
            icon: ShieldCheck,
        });
    }
    if (d.initial_training_completed_at) {
        events.push({
            at: d.initial_training_completed_at,
            label: 'Initial HSWA training completed',
            tone: 'success',
            icon: GraduationCap,
        });
    }
        ...
        <div className="flex flex-col gap-5">
                    ...
                    {events.map((e, i) => {
                        ...
                        return (
                            <li key={i} className="mb-5 ml-5">
                                <span
                                    className={`absolute -left-[7px] flex h-3.5 w-3.5 items-center justify-center rounded-full ${DOT[e.tone]}`}
                                />
            ...
            <ReviewCard icon={Clock} title="Record details" span>
                <ReviewRow
                    ...
                    value={d.created_at ? fmtDateTime(d.created_at) : undefined}
                />
