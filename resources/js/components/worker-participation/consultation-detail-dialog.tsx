...
import { WizardShell, ReviewCard, ReviewRow } from '@/components/wizard/shell';
import { Field, StepHead, Segmented, InfoCard } from '@/components/wizard/primitives';
import {
    WP_BASE,
    CONSULT_ORDER,
    CONSULT_STATUS,
...
export function ConsultationDetailDialog({
    ...
    const initialSection: SectionKey =
        initialAction === 'feedback' || initialAction === 'outcome' || initialAction === 'close'
            ? 'lifecycle'
    ...
    const SECTIONS: { key: SectionKey; label: string; blurb: string; icon: ComponentType<{ className?: string }> }[] = [
        { key: 'overview', label: 'Overview', blurb: 'Topic, site & detail', icon: FileText },
        { key: 'lifecycle', label: 'Lifecycle', blurb: `${st.label} · feedback → outcome`, icon: ListChecks },
        { key: 'documents', label: 'Documents', blurb: `${docs} file${docs === 1 ? '' : 's'} attached`, icon: ClipboardList },
    ];
    ...
    const footerStart = (
        <div className="flex flex-wrap items-center gap-2 text-xs">
            <span className="inline-flex items-center gap-1.5 rounded-md bg-muted px-2 py-0.5 font-medium text-foreground">
                <MessageSquare className="h-3 w-3" /> {consultationTypeLabel(d.consultation_type)}
            </span>
    ...
    const footerEnd =
        action || !can.manage || closed ? null : (
            <div className="flex flex-wrap items-center justify-end gap-2">
                <OptionBtn icon={MessageSquare} label="Record feedback" onClick={() => setAction('feedback')} />
                <OptionBtn icon={FileCheck2} label="Record outcome" onClick={() => setAction('outcome')} />
                <OptionBtn icon={Upload} label="Upload document" onClick={() => setAction('upload')} />
                <OptionBtn icon={XCircle} label="Close consultation" tone="critical" onClick={() => setAction('close')} />
            </div>
        );
    ...
    return (
        ...
        >
            {action === 'edit' ? (
                ...
                <>
                    {section === 'overview' ? (
                        <OverviewSection d={d} onEdit={can.manage && !closed ? () => setAction('edit') : undefined} />
                    ) : null}
                    {section === 'lifecycle' ? <LifecycleSection d={d} /> : null}
                    {section === 'documents' ? <DocumentsSection d={d} /> : null}
                </>
            )}
        </WizardShell>
    );
...
function OptionBtn({
    ...
    return (
        <Button
            ...
            variant="outline"
            onClick={onClick}
            className={tone === 'critical' ? 'text-status-critical hover:text-status-critical' : undefined}
        >
...
function OverviewSection({ d, onEdit }: { d: ConsultationDetail; onEdit?: () => void }) {
    return (
        <div className="grid gap-4 sm:grid-cols-2">
            <ReviewCard icon={MessageSquare} title="Consultation" span onEdit={onEdit}>
                <ReviewRow label="Topic" value={d.title} />
                <ReviewRow label="Type" value={consultationTypeLabel(d.consultation_type)} />
...
function LifecycleSection({ d }: { d: ConsultationDetail }) {
    ...
    return (
        <div className="flex flex-col gap-5">
            <p className="text-[11px] font-semibold tracking-wide text-muted-foreground uppercase">Consultation lifecycle</p>
            <ol className="relative ml-2 border-l border-border">
                {CONSULT_STAGES.map((stage, i) => {
                    const done = i < idx;
                    const current = i === idx;
                    const tone: Tone = done ? 'success' : current ? 'warning' : 'neutral';
                    return (
                        <li key={stage.key} className="mb-6 ml-5 last:mb-0">
                            <span
                                className={`absolute -left-[7px] flex h-3.5 w-3.5 items-center justify-center rounded-full ${
                                    done ? 'bg-status-success' : current ? 'bg-primary' : 'bg-muted-foreground/40'
                                }`}
                            >
                                {done ? <CheckCircle2 className="h-3 w-3 text-primary-foreground" /> : null}
                            </span>
                            <div className="flex flex-wrap items-center gap-2">
                                <span className={`text-sm font-semibold ${current || done ? 'text-foreground' : 'text-muted-foreground'}`}>
                                    {stage.label}
                                </span>
                                <StageChip tone={tone}>{done ? 'Done' : current ? 'Current' : 'Upcoming'}</StageChip>
                            </div>
                            <p className="mt-0.5 text-xs text-muted-foreground">{stage.blurb}</p>

                            {stage.key === 'feedback_received' && (done || current) ? (
                                <DetailNote icon={MessageSquare} title="Worker feedback" body={d.worker_feedback_summary} />
                            ) : null}
                            {stage.key === 'actioned' && (done || current) ? (
                                <>
                                    <DetailNote icon={FileCheck2} title="Outcome" body={d.outcome} />
                                    <DetailNote icon={ListChecks} title="Changes made" body={d.changes_made} />
                                </>
                            ) : null}
                        </li>
                    );
...
function StageChip({ tone, children }: { tone: Tone; children: ReactNode }) {
    ...
    return <span className={`inline-flex rounded-full px-2 py-0.5 text-[11px] font-medium ${cls[tone]}`}>{children}</span>;
}

function DetailNote({ icon: Icon, title, body }: { icon: LucideIcon; title: string; body: string | null | undefined }) {
    return (
        <div className="mt-2 rounded-lg border border-border bg-muted/30 p-3">
            ...
            <p className="text-[13px] whitespace-pre-wrap text-foreground">{body || '—'}</p>
        </div>
    );
}
            ...
            <div className="flex min-w-0 items-center gap-3">
                <span
                    className={`grid h-10 w-10 shrink-0 place-items-center rounded-lg ${
                        present ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground/50'
                    }`}
                >
                ...
                <div className="min-w-0">
                    ...
                    <p className="truncate text-xs text-muted-foreground">{present ? (name ?? 'Document attached') : 'None attached'}</p>
                </div>
            </div>
            {present ? (
                <a
        ...
        <form onSubmit={onSubmit} className="flex flex-col gap-4">
            ...
            <div className="flex justify-end gap-2">
                ...
                <Button
                    type="submit"
                    disabled={processing}
                    className={ctaTone === 'critical' ? 'bg-status-critical text-primary-foreground hover:bg-status-critical/90' : undefined}
                >
...
function EditPane({ d, onDone }: { d: ConsultationDetail; onDone: () => void }) {
    const form = useForm<{ title: string; consultation_type: string; description: string; consultation_date: string }>({
        title: d.title ?? '',
    ...
    const submit = (e: FormEvent) => {
        ...
        if (!form.data.description.trim()) {
            form.setError('description', 'Describe what is being consulted on.');
            return;
        }
    ...
    return (
        <>
            <StepHead icon={Pencil} title="Edit consultation" blurb="Update the topic, type, detail or date kaimahi were consulted." />
            <PaneShell onCancel={onDone} onSubmit={submit} cta="Save changes" processing={form.processing}>
                <Field label="Topic" required error={form.errors.title}>
                    <Input value={form.data.title} onChange={(e) => form.setData('title', e.target.value)} placeholder="e.g. New manual-handling procedure" />
                </Field>
                <Field label="Consultation type" required error={form.errors.consultation_type}>
                    <Segmented
                        value={form.data.consultation_type}
                        onChange={(v) => form.setData('consultation_type', v)}
                        options={CONSULTATION_TYPES.map((t) => ({ value: t.key, label: t.label, icon: t.icon }))}
                    />
                </Field>
                <Field label="What is being consulted on" required error={form.errors.description}>
                    <textarea
                ...
                <Field label="Consultation date" error={form.errors.consultation_date}>
                    <Input type="date" value={form.data.consultation_date} onChange={(e) => form.setData('consultation_date', e.target.value)} />
                </Field>
            </PaneShell>
        </>
    );
...
function FeedbackPane({ d, onDone }: { d: ConsultationDetail; onDone: () => void }) {
    // Non-regressing: never downgrade the lifecycle — send the LATER of the
    // current status and this pane's milestone.
    const target = CONSULT_ORDER[d.status] >= CONSULT_ORDER['feedback_received'] ? d.status : 'feedback_received';
    ...
    const submit = (e: FormEvent) => {
        ...
        if (!form.data.worker_feedback_summary.trim()) {
            form.setError('worker_feedback_summary', 'Summarise the feedback kaimahi gave.');
            return;
        }
    ...
    return (
        <>
            <StepHead icon={MessageSquare} title="Record worker feedback" blurb="Capture what kaimahi said — this moves the consultation to Feedback received." />
            <PaneShell onCancel={onDone} onSubmit={submit} cta="Record feedback" ctaIcon={MessageSquare} processing={form.processing}>
                <Field label="Worker feedback summary" required error={form.errors.worker_feedback_summary}>
                    <textarea
                        className={TEXTAREA_CLASS}
                        rows={5}
                        value={form.data.worker_feedback_summary}
                        onChange={(e) => form.setData('worker_feedback_summary', e.target.value)}
...
function OutcomePane({ d, onDone }: { d: ConsultationDetail; onDone: () => void }) {
    // Non-regressing: never downgrade the lifecycle — send the LATER of the
    // current status and this pane's milestone.
    const target = CONSULT_ORDER[d.status] >= CONSULT_ORDER['actioned'] ? d.status : 'actioned';
    const form = useForm<{ status: string; outcome: string; changes_made: string }>({
        status: target,
        outcome: d.outcome ?? '',
    ...
    return (
        <>
            <StepHead icon={FileCheck2} title="Record outcome" blurb="Document the decision and what changed as a result — this moves the consultation to Actioned." />
            <PaneShell onCancel={onDone} onSubmit={submit} cta="Record outcome" ctaIcon={FileCheck2} processing={form.processing}>
                ...
                <Field label="Changes made" hint="Optional" error={form.errors.changes_made}>
                    <textarea
                        className={TEXTAREA_CLASS}
                        rows={3}
                        value={form.data.changes_made}
                        onChange={(e) => form.setData('changes_made', e.target.value)}
...
function UploadPane({ d, onDone }: { d: ConsultationDetail; onDone: () => void }) {
    const { data, setData, post, processing, errors } = useForm<{ document: File | null; type: 'document' | 'outcome' }>({
        document: null,
        type: 'document',
    });
    const submit = () => {
    ...
    return (
        <>
            <StepHead icon={Upload} title="Upload a document" blurb="Attach the supporting material or the signed-off outcome for this consultation." />
            <form
                onSubmit={(e) => {
                    e.preventDefault();
                    submit();
            ...
            >
                <Field label="Document type" required>
                    <Segmented
                        value={data.type}
                        onChange={(v) => setData('type', v as 'document' | 'outcome')}
                        options={[
                            { value: 'document', label: 'Supporting document', icon: FileText },
                            { value: 'outcome', label: 'Outcome document', icon: FileCheck2 },
                        ]}
                    />
                </Field>
...
function ClosePane({ d, onDone }: { d: ConsultationDetail; onDone: () => void }) {
    ...
    return (
        <>
            <StepHead icon={XCircle} title="Close consultation" blurb="Mark this consultation as complete." />
            <PaneShell onCancel={onDone} onSubmit={submit} cta="Close consultation" ctaIcon={CheckCircle2} ctaTone="critical" processing={form.processing}>
                <InfoCard icon={CheckCircle2} tone="warn">
                    Closing records the consultation as complete. Make sure worker feedback and the outcome have been captured first — a closed
                    consultation can no longer be edited from this dialog.
                </InfoCard>
                <div className="rounded-xl border border-border bg-muted/20 p-3.5">
                    <p className="flex items-center gap-1.5 text-[13px] font-semibold text-foreground">
                        <UserCircle2 className="h-4 w-4 text-muted-foreground" /> {d.title}
                    </p>
                    <p className="mt-1 text-xs text-muted-foreground">
                        {consultationTypeLabel(d.consultation_type)} · {d.site?.name ?? 'All sites'} · {fmtDate(d.consultation_date)}
                    </p>
                </div>
