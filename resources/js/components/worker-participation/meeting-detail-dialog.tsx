...
export function MeetingDetailDialog({
    ...
    const isMeetingAction = (
        a: WpDetailAction | null,
    ): a is 'edit' | 'attendees' | 'complete' | 'minutes' =>
        a === 'edit' ||
        a === 'attendees' ||
        a === 'complete' ||
        a === 'minutes';
    const [section, setSection] = useState<SectionKey>(
        initialAction === 'attendees'
            ? 'attendees'
            : initialAction === 'complete' || initialAction === 'edit'
              ? 'agenda'
              : initialAction === 'minutes'
                ? 'minutes'
                : 'overview',
    );
    const [action, setAction] = useState<
        'edit' | 'attendees' | 'complete' | 'minutes' | null
    >(isMeetingAction(initialAction) ? initialAction : null);
    ...
    const SECTIONS: {
    ...
    }[] = [
        {
            key: 'overview',
            label: 'Overview',
            blurb: 'Committee & schedule',
            icon: FileText,
        },
        {
            key: 'attendees',
            label: 'Attendees',
            blurb: `${attendees.length} invited`,
            icon: Users,
        },
        {
            key: 'agenda',
            label: 'Agenda & actions',
            blurb: `${agenda.length} item${agenda.length === 1 ? '' : 's'} · ${actions.length} action${actions.length === 1 ? '' : 's'}`,
            icon: ClipboardList,
        },
        {
            key: 'minutes',
            label: 'Minutes',
            blurb: d.minutes_document_name
                ? 'Filed'
                : d.minutes
                  ? 'Recorded'
                  : 'None yet',
    ...
    const footerEnd =
        action || !can.manage || cancelled ? null : (
            <div className="flex flex-wrap items-center justify-end gap-2">
                ...
                {d.minutes_document_path ? null : (
                    <OptionBtn
                        icon={Upload}
                        label="Upload minutes"
                        onClick={() => setAction('minutes')}
                    />
                )}
                {scheduled ? (
                    <OptionBtn
            ...
            {action === 'edit' ? (
                ...
                <>
                    ...
                    {section === 'minutes' ? (
                        <MinutesSection
                            d={d}
                            canManage={can.manage && !cancelled}
                            onUpload={() => setAction('minutes')}
                        />
                    ) : null}
                </>
            )}
...
function ActionRepeater({
    ...
    const set = (i: number, patch: Partial<ActionItem>) =>
        onChange(
                ...
                <div key={i} className="rounded-xl border border-border p-3">
                    ...
                    <div className="grid gap-2 sm:grid-cols-3">
                        <SelectInput
                            value={
                                it.assigned_to != null
                                    ? String(it.assigned_to)
                                    : ''
                            }
                            onChange={(v) =>
                                set(i, { assigned_to: v ? Number(v) : null })
                            }
                        ...
                        <Input
                            ...
                            value={it.due_date ?? ''}
                            onChange={(e) =>
                                set(i, { due_date: e.target.value || null })
                            }
                        ...
                        <SelectInput
                            ...
                            onChange={(v) =>
                                set(i, { status: v as ActionItem['status'] })
                            }
                            placeholder="Status"
                            options={[
    ...
    return (
        <>
            <StepHead
                icon={CalendarClock}
                title="Edit meeting"
                blurb="Adjust the schedule, location and agenda for this committee meeting."
            />
            <PaneShell
                onCancel={onDone}
                onSubmit={submit}
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    >
                        <Input
                            ...
                            placeholder="e.g. Site office — meeting room"
                        />
                    </Field>
                </div>
                <Field label="Agenda items" hint="Optional">
                    <AgendaRepeater
                        items={form.data.agenda_items}
                        onChange={(v) => form.setData('agenda_items', v)}
                    />
                </Field>
            </PaneShell>
        </>
    ...
    return (
        <>
            <StepHead
                icon={UserPlus}
                title="Add attendees"
                blurb="Invite kaimahi to this meeting. People already on the list aren't shown."
            />
            <PaneShell
                onCancel={onDone}
                onSubmit={submit}
                cta={`Add ${form.data.user_ids.length || ''} attendee${form.data.user_ids.length === 1 ? '' : 's'}`.trim()}
                processing={form.processing}
            >
                <Field label="Invite" required error={form.errors.user_ids}>
                    {selectable.length ? (
                        <StaffMultiSelect
                            staff={selectable}
                            selected={form.data.user_ids}
                            onChange={(ids) => form.setData('user_ids', ids)}
                        />
                    ) : (
                        <p className="text-sm text-muted-foreground">
    ...
    return (
        <>
            <StepHead
                ...
                blurb="Capture the minutes, record who attended, and log any action items with owners and due dates."
            />
            <PaneShell
                onCancel={onDone}
                onSubmit={submit}
                cta="Complete meeting"
                processing={form.processing}
            >
                <Field
                    label="Minutes"
                    hint="Optional — a summary; you can also upload a signed copy"
                ...
                >
                    <textarea
                        ...
                        onChange={(e) =>
                            form.setData('minutes', e.target.value)
                        }
                ...
                <Field label="Who attended" hint="Defaults to everyone invited">
                    {invited.length ? (
                        <StaffMultiSelect
                            ...
                            selected={form.data.actual_attendee_ids}
                            onChange={(ids) =>
                                form.setData('actual_attendee_ids', ids)
                            }
                        />
                    ) : (
                        <p className="text-sm text-muted-foreground">
                            No attendees were invited to this meeting.
                        </p>
                    )}
                </Field>
                <Field label="Action items" hint="Optional">
                    <ActionRepeater
                        items={form.data.action_items}
                        onChange={(v) => form.setData('action_items', v)}
                        staff={staff}
                    />
                </Field>
            </PaneShell>
    ...
    return (
        <>
            <StepHead
                ...
                blurb="Attach the signed minutes document for this meeting (PDF or Word)."
            />
            <div className="flex flex-col gap-4">
                {data.document ? (
                    ...
                    <FileDropzone
                        ...
                        onFiles={(f) => setData('document', f[0])}
                    />
                )}
                {errors.document ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2">
            ...
            <ReviewCard icon={CalendarClock} title="Schedule">
                ...
                {completed ? (
                    <ReviewRow
                        label="Ended"
                        value={d.ended_at ? fmtDateTime(d.ended_at) : undefined}
                    />
                ) : null}
            </ReviewCard>
            ...
            <ReviewCard icon={MapPin} title="Location">
                ...
                <ReviewRow
                    label="Attendees"
                    value={`${(d.attendee_users ?? []).length} invited`}
                />
            </ReviewCard>
            ...
            {d.actions_due_count > 0 ? (
                <ReviewCard icon={ListTodo} title="Outstanding actions" span>
                    <p className="text-sm text-foreground">
                        {d.actions_due_count} action{' '}
                        {d.actions_due_count === 1 ? 'item is' : 'items are'}{' '}
                        due — see{' '}
...
function AttendeesSection({
    ...
    attendees: NonNullable<MeetingDetail['attendee_users']>;
}) {
    if (!attendees.length) {
    ...
    return (
        <div className="flex flex-col gap-2">
            {attendees.map((a) => {
                ...
                const badgeTone: Tone = attended
                    ? 'success'
                    : declined
                      ? 'critical'
                      : 'neutral';
                const badgeLabel = attended
                    ? 'Attended'
                    : declined
                      ? 'Declined'
                      : 'Invited';
                ...
                return (
                    ...
                    >
                        ...
                        <span
                            className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[11px] font-medium ${chipClass(badgeTone)}`}
                        >
...
function AgendaSection({
    ...
    const nameOf = (id: number | null | undefined) =>
        id != null
            ? (staff.find((s) => s.id === id)?.name ?? `User #${id}`)
            : null;
    return (
                ...
                {agenda.length ? (
                    <ol className="flex flex-col gap-1.5">
                        {agenda.map((item, i) => (
                            <li
                                key={i}
                                className="flex items-start gap-2.5 rounded-lg border border-border p-2.5 text-sm"
                            >
                                <span className="grid h-5 w-5 shrink-0 place-items-center rounded-md bg-muted text-[11px] font-semibold text-muted-foreground">
                                    {i + 1}
                                </span>
                    ...
                    <div className="flex flex-col gap-2">
                        {actions.map((a, i) => {
                            ...
                            return (
                                ...
                                >
                                    <ListTodo
                                        className={`mt-0.5 h-4 w-4 shrink-0 ${st.tone === 'success' ? 'text-status-success' : st.tone === 'warning' ? 'text-status-warning' : 'text-muted-foreground'}`}
                                    />
                                    ...
                                    <span
                                        className={`shrink-0 rounded-full px-2.5 py-0.5 text-[11px] font-medium ${chipClass(st.tone)}`}
                                    >
                                        {st.label}
                                    </span>
                                </div>
    ...
    return (
        <div className="flex flex-col gap-4">
                ...
                <div className="flex items-center gap-3 rounded-lg border border-border p-3">
                    ...
                    <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-medium text-foreground">
                            {d.minutes_document_name ?? 'Minutes document'}
                        </p>
                        <p className="text-xs text-muted-foreground">
                            Filed minutes
                        </p>
                    ...
                    <a
                        href={`${WP_BASE}/meetings/${d.id}/minutes/download`}
            ...
            {!d.minutes && !hasDoc ? (
                <EmptyState
                    icon={ClipboardCheck}
                    text="No minutes recorded or filed yet."
                />
            ) : null}
