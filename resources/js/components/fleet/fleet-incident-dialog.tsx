...
export function FleetIncidentDialog({
    ...
    const SECTIONS: (WizardStep & { key: SectionKey })[] = [
        ...
        },
        {
            key: 'vehicle',
            label: 'Vehicle / asset',
            blurb: d.asset?.name ?? 'No asset',
            icon: Truck,
        },
        {
            key: 'people',
            label: 'People',
            blurb: `${d.people_aboard_count ?? d.people_aboard?.length ?? 0} aboard`,
            icon: Users,
        },
        {
            key: 'scene',
            label: 'Scene & conditions',
            blurb: d.road_type ?? 'Location & road',
            icon: MapPin,
        },
        {
            key: 'damage',
            label: 'Damage & recovery',
            blurb: d.damage_classification
                ? titleCase(d.damage_classification)
                : 'VOR / tow',
            icon: Wrench,
        },
        {
            ...
            blurb: d.requires_police_report ? 's22 duty' : 'NZ regs',
            icon: ShieldAlert,
        },
        {
            key: 'insurance',
            label: 'Insurance & cost',
            blurb: d.insurer_name ?? 'Claim / cost',
            icon: CreditCard,
        },
        {
            key: 'photos',
        ...
        {
            key: 'investigation',
            label: 'Investigation & follow-ups',
            blurb: openFollowups > 0 ? `${openFollowups} open` : 'Root cause',
            icon: Search,
        },
        {
            key: 'linked',
            label: 'Linked records',
            blurb: 'H&S · incidents · asset',
            icon: Link2,
        },
    ];
    ...
    const footerStart = (
        <div className="flex items-center gap-2 text-xs">
            <span className="inline-flex items-center gap-1 rounded-full bg-muted px-2 py-0.5 font-medium">
                <span
                    className={`h-1.5 w-1.5 rounded-full ${SEV_DOT[d.severity] ?? 'bg-muted-foreground'}`}
                />
    ...
    const footerEnd = action ? null : (
        <div className="flex flex-wrap items-center gap-2">
            <Link
                href={`/fleet-assets/incidents?incident=${d.id}`}
                ...
                <>
                    ...
                    {d.is_police_report_due ? (
                        <Button
                            size="sm"
                            variant="outline"
                            onClick={() => setAction('police_report')}
                        >
                            <ShieldAlert className="mr-1.5 h-4 w-4" /> Log
                            Police report
                        </Button>
                    ) : null}
                    {!d.insurance_claimed ? (
                    ...
                    {d.is_off_road ? (
                        <Button
                            size="sm"
                            variant="outline"
                            onClick={() => setAction('back_in_service')}
                        >
                            <CircleSlash className="mr-1.5 h-4 w-4" /> Back in
                            service
                        </Button>
                    ) : (
                        <Button
                            size="sm"
                            variant="outline"
                            onClick={() => setAction('off_road')}
                        >
                            <CircleSlash className="mr-1.5 h-4 w-4" /> Mark
                            off-road
                        </Button>
                    )}
                </>
    ...
    return (
        <WizardShell
            ...
            railIcon={
                d.asset?.category && d.asset.category !== 'vehicle'
                    ? Box
                    : Truck
            }
    ...
    return (
        <div className="flex items-center gap-1">
            {STAGES.map((s, i) => (
                ...
                >
                    <div
                        className={`h-1.5 w-full rounded-full ${i <= cur ? 'bg-primary' : 'bg-muted'}`}
                    />
                    <span
                        className={`text-[10px] font-semibold ${i === cur ? 'text-primary' : 'text-muted-foreground'}`}
                    >
                        {titleCase(s)}
                    </span>
                </div>
...
function OverviewSection({ d }: { d: FleetIncidentDetail }) {
    return (
        <div className="grid gap-4 sm:grid-cols-2">
            {d.is_police_report_due ? (
                <div className="sm:col-span-2">
                    <InfoCard icon={ShieldAlert} tone="crit">
                        <strong>Police report due (s22 LTA 1998).</strong>{' '}
                        Injury/fatal crashes must be reported to NZ Police
                        within 24 hours via 105 or a Traffic Crash Report.
                        {d.police_report_due_at
                            ? ` Due ${formatDateTime(d.police_report_due_at)}.`
                            : ''}
                        {d.police_report_hours_remaining != null
                            ? ` ${d.police_report_hours_remaining}h remaining.`
                            : ''}
                    </InfoCard>
                </div>
            ) : null}
            ...
            d.worksafe_notification_status !== 'acknowledged' ? (
                <div className="sm:col-span-2">
                    <InfoCard icon={AlertTriangle} tone="warn">
                        <strong>
                            WorkSafe NZ notifiable event (HSWA 2015).
                        </strong>{' '}
                        Notify WorkSafe NZ as soon as practicable.
                    </InfoCard>
                </div>
            ) : null}
...
function VehicleSection({ d }: { d: FleetIncidentDetail }) {
    if (!d.asset)
        return (
            <EmptyState icon={Truck} text="No asset linked to this incident." />
        );
    return (
        <div className="grid gap-4 sm:grid-cols-2">
            <ReviewCard icon={Truck} title="Asset" span>
                ...
                <ReviewRow
                    label="Registration"
                    value={
                        d.vehicle_rego_snapshot ?? d.asset.registration_number
                    }
                />
                <ReviewRow
                    label="Category"
                    value={titleCase(d.asset.category)}
                />
                <ReviewRow label="Serial" value={d.asset_serial_snapshot} />
                <ReviewRow
                    label="Odometer"
                    value={
                        d.odometer_at_incident != null
                            ? `${d.odometer_at_incident} km`
                            : undefined
                    }
                />
            ...
            <ReviewCard icon={User} title="Driver">
                ...
                <ReviewRow
                    label="Licence no."
                    value={d.driver_licence_number}
                />
                <ReviewRow
                    label="Licence class"
                    value={d.driver_licence_class}
                />
                <ReviewRow label="Expiry" value={d.driver_licence_expiry} />
                <ReviewRow
                    label="Years held"
                    value={
                        d.driver_years_held != null
                            ? String(d.driver_years_held)
                            : undefined
                    }
                />
                <ReviewRow
            ...
            <div className="sm:col-span-2">
                <Link
                    href={`/fleet-assets/assets/${d.asset.id}`}
                    className="inline-flex items-center gap-1 text-sm font-medium text-primary hover:underline"
                >
                    <ExternalLink className="h-3.5 w-3.5" /> Open in Asset
                    register
                </Link>
                <span className="ml-3 text-xs text-muted-foreground">
                    Snapshot fields populate from the register once the wider
                    Fleet &amp; Assets module is wired.
                </span>
            </div>
        </div>
    );
        ...
        <div className="grid gap-4 sm:grid-cols-2">
            <ReviewCard icon={Users} title="Aboard">
                <ReviewRow
                    label="People aboard"
                    value={String(
                        d.people_aboard_count ?? d.people_aboard?.length ?? 0,
                    )}
                />
                <ReviewRow
                    label="Whānau informed"
                    value={
                        d.whanau_informed != null
                            ? d.whanau_informed
                                ? 'Yes'
                                : 'No'
                            : undefined
            ...
            <ReviewCard icon={Users} title="Third parties & witnesses">
                <ReviewRow
                    ...
                    value={
                        d.third_party_involved != null
                            ? d.third_party_involved
                                ? 'Yes'
                                : 'No'
                            : undefined
                    }
                ...
                <ReviewRow
                    label="Third parties"
                    value={String(d.third_parties?.length ?? 0)}
                />
                <ReviewRow
                    label="Witnesses"
                    value={String(d.witnesses?.length ?? 0)}
                />
                <ReviewRow
                    label="Attending officer"
                    value={d.attending_officer}
                />
            </ReviewCard>
            {d.booking ? (
                <ReviewCard icon={Calendar} title="Booking context">
                    ...
                    <ReviewRow
                        ...
                        value={
                            d.booking.starts_at
                                ? formatDateTime(d.booking.starts_at)
                                : undefined
                        }
                    />
                    <ReviewRow
                        ...
                        value={
                            d.booking.ends_at
                                ? formatDateTime(d.booking.ends_at)
                                : undefined
                        }
                    />
                </ReviewCard>
            ...
            <ReviewCard icon={ShieldAlert} title="Testing">
                <ReviewRow
                    label="Breath test"
                    value={
                        d.breath_test_administered
                            ? (d.breath_test_result ?? 'Administered')
                            : 'Not administered'
                    }
                />
                <ReviewRow
                    label="Drug test"
                    value={
                        d.drug_test_administered
                            ? (d.drug_test_result ?? 'Administered')
                            : 'Not administered'
                    }
                />
            </ReviewCard>
    ...
    return (
        <div className="grid gap-4 sm:grid-cols-2">
            <ReviewCard icon={MapPin} title="Location">
                ...
                <ReviewRow
                    label="Latitude"
                    value={d.latitude != null ? String(d.latitude) : undefined}
                />
                <ReviewRow
                    label="Longitude"
                    value={
                        d.longitude != null ? String(d.longitude) : undefined
                    }
                />
                {hasCoords ? (
            ...
            <ReviewCard icon={Cloud} title="Road & weather">
                ...
                <ReviewRow
                    ...
                    value={
                        d.estimated_speed != null
                            ? `${d.estimated_speed} km/h`
                            : undefined
                    }
                />
        ...
        <div className="grid gap-4 sm:grid-cols-2">
            <ReviewCard icon={Wrench} title="Damage" span>
                <ReviewRow
                    ...
                    value={titleCase(d.damage_classification)}
                />
                <ReviewRow
                    label="Drivable"
                    value={
                ...
                <ReviewRow
                    label="Damage areas"
                    value={d.damage_details?.areas?.join(', ')}
                />
                <ReviewRow
                    label="Cargo / equipment"
                    value={d.cargo_equipment_damage}
                />
            </ReviewCard>
            <ReviewCard icon={CircleSlash} title="Vehicle off-road (VOR)">
                {d.is_off_road || d.vehicle_off_road ? (
                    <>
                        <ReviewRow
                            label="Off-road from"
                            value={d.off_road_from}
                        />
                        <ReviewRow
                            label="Off-road to"
                            value={d.off_road_to ?? '(still off-road)'}
                        />
                        <ReviewRow
                            label="Service resumed"
                            value={d.service_resumed_at}
                        />
                    </>
                ) : (
            ...
            <ReviewCard icon={FileText} title="Asset condition & cost">
                <ReviewRow
                    ...
                    value={d.asset_condition_before}
                />
                <ReviewRow
                    label="Condition after"
                    value={d.asset_condition_after}
                />
                <ReviewRow
                    label="Replacement cost"
                    value={
                        d.replacement_cost != null
                            ? `$${d.replacement_cost}`
                            : undefined
                    }
                />
                <ReviewRow
                    ...
                    value={
                        d.damage_details?.estimated_cost != null
                            ? `$${d.damage_details.estimated_cost}`
                            : undefined
                    }
                />
            </ReviewCard>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
            {d.is_police_report_due ? (
                <div className="sm:col-span-2">
                    <InfoCard icon={ShieldAlert} tone="crit">
                        ...
                        {d.police_report_due_at
                            ? `Due by ${formatDateTime(d.police_report_due_at)}.`
                            : ''}
                        {d.police_report_hours_remaining != null
                            ? ` ${d.police_report_hours_remaining}h remaining.`
                            : ''}
            ...
            <ReviewCard icon={ShieldAlert} title="Injury & fatality">
                <ReviewRow
                    label="Injury"
                    value={
                        d.injury_involved != null
                            ? d.injury_involved
                                ? `Yes — ${titleCase(d.injury_severity)}`
                                : 'No'
                            : undefined
                    }
                />
            ...
            <ReviewCard icon={FileText} title="NZ Police">
                ...
                <ReviewRow
                    ...
                    value={d.police_reference}
                />
                <ReviewRow
                    label="TCR reference"
                    value={d.traffic_crash_report_reference}
                />
                <ReviewRow
                    label="Logged at"
                    value={
                        d.police_report_logged_at
                            ? formatDateTime(d.police_report_logged_at)
                            : undefined
                    }
                />
            </ReviewCard>
            <ReviewCard icon={AlertTriangle} title="WorkSafe NZ">
                ...
                <ReviewRow
                    label="Status"
                    value={titleCase(d.worksafe_notification_status)}
                />
                <ReviewRow
                    label="Notified at"
                    value={
                        d.worksafe_notified_at
                            ? formatDateTime(d.worksafe_notified_at)
                            : undefined
                    }
                />
    ...
    return (
        <div className="grid gap-4 sm:grid-cols-2">
            <ReviewCard icon={CreditCard} title="Claim">
                ...
                <ReviewRow
                    label="Status"
                    value={titleCase(d.insurance_claim_status)}
                />
                <ReviewRow
                    label="Excess"
                    value={
                        d.insurance_excess != null
                            ? `$${d.insurance_excess}`
                            : undefined
                    }
                />
                <ReviewRow
                    label="Amount sought"
                    value={
                        d.insurance_amount_sought != null
                            ? `$${d.insurance_amount_sought}`
                            : undefined
                    }
                />
                <ReviewRow
                    ...
                    value={
                        d.insurance_amount_approved != null
                            ? `$${d.insurance_amount_approved}`
                            : undefined
                    }
                />
            </ReviewCard>
            <ReviewCard icon={Wrench} title="Repair & cost">
                <ReviewRow
                    label="Repair contractor"
                    value={d.repair_contractor}
                />
                <ReviewRow
                    label="Actual repair cost"
                    value={
                        d.actual_repair_cost != null
                ...
                <ReviewRow
                    ...
                    value={
                        d.total_incident_cost != null
                            ? `$${d.total_incident_cost}`
                            : undefined
                    }
                />
            </ReviewCard>
...
function PhotosSection({ d }: { d: FleetIncidentDetail }) {
    ...
    return (
        <div className="flex flex-col gap-3">
            {canUpload ? (
                <AttachmentUploader
                    endpoint={`/fleet-assets/incidents/${d.id}/attachments`}
                    noteField="notes"
                    accept="image/*,.pdf,.doc,.docx"
                    hint="Scene/damage photos, dashcam, the TCR or insurance PDF — up to 20 MB each"
                />
            ) : null}
                        ...
                        >
                            ...
                            <div className="flex items-center justify-between gap-1 px-1.5 py-1">
                                ...
                                <span className="flex items-center gap-1">
                                    <a
                                        href={`/fleet-assets/incidents/${d.id}/attachments/${a.id}/download`}
                                    ...
                                    {canUpload ? (
                                        <Button
                                            ...
                                            onClick={() =>
                                                router.delete(
                                                    `/fleet-assets/incidents/${d.id}/attachments/${a.id}`,
                                                    { preserveScroll: true },
                                                )
                                            }
                                            className="rounded px-1 py-0.5 text-status-critical hover:bg-muted"
                                            aria-label="Remove"
                                        >
...
function InvestigationSection({ d }: { d: FleetIncidentDetail }) {
    const completeFollowup = (fid: number) =>
        router.post(
            `/fleet-assets/incidents/${d.id}/followups/${fid}/complete`,
            {},
            { preserveScroll: true },
        );
    return (
        <div className="flex flex-col gap-4">
            <ReviewCard icon={Search} title="Investigation">
                ...
                <ReviewRow
                    ...
                    value={d.corrective_actions}
                />
                <ReviewRow
                    label="Completed at"
                    value={
                        d.investigation_completed_at
                            ? formatDateTime(d.investigation_completed_at)
                            : undefined
                    }
                />
            </ReviewCard>
                ...
                {d.followups.length ? (
                    <div className="flex flex-col gap-2">
                        {d.followups.map((f) => (
                            ...
                            >
                                <ListTodo
                                    className={`mt-0.5 h-4 w-4 shrink-0 ${f.is_completed ? 'text-status-success' : 'text-status-warning'}`}
                                />
        ...
        <div className="flex flex-col gap-2">
            {d.hs_event ? (
                <LinkedRow
                    icon={ShieldAlert}
                    title="H&S event"
                    sub={`${d.hs_event.reference ?? `#${d.hs_event.id}`} · ${titleCase(d.hs_event.status)}`}
            ...
            {d.hs_event?.control_room_alert_id ? (
                <LinkedRow
                    ...
                    sub="Active operational alert"
                    href={`/control-room/alerts/${d.hs_event.control_room_alert_id}`}
                />
            ) : null}
            {d.asset ? (
                <LinkedRow
                    ...
                    sub={`${d.asset.name}${d.asset.registration_number ? ` · ${d.asset.registration_number}` : ''}`}
                    href={`/fleet-assets/assets/${d.asset.id}`}
                />
            ) : null}
            {d.asset?.hr_asset ? (
                d.can.view_hr_assets ? (
                    <LinkedRow
                        ...
                        sub={
                            d.asset.hr_asset.holder_name
                                ? `HR: assigned to ${d.asset.hr_asset.holder_name}`
                                : 'Tracked in HR Asset Register'
                        }
            ...
            {d.client_incidents.map((ci) => (
                <LinkedRow
                    key={ci.id}
                    icon={User}
                    title="Client incident (resident aboard)"
                    sub={`${ci.client?.name ?? 'Resident'} · ${titleCase(ci.status)} · ${titleCase(ci.type)}`}
                    href={`/incidents?incident=${ci.id}`}
                />
            ))}
            {empty ? (
                <p className="text-sm text-muted-foreground">
...
function ActionPane({
    ...
    switch (action) {
        ...
        case 'back_in_service':
            return (
                <BackInServicePane incidentId={incidentId} onDone={onDone} />
            );
    }
}
    ...
    const nextStatus =
        d.status === 'reported'
            ...
            : d.status === 'investigating'
              ? 'resolved'
              : 'closed';
    const form = useForm<{ status: string; resolution_notes: string }>({
        status: nextStatus,
        resolution_notes: d.resolution_notes ?? '',
    });
    const submit = (e: FormEvent) => {
        e.preventDefault();
        form.post(`/fleet-assets/incidents/${incidentId}/status`, {
            preserveScroll: true,
            onSuccess: () => onDone(),
        });
    };
    return (
        <form onSubmit={submit} className="flex flex-col gap-4">
            <StepHead
                ...
                blurb={
                    nextStatus === 'closed'
                        ? 'Resolution notes are required to close. WorkSafe-notifiable events should be notified and any open follow-ups completed first.'
                        : 'Advance the incident lifecycle.'
                }
            />
            {nextStatus === 'closed' &&
            d.requires_police_report &&
            !d.police_report_logged_at &&
            !d.traffic_crash_report_reference ? (
                <InfoCard icon={ShieldAlert} tone="warn">
                    No Police report (TCR) has been logged for this injury/fatal
                    crash — Land Transport Act s22.
                </InfoCard>
            ) : null}
            {nextStatus === 'resolved' || nextStatus === 'closed' ? (
                <Field
                    label="Resolution notes"
                    required={nextStatus === 'closed'}
                ...
                >
                    <Textarea
                        ...
                        placeholder="What was the outcome and how was it resolved?"
                    />
                </Field>
            ) : null}
    ...
    return (
        <form onSubmit={submit} className="flex flex-col gap-4">
            <StepHead
                icon={Plus}
                title="Add follow-up"
                blurb="An operational task to track to completion."
            />
            ...
            <div className="grid gap-4 sm:grid-cols-2">
                ...
                >
                    <select
                        ...
                        onChange={(e) =>
                            form.setData('assigned_to_user_id', e.target.value)
                        }
...
function PoliceReportPane({
    ...
    const submit = (e: FormEvent) => {
        e.preventDefault();
        form.post(`/fleet-assets/incidents/${incidentId}/police-report`, {
            preserveScroll: true,
            onSuccess: () => onDone(),
        });
    };
    return (
        <form onSubmit={submit} className="flex flex-col gap-4">
            <StepHead
                icon={ShieldAlert}
                title="Log Police report (TCR)"
                blurb="Land Transport Act 1998 s22 — record the Traffic Crash Report for this injury/fatal crash."
            />
            <div className="grid gap-4 sm:grid-cols-2">
                <Field
                    label="TCR reference"
                    error={form.errors.traffic_crash_report_reference}
                >
                    <Input
                        value={form.data.traffic_crash_report_reference}
                        onChange={(e) =>
                            form.setData(
                                'traffic_crash_report_reference',
                                e.target.value,
                ...
                <Field
                    label="Police reference"
                    error={form.errors.police_reference}
                >
                    <Input
                        value={form.data.police_reference}
                        onChange={(e) =>
                            form.setData('police_reference', e.target.value)
                        }
                    />
                </Field>
                <Field
                    label="Attending officer"
                    error={form.errors.attending_officer}
                >
                    <Input
                        value={form.data.attending_officer}
                        onChange={(e) =>
                            form.setData('attending_officer', e.target.value)
                        }
                    />
                </Field>
...
function ClaimPane({
    ...
    return (
        <form onSubmit={submit} className="flex flex-col gap-4">
            <StepHead
                icon={CreditCard}
                title="Log insurance claim"
                blurb="Record the insurer and claim details."
            />
            <div className="grid gap-4 sm:grid-cols-2">
                <Field label="Insurer" error={form.errors.insurer_name}>
                    <Input
                        value={form.data.insurer_name}
                        onChange={(e) =>
                            form.setData('insurer_name', e.target.value)
                        }
                    />
                </Field>
                <Field
                    label="Claim reference"
                    error={form.errors.insurance_reference}
                >
                    <Input
                        value={form.data.insurance_reference}
                        onChange={(e) =>
                            form.setData('insurance_reference', e.target.value)
                        }
                    />
                </Field>
                <Field label="Excess ($)" error={form.errors.insurance_excess}>
                    <Input
                        type="number"
                        min="0"
                        value={form.data.insurance_excess}
                        onChange={(e) =>
                            form.setData('insurance_excess', e.target.value)
                        }
                    />
                </Field>
                <Field
                    label="Amount sought ($)"
                    error={form.errors.insurance_amount_sought}
                >
                    <Input
                        type="number"
                        min="0"
                        value={form.data.insurance_amount_sought}
                        onChange={(e) =>
                            form.setData(
                                'insurance_amount_sought',
                                e.target.value,
                ...
                <Field
                    label="Status"
                    error={form.errors.insurance_claim_status}
                >
                    <Input
                        value={form.data.insurance_claim_status}
                        onChange={(e) =>
                            form.setData(
                        ...
                        placeholder="lodged / assessing / approved / settled"
                    />
                </Field>
            </div>
            <PaneButtons
...
function OffRoadPane({
    ...
    const form = useForm<{ off_road_from: string; off_road_to: string }>({
        off_road_from: '',
        off_road_to: '',
    });
    const submit = (e: FormEvent) => {
        e.preventDefault();
        form.post(`/fleet-assets/incidents/${incidentId}/off-road`, {
            preserveScroll: true,
            onSuccess: () => onDone(),
        });
    };
    return (
        <form onSubmit={submit} className="flex flex-col gap-4">
            <StepHead
                icon={CircleSlash}
                title="Mark off-road (VOR)"
                blurb="Take the vehicle out of service — surfaces on the Off-road worklist."
            />
            <div className="grid gap-4 sm:grid-cols-2">
                <Field label="Off-road from" error={form.errors.off_road_from}>
                    <Input
                        type="date"
                        value={form.data.off_road_from}
                        onChange={(e) =>
                            form.setData('off_road_from', e.target.value)
                        }
                    />
                </Field>
                <Field label="Expected return" error={form.errors.off_road_to}>
                    <Input
                        type="date"
                        value={form.data.off_road_to}
                        onChange={(e) =>
                            form.setData('off_road_to', e.target.value)
                        }
                    />
                </Field>
            </div>
...
function BackInServicePane({
    ...
    const form = useForm<{ service_resumed_at: string }>({
        service_resumed_at: '',
    });
    const submit = (e: FormEvent) => {
        e.preventDefault();
        form.post(`/fleet-assets/incidents/${incidentId}/back-in-service`, {
            preserveScroll: true,
            onSuccess: () => onDone(),
        });
    };
    return (
        <form onSubmit={submit} className="flex flex-col gap-4">
            <StepHead
                ...
                blurb="Mark the vehicle back in service."
            />
            <Field
                label="Service resumed"
                error={form.errors.service_resumed_at}
            >
                <Input
                    type="date"
                    value={form.data.service_resumed_at}
                    onChange={(e) =>
                        form.setData('service_resumed_at', e.target.value)
                    }
                />
            </Field>
            <PaneButtons
