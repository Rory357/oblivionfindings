...
export function SafeguardingConcernDialog({
            ...
            >
                <div className="flex flex-col items-center justify-center py-16 text-center">
                    ...
                    <p className="mt-1 max-w-sm text-sm text-muted-foreground">
                        This is a sensitive safeguarding allegation. It is
                        visible only to the assigned lead, the reporter, and
                        staff cleared to view sensitive concerns.
                    </p>
                </div>
            </WizardShell>
        );
    ...
    const SECTIONS: {
        ...
        },
        {
            key: 'risk',
            label: 'Risk',
            blurb: `${d.risk_assessments?.length ?? 0} assessment${(d.risk_assessments?.length ?? 0) === 1 ? '' : 's'}`,
            icon: Activity,
        },
        {
            key: 'investigation',
            label: 'Investigation',
            blurb: d.hs_event
                ? d.hs_event.reference_number
                : `${d.investigations?.length ?? 0} record${(d.investigations?.length ?? 0) === 1 ? '' : 's'}`,
            icon: Search,
        },
        {
            key: 'reports',
            label: 'External reports',
            blurb: `${d.external_reports?.length ?? 0} logged`,
            icon: Landmark,
        },
        {
            key: 'actions',
            label: 'Action plan',
            blurb: `${d.action_plans?.length ?? 0} item${(d.action_plans?.length ?? 0) === 1 ? '' : 's'}`,
            icon: ListTodo,
        },
        {
            key: 'evidence',
            label: 'Evidence',
            blurb: `${d.attachments?.length ?? 0} file${(d.attachments?.length ?? 0) === 1 ? '' : 's'}`,
            icon: Paperclip,
        },
        {
            key: 'linked',
            label: 'Linked records',
            blurb: 'incident · H&S · alerts',
            icon: LinkIcon,
        },
    ];
    ...
    const markInformed = () =>
        router.post(
            `/safeguarding/${d.id}/subject-informed`,
            {},
            { preserveScroll: true },
        );
        ...
        <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            {can.update && !terminal ? (
                <OptionBtn
                    icon={Activity}
                    label="Add risk"
                    onClick={() => setAction('risk')}
                />
            ) : null}
            {can.investigate && !terminal ? (
                <OptionBtn
                    icon={Search}
                    label="Start investigation"
                    onClick={() => setAction('investigation')}
                    disabled={reported}
                    reason={triageFirst}
                />
            ) : null}
            {can.report_external && !terminal ? (
                <OptionBtn
                    icon={Landmark}
                    label="Log referral"
            ...
            {can.update && !terminal && !d.subject_informed ? (
                <OptionBtn
                    icon={BadgeCheck}
                    label="Mark informed"
        ...
        >
            {action === 'triage' ? (
                ...
                <>
                    ...
                    {section === 'linked' ? (
                        <LinkedSection d={d} subject={subject} />
                    ) : null}
                </>
            )}
        </WizardShell>
...
function AssignPane({ d, onDone }: { d: ConcernDetail; onDone: () => void }) {
    ...
    const form = useForm<{ assigned_to_user_id: string }>({
        assigned_to_user_id: '',
    });
    const submit = (e: FormEvent) => {
        ...
        form.post(`/safeguarding/${d.id}/assign`, {
            preserveScroll: true,
    ...
    return (
        <>
            <StepHead
                icon={UserCog}
                title="Assign a lead"
                blurb="The lead owns the concern through triage, investigation and closure."
            />
            <PaneShell
                onCancel={onDone}
                onSubmit={submit}
...
function InvestigationPane({
    ...
    const submit = (e: FormEvent) => {
        ...
        if (!form.data.lead_investigator_id) {
            form.setError(
                'lead_investigator_id',
                'Choose a lead investigator.',
            );
            return;
        }
        form.post(`/safeguarding/${d.id}/investigations`, {
            preserveScroll: true,
            onSuccess: onSuccessGuard(onDone),
        });
    };
    return (
        <>
            <StepHead
                ...
                blurb="Opening an investigation record moves the concern to Under investigation. Completing it auto-advances the concern."
            />
            <PaneShell
                onCancel={onDone}
                onSubmit={submit}
            ...
            >
                <div className="grid gap-3 sm:grid-cols-2">
                    <Field label="Type" required>
                        <SelectInput
                            value={form.data.investigation_type}
                            onChange={(v) =>
                                form.setData('investigation_type', v)
                            }
                            placeholder="Type"
                            options={[
                    ...
                    >
                        <SelectInput
                            ...
                            onChange={(v) =>
                                form.setData('lead_investigator_id', v)
                            }
                            placeholder="Select"
                            options={staff.map((s) => ({
                                value: String(s.id),
                ...
                <Field label="Terms of reference" hint="Optional">
                    <Textarea
                        rows={2}
                        value={form.data.terms_of_reference}
                        onChange={(e) =>
                            form.setData('terms_of_reference', e.target.value)
                        }
                    />
                </Field>
                <Field label="Methodology" hint="Optional">
                    <Input
                        ...
                        onChange={(e) =>
                            form.setData('methodology', e.target.value)
                        }
                    />
                </Field>
            </PaneShell>
...
function ReportPane({ d, onDone }: { d: ConcernDetail; onDone: () => void }) {
    ...
    const pickAuthority = (v: string) => {
        ...
        if (
            !form.data.authority_name ||
            AUTHORITIES.some((a) => a.label === form.data.authority_name)
        ) {
    ...
    const submit = (e: FormEvent) => {
        e.preventDefault();
        form.post(`/safeguarding/${d.id}/external-reports`, {
            preserveScroll: true,
            onSuccess: onSuccessGuard(onDone),
        });
    };
    return (
        <>
            <StepHead
                icon={Landmark}
                title="Log an external referral"
                blurb="Record a report to an external authority. NZ has no single adult-safeguarding statute — you choose the right authority."
            />
            <PaneShell
                onCancel={onDone}
                onSubmit={submit}
            ...
            >
                <div className="grid gap-3 sm:grid-cols-2">
                    <Field
                        label="Authority"
                        required
                        error={form.errors.authority_type}
                    >
                        <SelectInput
                            value={form.data.authority_type}
                    ...
                    <Field
                        ...
                        required
                        error={form.errors.authority_name}
                    >
                        <Input
                            value={form.data.authority_name}
                            onChange={(e) =>
                                form.setData('authority_name', e.target.value)
                            }
                            placeholder="e.g. NZ Police — Central"
                        />
                    </Field>
                </div>
                <div className="grid gap-3 sm:grid-cols-2">
                    <Field label="Method">
                        <SelectInput
                            value={form.data.report_method}
                            onChange={(v) => form.setData('report_method', v)}
                            placeholder="Method"
                            options={[
                    ...
                    >
                        <Input
                            ...
                            onChange={(e) =>
                                form.setData('reported_at', e.target.value)
                            }
                        />
                    </Field>
                </div>
                <Field
                    ...
                    required
                    error={form.errors.report_summary}
                >
                    <Textarea
                        rows={3}
                        value={form.data.report_summary}
                        onChange={(e) =>
                            form.setData('report_summary', e.target.value)
                        }
                    />
                </Field>
            </PaneShell>
...
function RiskPane({ d, onDone }: { d: ConcernDetail; onDone: () => void }) {
    const form = useForm<{
        ...
        risk_from_others: string;
        mental_capacity: string;
    ...
    const submit = (e: FormEvent) => {
        ...
        form.post(`/safeguarding/${d.id}/risk-assessments`, {
            preserveScroll: true,
            onSuccess: onSuccessGuard(onDone),
        });
    };
    return (
        <>
            <StepHead
                ...
                blurb="Record the current risk picture and when it should next be reviewed."
            />
            <PaneShell
                onCancel={onDone}
                onSubmit={submit}
            ...
            >
                <Field
                    label="Overall risk"
                    required
                    error={form.errors.overall_risk_level}
                >
                    <SelectInput
                        value={form.data.overall_risk_level}
                        onChange={(v) => form.setData('overall_risk_level', v)}
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                    <Field label="To self">
                        <SelectInput
                            value={form.data.risk_to_self}
                            onChange={(v) => form.setData('risk_to_self', v)}
                            placeholder="—"
                            options={RISK_OPTS}
                        />
                    </Field>
                    <Field label="To others">
                        <SelectInput
                            value={form.data.risk_to_others}
                            onChange={(v) => form.setData('risk_to_others', v)}
                            placeholder="—"
                            options={RISK_OPTS}
                        />
                    </Field>
                    <Field label="From others">
                        <SelectInput
                            value={form.data.risk_from_others}
                            onChange={(v) =>
                                form.setData('risk_from_others', v)
                            }
                            placeholder="—"
                            options={RISK_OPTS}
                        />
                ...
                <Field label="Protective measures" hint="One per line">
                    <Textarea
                        rows={2}
                        value={form.data.protective_measures}
                        onChange={(e) =>
                            form.setData('protective_measures', e.target.value)
                        }
                    />
                </Field>
                <div className="grid gap-3 sm:grid-cols-2">
                    <Field label="Mental capacity" hint="Optional">
                        <SelectInput
                            value={form.data.mental_capacity}
                            onChange={(v) => form.setData('mental_capacity', v)}
                            placeholder="—"
                            options={[
                                {
                                    value: 'has_capacity',
                ...
                <Field label="Notes" hint="Optional">
                    <Textarea
                        rows={2}
                        value={form.data.assessment_notes}
                        onChange={(e) =>
                            form.setData('assessment_notes', e.target.value)
                        }
                    />
                </Field>
            </PaneShell>
...
function ActionItemPane({
    ...
    const submit = (e: FormEvent) => {
        ...
        form.post(`/safeguarding/${d.id}/action-plans`, {
            preserveScroll: true,
            onSuccess: onSuccessGuard(onDone),
        });
    };
    return (
        <>
            <StepHead
                icon={ListTodo}
                title="Add a protective action"
                blurb="Track a protective measure or corrective action with an owner and due date."
            />
            <PaneShell
                onCancel={onDone}
                onSubmit={submit}
            ...
            >
                <Field
                    label="Action"
                    required
                    error={form.errors.action_description}
                >
                    <Textarea
                        rows={2}
                        ...
                        placeholder="e.g. Increase observations and review the support plan"
                    />
                </Field>
                <div className="grid gap-3 sm:grid-cols-3">
                    <Field label="Type">
...
const SUBSTANTIATE = [
    {
        key: 'substantiated',
        label: 'Substantiated',
        description: 'Evidence supports it',
    },
    {
        key: 'needs_enquiry',
        label: 'Needs enquiry',
        description: 'Unclear — look into it',
    },
    {
        key: 'not_substantiated',
        label: 'Not substantiated',
        description: 'No basis found',
    },
];
const TRIAGE_PATHS = [
    {
        key: 'investigate',
        label: 'Investigate',
        description: 'Open an internal enquiry',
        icon: Search,
    },
    {
        key: 'refer',
        label: 'Refer externally',
        description: 'Notify an authority',
        icon: Landmark,
    },
    {
        key: 'no_action',
        label: 'No further action',
        description: 'Close with rationale',
        icon: CircleSlash,
    },
];
...
function TriagePane({ d, onDone }: { d: ConcernDetail; onDone: () => void }) {
    ...
    const submit = (e: FormEvent) => {
        ...
        if (!form.data.substantiation) {
            form.setError(
                'substantiation',
                'Record a substantiation judgement.',
            );
            return;
        }
        ...
        form.post(`/safeguarding/${d.id}/triage`, {
            preserveScroll: true,
            onSuccess: onSuccessGuard(onDone),
        });
    };
    ...
    return (
        <>
            <StepHead
                icon={ClipboardCheck}
                title="Triage concern"
                blurb="The first gate. Substantiate, set the initial risk, assign a lead, and decide the path."
            />
            <PaneShell
                onCancel={onDone}
                onSubmit={submit}
            ...
            >
                <Field
                    label="1 · Is the concern substantiated?"
                    required
                    error={form.errors.substantiation}
                >
                    <TilePicker
                        cols={3}
                        value={form.data.substantiation}
                        onChange={(v) => form.setData('substantiation', v)}
                        options={SUBSTANTIATE}
                    />
                </Field>
                <Field
                    label="2 · Initial risk level"
                    required
                    error={form.errors.initial_risk}
                >
                    <Segmented
                        value={form.data.initial_risk}
                        onChange={(v) => form.setData('initial_risk', v)}
                        options={RISK_OPTS.map((o) => ({
                            value: o.value,
                            label: o.label,
                        }))}
                    />
                ...
                <Field label="3 · Assign a safeguarding lead" hint="Optional">
                    <SelectInput
                        ...
                        onChange={(v) => form.setData('lead_user_id', v)}
                        placeholder="Select a lead"
                        options={staff.map((s) => ({
                            value: String(s.id),
                            label: s.name,
                ...
                >
                    <TilePicker
                        ...
                        onChange={(v) => form.setData('path', v)}
                        options={TRIAGE_PATHS}
                    />
                </Field>
                {form.data.path === 'refer' ? (
                    <InfoCard icon={Landmark} tone="crit">
                        Referring externally requires a report to an authority —
                        you'll log it next, before the concern moves to{' '}
                        <b>Referred external</b>.
                    </InfoCard>
                ) : null}
                {form.data.path === 'investigate' ? (
                    <InfoCard icon={Search} tone="info">
                        An investigation record opens automatically — the
                        concern can't enter <b>Investigating</b> without one.
                    </InfoCard>
                ) : null}
                {form.data.path === 'no_action' ? (
                    <Field label="Rationale" required error={form.errors.notes}>
                        <Textarea
                            ...
                            placeholder="Why does this need no further safeguarding action?"
                        />
                    </Field>
                ) : null}
            </PaneShell>
...
function ClosePane({ d, onDone }: { d: ConcernDetail; onDone: () => void }) {
    const invOpen = (d.investigations ?? []).some(
        (i) => !['completed', 'abandoned'].includes(i.status),
    );
    const actOpen = (d.action_plans ?? []).some(
        (a) => !['completed', 'cancelled'].includes(a.status),
    );
    const refOk =
        !d.requires_external_referral || (d.external_reports?.length ?? 0) > 0;
    ...
    const checks = [
        ...
        {
            ok: !actOpen,
            label: 'Action plan complete',
            detail: actOpen
                ? 'One or more actions are still open'
                : 'No open actions',
        },
        {
            ok: infOk,
            label: 'Subject informed (or recorded N/A)',
            detail: infOk ? 'Recorded' : 'Subject not recorded as informed',
        },
        {
            ok: refOk,
            label: 'External referral logged (if required)',
            detail: refOk
                ? 'Not required or logged'
                : 'Referral indicated but no report logged',
        },
    ];
    ...
    }>({ closure_summary: '', lessons_learned: '', override_reason: '' });
    const submit = (e: FormEvent) => {
        e.preventDefault();
        ...
        if (blocked && !form.data.override_reason.trim()) {
            form.setError(
                'override_reason',
                'Record why you are closing with open work.',
            );
            return;
        }
        form.post(`/safeguarding/${d.id}/close`, {
            preserveScroll: true,
            onSuccess: onSuccessGuard(onDone),
        });
    };
    ...
    return (
        <>
            <StepHead
                icon={CheckCircle2}
                title="Close concern"
                blurb="Confirm the closure checks, summarise how it was resolved, and capture any lessons."
            />
            <PaneShell
                onCancel={onDone}
                onSubmit={submit}
            ...
            >
                <div>
                    ...
                    <div className="overflow-hidden rounded-xl border border-border">
                        {checks.map((c, i) => (
                            ...
                            >
                                <span
                                    className={`grid h-6 w-6 shrink-0 place-items-center rounded-md ${c.ok ? 'bg-status-success-bg text-status-success' : 'bg-status-warning-bg text-status-warning'}`}
                                >
                                    {c.ok ? (
                ...
                {blocked ? (
                    <>
                        <InfoCard icon={AlertTriangle} tone="warn">
                            <b>Open work remains.</b> Closing now is
                            soft-blocked. You can override and close anyway, but
                            you must record why below.
                        </InfoCard>
                        <Field
                        ...
                        >
                            <Input
                                ...
                                placeholder="Why are you closing with open work?"
                            />
                        </Field>
                    </>
                ) : null}
                ...
                >
                    <Textarea
                        ...
                        placeholder="How was the concern resolved and what makes it safe to close?"
                    />
                </Field>
                <Field label="Lessons learned" hint="Optional">
                    <Textarea
                        ...
                        placeholder="What would prevent or improve handling of a similar concern?"
                    />
                </Field>

                {!infOk ? (
                    <InfoCard icon={AlertTriangle} tone="warn">
                        The subject has not been recorded as informed. Confirm
                        whether telling them is appropriate (it isn't always —
                        e.g. an active Police matter) before closing.
                    </InfoCard>
                ) : null}
            </PaneShell>
        </>
...
function LifecycleTracker({ d }: { d: ConcernDetail }) {
    ...
    const branch =
        d.status === 'referred_external'
            ? 'Referred external'
            : d.status === 'no_action_required'
              ? 'No further action'
              : null;
    return (
...
function OverviewSection({
    ...
    const informedLabel = d.subject_informed
        ? `Yes${d.subject_informed_at ? ` · ${formatDateTime(d.subject_informed_at)}` : ''}`
        : 'Not yet';
    return (
        <div className="grid gap-4 sm:grid-cols-2">
                ...
                <InfoCard icon={ClipboardCheck} tone="warn">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                        <span>
                            ...
                            Triage decides the path — investigate, refer
                            externally, or no further action.
                        </span>
                        {onTriage ? (
            ...
            {d.alerts?.some((a) => a.active) ? (
                <InfoCard icon={RadioTower} tone="crit">
                    <span className="font-semibold">
                        Active protective alert
                    </span>{' '}
                    on the subject — see Linked records.
                </InfoCard>
            ) : null}
            ...
            <ReviewCard icon={Users} title="People">
                ...
                <ReviewRow
                    label="Alleged person"
                    value={d.people?.alleged_perpetrator}
                />
                <ReviewRow
                    label="Lead"
                    value={d.people?.assigned_to ?? 'Unassigned'}
                />
            </ReviewCard>
                ...
                <ReviewRow
                    ...
                    value={
                        d.current_risk_level
                            ? titleCase(d.current_risk_level)
                            : undefined
                    }
                />
                <ReviewRow
...
function TimelineSection({ d }: { d: ConcernDetail }) {
    ...
    if (d.reported_at)
        events.push({
            at: d.reported_at,
            label: 'Concern raised — created as Awaiting triage',
            tone: 'primary',
            icon: ShieldAlert,
        });
    if (d.triage?.at)
        events.push({
            at: d.triage.at,
            label: `Triaged${d.triage.decision ? ` · ${titleCase(d.triage.decision)}` : ''}${d.triage.by ? ` by ${d.triage.by}` : ''}`,
            tone: 'info',
            icon: ClipboardCheck,
        });
    (d.investigations ?? []).forEach((i) => {
        if (i.started_at)
            events.push({
                at: i.started_at,
                label: 'Investigation opened — required to enter Investigating',
                tone: 'primary',
                icon: Search,
            });
        if (i.completed_at)
            events.push({
                at: i.completed_at,
                label: `Investigation completed${i.outcome ? ` · ${titleCase(i.outcome)}` : ''}`,
                tone: 'success',
                icon: CheckCircle2,
            });
    });
    (d.external_reports ?? []).forEach((r) => {
        if (r.reported_at)
            events.push({
                at: r.reported_at,
                label: `Reported to ${r.authority_name}`,
                tone: 'warning',
                icon: Landmark,
            });
        if (r.acknowledged_at)
            events.push({
                ...
                label: `${r.authority_name} acknowledged`,
                tone: 'success',
                icon: CheckCircle2,
            });
    });
    if (d.closure?.at)
        events.push({
            at: d.closure.at,
            label: `Closed${d.closure.by ? ` by ${d.closure.by}` : ''}`,
            tone: 'success',
            icon: CheckCircle2,
        });
...
function RiskSection({ d }: { d: ConcernDetail }) {
    if (!d.risk_assessments?.length) {
        return (
            <EmptyState
                icon={Activity}
                text="No risk assessment recorded yet."
            />
        );
    }
    return (
        <div className="flex flex-col gap-3">
            {d.risk_assessments.map((r) => (
                <ReviewCard
                    key={r.id}
                    icon={Activity}
                    title={`Risk assessment${r.assessed_at ? ` · ${formatDateTime(r.assessed_at)}` : ''}`}
                    span
                >
                    <ReviewRow
                        label="Overall risk"
                    ...
                    <ReviewRow
                        label="Risk to self"
                        value={titleCase(r.risk_to_self)}
                    />
                    <ReviewRow
                        label="Risk to others"
                        value={titleCase(r.risk_to_others)}
                    />
                    <ReviewRow
                        label="Risk from others"
                        value={titleCase(r.risk_from_others)}
                    />
                    <ReviewRow
                        label="Mental capacity"
                        value={
                            r.mental_capacity
                                ? titleCase(r.mental_capacity)
                                : undefined
                        }
                    />
                    <ReviewRow
                        label="Protective measures"
                        value={r.protective_measures}
                    />
                    ...
                    <ReviewRow
                        ...
                        value={
                            r.next_review_date
                                ? formatDateTime(r.next_review_date)
                                : undefined
                        }
                    />
                </ReviewCard>
    ...
    return (
        <div className="flex flex-col gap-4">
                ...
                <div className="flex items-center justify-between rounded-lg border border-border bg-muted/30 p-3">
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                            H&amp;S event · {titleCase(d.hs_event.status)}
                        </p>
                    </div>
                    <Link
                        href={`/health-safety/events/${d.hs_event.id}`}
            ...
            {d.investigations?.length ? (
                d.investigations.map((i) => {
                    ...
                    return (
                        <ReviewCard
                            key={i.id}
                            icon={Search}
                            title={`Investigation · ${titleCase(i.status)}`}
                            span
                        >
                            ...
                            <ReviewRow
                                label="Outcome"
                                value={
                                    i.outcome ? titleCase(i.outcome) : undefined
                                }
                            />
                            {i.findings ? (
                                <ReviewRow
                            ...
                            {canInvestigate && open && !terminal ? (
                                <InvestigationCompleteForm
                                    concernId={d.id}
                ...
                <EmptyState
                    icon={Search}
                    text="No investigation opened. Completing an investigation auto-advances the concern."
                />
            )}
        </div>
    );
...
function InvestigationCompleteForm({
    concernId,
    investigationId,
}: {
    concernId: number;
    ...
    const submit = (e: FormEvent) => {
        ...
        form.put(
            `/safeguarding/${concernId}/investigations/${investigationId}`,
            { preserveScroll: true, onSuccess: () => setOpenForm(false) },
        );
    };
    if (!openForm) {
        return (
    ...
    return (
        <form
            onSubmit={submit}
            className="mt-2 flex flex-col gap-2 rounded-xl border border-dashed border-border p-3"
        >
            <p className="text-xs font-semibold text-foreground">
                Complete investigation
            </p>
            <p className="text-[11px] text-muted-foreground">
                Recording the outcome moves the concern on to its action-plan
                stage.
            </p>
    ...
    if (!d.external_reports?.length) {
        return (
            <EmptyState
                ...
                text={
                    d.requires_external_referral
                        ? 'Referral indicated at triage — no report logged yet.'
                        : 'No external reports logged.'
                }
            ...
            {d.external_reports.map((r) => (
                ...
                >
                    ...
                    <ReviewRow
                        ...
                        value={
                            r.ack_received
                                ? `Yes${r.acknowledged_at ? ` · ${formatDateTime(r.acknowledged_at)}` : ''}${r.ack_reference ? ` · ${r.ack_reference}` : ''}`
                                : 'Awaiting'
                        }
                    />
                    <ReviewRow
                        ...
                        value={
                            r.authority_action
                                ? titleCase(r.authority_action)
                                : undefined
                        }
                    />
                    {canReport && !r.ack_received ? (
                        <ReportAckForm concernId={d.id} reportId={r.id} />
                    ) : null}
                </ReviewCard>
            ))}
        </div>
...
function ReportAckForm({
    ...
    const submit = (e: FormEvent) => {
        e.preventDefault();
        form.put(`/safeguarding/${concernId}/external-reports/${reportId}`, {
            preserveScroll: true,
        ...
        >
            ...
            <div className="grid gap-2 sm:grid-cols-2">
                ...
                <Field label="Reference" hint="Optional">
                    <Input
                        value={form.data.acknowledgement_reference}
                        onChange={(e) =>
                            form.setData(
                                'acknowledgement_reference',
                                e.target.value,
            ...
            <Field label="Authority outcome / action" hint="Optional">
                <Input
                    value={form.data.authority_action}
                    onChange={(e) =>
                        form.setData('authority_action', e.target.value)
                    }
                    placeholder="What did the authority decide or do?"
                />
            </Field>
            <div className="flex justify-end gap-2">
                <Button
...
function ActionsSection({ d }: { d: ConcernDetail }) {
    ...
    const complete = (id: number) =>
        router.post(
            `/safeguarding/${d.id}/action-plans/${id}/complete`,
            {},
            { preserveScroll: true },
        );
    return (
        <div className="flex flex-col gap-2">
            {d.action_plans.map((a) => {
                ...
                return (
                    ...
                    >
                        <ListTodo
                            className={`mt-0.5 h-4 w-4 shrink-0 ${a.completed_at ? 'text-status-success' : a.overdue ? 'text-status-critical' : 'text-status-warning'}`}
                        />
        ...
        <div className="flex flex-col gap-3">
            {canEdit ? (
                <AttachmentUploader
                    ...
                    sensitive={{
                        field: 'is_sensitive',
                        label: 'Sensitive — restrict to cleared staff',
                    }}
                            ...
                            >
                                ...
                                {canEdit ? (
                                    <Button
                                        ...
                                        onClick={() =>
                                            router.delete(
                                                `/safeguarding/${d.id}/attachments/${a.id}`,
                                                { preserveScroll: true },
                                            )
                                        }
        ...
        <div className="flex flex-col gap-2">
            {d.control_room_alert_id ? (
                <LinkedRow
                    ...
                    sub="Active alert"
                    href={`/control-room/alerts/${d.control_room_alert_id}`}
                />
            ) : null}
            {d.related_incident_id ? (
                <LinkedRow
                    icon={ShieldAlert}
                    title="Originating incident"
                    sub={
                        d.related_incident_ref ?? `INC-${d.related_incident_id}`
                    }
                    href={`/incidents?incident=${d.related_incident_id}`}
                />
            ) : null}
            {d.hs_event ? (
                <LinkedRow
...
function EmptyState({
    ...
    return (
        <div
            className={`rounded-xl border border-dashed py-12 text-center ${tone === 'warn' ? 'border-status-warning/40 bg-status-warning-bg/30' : 'border-border'}`}
        >
            <Icon
                className={`mx-auto mb-2 h-8 w-8 ${tone === 'warn' ? 'text-status-warning' : 'text-muted-foreground/40'}`}
            />
            <p
                className={`text-sm ${tone === 'warn' ? 'text-status-warning' : 'text-muted-foreground'}`}
            >
                {text}
            </p>
        </div>
