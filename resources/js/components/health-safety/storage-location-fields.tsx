...
export function StorageLocationFields({
    ...
    return (
        <>
            ...
            >
                <Input
                    value={s('location_description')}
                    onChange={(e) =>
                        set('location_description', e.target.value)
                    }
                    placeholder="e.g. Chemical store, bay 1"
                />
            </Field>
            <div className="grid gap-3 sm:grid-cols-3">
                <Field label="Quantity held" error={errors.current_quantity}>
                    <Input
                        ...
                        min="0"
                        step="0.01"
                        value={s('current_quantity')}
                        onChange={(e) =>
                            set('current_quantity', e.target.value)
                        }
                    />
                </Field>
                <Field label="Maximum" error={errors.maximum_quantity}>
                    <Input
                        ...
                        min="0"
                        step="0.01"
                        value={s('maximum_quantity')}
                        onChange={(e) =>
                            set('maximum_quantity', e.target.value)
                        }
                    />
                </Field>
                <Field label="Unit">
                    <SelectInput
                        value={s('quantity_unit')}
            ...
            <div className="grid gap-3 sm:grid-cols-2">
                <Field label="Container">
                    <Input
                        ...
                        placeholder="e.g. HDPE drum, 5 L bottles"
                    />
                </Field>
                <Field label="Last audited">
                    <Input
                        type="date"
                        value={s('last_audit_date')}
                        onChange={(e) => set('last_audit_date', e.target.value)}
                    />
                </Field>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
                <Field label="Properly labelled">
                    <Bool
                        value={!!values.properly_labelled}
                        onChange={(v) => set('properly_labelled', v)}
                    />
                </Field>
                <Field label="Segregation compliant">
                    <Bool
                        value={!!values.segregation_compliant}
                        onChange={(v) => set('segregation_compliant', v)}
                    />
                </Field>
            </div>
