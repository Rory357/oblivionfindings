...
const STEPS: WizardStep[] = [
    {
        ...
        blurb: 'Who, where, linked plan',
        icon: User,
    },
    {
        key: 'episode',
        label: 'The episode',
        blurb: 'Type, time & severity',
        icon: ShieldAlert,
    },
    {
        key: 'trigger',
        label: 'Trigger & de-escalation',
        blurb: 'What led to it',
        icon: Activity,
    },
    {
        key: 'response',
        label: 'Restraint & response',
        blurb: 'What was done',
        icon: ClipboardList,
    },
    {
...
export function RestraintEventWizard({
    ...
    const form = useForm<EventForm>({
        ...
        behaviour_support_plan_id: prescope?.behaviour_support_plan_id
            ? String(prescope.behaviour_support_plan_id)
            : '',
        related_incident_id: '',
    ...
    const clientPlans = useMemo(
        () =>
            plans
                ...
                .map((p) => ({
                    value: String(p.id),
                    label: `${p.reference} · ${p.title}${p.status !== 'active' ? ` (${titleCase(p.status)})` : ''}`,
                })),
        [plans, clientId],
    );
    ...
    const onClient = (v: string) => {
        setData((d) => {
            ...
            if (c?.site_id && !prescope?.site_id)
                next.site_id = String(c.site_id);
            return next;
        });
    };
    ...
    const submit = (addAnother: boolean) => {
        form.transform((d) => ({
            ...
            behaviour_support_plan_id: d.behaviour_support_plan_id
                ? Number(d.behaviour_support_plan_id)
                : null,
    ...
    const success = submitted ? (
        <WizardSuccessPane
            ...
            blurb={
                <>
                    Recorded against the least-restrictive-practice register and
                    mirrored to Health &amp; Safety.
                    {data.injury_occurred || !data.within_support_plan
                        ? ' A Control Room alert was raised for review.'
                        : ''}{' '}
                    You can now attach body maps, injury photos and
                    authorisation forms from the event detail.
                </>
            }
            actions={
                <>
                    {createdId && onOpenEvent ? (
                        <Button onClick={() => onOpenEvent(createdId)}>
                            Open event
                        </Button>
                    ) : null}
    ...
    return (
        <WizardShell
            ...
            railSub={
                prescope?.client_name
                    ? `For ${prescope.client_name}`
                    : 'Restrictive-practice episode'
            }
            steps={STEPS}
            stepIndex={stepIndex}
            ...
            <WizardStepPane>
                {stepKey === 'context' ? (
                    <div className="flex flex-col gap-4">
                        <StepHead
                            ...
                            blurb="Who was supported, where, and which plan applies."
                        />
                        {prescope ? (
                            <InfoCard icon={User} tone="info">
                                Recording against{' '}
                                <span className="font-semibold">
                                    {prescope.client_name ?? 'this client'}
                                </span>
                                {prescope.stay_id
                                    ? ' for the current respite stay'
                                    : ''}
                                . The active behaviour support plan links
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            ...
                            <Field
                                label="Linked behaviour support plan"
                                hint={
                                    clientId ? undefined : 'Pick a client first'
                                }
                                error={errors.behaviour_support_plan_id}
                            >
                                <SelectInput
                                    value={data.behaviour_support_plan_id}
                                    onChange={(v) =>
                                        setData('behaviour_support_plan_id', v)
                                    }
                                    placeholder={
                                        clientPlans.length
                                            ? 'Link a plan'
                        ...
                        <Field
                            ...
                            hint="Optional — link an incident report"
                            error={errors.related_incident_id}
                        >
                            <SelectInput
                                value={data.related_incident_id}
                                onChange={(v) =>
                                    setData('related_incident_id', v)
                                }
                                placeholder={
                        ...
                        {clientId && clientPlans.length === 0 ? (
                            <InfoCard icon={AlertTriangle} tone="warn">
                                This client has no active behaviour support
                                plan. Restrictive practice should be governed by
                                a current plan — consider creating one.
                            </InfoCard>
                        ) : null}
                    </div>
                ) : null}
                ...
                {stepKey === 'episode' ? (
                    <div className="flex flex-col gap-4">
                        <StepHead
                            icon={ShieldAlert}
                            title="The episode"
                            blurb="What type of restrictive practice, and for how long."
                        />
                        <Field
                            label="Type of restrictive practice"
                            required
                            error={errors.restraint_type}
                        >
                            <TilePicker
                                value={data.restraint_type}
                                onChange={(v) => setData('restraint_type', v)}
                                options={TYPE_TILES}
                                cols={3}
                            />
                        </Field>
                        ...
                        {durationPreview != null ? (
                            <InfoCard icon={Activity} tone="info">
                                Duration:{' '}
                                <span className="font-semibold">
                                    {durationLabel(durationPreview)}
                                </span>
                                . Shorter is safer — least-restrictive practice
                                means the minimum necessary.
                            </InfoCard>
                        ) : null}
                        <Field
                            label="Severity"
                ...
                {stepKey === 'trigger' ? (
                    <div className="flex flex-col gap-4">
                        <StepHead
                            icon={Activity}
                            title="Trigger & de-escalation"
                            blurb="What led to the episode, and what was tried first."
                        />
                        <Field
                            label="Trigger / antecedent"
                            required
                            error={errors.trigger_description}
                        >
                            <Textarea
                                rows={3}
                                ...
                                placeholder="What was happening before the restraint?"
                            />
                        </Field>
                        <InfoCard icon={ShieldCheck} tone="info">
                            ...
                            restrictive practice is a last resort. Record the
                            de-escalation and least-restrictive strategies tried
                            first.
                        </InfoCard>
                        <Field
                            label="De-escalation attempted"
                            required
                            error={errors.de_escalation_attempted}
                        >
                            <Textarea
                                rows={3}
                                value={data.de_escalation_attempted}
                                onChange={(e) =>
                                    setData(
                                        'de_escalation_attempted',
                                        e.target.value,
                                ...
                                placeholder="What less-restrictive approaches were tried before restraint?"
                            />
                        </Field>
                    </div>
                ) : null}
                ...
                {stepKey === 'response' ? (
                    <div className="flex flex-col gap-4">
                        <StepHead
                            icon={ClipboardList}
                            title="Restraint & response"
                            blurb="What was done, who was involved, and how the person responded."
                        />
                        <Field
                            label="What restraint was used"
                            required
                            error={errors.restraint_description}
                        >
                            <Textarea
                                rows={3}
                                value={data.restraint_description}
                                onChange={(e) =>
                                    setData(
                                        'restraint_description',
                                        e.target.value,
                                ...
                                placeholder="Describe the restrictive practice applied"
                            />
                        </Field>
                        <Field
                            label="How the person responded"
                            error={errors.person_response}
                        >
                            <Textarea
                                rows={2}
                                value={data.person_response}
                                onChange={(e) =>
                                    setData('person_response', e.target.value)
                                }
                                placeholder="The person's response during and after"
                            />
                        </Field>
                        <Field
                            label="Staff involved"
                            hint="Select everyone present"
                        >
                            <StaffMultiSelect
                                options={staffOptions}
                                value={data.staff_involved}
                                onChange={(v) => setData('staff_involved', v)}
                            />
                        </Field>
                    </div>
                ) : null}
                ...
                {stepKey === 'injury' ? (
                    <div className="flex flex-col gap-4">
                        <StepHead
                            icon={HeartPulse}
                            title="Injury & aftercare"
                            blurb="Was anyone harmed, and what support followed."
                        />
                        <label className="flex items-center gap-2.5 rounded-lg border border-border p-3 text-sm">
                            <input
                                type="checkbox"
                                checked={data.injury_occurred}
                                onChange={(e) =>
                                    setData('injury_occurred', e.target.checked)
                                }
                                className="h-4 w-4 rounded border-border"
                            />
                            <span className="font-medium">
                                An injury occurred during this episode
                            </span>
                        </label>
                        {data.injury_occurred ? (
                            <>
                                <InfoCard icon={AlertTriangle} tone="warn">
                                    An injury escalates this event — it raises a
                                    Control Room alert and is mirrored to Health
                                    &amp; Safety at higher severity.
                                </InfoCard>
                                <Field
                                ...
                                >
                                    <Textarea
                                        ...
                                        placeholder="Who was injured, the nature of the injury, and treatment given"
                                    />
                                </Field>
                            </>
                        ) : null}
                        <Field
                            label="Post-incident support"
                            hint="Debrief, comfort, follow-up"
                            error={errors.post_incident_support}
                        >
                            <Textarea
                                rows={2}
                                value={data.post_incident_support}
                                onChange={(e) =>
                                    setData(
                                        'post_incident_support',
                                        e.target.value,
                                ...
                                placeholder="Support offered to the person (and staff) after the episode"
                            />
                        </Field>
                    </div>
                ) : null}
                ...
                {stepKey === 'adherence' ? (
                    <div className="flex flex-col gap-4">
                        <StepHead
                            ...
                            blurb="Was this within the behaviour support plan?"
                        />
                        <Field label="Within the support plan?" required>
                            <Segmented
                                value={data.within_support_plan ? 'yes' : 'no'}
                                onChange={(v) =>
                                    setData('within_support_plan', v === 'yes')
                                }
                                options={[
                        ...
                        {!data.within_support_plan ? (
                            <>
                                <InfoCard icon={AlertTriangle} tone="crit">
                                    Restraint outside the agreed plan is a
                                    deviation. It raises a Control Room alert
                                    for review and must be explained.
                                </InfoCard>
                                <Field
                                    label="Reason for deviation"
                                    required
                                    error={errors.deviation_reason}
                                >
                                    <Textarea
                                        rows={3}
                                        value={data.deviation_reason}
                                        onChange={(e) =>
                                            setData(
                                                'deviation_reason',
                                                e.target.value,
                                        ...
                                        placeholder="Why was restraint used outside the plan?"
                                    />
                                </Field>
                            </>
                        ) : null}
                        <Field
                            label="Authorised by"
                            hint="Who authorised the restraint"
                            error={errors.authorised_by}
                        >
                            <SelectInput
                                value={data.authorised_by}
                                onChange={(v) => setData('authorised_by', v)}
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                                ...
                                <ReviewRow
                                    ...
                                    value={
                                        clientPlans.find(
                                            (o) =>
                                                o.value ===
                                                data.behaviour_support_plan_id,
                                        )?.label
                                    }
                                />
                                <ReviewRow
                            ...
                            >
                                ...
                                <ReviewRow
                                    label="Started"
                                    value={data.started_at?.replace('T', ' ')}
                                />
                                <ReviewRow
                                    label="Duration"
                                    value={
                            ...
                            <ReviewCard
                                icon={ShieldCheck}
                                title="Adherence & injury"
                                onEdit={() => setStepIndex(5)}
                            >
                                <ReviewRow
                                    label="Within plan"
                                    value={
                                ...
                                <ReviewRow
                                    label="Injury"
                                    value={data.injury_occurred ? 'Yes' : 'No'}
                                />
                                <ReviewRow
                                    label="Authorised by"
                                    value={
                        ...
                        <InfoCard icon={Camera} tone="info">
                            Body maps, injury photos and authorisation forms
                            attach from the event detail once it&apos;s saved —
                            open the event and use the Evidence section.
                        </InfoCard>
                    </div>
                ) : null}
            </WizardStepPane>
...
function StaffMultiSelect({
    ...
    return (
        <GuardrailCard
            unstyled
            className="flex max-h-44 flex-wrap gap-1.5 overflow-y-auto rounded-lg border border-border bg-card/50 p-2"
        >
            {options.map((o) => {
