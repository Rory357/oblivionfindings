            ...
            {procedures.length ? (
                <ul className="divide-y divide-border">
                    {procedures.map((p) => {
                        ...
                        return (
                            <li key={p.id} className="flex items-center gap-2 px-4 py-3 transition-colors hover:bg-muted/45 md:px-5">
                                <Link href={`/health-safety/procedures?procedure=${p.id}`} className="flex min-w-0 flex-1 items-center gap-3 focus-visible:outline-none">
                                ...
                                {(showAcknowledge || ackReadonly) && p.acknowledged ? (
                                ...
                                ) : showAcknowledge && p.status === 'approved' ? (
                                    // eslint-disable-next-line no-restricted-syntax -- inline ack affordance on a list row, not a form control
                                    <button
                                        type="button"
                                        onClick={() => router.post(`/health-safety/procedures/${p.id}/acknowledge`, {}, { preserveScroll: true, preserveState: true })}
