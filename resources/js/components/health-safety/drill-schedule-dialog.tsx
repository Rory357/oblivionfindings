...
const STEPS: WizardStep[] = [
    ...
    { key: 'schedule', label: 'Scenario & schedule', blurb: 'When it runs', icon: CalendarClock },
    { key: 'people', label: 'Wardens & people', blurb: 'Who takes part', icon: Users },
    { key: 'review', label: 'Review', blurb: 'Confirm & schedule', icon: CheckCircle2 },
];
...
export function DrillScheduleDialog({
    ...
    const typeTiles = useMemo(
        () =>
            SCHEDULE_TYPE_KEYS.map((key) => {
                ...
                return { key, label: m.label, icon: m.icon, description: TYPE_DESCRIPTIONS[key] };
            }),
        [],
    );
    ...
    const coordinatorName = staff.find((s) => String(s.id) === form.data.conducted_by)?.name ?? '—';
    const wardenNames = staff.filter((s) => form.data.warden_ids.includes(s.id)).map((s) => s.name);
    ...
    return (
        ...
        >
            {step === 0 ? (
                <WizardStepPane>
                    <StepHead icon={MapPin} title="Site & drill type" blurb="Where, and what kind of drill" />
                    <div className="flex flex-col gap-4">
                        ...
                        <Field label="Drill type" required error={form.errors.drill_type}>
                            <TilePicker value={form.data.drill_type} onChange={(v) => form.setData('drill_type', v)} options={typeTiles} />
                        </Field>
                    </div>
                </WizardStepPane>
            ) : null}
            ...
            {step === 1 ? (
                <WizardStepPane>
                    <StepHead icon={CalendarClock} title="Scenario & schedule" blurb="When it runs and the scenario brief" />
                    <div className="grid gap-4 sm:grid-cols-2">
                        <Field label="Date" required error={form.errors.date ?? (form.errors as Record<string, string | undefined>).scheduled_at}>
                            <Input type="date" value={form.data.date} onChange={(e) => form.setData('date', e.target.value)} />
                        </Field>
                        <Field label="Start time" required error={form.errors.time}>
                            <Input type="time" value={form.data.time} onChange={(e) => form.setData('time', e.target.value)} />
                        </Field>
                        <Field label="Drill title" error={form.errors.title} span>
                            <Input
            ...
            {step === 2 ? (
                <WizardStepPane>
                    <StepHead icon={Users} title="Wardens & participants" blurb="Who runs it and who's expected" />
                    <div className="grid gap-4 sm:grid-cols-2">
                        <Field label="Drill coordinator" required error={form.errors.conducted_by}>
                            <SelectInput
                                value={form.data.conducted_by}
                                onChange={(v) => form.setData('conducted_by', v)}
                                placeholder="Select coordinator"
                                options={staff.map((s) => ({ value: String(s.id), label: s.name }))}
                            />
                        </Field>
                        <Field label="Expected residents" error={form.errors.total_participants}>
                            <Input
                                type="number"
                                min={0}
                                value={form.data.total_participants}
                                onChange={(e) => form.setData('total_participants', e.target.value)}
                            />
                        </Field>
                        <Field label="Fire wardens on shift" hint="Seeds the roll-call" span>
                            <div className="flex flex-wrap gap-1.5">
                                {staff.slice(0, 40).map((s) => {
                                    const active = form.data.warden_ids.includes(s.id);
                                    return (
                                        <button
                                            key={s.id}
                                            type="button"
                                            ...
                                            onClick={() =>
                                                form.setData(
                                                    'warden_ids',
                                                    active ? form.data.warden_ids.filter((id) => id !== s.id) : [...form.data.warden_ids, s.id],
                                                )
                                            }
                                            className={cn(
            ...
            {step === 3 ? (
                <WizardStepPane>
                    <StepHead icon={CheckCircle2} title="Review & schedule" blurb="Confirm the drill before it's added to the calendar" />
                    <div className="grid gap-4 sm:grid-cols-2">
                        <ReviewCard icon={MapPin} title="Drill" onEdit={() => setStep(0)}>
                            <ReviewRow label="Site" value={siteName} />
                            <ReviewRow label="Drill type" value={typeMeta(form.data.drill_type).label} />
                            <ReviewRow label="Title" value={form.data.title} />
                        </ReviewCard>
                        <ReviewCard icon={CalendarClock} title="Schedule" onEdit={() => setStep(1)}>
                            <ReviewRow label="Date" value={form.data.date} />
                            <ReviewRow label="Start time" value={form.data.time} />
                            <ReviewRow label="Assembly point" value={form.data.assembly_point} />
                            <ReviewRow label="Unannounced" value={form.data.is_unannounced ? 'Yes' : 'No'} />
                        </ReviewCard>
                        <ReviewCard icon={Users} title="People" onEdit={() => setStep(2)} span>
                            <ReviewRow label="Coordinator" value={coordinatorName} />
                            <ReviewRow label="Expected residents" value={form.data.total_participants} />
                            <ReviewRow label="Wardens" value={wardenNames.length ? wardenNames.join(', ') : undefined} />
                        </ReviewCard>
                    </div>
                    <div className="mt-4">
                        <InfoCard icon={CalendarCheck} tone="info">
                            This drill will appear on the site's calendar and the site profile's Drills tab as soon as it's scheduled.
                        </InfoCard>
                    </div>
                </WizardStepPane>
            ) : null}
