...
function SessionDetailDialog({
    ...
    const canAct =
        can.manage && (d.status === 'active' || d.status === 'overdue');
    ...
    const SECTIONS: WizardStep[] = [
        ...
        },
        {
            key: 'checkins',
            label: 'Check-ins',
            blurb: `${d.check_ins.length} logged`,
            icon: ListChecks,
        },
        {
            key: 'alerts',
            label: 'Alerts',
            blurb: d.alerts.length ? `${d.alerts.length} raised` : 'None',
            icon: Bell,
        },
    ];
            ...
            {canAct ? (
                <>
                    <ActBtn
                        icon={CheckCircle2}
                        onClick={() =>
                            setAction({ kind: 'checkin', session: d })
                        }
                    >
                        Record check-in
                    </ActBtn>
                    <ActBtn
                        icon={Clock}
                        onClick={() =>
                            setAction({ kind: 'extend', session: d })
                        }
                    >
                        Extend / edit
                    </ActBtn>
                    <ActBtn
                        icon={XCircle}
                        onClick={() => setAction({ kind: 'end', session: d })}
                    >
                        End session
                    </ActBtn>
                    <GuardrailButton
                        ...
                        type="button"
                        onClick={() =>
                            setAction({ kind: 'emergency', session: d })
                        }
                ...
                <span className="text-xs text-muted-foreground">
                    No lifecycle actions — session{' '}
                    {SESSION_LABEL[d.status]?.toLowerCase() ?? d.status}.
                </span>
            )}
            {canRemove ? (
                <GuardrailButton
        ...
        >
            {action ? (
                <LoneWorkerActionForm
                    target={action}
                    onDone={() =>
                        action.kind === 'delete' ? onClose() : setAction(null)
                    }
    ...
    return (
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            <ReviewCard icon={ClipboardCheck} title="Monitoring plan">
                ...
                <ReviewRow
                    label="Linked shift"
                    value={d.shift ? `SH-${d.shift.id}` : 'Ad-hoc (no shift)'}
                />
                <ReviewRow
                    label="Activity"
                ...
                <ReviewRow
                    label="Started"
                    value={formatDateTime(d.started_at)}
                />
                <ReviewRow
                    label="Expected end"
                    value={formatDateTime(d.expected_end_at)}
                />
                <ReviewRow
                    label="Check-in interval"
                    value={
                        d.check_in_interval_minutes
                            ? `Every ${d.check_in_interval_minutes} min`
                            : '—'
                    }
                />
                <ReviewRow
            ...
            <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 px-3 py-2.5">
                        <div className="min-w-0">
                            <div className="truncate text-sm font-medium text-foreground">
                                {d.location || 'No location recorded'}
                            </div>
                            {hasCoords ? (
                        ...
                        <div className="flex items-center justify-between gap-2">
                            <div className="min-w-0">
                                <div className="flex items-center gap-1.5 text-sm font-medium text-foreground">
                                    ...
                                    {d.tracker.name || 'GPS tracker'}
                                </div>
                                <div className="text-xs text-muted-foreground">
                                    {d.tracker.battery_level != null
                                        ? `Battery ${d.tracker.battery_level}% · `
                                        : ''}
                                    Last seen{' '}
                                    {formatRelative(d.tracker.last_seen_at)}
                                </div>
                            </div>
                            <GuardrailButton
                                unstyled
                        ...
                        {d.tracker.panic_active ? (
                            <div className="mt-2 flex items-center justify-between gap-2 rounded-lg bg-status-critical-bg px-3 py-2">
                                ...
                                <GuardrailButton
                                    ...
                                    onClick={() =>
                                        router.post(
                                            d.tracker!.acknowledge_panic_url,
                                            {},
                                            { preserveScroll: true },
                                        )
                                    }
                ...
                ) : d.tracker_state === 'integrity_error' ? (
                    <InfoCard icon={AlertTriangle} tone="crit">
                        Tracker details are unavailable because the assignment
                        or Site record needs repair. Location and panic actions
                        are disabled until the record is corrected.
                    </InfoCard>
                ) : null}
...
function SessionCheckins({ d }: { d: SessionDetail }) {
    const events: {
        ...
        ...(d.status === 'completed' && d.ended_at
            ? [{ kind: 'end', label: 'Session ended', at: d.ended_at }]
            : []),
    ].sort(
    ...
    const kindStyle = (
        kind: string,
    ): { icon: typeof Check; dot: string; fg: string } => {
        switch (kind) {
            case 'ok':
                return {
                    icon: Check,
                    ...
                    {events.map((ev, i) => {
                        ...
                        return (
                            <li key={i} className="relative">
                                <span
                                    className={cn(
                                        'absolute -left-[26px] flex h-4 w-4 items-center justify-center rounded-full ring-4 ring-background',
                                        s.dot,
                                    )}
                                >
...
function SessionAlerts({
    ...
    return (
        <div className="flex flex-col gap-3">
            ...
            {d.alerts.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                    No operational alerts have been raised for this session.
                </p>
            ) : (
                <div className="flex flex-col gap-2">
                    {d.alerts.map((a) => {
                        ...
                        const status = ALERT_STATUS_META[a.status] ?? {
                            tone: 'neutral' as const,
                            label: a.status,
                        };
                        return (
            ...
            {d.legacy_alert_history.length > 0 ? (
                <div className="mt-2 flex flex-col gap-2 border-t border-border pt-3">
                    <p className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">
                        Historical records · read only
                    </p>
                    {d.legacy_alert_history.map((a) => {
                        const meta = ALERT_TYPE_META[a.type] ?? {
                            tone: 'neutral' as const,
                            label: a.type,
                        };
...
function AlertDetailDialog({
    ...
    const meta = ALERT_TYPE_META[d.type] ?? {
        tone: 'neutral' as const,
        label: d.type,
    };
    const Icon = ALERT_TYPE_ICON[d.type] ?? Bell;
    const status = ALERT_STATUS_META[d.status] ?? {
        tone: 'neutral' as const,
        label: d.status,
    };
    const isHistorical =
        d.source === 'legacy_history' || d.is_historical === true;
                ...
                <div className="flex items-center gap-3 border-b border-border px-5 py-3.5">
                    ...
                    <div className="min-w-0 flex-1">
                        ...
                        <p className="truncate text-xs text-muted-foreground">
                            {d.session?.user?.name ?? 'Lone worker'} ·{' '}
                            {d.session?.site?.name ?? 'No site'}
                        </p>
                    </div>
                    <GuardrailButton
                        unstyled
                    ...
                    {action ? (
                        ...
                        <div className="flex flex-col gap-4">
                            <div className="rounded-xl border border-border">
                                <SummaryRow
                                    label="Worker"
                                    value={d.session?.user?.name ?? '—'}
                                />
                                <SummaryRow
                                    label="Site / Client"
                                    value={`${d.session?.site?.name ?? '—'}${d.session?.client?.name ? ` · ${d.session.client.name}` : ''}`}
                                />
                                <SummaryRow
                                    label="Triggered"
                                    value={formatDateTime(d.triggered_at)}
                                />
                                <SummaryRow
                                    label="Status"
                                    value={
                                        ...
                                        <FlagBadge
                                            ...
                                            title={
                                                isHistorical
                                                    ...
                                                    : 'Canonical · owned by Control Room'
                                            }
                                        >
                                            {isHistorical
                                                ? 'Historical'
                            ...
                            {d.incident_id ? (
                                <div className="flex items-center gap-1.5 rounded-lg bg-status-info-bg px-3 py-2 text-xs font-medium text-status-info">
                                    ...
                                    Escalated to incident INC-{d.incident_id}
                                </div>
                            ) : null}

                            {isHistorical ? (
                                <InfoCard icon={FileText} tone="info">
                                    This is read-only historical evidence. It is
                                    not an active alert and cannot be
                                    acknowledged or resolved.
                                </InfoCard>
                            ) : (
                                <InfoCard icon={RadioTower} tone="info">
                                    This is the canonical operational alert. Its
                                    status, actions, SLA, escalation and history
                                    are shared with the{' '}
                            ...
                            {sessionId ? (
                                ...
                                >
                                    <Activity className="h-4 w-4" /> View linked
                                    session <ChevronRight className="h-3 w-3" />
                                </GuardrailButton>
                            ) : null}
                        </div>
                    )}
