...
import {
    SiteAddStorageDialog,
    type StorageSubstanceOption,
} from '@/components/health-safety/site-chemical-storage-dialog';
...
export function SiteChemicalsPanel({
    data,
    siteId,
    siteName,
}: {
    data: SiteChemicalsData;
    siteId: number;
    siteName: string;
}) {
            ...
            <CardHeader className="gap-3">
                <div className="flex flex-wrap items-start justify-between gap-3">
                        ...
                        <div>
                            <CardTitle className="flex items-center gap-2">
                                ...
                                <span className="text-sm font-normal text-muted-foreground">
                                    · {summary.count} substance
                                    {summary.count === 1 ? '' : 's'}
                                </span>
                            </CardTitle>
                            <p className="mt-0.5 text-sm text-muted-foreground">
                                Site-scoped storage. The substance master record
                                lives in the Chemical register.
                            </p>
                        </div>
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                        {data.can_add ? (
                            ...
                            >
                                <Plus className="mr-1.5 h-4 w-4" /> Add storage
                                here
                            </Button>
                        ) : null}
                ...
                {summary.count > 0 ? (
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 rounded-lg bg-muted/40 px-3 py-2 text-xs">
                        ...
                        >
                            {summary.controlled} controlled
                        </span>
                        <span
                            className={
                                summary.sds_to_action > 0
                                    ? 'text-status-critical'
                                    : 'text-muted-foreground'
                            }
                        >
                            {summary.sds_to_action} SDS to action
                        </span>
                        <span
                            className={
                                summary.segregation_gaps > 0
                                    ? 'text-status-critical'
                                    : 'text-muted-foreground'
                            }
                        >
                            {summary.segregation_gaps} segregation gap
                            {summary.segregation_gaps === 1 ? '' : 's'}
                        </span>
                    </div>
                ) : null}
            </CardHeader>
                ...
                {rows.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-10 text-center">
                        ...
                        <p className="text-sm font-semibold text-foreground">
                            No chemicals stored here
                        </p>
                        <p className="mt-0.5 max-w-xs text-xs text-muted-foreground">
                            Storage locations are added from each substance's
                            record in the Chemical register.
                        </p>
                    </div>
                ) : (
                    <div className="overflow-x-auto">
                                ...
                                {rows.map((r) => (
                                    <tr
                                        ...
                                        onClick={() =>
                                            r.substance &&
                                            router.visit(
                                                `/health-safety/substances?substance=${r.substance.id}`,
                                            )
                                        }
                                        onKeyDown={(e) => {
                                            if (
                                    ...
                                    >
                                        ...
                                        <td className="px-3 py-3 align-top">
                                            <div className="flex flex-wrap gap-1.5">
                                                <CompChip
                                                    ok={r.properly_labelled}
                                                    label="Labelled"
                                                />
                                                <CompChip
                                                    ok={r.segregation_compliant}
                                                    label="Segregated"
                                                />
                                            </div>
                                        </td>
                        ...
                        <p className="mt-3 text-xs text-muted-foreground">
                            Storage locations are site-scoped but managed from
                            each substance's record. Adding, editing or removing
                            a location updates the register.
                        </p>
                    </div>
                )}
            </CardContent>
