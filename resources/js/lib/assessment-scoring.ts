...
export type AssessmentInputs = Record<
    string,
...
function must(inputs: AssessmentInputs): ScoreResult {
    ...
    const band: RiskBandValue =
        total >= 2 ? 'high' : total === 1 ? 'medium' : 'low';
        ...
        breakdown: [
            {
                ...
                label: 'BMI',
                detail:
                    bmi === null ? 'Not provided' : `${bmi.toFixed(1)} kg/m²`,
                points: bmiPts,
            },
            {
                key: 'weight_loss',
...
const FRAT_MAPS: Record<
    string,
    Record<string, { points: number; label: string }>
> = {
    recent_falls: {
...
function frat(inputs: AssessmentInputs): ScoreResult {
    ...
    const breakdown: BreakdownRow[] = FRAT_FACTORS.map((f) => {
        ...
        const chosen =
            typeof inputs[f.key] === 'string'
                ? map[inputs[f.key] as string]
                : undefined;
        ...
        return {
            key: f.key,
            label: f.label,
            detail: chosen ? opt.label : 'Not specified',
            points: opt.points,
        };
    });
    const band: RiskBandValue =
        total >= 16 ? 'high' : total >= 12 ? 'medium' : 'low';
...
function braden(inputs: AssessmentInputs): ScoreResult {
    ...
    const breakdown: BreakdownRow[] = BRADEN_SUBSCALES.map((s) => {
        ...
        const value =
            raw === null
                ? s.max
                : Math.max(1, Math.min(s.max, Math.trunc(raw)));
        total += value;
        return {
            key: s.key,
            label: s.label,
            detail: s.options[value] ?? String(value),
            points: value,
        };
    });
    const band: RiskBandValue =
...
const IDDSI_DRINKS: Record<number, string> = {
    0: 'Thin',
    1: 'Slightly Thick',
    2: 'Mildly Thick',
    3: 'Moderately Thick',
...
const IDDSI_FOODS: Record<number, string> = {
    3: 'Liquidised',
    4: 'Pureed',
    5: 'Minced & Moist',
    6: 'Soft & Bite-Sized',
        ...
        breakdown: [
            {
                ...
                detail:
                    drink === null
                        ? 'Not specified'
                        : `Level ${drink} · ${IDDSI_DRINKS[drink]}`,
                points: null,
            },
            {
                key: 'food_level',
