...
export default function BillCreate({
    ...
    useEffect(() => {
        if (data.vendor_id && data.bill_date) {
            ...
            if (vendor?.payment_terms_days) {
                ...
                billDate.setDate(
                    billDate.getDate() + vendor.payment_terms_days,
                );
                setData('due_date', billDate.toISOString().split('T')[0]);
            }
        }
    }, [data.vendor_id, data.bill_date, setData, vendors]);
    ...
    const calcLineTotal = (line: LineItem) =>
        calcLineSubtotal(line) + calcLineGst(line);

    const subtotal = data.lines.reduce(
        (sum, line) => sum + calcLineSubtotal(line),
        0,
    );
    const gstTotal = data.lines.reduce(
        (sum, line) => sum + calcLineGst(line),
        0,
    );
            ...
            >
                <form onSubmit={handleSubmit}>
                    ...
                    <Card className="mb-6">
                        ...
                        <CardContent>
                            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                                ...
                                <div>
                                    <Label htmlFor="vendor_reference">
                                        Vendor Reference
                                    </Label>
                                    <Input
                                        id="vendor_reference"
                    ...
                    <Card className="mb-6">
                        ...
                        <CardContent>
                            {errors.lines && (
                                <p className="mb-2 text-sm text-status-critical">
                                    {errors.lines}
                                </p>
                            )}
