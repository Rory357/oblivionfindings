...
export default function PaymentMatchingIndex({ matches, filters }: Props) {
    ...
    function handleConfirm(matchId: number) {
        router.post(
            `/finance/payment-matching/${matchId}/confirm`,
            {},
            {
                preserveScroll: true,
            },
    ...
    function handleReject(matchId: number) {
        router.post(
            `/finance/payment-matching/${matchId}/reject`,
            {},
            {
                preserveScroll: true,
            },
    ...
    const suggestedCount = useMemo(
        () => matches.data.filter((m) => m.status === 'suggested').length,
        [matches.data],
    );
    ...
    const rejectedCount = useMemo(
        () => matches.data.filter((m) => m.status === 'rejected').length,
        [matches.data],
    );
            ...
            >
                        ...
                        {matches.data.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-12 text-center">
                                ...
                                <p className="mb-4 text-muted-foreground">
                                    Run auto-match to find matches between bank
                                                ...
                                                {match.status ===
                                                    ...
                                                    <div className="flex items-center justify-end gap-1">
                                                        <Button
                                                            variant="ghost"
                                                            size="sm"
                                                            className="text-status-success hover:bg-status-success hover:text-status-success"
                                                            onClick={() =>
                                                                handleConfirm(
                                                                    match.id,
                                                                )
                ...
                {matches.last_page > 1 && (
                    <div className="mt-4 flex items-center justify-between">
                        <p className="text-sm text-muted-foreground">
                            Showing{' '}
                            {(matches.current_page - 1) * matches.per_page + 1}{' '}
                            to{' '}
                            {Math.min(
