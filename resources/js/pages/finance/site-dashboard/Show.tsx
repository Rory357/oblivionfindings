...
const varianceBadge = (status: string) => {
    switch (status) {
        ...
        case 'approaching':
            return (
                <Badge className="border-0 bg-status-warning text-white">
                    Approaching
                </Badge>
            );
        case 'under_budget':
            ...
            <div className="flex flex-col gap-6 p-6">
                ...
                <PageHero
                    ...
                    stats={[
                        {
                            label: 'Period',
                            value: `${filters.from.slice(5)} to ${filters.to.slice(5)}`,
                        },
                    ]}
                />
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="space-y-2">
                            {insights.map((insight, i) => (
                                <div
                                    key={i}
                                    className={`flex items-start gap-3 rounded-lg border p-3 ${severityColor[insight.severity] || severityColor.info}`}
                                >
                    ...
                    <CardContent>
                        <div className="grid gap-4 sm:grid-cols-4">
                            <div className="rounded-lg bg-primary/10 p-4 dark:bg-primary/30">
                                ...
                                <p className="mt-1 text-xl font-bold text-primary dark:text-primary/70">
                                    {$(dashboard.staffing.wages)}
                                </p>
                            </div>
                            <div className="rounded-lg bg-status-info-bg p-4">
                                ...
                                <p className="mt-1 text-xl font-bold text-status-info dark:text-status-info">
                                    {$(dashboard.staffing.employer_oncost)}
                                </p>
                                <p className="mt-0.5 text-[10px] text-muted-foreground">
                                    {pct(
                                        dashboard.staffing.oncost_pct_of_wages,
                                    )}{' '}
                                    of wages
                                </p>
                            </div>
                            ...
                            <div className="rounded-lg bg-muted p-4 dark:bg-muted/30">
                                ...
                                <p className="mt-1 text-xl font-bold text-foreground dark:text-muted-foreground">
                                    {pct(
                                        dashboard.staffing
                                            .staffing_pct_of_total,
                                    )}
                                </p>
                            </div>
                        </div>
                                ...
                                <TableBody>
                                    {variance.lines.map((line) => (
                                        <TableRow key={line.category}>
                                            ...
                                            <TableCell
                                                className={`text-right tabular-nums ${Number(line.variance) > 0 ? 'text-status-critical' : 'text-status-success'}`}
                                            >
                                                {$(line.variance)}
                                            </TableCell>
                                            <TableCell className="text-right tabular-nums">
                                    ...
                                    <TableRow className="border-t-2 font-semibold">
                                        ...
                                        <TableCell className="text-right tabular-nums">
                                            {$(variance.totals.planned)}
                                        </TableCell>
                                        <TableCell className="text-right tabular-nums">
                                            {$(variance.totals.actual)}
                                        </TableCell>
                                        <TableCell
                                            className={`text-right tabular-nums ${Number(variance.totals.variance) > 0 ? 'text-status-critical' : 'text-status-success'}`}
                                        >
