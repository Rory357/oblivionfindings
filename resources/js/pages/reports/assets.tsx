...
export default function AssetsReport() {
    const { overdueInspections, overdueMaintenance, expiringWarranties } =
                ...
                <Card>
                    ...
                    <CardContent className="space-y-2">
                        {overdueInspections?.length ? (
                            overdueInspections.map((a: AssetRow) => (
                                <Row
                                    key={a.id}
                                    a={a}
                                    right={a.inspection_due_at ?? ''}
                                />
                            ))
                        ) : (
                ...
                <Card>
                    ...
                    <CardContent className="space-y-2">
                        {overdueMaintenance?.length ? (
                            overdueMaintenance.map((a: AssetRow) => (
                                <Row
                                    key={a.id}
                                    a={a}
                                    right={a.maintenance_due_at ?? ''}
                                />
                            ))
                        ) : (
                            <div className="text-sm text-muted-foreground">
                ...
                <Card>
                    ...
                    <CardContent className="space-y-2">
                        {expiringWarranties?.length ? (
                            expiringWarranties.map((a: AssetRow) => (
                                <Row
                                    key={a.id}
                                    a={a}
                                    right={a.warranty_expires_at ?? ''}
                                />
                            ))
                        ) : (
                            <div className="text-sm text-muted-foreground">
