  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      ...
      <div className="space-y-4 p-4">
          ...
          <CardContent>
            {props.summary ? (
              <div className="space-y-2">
                <div className="text-xs text-muted-foreground">
                  {props.summary.model ? `Model: ${props.summary.model}` : null}
                  {props.summary.generated_at ? ` • Generated: ${new Date(props.summary.generated_at).toLocaleString()}` : null}
                </div>
            ...
            {form.errors.scope_type || form.errors.scope_id || form.errors.from || form.errors.to ? (
        ...
        <Card className="rounded-2xl">
          <CardHeader>
            <CardTitle className="text-base">How this works (MVP)</CardTitle>
          </CardHeader>
          <CardContent className="text-sm text-muted-foreground">
            This is using your internal timeline events to build a deterministic summary. When you’re ready, we can swap the job
            to call a real LLM (OpenAI, Azure OpenAI, etc.) and keep the same UI and storage.
          </CardContent>
        </Card>
      </div>
    </AppLayout>
