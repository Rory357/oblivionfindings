...
function summariseData(type: string, data: Record<string, unknown>): string {
    switch (type) {
        ...
        case 'sleep':
            return [
                data.quality
                    ? `${String(data.quality).charAt(0).toUpperCase()}${String(data.quality).slice(1)} sleep`
                    : null,
                data.interruptions && Number(data.interruptions) > 0
        ...
        case 'fluid_intake':
            return [
                data.amount_ml ? `${data.amount_ml}ml` : null,
                data.fluid_type,
            ]
                .filter(Boolean)
                .join(', ');
        case 'pain':
            return [data.score ? `Pain ${data.score}/10` : null, data.location]
                .filter(Boolean)
                .join(', ');
        default:
            return '';
...
export default function ObservationRegister({
    ...
    const typeLabel = (value: string) =>
        filter_options.observation_types.find((t) => t.value === value)
            ?.label ?? value;
                                        ...
                                        >
                                            <td className="px-4 py-3">
                                                {obs.client ? (
                                                    ...
                                                    >
                                                        {obs.client.first_name}{' '}
                                                        {obs.client.last_name}
                                                    </Link>
                                                ) : (
                    ...
                    {observations.last_page > 1 && (
                        <div className="flex items-center justify-between border-t px-4 py-3">
                            <p className="text-xs text-muted-foreground">
                                ...
                                {observations.last_page} ({observations.total}{' '}
                                total)
                            </p>
