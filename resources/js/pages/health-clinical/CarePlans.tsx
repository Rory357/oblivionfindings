...
export default function CarePlansLens({
    plans,
    stats,
    kpis,
    tab_counts,
        ...
        >
            ...
            <div className="flex flex-wrap items-center gap-3 rounded-xl border border-primary/25 bg-primary/5 px-4 py-3">
                ...
                <p className="flex-1 text-[13px] text-foreground">
                    A read-only review &amp; sign-off lens. Care plans are
                    created, reviewed and signed off in the
                    <strong> Care Plans module</strong> — this surfaces what
                    clinically needs attention.
                </p>
                <Link href="/operations/care-plans">
                    <Button size="sm" variant="outline" className="gap-1.5">
                        Open Care Plans module{' '}
            ...
            <RegisterStatStrip
                stats={[
                    { label: 'Active plans', value: stats.active },
                    {
                        ...
                        tone:
                            stats.reviews_overdue > 0 ? 'critical' : 'default',
                    },
                    {
                        label: 'Awaiting sign-off',
                        value: stats.awaiting_sign_off,
                        tone:
                            stats.awaiting_sign_off > 0 ? 'warning' : 'default',
                    },
                ]}
            />
                    ...
                    {plans.length === 0 ? (
                        <div className="p-12 text-center">
                            ...
                            <p className="mt-1 text-sm text-muted-foreground/70">
                                Nothing needs clinical review right now.
                            </p>
                        </div>
                    ) : (
                                    ...
                                    {plans.map((p) => (
                                        ...
                                        >
                                            <td className="px-4 py-3">
                                                ...
                                                {p.client ? (
                                                    <Link
                                                        href={`/operations/clients/${p.client.id}`}
                                            ...
                                            <td className="px-4 py-3 text-right">
                                                <Link
                                                    href={`/operations/care-plans/${p.id}`}
