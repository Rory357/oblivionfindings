...
export function ClientPicker({
    ...
    useEffect(() => {
        ...
        const handle = setTimeout(() => {
            fetch(`/health-clinical/clients/search?q=${encodeURIComponent(query)}`, {
    ...
    return (
        <div className="flex flex-col gap-2.5">
            ...
            <div className="flex items-center justify-between px-0.5 text-[11px] text-muted-foreground">
                <span>{loading ? 'Searching…' : `${results.length} ${results.length === 1 ? 'match' : 'matches'}`}</span>
                {results.length >= 20 ? <span>Showing first 20 — refine to narrow</span> : null}
            </div>
            <div className="max-h-[280px] overflow-y-auto rounded-lg border border-border">
                {results.length === 0 && !loading ? (
                    <div className="px-4 py-10 text-center text-sm text-muted-foreground">
                        {query ? 'No clients match that search.' : 'Start typing to find a client.'}
                    </div>
                ) : (
                            ...
                            >
                                ...
                                <span className="min-w-0 flex-1">
                                    ...
                                    <span className="block truncate text-xs text-muted-foreground">
                                        {[c.site, c.nhi].filter(Boolean).join(' · ') || 'No site'}
                                    </span>
                                </span>
                            </button>
                        ))}
...
export function ClientChip({ client, onClear }: { client: ClientResult; onClear?: () => void }) {
    return (
        <div className="flex items-center gap-3 rounded-lg border border-primary/30 bg-primary/5 px-3 py-2.5">
            ...
            <div className="min-w-0 flex-1">
                ...
                <div className="truncate text-xs text-muted-foreground">
                    {[client.site, client.nhi].filter(Boolean).join(' · ') || 'No site'}
                </div>
            </div>
            {onClear ? (
                <button
...
export function ClinicalCardRail({ clientId }: { clientId: number | null }) {
    ...
    useEffect(() => {
        ...
        let active = true;
        setLoading(true);
        fetch(`/health-clinical/clients/${clientId}/clinical-card`, { headers: { Accept: 'application/json' } })
            .then((r) => r.json())
    ...
    if (!clientId) {
        return (
            <div className="rounded-lg border border-dashed border-sidebar-border bg-card/40 p-3 text-[11px] leading-relaxed text-muted-foreground">
                ...
                Allergies, baseline vitals &amp; active protocols load here once a client is chosen.
            </div>
        );
    }
    ...
    return (
        <div className="rounded-lg border border-sidebar-border bg-card/60 p-3 text-[12px]">
            <div className="mb-2 flex items-center gap-1.5 text-[11px] font-bold tracking-wide text-muted-foreground uppercase">
                <HeartPulse className="h-3.5 w-3.5 text-primary" /> Clinical card
                {loading ? <Loader2 className="ml-auto h-3 w-3 animate-spin" /> : null}
            </div>

            {/* Allergies */}
            <div className="mb-2.5">
                <div className="mb-1 flex items-center gap-1 text-[10.5px] font-semibold text-muted-foreground">
                    <ShieldAlert className="h-3 w-3 text-status-critical" /> Allergies
                </div>
                {card?.allergies?.length ? (
                    <div className="flex flex-wrap gap-1">
                        {card.allergies.map((a) => (
                            <span key={a} className="rounded-full bg-status-critical-bg px-2 py-0.5 text-[10.5px] font-medium text-status-critical">
                                {a}
                            </span>
                        ))}
                    </div>
            ...
            {/* Baseline vitals */}
            <div className="mb-2.5">
                <div className="mb-1 text-[10.5px] font-semibold text-muted-foreground">Baseline vitals</div>
                {card?.baseline_vitals ? (
                    <div className="flex flex-wrap items-center gap-1.5">
                        <span className="text-[11px] text-foreground">{card.baseline_vitals.summary || '—'}</span>
                        {card.baseline_vitals.news2_band ? (
                            <span className={cn('rounded-full px-1.5 py-0.5 text-[10px] font-semibold', BAND_PILL[card.baseline_vitals.news2_band])}>
                                NEWS2 {card.baseline_vitals.news2_score} · {NEWS2_BAND_LABEL[card.baseline_vitals.news2_band]}
                            </span>
                        ) : null}
                    </div>
                ) : (
                    <span className="text-[11px] text-muted-foreground">No vitals recorded yet</span>
                )}
            </div>
            ...
            <div>
                <div className="mb-1 flex items-center gap-1 text-[10.5px] font-semibold text-muted-foreground">
                    <Workflow className="h-3 w-3 text-primary" /> Active protocols
                </div>
                {card?.active_protocols?.length ? (
                    <ul className="space-y-0.5">
                        {card.active_protocols.slice(0, 4).map((p) => (
                            <li key={p.id} className="truncate text-[11px] text-foreground">
