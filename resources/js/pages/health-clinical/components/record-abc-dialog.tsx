...
const FUNCTIONS: {
...
}[] = [
    {
        ...
        description: 'To get away from a demand or situation',
        icon: ShieldAlert,
    },
    {
        key: 'attention_social',
        label: 'Attention / social',
        description: 'To gain attention or interaction',
        icon: Users,
    },
    {
        key: 'tangible_access',
        label: 'Tangible / access',
        description: 'To obtain an item, activity or outcome',
        icon: Target,
    },
    {
...
const STEPS: readonly WizardStep[] = [
    ...
    },
    {
        ...
        blurb: 'Strategies, harm, evidence',
        icon: HeartHandshake,
    },
...
function Body({ onClose, client, onSaved }: RecordAbcDialogProps) {
    ...
    const addTag = () => {
        ...
        if (t && !data.behaviour_tags.includes(t))
            setData('behaviour_tags', [...data.behaviour_tags, t]);
        setTagDraft('');
    };
    ...
    return (
        <WizardShell
            ...
            railExtra={
                <ClinicalCardRail clientId={picked ? picked.id : null} />
            }
            footerStart={
                stepIndex > 0 ? (
            ...
            {STEPS[stepIndex].key === 'context' ? (
                <WizardStepPane>
                    <StepHead
                        icon={MapPin}
                        title="Context"
                        blurb="Who the entry is for, and when and where it happened."
                    />
                    <div className="grid gap-4">
                        <Field label="Client" required>
                            <ClientPicker
                                value={picked}
                                onChange={lockedClient ? () => {} : setPicked}
                            />
                        </Field>
                        <div className="grid gap-4 sm:grid-cols-2">
                            ...
                            <Field label="Setting" hint="where / activity">
                                <Input
                                    value={data.setting}
                                    onChange={(e) =>
                                        setData('setting', e.target.value)
                                    }
                                    placeholder="e.g. dining room, morning routine"
                                />
                            </Field>
                            <Field
                                label="Others present"
                                span
                                hint="who else was there"
                            >
                                <Input
                                    value={data.others_present}
                                    onChange={(e) =>
                                        setData(
                                            'others_present',
                                            e.target.value,
                                    ...
                                    placeholder="e.g. two staff, one peer"
                                />
                            </Field>
                        </div>
                    </div>
            ...
            {STEPS[stepIndex].key === 'abc' ? (
                <WizardStepPane>
                    <StepHead
                        ...
                        blurb="What led up to it, what happened, and what followed."
                    />
                    <div className="grid gap-4">
                        <Field
                            label="Antecedent"
                            required
                            hint="what happened just before"
                        >
                            <Textarea
                                rows={2}
                                value={data.antecedent}
                                onChange={(e) =>
                                    setData('antecedent', e.target.value)
                                }
                                placeholder="The trigger or situation before the behaviour."
                            />
                        </Field>
                        <Field
                            label="Behaviour"
                            required
                            hint="observable & measurable"
                        >
                            <Textarea
                                rows={2}
                                value={data.behaviour}
                                onChange={(e) =>
                                    setData('behaviour', e.target.value)
                                }
                                placeholder="Exactly what the person did."
                            />
                        </Field>
                        <Field
                            label="Consequence"
                            required
                            hint="what happened after"
                        >
                            <Textarea
                                rows={2}
                                value={data.consequence}
                                onChange={(e) =>
                                    setData('consequence', e.target.value)
                                }
                                placeholder="The response and outcome."
                            />
                        </Field>

                        <Field
                            label="Behaviour tags"
                            hint="optional — add keywords"
                        >
                        ...
                        <Field
                            label="Hypothesised function"
                            hint="the 'why' (PBS)"
                        >
                            <TilePicker
                                value={data.behaviour_function}
                                onChange={(v) =>
                                    setData('behaviour_function', v)
                                }
                                cols={3}
            ...
            {STEPS[stepIndex].key === 'response' ? (
                <WizardStepPane>
                    <StepHead
                        ...
                        blurb="Strategies used, any harm, and supporting evidence."
                    />
                    <div className="grid gap-4">
                        <Field label="Strategies used">
                            <Textarea
                                rows={2}
                                value={data.strategies_used}
                                onChange={(e) =>
                                    setData('strategies_used', e.target.value)
                                }
                                placeholder="De-escalation / support strategies and how the person responded."
                            />
                        </Field>
                        ...
                        <div className="rounded-lg border border-border bg-muted/30 p-3">
                            <label className="flex items-start gap-3">
                                <Switch
                                    checked={data.harm_occurred}
                                    onCheckedChange={(v) =>
                                        setData('harm_occurred', v)
                                    }
                                />
                                <span>
                            ...
                            {data.harm_occurred ? (
                                <div className="mt-2.5">
                                    <Textarea
                                        ...
                                        placeholder="Describe the harm — attach a body map / photo below."
                                    />
                                </div>
                            ) : null}
                        </div>
                        ...
                        <div>
                            ...
                            <p className="mb-2 text-[13px] text-muted-foreground">
                                A body map / injury photo when harm occurred,
                                property-damage photos, or a scanned paper
                                chart.
                            </p>
                            <FileDropzone
                                onFiles={addFiles}
            ...
            {STEPS[stepIndex].key === 'review' ? (
                <WizardStepPane>
                    <StepHead
                        icon={Check}
                        title="Review & record"
                        blurb="Confirm the ABC entry before recording."
                    />
                    <div className="grid gap-3 sm:grid-cols-2">
                        <ReviewCard
                            icon={MapPin}
                    ...
                    {!picked ? (
                        <InfoCard icon={Activity} tone="warn">
                            Choose a client on the Context step before
                            recording.
                        </InfoCard>
                    ) : null}
                </WizardStepPane>
