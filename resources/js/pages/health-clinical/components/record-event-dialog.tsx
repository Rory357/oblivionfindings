...
const TYPES: { key: string; label: string; icon: ComponentType<{ className?: string }> }[] = [
    ...
    { key: 'allergic_reaction', label: 'Allergic reaction', icon: AlertTriangle },
...
const STEPS: readonly WizardStep[] = [
    { key: 'client', label: 'Client & type', blurb: 'Who & what happened', icon: Stethoscope },
    { key: 'what', label: 'What happened', blurb: 'Description & witnesses', icon: AlertTriangle },
    { key: 'response', label: 'Response & evidence', blurb: 'Action, follow-up, files', icon: ShieldAlert },
...
function Body({ onClose, client, onSaved }: RecordEventDialogProps) {
    ...
    const stepValid = (i: number): boolean => {
        if (i === 0) return !!data.client_id && !!data.event_type && !!data.severity;
    ...
    return (
        ...
        >
            {STEPS[stepIndex].key === 'client' ? (
                <WizardStepPane>
                    <StepHead icon={Stethoscope} title="What happened, and to whom?" blurb="Pick the client, the event type and how severe it was." />
                    <div className="grid gap-5">
                                ...
                                {SEVERITY.map((s) => {
                                    ...
                                    return (
                                        <button
                                            key={s.key}
                                            type="button"
                                            onClick={() => setData('severity', s.key)}
                                            className={cn(
                        ...
                        {hsLinked ? (
                            <InfoCard icon={ShieldAlert} tone="warn">
                                Falls, seizures and choking automatically raise a linked <strong>Health &amp; Safety event</strong>{' '}
                                (with a WorkSafe-notifiable check) when you log this.
                            </InfoCard>
                        ) : null}
                    </div>
                </WizardStepPane>
            ...
            {STEPS[stepIndex].key === 'what' ? (
                <WizardStepPane>
                    <StepHead icon={AlertTriangle} title="What happened" blurb="Describe the event, when it occurred and who witnessed it." />
                    <div className="grid gap-4">
                        <Field label="Description" required>
                            <Textarea rows={4} value={data.description} onChange={(e) => setData('description', e.target.value)} placeholder="Describe what happened, in clinical detail." />
                        </Field>
                                    ...
                                    <div className="flex flex-wrap gap-1.5">
                                        {data.witnesses.map((w, i) => (
                                            <span key={i} className="inline-flex items-center gap-1 rounded-full border border-border bg-card px-2.5 py-1 text-[13px]">
                                                {w}
                                                <button type="button" onClick={() => setData('witnesses', data.witnesses.filter((_, idx) => idx !== i))} aria-label={`Remove ${w}`}>
            ...
            {STEPS[stepIndex].key === 'response' ? (
                <WizardStepPane>
                    <StepHead icon={ShieldAlert} title="Response & evidence" blurb="What was done, any follow-up, and supporting evidence." />
                    <div className="grid gap-4">
                        <div className="grid gap-4 sm:grid-cols-2">
                            <SubHead icon={ShieldAlert}>Immediate response</SubHead>
                            <Field label="Immediate action taken" span>
                                <Textarea rows={2} value={data.immediate_action_taken} onChange={(e) => setData('immediate_action_taken', e.target.value)} placeholder="First aid given, repositioning, observations started…" />
                            </Field>
                            <Field label="Outcome" span>
                                <Textarea rows={2} value={data.outcome} onChange={(e) => setData('outcome', e.target.value)} placeholder="Current condition / how it resolved." />
                            </Field>
                        </div>
                        ...
                        <div className="rounded-lg border border-border bg-muted/30 p-3">
                            <label className="flex items-start gap-3">
                                ...
                                <span>
                                    ...
                                    <span className="mt-0.5 block text-[13px] text-muted-foreground">Track an action that still needs completing.</span>
                                </span>
                            </label>
                            {data.requires_followup ? (
                                <div className="mt-2.5">
                                    <Textarea rows={2} value={data.followup_notes} onChange={(e) => setData('followup_notes', e.target.value)} placeholder="What needs to happen, and by when." />
                                </div>
                            ) : null}
                        </div>
                        ...
                        <div>
                            ...
                            <p className="mb-2 text-[13px] text-muted-foreground">
                                Injury / wound photos, a body map, observation charts or the WorkSafe PDF. Image, PDF or Word, up to 10&nbsp;MB each.
                            </p>
            ...
            {STEPS[stepIndex].key === 'review' ? (
                <WizardStepPane>
                    <StepHead icon={Check} title="Review & log" blurb="Confirm the event details before logging." />
                    <div className="grid gap-3 sm:grid-cols-2">
                        <ReviewCard icon={AlertTriangle} title="Event" onEdit={() => setStepIndex(0)}>
                            <ReviewRow label="Client" value={picked?.name} />
                            <ReviewRow label="Type" value={TYPES.find((t) => t.key === data.event_type)?.label} />
                            <ReviewRow label="Severity" value={SEVERITY.find((s) => s.key === data.severity)?.label} />
                            <ReviewRow label="Occurred at" value={data.occurred_at ? new Date(data.occurred_at).toLocaleString('en-NZ') : 'Now'} />
                        </ReviewCard>
                        <ReviewCard icon={ShieldAlert} title="Response" onEdit={() => setStepIndex(2)}>
                            <ReviewRow label="Action" value={data.immediate_action_taken} />
                            <ReviewRow label="Outcome" value={data.outcome} />
                            <ReviewRow label="Follow-up" value={data.requires_followup ? 'Required' : 'None'} />
                            <ReviewRow label="Witnesses" value={data.witnesses.length ? data.witnesses.join(', ') : undefined} />
                        ...
                        <ReviewCard icon={AlertTriangle} title="Description" span onEdit={() => setStepIndex(1)}>
                            <p className="text-[13px] whitespace-pre-wrap">{data.description || '—'}</p>
                        </ReviewCard>
                    </div>
                    {hsLinked ? (
                        <InfoCard icon={ShieldAlert} tone="warn">
                            A linked Health &amp; Safety event will be raised automatically.
                        </InfoCard>
                    ) : null}
                </WizardStepPane>
            ) : null}
