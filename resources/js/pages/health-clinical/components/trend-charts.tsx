...
export type VitalsPoint = Base & { systolic: number; diastolic: number; pulse: number };
export type FluidPoint = Base & { amount_ml: number; fluid_type: string | null };
...
function formatDateTime(value: string): string {
    return new Date(value).toLocaleDateString('en-NZ', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' });
}

type GenericLine = { dataKey: string; name: string; color: string };

export function TrendChartCard<TPoint extends { recorded_at: string; short_label: string }>({
    title,
    description,
    points,
    latestLabel,
                    ...
                    <div className="h-[260px]">
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={points}>
                                ...
                                <XAxis dataKey="short_label" tick={{ fontSize: 12 }} className="text-muted-foreground" />
                                ...
                                <Tooltip
                                    ...
                                    formatter={(value, name, item) => {
                                        ...
                                        if (tooltipFormatter) {
                                            return tooltipFormatter(value as number | string, name as string, point);
                                        }
                                ...
                                <Legend />
                                {lines.map((line) => (
                                    <Line key={line.dataKey} type="monotone" dataKey={line.dataKey} name={line.name} stroke={line.color} strokeWidth={2} dot={{ r: 3 }} activeDot={{ r: 5 }} />
                                ))}
                            </LineChart>
                        </ResponsiveContainer>
                    </div>
        ...
        <div className="grid gap-6 xl:grid-cols-2">
            {trendSets.news2 ? (
                <TrendChartCard
                    ...
                    latestLabel={trendSets.news2.latest ? `Latest ${trendSets.news2.latest.score}` : null}
                    emptyLabel="No NEWS2-scored vitals in this date range."
                    lines={[{ dataKey: 'score', name: 'NEWS2', color: 'var(--primary)' }]}
                    tooltipFormatter={(value, _n, p) => [`${value}${p.band ? ` · ${p.band.replace('_', '-')}` : ''}`, 'NEWS2']}
            ...
            <TrendChartCard
                ...
                latestLabel={trendSets.vitals.latest ? `Latest ${trendSets.vitals.latest.systolic}/${trendSets.vitals.latest.diastolic}, pulse ${trendSets.vitals.latest.pulse}` : null}
                ...
                lines={[
                    ...
                    { dataKey: 'systolic', name: 'Systolic', color: 'var(--chart-1)' },
                    { dataKey: 'diastolic', name: 'Diastolic', color: 'var(--chart-2)' },
                    { dataKey: 'pulse', name: 'Pulse', color: 'var(--chart-4)' },
                ]}
            ...
            <TrendChartCard
                ...
                latestLabel={trendSets.weight.latest ? `Latest ${trendSets.weight.latest.weight_kg} kg` : null}
                emptyLabel="No weight observations in this date range."
                lines={[{ dataKey: 'weight_kg', name: 'Weight (kg)', color: 'var(--chart-2)' }]}
            ...
            <TrendChartCard
                ...
                latestLabel={trendSets.pain.latest ? `Latest ${trendSets.pain.latest.score}/10` : null}
                emptyLabel="No pain observations in this date range."
                lines={[{ dataKey: 'score', name: 'Pain Score', color: 'var(--chart-3)' }]}
                tooltipFormatter={(value, _name, payload) => [`${Number(value).toFixed(0)}/10${payload.location ? ` · ${payload.location}` : ''}`, 'Pain Score']}
            ...
            <TrendChartCard
                ...
                latestLabel={trendSets.fluid_intake.latest ? `Latest ${trendSets.fluid_intake.latest.amount_ml} ml` : null}
                emptyLabel="No fluid intake observations in this date range."
                lines={[{ dataKey: 'amount_ml', name: 'Amount (ml)', color: 'var(--chart-1)' }]}
                tooltipFormatter={(value, _name, payload) => [`${Number(value).toFixed(0)} ml${payload.fluid_type ? ` · ${payload.fluid_type}` : ''}`, 'Fluid Intake']}
