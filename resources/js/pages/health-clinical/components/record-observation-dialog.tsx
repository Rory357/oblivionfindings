...
import { ACVPU_OPTIONS, scoreNews2, type News2Band } from '@/lib/news2';
...
const TYPES: {
...
}[] = [
    {
        key: 'vitals',
        label: 'Vital signs',
        description: 'BP, pulse, temp, SpO₂ → NEWS2',
        icon: HeartPulse,
        clinical: true,
    },
    {
    ...
    },
    {
        key: 'bowel',
        label: 'Bowel',
        description: 'Bristol stool type',
        icon: Toilet,
    },
    {
        key: 'sleep',
        label: 'Sleep',
        description: 'Bed/wake & quality',
        icon: Moon,
    },
    {
        key: 'general',
        label: 'General',
        description: 'Notes-only observation',
        icon: ClipboardList,
    },
];
...
const STEPS: readonly WizardStep[] = [
    {
        key: 'client',
        label: 'Client & type',
        blurb: 'Who & what to record',
        icon: Stethoscope,
    },
    {
        key: 'measure',
        label: 'Measurements',
        blurb: 'The clinical readings',
        icon: Activity,
    },
    {
        key: 'context',
        label: 'Context',
        blurb: 'When, notes & flags',
        icon: Clock,
    },
...
function Body({
    ...
    const form = useForm<ObsForm>({
        ...
        is_flagged: false,
        flagged_reason: '',
        protocol_schedule_id: protocolScheduleId
            ? String(protocolScheduleId)
            : '',
    });
    ...
    const setD = (key: string, value: string | boolean) =>
    ...
    const availableTypes = TYPES.filter(
        (t) => canRecordClinical || !t.clinical,
    );
    ...
    const chooseType = (key: string) => {
        ...
        form.setData((prev) => ({
            ...prev,
            observation_type: key as ObsTypeKey,
            data: {},
        }));
    };
    ...
    const stepValid = (i: number): boolean => {
        ...
        if (i === 1) {
            const req = data.observation_type
                ? REQUIRED[data.observation_type]
                : [];
            return req.every((k) => {
                const v = data.data[k];
                ...
                success={
                    <WizardSuccessPane
                        ...
                        blurb={
                            news2 ? (
                                <>
                                    NEWS2 <strong>{news2.score}</strong> ·{' '}
                                    {news2.bandLabel}. {news2.advice}
                                </>
                            ) : (
        ...
        >
            {STEPS[stepIndex].key === 'client' ? (
                <WizardStepPane>
                    <StepHead
                        icon={Stethoscope}
                        title="Who & what are we recording?"
                        blurb="Pick the client, then the observation type."
                    />
                    <div className="grid gap-5">
                        ...
                        {!canRecordClinical ? (
                            <InfoCard icon={Stethoscope}>
                                Vitals &amp; pain need the clinical recording
                                permission, so they're hidden here.
                            </InfoCard>
                        ) : null}
                    </div>
                </WizardStepPane>
            ...
            {STEPS[stepIndex].key === 'measure' ? (
                <WizardStepPane>
                    <StepHead
                        icon={Activity}
                        title="Measurements"
                        blurb="Enter the readings for this observation."
                    />
                    <MeasureFields
                        type={data.observation_type as ObsTypeKey}
                        data={data.data}
                        setD={setD}
                        news2={news2}
                    />
            ...
            {STEPS[stepIndex].key === 'context' ? (
                <WizardStepPane>
                    <StepHead
                        icon={Clock}
                        title="Context"
                        blurb="When it was taken, any notes, and whether it needs review."
                    />
                    <div className="grid gap-4">
                        <Field
                            label="Recorded at"
                            hint="leave blank for now (back-date for retrospective entry)"
                        >
                            <Input
                                type="datetime-local"
                        ...
                        <Field label="Clinical notes">
                            <Textarea
                                ...
                                placeholder="Anything the care team should know about this reading."
                            />
                        </Field>
                        <div className="rounded-lg border border-border bg-muted/30 p-3">
                            <label className="flex items-start gap-3">
                                <Switch
                                    checked={data.is_flagged}
                                    onCheckedChange={(v) =>
                                        setData('is_flagged', v)
                                    }
                                />
                                <span>
                                    <span className="flex items-center gap-1.5 text-sm font-semibold">
                                        <Flag className="h-3.5 w-3.5 text-status-warning" />{' '}
                                        Flag for clinical review
                                    </span>
                                    <span className="mt-0.5 block text-[13px] text-muted-foreground">
                                        Pushes this record to the
                                        registered-nurse sign-off queue.
                                    </span>
                                </span>
                            </label>
                            {data.is_flagged ? (
                                <div className="mt-2.5">
                                    <Input
                                        ...
                                        placeholder="Reason for the flag (optional)"
                                    />
                                </div>
                            ) : null}
                        </div>
            ...
            {STEPS[stepIndex].key === 'review' ? (
                <WizardStepPane>
                    <StepHead
                        icon={Check}
                        title="Review & record"
                        blurb="Confirm the details below, then record."
                    />
                    ...
                    {Object.keys(errors).length ? (
                        <p className="mt-3 text-xs text-status-critical">
                            Some fields need attention — check the measurements
                            step.
                        </p>
                    ) : null}
                    <p className="mt-4 text-[11px] text-muted-foreground">
                        Recorded against your account and added to the client
                        timeline. Vitals are scored with NEWS2 on save.
                    </p>
                </WizardStepPane>
            ) : null}
        </WizardShell>
...
function MeasureFields({
        ...
        return (
            <div className="grid gap-4">
                <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    {numField('systolic', 'Systolic (mmHg)', {
                        required: true,
                    })}
                    {numField('diastolic', 'Diastolic (mmHg)', {
                        required: true,
                    })}
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    {numField('respiration_rate', 'Respiratory rate', {
                        hint: 'breaths/min',
                    })}
                    ...
                    <Field label="Supplemental oxygen">
                        <div className="flex h-10 items-center gap-2.5">
                            <Switch
                                checked={!!data.on_oxygen}
                                onCheckedChange={(v) => setD('on_oxygen', v)}
                            />
                            <span className="text-[13px] text-muted-foreground">
                                {data.on_oxygen ? 'On oxygen' : 'On air'}
                            </span>
                        </div>
                    </Field>
                    <Field
                        label="SpO₂ Scale 2"
                        hint="hypercapnic / COPD target 88–92%"
                    >
                        <div className="flex h-10 items-center gap-2.5">
                            <Switch
                                checked={data.spo2_scale === '2'}
                                onCheckedChange={(v) =>
                                    setD('spo2_scale', v ? '2' : '1')
                                }
                            />
                            <span className="text-[13px] text-muted-foreground">
                                {data.spo2_scale === '2'
    ...
    if (type === 'weight') {
        return (
            <div className="grid gap-4 sm:grid-cols-2">
                {numField('weight_kg', 'Weight (kg)', { required: true })}
            </div>
        );
    }
        ...
        return (
            <div className="grid gap-4 sm:grid-cols-2">
                <Field label="Bed time" required>
                    <Input
                        type="time"
                        value={(data.bed_time as string) ?? ''}
                        onChange={(e) => setD('bed_time', e.target.value)}
                    />
                </Field>
                <Field label="Wake time" required>
                    <Input
                        type="time"
                        value={(data.wake_time as string) ?? ''}
                        onChange={(e) => setD('wake_time', e.target.value)}
                    />
                </Field>
    ...
    return (
        <InfoCard icon={ClipboardList}>
            A general observation captures a free-text note only — add it on the
            next step.
        </InfoCard>
    );
}
    ...
    if (!news2) {
        return (
            <InfoCard icon={Activity}>
                Enter respiratory rate, SpO₂, BP, pulse and temperature to see
                the live NEWS2 score.
            </InfoCard>
        );
    }
    return (
