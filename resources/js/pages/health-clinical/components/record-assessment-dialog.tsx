...
import { Field, InfoCard, StepHead, SubHead, TilePicker } from '@/components/wizard/primitives';
...
import { Check, ChevronLeft, ChevronRight, ClipboardList, Droplets, Loader2, Paperclip, Scale, ShieldAlert, Stethoscope, TrendingDown } from 'lucide-react';
...
const TYPES: { key: AssessmentTypeValue; label: string; blurb: string; icon: ComponentType<{ className?: string }> }[] = [
    { key: 'falls_frat', label: 'Falls (FRAT)', blurb: 'Peninsula Health Falls Risk Assessment Tool', icon: TrendingDown },
    { key: 'pressure_braden', label: 'Pressure (Braden)', blurb: 'Braden Scale for pressure-injury risk', icon: ShieldAlert },
    { key: 'malnutrition_must', label: 'Malnutrition (MUST)', blurb: 'BAPEN Malnutrition Universal Screening Tool', icon: Scale },
    { key: 'dysphagia_iddsi', label: 'Dysphagia (IDDSI)', blurb: 'IDDSI texture-level classification', icon: Droplets },
];

const SELECT_OPTIONS: Record<string, { value: string; label: string }[]> = {
    recent_falls: [
        { value: 'none_12mo', label: 'No falls in the last 12 months' },
        { value: 'one_plus_3_12mo', label: 'One or more between 3 and 12 months ago' },
        { value: 'one_plus_3mo', label: 'One or more in the last 3 months' },
        { value: 'one_plus_3mo_resident', label: 'One or more in the last 3 months whilst a resident' },
    ],
    medications: [
    ...
    psychological: [
        { value: 'none', label: 'No apparent anxiety, depression or agitation' },
...
const BRADEN_SUBSCALES: { key: string; label: string; options: { value: string; label: string }[] }[] = [
    { key: 'sensory_perception', label: 'Sensory perception', options: [
        { value: '1', label: '1 — Completely limited' }, { value: '2', label: '2 — Very limited' }, { value: '3', label: '3 — Slightly limited' }, { value: '4', label: '4 — No impairment' },
    ] },
    { key: 'moisture', label: 'Moisture', options: [
        { value: '1', label: '1 — Constantly moist' }, { value: '2', label: '2 — Very moist' }, { value: '3', label: '3 — Occasionally moist' }, { value: '4', label: '4 — Rarely moist' },
    ] },
    { key: 'activity', label: 'Activity', options: [
        { value: '1', label: '1 — Bedfast' }, { value: '2', label: '2 — Chairfast' }, { value: '3', label: '3 — Walks occasionally' }, { value: '4', label: '4 — Walks frequently' },
    ] },
    { key: 'mobility', label: 'Mobility', options: [
        { value: '1', label: '1 — Completely immobile' }, { value: '2', label: '2 — Very limited' }, { value: '3', label: '3 — Slightly limited' }, { value: '4', label: '4 — No limitations' },
    ] },
    { key: 'nutrition', label: 'Nutrition', options: [
        { value: '1', label: '1 — Very poor' }, { value: '2', label: '2 — Probably inadequate' }, { value: '3', label: '3 — Adequate' }, { value: '4', label: '4 — Excellent' },
    ] },
    { key: 'friction_shear', label: 'Friction & shear', options: [
        { value: '1', label: '1 — Problem' }, { value: '2', label: '2 — Potential problem' }, { value: '3', label: '3 — No apparent problem' },
    ] },
];

const IDDSI_DRINKS = [
    { value: '0', label: 'Level 0 — Thin' }, { value: '1', label: 'Level 1 — Slightly Thick' }, { value: '2', label: 'Level 2 — Mildly Thick' }, { value: '3', label: 'Level 3 — Moderately Thick' }, { value: '4', label: 'Level 4 — Extremely Thick' },
];
const IDDSI_FOODS = [
    { value: '3', label: 'Level 3 — Liquidised' }, { value: '4', label: 'Level 4 — Pureed' }, { value: '5', label: 'Level 5 — Minced & Moist' }, { value: '6', label: 'Level 6 — Soft & Bite-Sized' }, { value: '7', label: 'Level 7 — Regular' },
];

const STEPS: readonly WizardStep[] = [
    { key: 'client', label: 'Client & tool', blurb: 'Who & which assessment', icon: Stethoscope },
    { key: 'form', label: 'Assessment', blurb: 'Complete the tool', icon: ClipboardList },
    { key: 'evidence', label: 'Notes & evidence', blurb: 'Context & files', icon: Paperclip },
...
function Body({ onClose, client, onSaved }: RecordAssessmentDialogProps) {
    ...
    const removeFile = (i: number) => setData('attachments', data.attachments.filter((_, idx) => idx !== i));
    ...
    return (
        ...
        >
            {STEPS[stepIndex].key === 'client' ? (
                <WizardStepPane>
                    <StepHead icon={Stethoscope} title="Who, and which assessment?" blurb="Pick the client and the standardised tool to complete." />
                    <div className="grid gap-5">
                        <Field label="Client" required>
                            <ClientPicker value={picked} onChange={lockedClient ? () => {} : choosePatient} />
                        </Field>
                        <Field label="Assessment tool" required>
                            <TilePicker
                                ...
                                onChange={chooseType}
                                cols={2}
                                options={TYPES.map((t) => ({ key: t.key, label: t.label, icon: t.icon }))}
                            />
                            {typeMeta ? <p className="mt-2 text-[13px] text-muted-foreground">{typeMeta.blurb}</p> : null}
                        </Field>
                    </div>
                </WizardStepPane>
            ) : null}
            ...
            {STEPS[stepIndex].key === 'evidence' ? (
                <WizardStepPane>
                    <StepHead icon={Paperclip} title="Notes & evidence" blurb="Optional clinical context and supporting documents." />
                    <div className="grid gap-4">
                        <Field label="Notes" hint="optional">
                            <Textarea rows={3} value={data.notes} onChange={(e) => setData('notes', e.target.value)} placeholder="Clinical context, who was consulted, planned actions…" />
                        </Field>
                        <div>
                            <SubHead icon={Paperclip}>Evidence &amp; attachments</SubHead>
                            <p className="mb-2 text-[13px] text-muted-foreground">
                                Completed paper assessment, SLT report, body map or photos. Image, PDF or Word, up to 10&nbsp;MB each.
                            </p>
            ...
            {STEPS[stepIndex].key === 'review' ? (
                <WizardStepPane>
                    <StepHead icon={Check} title="Review & record" blurb="Confirm the computed result before recording." />
                    <div className="grid gap-3 sm:grid-cols-2">
                        <ReviewCard icon={ShieldAlert} title="Assessment" onEdit={() => setStepIndex(0)}>
                            ...
                            <ReviewRow label="Notes" value={data.notes || undefined} />
                        </ReviewCard>
                        {result ? (
                            <ReviewCard icon={ClipboardList} title="Result" onEdit={() => setStepIndex(1)}>
                                <ReviewRow label="Outcome" value={result.summary} />
                                {result.advice ? <ReviewRow label="Advice" value={result.advice} /> : null}
                            </ReviewCard>
                        ) : null}
                    </div>
                    {result ? <LiveScore result={result} /> : null}
                    <InfoCard icon={ShieldAlert} tone="info">
                        The score is computed transparently and verified server-side. It supports — but does not replace — your clinical judgement.
                    </InfoCard>
                </WizardStepPane>
            ) : null}
        </WizardShell>
...
function AssessmentFields({ type, inputs, setInput }: { type: AssessmentTypeValue | ''; inputs: AssessmentInputs; setInput: (k: string, v: string | number | boolean | null) => void }) {
            ...
            <div className="grid gap-4">
                ...
                <div className="rounded-lg border border-border bg-muted/30 p-3">
                    <label className="flex items-start gap-3">
                        <Switch checked={!!inputs.acute_disease_effect} onCheckedChange={(v) => setInput('acute_disease_effect', v ? 1 : 0)} />
                        <span>
                            <span className="text-sm font-semibold">Acute disease effect</span>
                            <span className="mt-0.5 block text-[13px] text-muted-foreground">Acutely ill and there has been, or is likely to be, no nutritional intake for more than 5 days.</span>
                        </span>
                    </label>
                </div>
            </div>
    ...
    if (type === 'falls_frat') {
        return (
            <div className="grid gap-4">
                {(['recent_falls', 'medications', 'psychological', 'cognitive'] as const).map((key) => (
                    <SelectField key={key} label={FRAT_LABELS[key]} value={str(inputs[key])} onChange={(v) => setInput(key, v)} options={SELECT_OPTIONS[key]} />
                ))}
            </div>
        );
    }
        ...
        return (
            <div className="grid gap-4 sm:grid-cols-2">
                {BRADEN_SUBSCALES.map((s) => (
                    <SelectField key={s.key} label={s.label} value={str(inputs[s.key])} onChange={(v) => setInput(s.key, v)} options={s.options} />
                ))}
            </div>
        );
    }
    ...
    if (type === 'dysphagia_iddsi') {
        return (
            <div className="grid gap-4 sm:grid-cols-2">
                <SelectField label="Recommended drink level" value={str(inputs.drink_level)} onChange={(v) => setInput('drink_level', v)} options={IDDSI_DRINKS} placeholder="Select a drink level" />
                <SelectField label="Recommended food level" value={str(inputs.food_level)} onChange={(v) => setInput('food_level', v)} options={IDDSI_FOODS} placeholder="Select a food level" />
            </div>
        );
    }
...
const FRAT_LABELS: Record<string, string> = {
    ...
    medications: 'Medications (sedatives, anti-Parkinson’s, antidepressants, antihypertensives, hypnotics)',
...
function LiveScore({ result }: { result: NonNullable<ReturnType<typeof computeAssessment>> }) {
    return (
        <div className={cn('mt-4 rounded-xl border p-4', TONE_CLASS[result.bandTone ?? 'neutral'])}>
            <div className="flex items-center justify-between gap-3">
                <p className="text-sm font-bold">{result.summary}</p>
                {result.score !== null ? <span className="text-2xl font-bold tabular-nums">{result.score}</span> : null}
            </div>
                ...
                {result.breakdown.map((row) => (
                    <div key={row.key} className="flex items-center justify-between gap-3 text-[13px]">
                        ...
                        <span className="flex items-center gap-2 text-right">
                            ...
                            {row.points !== null ? <span className="min-w-[20px] rounded bg-background/60 px-1 text-center font-semibold tabular-nums">{row.points}</span> : null}
                        </span>
                    </div>
                ))}
            </div>
            {result.advice ? <p className="mt-3 border-t border-current/15 pt-2 text-[13px] font-medium">{result.advice}</p> : null}
        </div>
    );
}
