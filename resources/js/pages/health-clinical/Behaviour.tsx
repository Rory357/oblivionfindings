...
export default function BehaviourRegister({
    entries,
    stats,
    filters,
    filter_options,
    ...
    const page = usePage<{
        auth?: { can?: { clinical?: { eventsRecord?: boolean } } };
    }>();
    ...
    const apply = () => {
        ...
        router.get('/health-clinical/behaviour', clean, {
            preserveState: true,
            replace: true,
        });
    };
    ...
    return (
        ...
        >
            <div className="inline-flex gap-1 rounded-lg bg-muted p-1">
                ...
                >
                    Restraint register
                    {restraint.stats.review_due > 0 ? (
                        <span className="rounded-full bg-status-warning-bg px-1.5 text-[10px] font-bold text-status-warning">
                            {restraint.stats.review_due}
                        </span>
                    ) : null}
                </button>
            </div>
            ...
            {view === 'restraint' ? (
                <RestraintLensView restraint={restraint} />
            ) : (
                <>
                        ...
                        <RegisterStatStrip
                            stats={[
                                ...
                                { label: '30d', value: stats.total_30d },
                                {
                                    ...
                                    tone:
                                        stats.escalated_30d > 0
                                            ? 'warning'
                                            : 'default',
                                },
                                {
                                    label: 'Harm',
                                    value: stats.harm_30d,
                                    tone:
                                        stats.harm_30d > 0
                                            ? 'critical'
                                            : 'default',
                        ...
                        <CardContent>
                            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
                                <FilterSelect
                                    ...
                                    options={filter_options.clients.map(
                                        (c) => ({
                                            value: String(c.id),
                                ...
                                <FilterSelect
                                    label="Function"
                                    value={local.behaviour_function}
                                    onChange={(v) =>
                                        setLocal((c) => ({
                                            ...c,
                                            behaviour_function: v,
                                        }))
                                    }
                                    placeholder="Any function"
                                    options={filter_options.functions.map(
                                        (f) => ({
                                            value: f.value,
                                            label: f.label,
                                        }),
                                ...
                                <FilterSelect
                                    ...
                                    placeholder="Any intensity"
                                    options={filter_options.intensities}
                                />
                                <div>
                        ...
                        <Card>
                            <CardContent className="p-12 text-center">
                                ...
                                <p className="mt-1 text-sm text-muted-foreground/70">
                                    {canRecord
                                        ? 'Use “Record ABC” above to chart a behaviour, or adjust the filters.'
                                        : 'No ABC entries match the current filters.'}
                                </p>
                            </CardContent>
                        </Card>
                    ) : (
                            ...
                            {entries.data.map((e) => (
                                ...
                                >
                                    <div className="mb-3 flex flex-wrap items-center gap-2">
                                        {e.client ? (
                                            <Link
                                                href={`/operations/clients/${e.client.id}`}
                    ...
                    {entries.last_page > 1 ? (
                        <div className="flex items-center justify-between px-1">
                            <p className="text-xs text-muted-foreground">
                                ...
                                {entries.last_page} ({entries.total} total)
                            </p>
        ...
        <div className="flex flex-col gap-4">
            <div className="flex flex-wrap items-center gap-3 rounded-xl border border-primary/25 bg-primary/5 px-4 py-3">
                ...
                <p className="flex-1 text-[13px] text-foreground">
                    A read-only behaviour-context lens. The restraint register
                    is owned by <strong>Health &amp; Safety</strong> — episodes,
                    authorisation and PBS plan reviews are managed there.
                </p>
                <Link href="/health-safety/restraints">
                    <Button size="sm" variant="outline" className="gap-1.5">
                        Open Restraint register{' '}
            ...
            <RegisterStatStrip
                stats={[
                    {
                        label: 'Episodes · 30d',
                        value: restraint.stats.total_30d,
                    },
                    {
                        label: 'Off-plan',
                        value: restraint.stats.off_plan,
                        tone:
                            restraint.stats.off_plan > 0
                                ? 'critical'
                                : 'default',
                    },
                    {
                        label: 'With injury',
                        value: restraint.stats.with_injury,
                        tone:
                            restraint.stats.with_injury > 0
                                ? 'critical'
                                : 'default',
                    },
                    {
                        label: 'Review due',
                        value: restraint.stats.review_due,
                        tone:
                            restraint.stats.review_due > 0
                                ? 'warning'
                                : 'default',
                ...
                <CardContent className="p-0">
                    {restraint.events.length === 0 ? (
                        <div className="p-12 text-center">
                            ...
                            <p className="font-medium text-muted-foreground">
                                No restraint episodes
                            </p>
                            <p className="mt-1 text-sm text-muted-foreground/70">
                                No restrictive practice recorded for your
                                clients.
                            </p>
                        </div>
                    ) : (
                                    ...
                                    {restraint.events.map((r) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-3">
                                                {r.client ? (
                                                    <Link
                                                        href={`/operations/clients/${r.client.id}`}
