...
const sections: Array<{ title: string; items: Array<{ label: string; why: string; expected: string }>; }> = [
  {
    ...
    items: [
      {
        ...
        expected: 'Status changes correctly; close available only after review.'
      },
      {
      ...
      },
      {
        ...
        expected: 'Notifications created + delivered per escalation rules.'
      },
    ]
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        expected: 'Blocked until a different authorised witness is selected.'
      },
      {
    ...
    items: [
      {
        ...
        expected: 'Blocked until a note exists or a summary is provided.'
      },
      {
...
export default function QualityChecklist() {
  return (
    <AppLayout breadcrumbs={[{ title: 'QA Checklist', href: '/quality/checklist' }]}>
      <Head title="QA Checklist" />
      <div className="space-y-4 p-4">
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Unhappy-path testing checklist</CardTitle>
          </CardHeader>
          <CardContent className="text-sm text-muted-foreground">
            Use this to deliberately try break the system. If any of these fail, we fix it before adding new features.
          </CardContent>
        </Card>
              ...
              {s.items.map((i) => (
                <div key={i.label} className="rounded-md border p-3">
                  ...
                  <div className="mt-2 text-sm">
                    <div><span className="text-muted-foreground">Why:</span> {i.why}</div>
                    <div className="mt-1"><span className="text-muted-foreground">Expected:</span> {i.expected}</div>
                  </div>
                </div>
              ))}
            </CardContent>
