...
> = {
    planned: {
        ...
        bgColor:
            'bg-status-info-bg border-status-info/30 dark:border-status-info/30',
        label: 'Planned',
    },
    active: {
        color: 'text-primary dark:text-primary',
        bgColor:
            'bg-primary/10 border-primary dark:bg-primary/30 dark:border-primary/30',
        label: 'Active',
    },
    completed: {
        color: 'text-foreground dark:text-muted-foreground',
        bgColor: 'bg-muted border-border dark:bg-muted/30 dark:border-border',
        label: 'Completed',
    },
    cancelled: {
        color: 'text-status-critical dark:text-status-critical',
        bgColor:
            'bg-status-critical-bg border-status-critical/30 dark:border-status-critical/30',
        label: 'Cancelled',
    },
};
...
export default function OutingShow({ outing, vehicle_state, can }: Props) {
    ...
    const statusConfig =
        STATUS_CONFIG[safeOuting.status] ?? STATUS_CONFIG.planned;
        ...
        >
            ...
            <PageShell>
                            ...
                            <div className="flex gap-2">
                                        ...
                                        return (
                                            <>
                                                <Button
                                                    ...
                                                    title={
                                                        !allPreChecked
                                                            ? 'All residents must complete their pre-departure check first'
                                                            : undefined
                                                    }
                                                >
                                ...
                                {safeOuting.status === 'active' &&
                                    (() => {
                                        ...
                                        const unreturned = safeResidents.filter(
                                            (r) => !r.returned_at,
                                        );
                                        return (
                                            <>
                                                ...
                                                <Button
                                                    ...
                                                    title={
                                                        !allReturned
                                                            ? `All residents must be marked as returned first (${returnedCount} of ${totalCount} returned)`
                                                            : undefined
                                                    }
                                                >
                ...
                >
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            {safeOuting.purpose && (
                                <Badge variant="outline" className="text-xs">
                                    {PURPOSE_LABELS[safeOuting.purpose] ??
                                        safeOuting.purpose}
                                </Badge>
                            )}
                        </div>
                        <span className="text-xs text-muted-foreground">
                            ...
                            {safeOuting.created_by &&
                                ` by ${safeOuting.created_by.name}`}
                        </span>
                    </div>
                </div>

                {!canManage &&
                    ['planned', 'active'].includes(safeOuting.status) && (
                            ...
                            <CardContent className="space-y-2 text-sm">
                                    ...
                                    <span className="flex items-center gap-1 font-medium">
                                        {safeOuting.asset ? (
                                            <>
                                                ...
                                                <Link
                                                    href={`/fleet-assets/vehicles/${safeOuting.asset.id}`}
                                ...
                                {safeOuting.booking && (
                                    <div className="flex justify-between">
                                        ...
                                        <Link
                                            href={`/fleet-assets/bookings/${safeOuting.booking.id}`}
                            ...
                            <CardContent>
                                {safeResidents.length > 0 ? (
                                            ...
                                            >
                                                <div>
                                                    <p className="text-sm font-medium">
                                                        {resident.client_name}
                                                    </p>
                                                    {resident.transport_needs &&
                                                        Object.values(
                                                            resident.transport_needs,
                                                        ).some(Boolean) && (
                                                ...
                                                <div className="flex items-center gap-3">
                                                    <div className="flex items-center gap-1.5">
                                                        <div
                                                            className={`flex h-5 w-5 items-center justify-center rounded-full ${resident.pre_check_completed ? 'bg-status-success-bg text-status-success' : 'bg-muted'}`}
                                                        >
                                                            {resident.pre_check_completed && (
                                                    ...
                                                    <div className="flex items-center gap-1.5">
                                                        <div
                                                            className={`flex h-5 w-5 items-center justify-center rounded-full ${resident.medication_packed ? 'bg-status-success-bg text-status-success' : 'bg-muted'}`}
                                                        >
                                                            {resident.medication_packed && (
                                                    ...
                                                    {safeOuting.status ===
                                                        ...
                                                        (resident.returned_at ? (
                                                            <Badge className="h-5 border-status-success/30 bg-status-success-bg text-[10px] text-status-success dark:border-status-success/30 dark:bg-status-success-bg dark:text-status-success">
                                                            ...
                                                            <Button
                                                                ...
                                                                onClick={() =>
                                                                    router.post(
                                                                        `/fleet-assets/outings/${safeOuting.id}/residents/${resident.id}/return`,
                                                                    )
                                                                }
                                                            >
                                                                <UserCheck className="mr-1 h-3 w-3" />
                                    ...
                                    <p className="py-4 text-center text-xs text-muted-foreground">
                                        No residents assigned to this outing.
                                    </p>
                                )}
                            </CardContent>
                        </Card>
                                                ...
                                                <div className="flex flex-col items-center">
                                                    ...
                                                    {i <
                                                        timelineSteps.length -
                                                            1 && (
                                                        <div
                                                            className={`mt-1 h-8 w-0.5 ${step.completed ? 'bg-primary/20 dark:bg-primary' : 'bg-muted'}`}
                                                        />
                                                    )}
                                                </div>
                                                <div className="pt-1">
                ...
                <ConfirmDialog
                    ...
                    onConfirm={() => {
                        router.post(
                            `/fleet-assets/outings/${safeOuting.id}/cancel`,
                        );
                    }}
                    title="Cancel Outing"
                ...
                <AlertDialog
                    open={showReturnAllDialog}
                    onOpenChange={(open) => {
                        if (!open) setShowReturnAllDialog(false);
                    }}
                >
                    <AlertDialogContent>
                        <AlertDialogHeader>
                        ...
                        <AlertDialogFooter>
                            ...
                            <AlertDialogAction
                                onClick={() => {
                                    router.post(
                                        `/fleet-assets/outings/${safeOuting.id}/residents/return-all`,
                                    );
                            ...
                            >
                                ...
                                Return All (
                                {
                                    safeResidents.filter((r) => !r.returned_at)
                                        .length
                                }
                                )
                            </AlertDialogAction>
