...
export default function ReportByHouse({
    ...
    selected_month,
    available_months,
    house_summaries: rawSummaries,
    vehicle_details: rawDetails,
}: Props) {
    ...
    const handleHouseChange = (value: string) => {
        router.get(
            '/fleet-assets/reports/by-house',
            {
                house_id: value === 'all' ? undefined : value,
                month: currentMonth,
            },
            { preserveState: true },
        );
    ...
    const handleMonthChange = (value: string) => {
        router.get(
            '/fleet-assets/reports/by-house',
            { house_id: selected_house_id ?? undefined, month: value },
            { preserveState: true },
        );
    };
            ...
            <PageShell>
                ...
                {selectedSummary && (
                    <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
                        ...
                        <FleetStatCard
                            label="TRANSPORTS"
                            value={selectedSummary.transport_logs}
                            icon={Users}
                            subtitle="Transport logs"
                        />
                    </div>
                )}
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle className="text-base">
                                Vehicles at{' '}
                                {selectedSummary?.name ?? 'Selected House'}
                            </CardTitle>
                        </CardHeader>
                        <CardContent>
                            <div
                                    ...
                                    <tbody>
                                        {details.map((v) => (
                                            ...
                                            >
                                                ...
                                                <td className="px-3 py-2 text-right">
                                                    {v.avg_trip_duration_minutes >
                                                    0
                                                        ? `${v.avg_trip_duration_minutes} min`
                                                        : '---'}
                                                </td>
                ...
                {selected_house_id && details.length === 0 && (
                    <FleetEmptyState
                        ...
                        description="No vehicles are assigned to this house. Assign vehicles via the home site field."
                    />
                )}
                                    ...
                                    <tbody>
                                        {summaries.map((s) => (
                                            ...
                                            >
                                                ...
                                                <td className="px-3 py-2 text-right">
                                                    {s.distance_this_month.toLocaleString()}
                                                </td>
                                                <td className="px-3 py-2 text-right">
                                                    {formatCurrency(
                                                        s.fuel_cost_this_month,
                                                    )}
                                                </td>
                                                <td className="px-3 py-2 text-right">
                                                    {s.transport_logs}
                ...
                {summaries.length === 0 && (
                    <FleetEmptyState
                        icon={Building2}
                        title="No houses configured"
                        description="Add houses in the Sites section to see vehicle usage by house."
                    />
                )}
            </PageShell>
        </AppLayout>
