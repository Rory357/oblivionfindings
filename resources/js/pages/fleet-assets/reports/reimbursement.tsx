...
export default function MileageReimbursement() {
    ...
    const handleGenerate = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch(
                `/fleet-assets/reports/reimbursement/data?${params.toString()}`,
                {
                    headers: {
    ...
    const totalDistance = useMemo(
        () => data?.reduce((sum, r) => sum + r.distance_km, 0) ?? 0,
        [data],
    );
    const totalTrips = useMemo(
        () => data?.reduce((sum, r) => sum + r.trips, 0) ?? 0,
        [data],
    );
                ...
                {data !== null && data.length > 0 && (
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                        ...
                        <FleetStatCard
                            ...
                            icon={DollarSign}
                            subtitle={`At ${formatCurrency(Number(rate))}/km`}
                        />
                    </div>
                )}
