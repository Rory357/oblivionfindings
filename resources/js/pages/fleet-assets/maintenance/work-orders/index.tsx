...
import {
    CheckCircle, ChevronDown, ChevronUp, ChevronsUpDown, ClipboardList,
    Download, Loader, Plus, User, Wrench, X,
} from 'lucide-react';
...
export default function WorkOrdersIndex({ work_orders, filters, users }: Props) {
    const safeFilters = filters ?? {};
    const safeData = work_orders?.data ?? [];
    const safeMeta = work_orders?.meta ?? { current_page: 1, last_page: 1, total: 0 };
    ...
    const inProgressCount = safeData.filter((wo) => wo.status === 'in_progress').length;
    const completedCount = safeData.filter((wo) => wo.status === 'completed').length;

    const criticalCount = safeData.filter((wo) => wo.priority === 'critical').length;
    const highCount = safeData.filter((wo) => wo.priority === 'high').length;
    const mediumCount = safeData.filter((wo) => wo.priority === 'medium').length;
    ...
    function handleSort(field: string) {
        ...
        router.get(window.location.pathname, { ...safeFilters, sort: field, direction: newDir }, { preserveState: true });
    }
    ...
    const applyFilters = (newFilters: Partial<typeof safeFilters>) => {
        router.get('/fleet-assets/maintenance/work-orders', { ...safeFilters, ...newFilters, page: 1 }, { preserveState: true });
    };
    ...
    const toggleSelectAll = useCallback(() => {
        setSelectedIds(selectedIds.length === safeData.length ? [] : safeData.map((wo) => wo.id));
    }, [safeData, selectedIds.length]);

    const handleBulkAction = useCallback((action: string) => {
        if (selectedIds.length === 0) return;
        const payload: Record<string, unknown> = { action, ids: selectedIds };
        if (action === 'assign' && bulkAssignee) payload.assigned_to_user_id = Number(bulkAssignee);
        router.post('/fleet-assets/maintenance/work-orders/bulk-action', payload as any, {
        ...
        >
            ...
            <PageShell>
                    ...
                    actions={
                        <div className="flex gap-2">
                            <Button variant="outline" size="sm" asChild>
                                <a href="/fleet-assets/maintenance/work-orders?export=csv"><Download className="mr-2 h-4 w-4" />Export CSV</a>
                            </Button>
                            <Button asChild>
                                <Link href="/fleet-assets/maintenance/work-orders/create"><Plus className="mr-2 h-4 w-4" />Create Work Order</Link>
                            </Button>
                        </div>
                    }
                />
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                    <Select value={safeFilters.status || 'all'} onValueChange={(v) => applyFilters({ status: v === 'all' ? '' : v })}>
                    ...
                    <Select value={safeFilters.priority || 'all'} onValueChange={(v) => applyFilters({ priority: v === 'all' ? '' : v })}>
                            ...
                            {safeData.length > 0 ? (
                                safeData.map((wo) => (
                                    <tr key={wo.id} className={`border-b transition-colors hover:bg-muted/30 transition-colors cursor-pointer ${PRIORITY_BORDER[wo.priority] ?? ''}`} onClick={() => router.visit(`/fleet-assets/maintenance/work-orders/${wo.id}`)}>
                                        ...
                                        <td className="px-4 py-3">
                                            {wo.asset ? (<Link href={`/fleet-assets/assets/${wo.asset.id}`} className="text-primary hover:underline" onClick={(e) => e.stopPropagation()}>{wo.asset.name}</Link>) : (<span className="text-muted-foreground">---</span>)}
                                        </td>
                                        <td className="px-4 py-3"><Badge variant={priorityVariant(wo.priority)}>{wo.priority}</Badge></td>
                                        <td className="px-4 py-3"><Badge variant={statusVariant(wo.status)}>{(wo.status ?? '').replace(/_/g, ' ')}</Badge></td>
                                        <td className="px-4 py-3">
                                            {wo.assigned_to ? (<span className="inline-flex items-center gap-1"><User className="h-3 w-3" />{wo.assigned_to.name}</span>) : (<span className="text-muted-foreground">Unassigned</span>)}
                                        </td>
                                        <td className="px-4 py-3 text-muted-foreground">{wo.created_at ? formatDate(wo.created_at) : '---'}</td>
                                    </tr>
                                ))
                            ) : (
                                <tr>
                                    <td colSpan={7} className="px-4 py-12">
                                        <FleetEmptyState icon={Wrench} title="No work orders" description="Create a work order to track vehicle and asset maintenance tasks." actionLabel="Create Work Order" actionHref="/fleet-assets/maintenance/work-orders/create" />
                                    </td>
                                </tr>
                            )}
                        </tbody>
                ...
                {selectedIds.length > 0 && (
                    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 rounded-lg border bg-background px-4 py-3 shadow-lg">
                        ...
                        <div className="flex items-center gap-2">
                            <Button size="sm" variant="outline" onClick={() => handleBulkAction('complete')}>Mark Complete</Button>
                            <Button size="sm" variant="outline" onClick={() => handleBulkAction('in_progress')}>Mark In Progress</Button>
                            {(users ?? []).length > 0 && (
                                <>
                                    <Select value={bulkAssignee} onValueChange={setBulkAssignee}>
                                        <SelectTrigger className="w-36 h-8 text-xs"><SelectValue placeholder="Assign to..." /></SelectTrigger>
                                        <SelectContent>{(users ?? []).map((u) => (<SelectItem key={u.id} value={String(u.id)}>{u.name}</SelectItem>))}</SelectContent>
                                    </Select>
                                    <Button size="sm" variant="outline" onClick={() => handleBulkAction('assign')} disabled={!bulkAssignee}>Assign</Button>
                                </>
                            )}
                        </div>
