...
const TRANSPORT_TYPE_BANNER: Record<string, string> = {
    medical: 'bg-red-50 border-red-200 text-red-900 dark:bg-red-950/30 dark:border-red-800 dark:text-red-200',
    appointment: 'bg-blue-50 border-blue-200 text-blue-900 dark:bg-blue-950/30 dark:border-blue-800 dark:text-blue-200',
    social: 'bg-green-50 border-green-200 text-green-900 dark:bg-green-950/30 dark:border-green-800 dark:text-green-200',
    shopping: 'bg-purple-50 border-purple-200 text-purple-900 dark:bg-purple-950/30 dark:border-purple-800 dark:text-purple-200',
    community: 'bg-teal-50 border-teal-200 text-teal-900 dark:bg-teal-950/30 dark:border-teal-800 dark:text-teal-200',
    respite: 'bg-orange-50 border-orange-200 text-orange-900 dark:bg-orange-950/30 dark:border-orange-800 dark:text-orange-200',
...
export default function TransportShow({ transport, vehicle_position, pre_check_status, care_needs }: Props) {
    ...
    useEffect(() => {
        if (t.status === 'in_progress') {
            refreshTimerRef.current = setInterval(() => {
                router.reload({ only: ['vehicle_position'], preserveState: true, preserveScroll: true });
            }, 15000);
        }
        return () => {
    ...
    return (
        ...
        >
            ...
            <PageShell>
                ...
                <div className={cn('rounded-lg border px-5 py-4', TRANSPORT_TYPE_BANNER[t.transport_type] ?? TRANSPORT_TYPE_BANNER.other)}>
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <Badge className="text-sm capitalize">{t.transport_type}</Badge>
                            <span className="font-medium">Transport #{t.id}</span>
                            <span className="opacity-50">|</span>
                            <span className="font-medium">{t.resident_name ?? '---'}</span>
                        </div>
                        <div className="flex items-center gap-2">
                            {/* Pre-Check Status Badge */}
                            {pre_check_status && (
                                <Badge
                                    variant={pre_check_status === 'completed' ? 'secondary' : 'outline'}
                                    className={cn(
                                        'text-xs',
                                ...
                                >
                                    ...
                                    Pre-Check: {pre_check_status === 'completed' ? 'Done' : 'Pending'}
                                </Badge>
                            )}
                ...
                {t.status === 'in_progress' && vehicle_position?.lat && vehicle_position?.lng && (
                    <Card>
                    ...
                    <Card className="border bg-purple-50 dark:bg-purple-950/30">
                        ...
                        <CardContent>
                            <div className="grid gap-2 sm:grid-cols-2">
                                {(care_needs ?? []).map((need) => (
                                    <div key={need.id} className="rounded-md bg-white/60 dark:bg-black/20 p-3">
                                        <p className="text-sm font-medium">{need.label}</p>
                                        {need.notes && (
                                            <p className="mt-0.5 text-xs text-muted-foreground">{need.notes}</p>
                                        )}
                                    </div>
                                ))}
                            </div>
                    ...
                    <div className="space-y-4">
                                ...
                                <dl className="space-y-3 text-sm">
                                    <div className="grid gap-3 sm:grid-cols-2">
                                        <div className="flex items-center gap-2 rounded-md bg-muted/40 p-3">
                                            ...
                                            <div>
                                                ...
                                                <dd className="font-medium">
                                                    {t.asset ? (
                                                        <Link href={`/fleet-assets/vehicles/${t.asset.id}`} className="text-primary hover:underline">
                                                            {t.asset.name}{t.asset.asset_tag ? ` (${t.asset.asset_tag})` : ''}
                                                        </Link>
                                                    ) : '---'}
                                                </dd>
                                            </div>
                                        ...
                                        <div className="flex items-center gap-2 rounded-md bg-muted/40 p-3">
                                            ...
                                            <div>
                                                ...
                                                <dd className="font-medium">{t.driver?.name ?? '---'}</dd>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="grid gap-3 sm:grid-cols-2">
                                        <div className="flex items-center gap-2 rounded-md bg-muted/40 p-3">
                                            ...
                                            <div>
                                                ...
                                                <dd className="font-medium">{t.resident_name ?? '---'}</dd>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-2 rounded-md bg-muted/40 p-3">
                                            <Users className="h-4 w-4 text-muted-foreground" />
                                            <div>
                                                ...
                                                <dd className="font-medium">{t.passengers_count ?? 1}</dd>
                                            </div>
                                        </div>
                                    </div>
                                    {t.supervisor_name && (
                                        <div className="rounded-md bg-muted/40 p-3">
                                            ...
                                            <dd className="mt-1 font-medium">{t.supervisor_name}</dd>
                                        </div>
                                    )}
                                    {t.booking && (
                                        <div className="rounded-md bg-muted/40 p-3">
                                            ...
                                            <dd className="mt-1">
                                                <Link href={`/fleet-assets/bookings/${t.booking.id}`} className="text-primary hover:underline font-medium">
                                                    Booking #{t.booking.id} - {t.booking.purpose}
                                                </Link>
                                            </dd>
                                        </div>
                                    )}
                                    <div className="flex items-center gap-2 rounded-md bg-muted/40 p-3">
                                        ...
                                        <div>
                                            ...
                                            <dd className="font-medium">
                                                {t.created_at ? new Date(t.created_at).toLocaleString('en-NZ') : '---'}
                                            </dd>
                                        </div>
                                    </div>
                                    {t.notes && (
                        ...
                        {t.status === 'in_progress' && (
                            <Card className="border-2 border-dashed border-primary/30">
                                <CardHeader>
                                    <CardTitle className="text-base">Mark as Completed</CardTitle>
                                </CardHeader>
                                <CardContent>
                                    <form onSubmit={handleComplete} className="grid gap-3 sm:grid-cols-2">
                                        <div>
                                            <label className="text-sm font-medium">Arrival Time</label>
                                            <Input
                                                type="datetime-local"
                                                value={completeForm.data.arrived_at}
                                                onChange={(e) => completeForm.setData('arrived_at', e.target.value)}
                                            />
                                        </div>
                                        <div>
                                        ...
                                        <div className="sm:col-span-2">
                                            <Button type="submit" size="lg" disabled={completeForm.processing} className="w-full">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="flex gap-4">
                                        <div className="flex flex-col items-center">
                                            ...
                                            <div className={cn('mt-2 w-0.5 flex-1', t.arrived_at ? 'bg-primary' : 'bg-muted')} />
                                        </div>
                                        <div className="pb-6">
                                            <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Departed</p>
                                            <p className="mt-1 text-sm font-medium">
                                                {t.departed_at ? new Date(t.departed_at).toLocaleString('en-NZ') : '---'}
                                            </p>
                                            {t.pickup_location && (
                                                <p className="mt-0.5 text-xs text-muted-foreground">{t.pickup_location}</p>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    {t.duration_minutes != null && (
                                        <div className="flex gap-4">
                                            ...
                                            <div className="pb-6">
                                                ...
                                                <p className="mt-1 text-lg font-bold">{formatDurationMinutes(t.duration_minutes)}</p>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    <div className="flex gap-4">
                                        <div className="flex flex-col items-center">
                                            <div className={cn(
                                                ...
                                                t.arrived_at ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'
                                            )}>
                                                {t.arrived_at ? <CheckCircle className="h-5 w-5" /> : <Clock className="h-5 w-5" />}
                                            </div>
                                        </div>
                                        <div>
                                            <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Arrived</p>
                                            <p className="mt-1 text-sm font-medium">
                                                {t.arrived_at ? new Date(t.arrived_at).toLocaleString('en-NZ') : 'In progress...'}
                                            </p>
                                            {t.dropoff_location && (
                                                <p className="mt-0.5 text-xs text-muted-foreground">{t.dropoff_location}</p>
                                            )}
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="space-y-3">
                                        {t.pickup_location && (
                                            <div className="rounded-md bg-muted/40 p-3">
                                                ...
                                                <div className="mt-1 font-medium text-sm">{t.pickup_location}</div>
                                            </div>
                                        )}
                                        {t.dropoff_location && (
                                            <div className="rounded-md bg-muted/40 p-3">
                                                <div className="text-xs text-muted-foreground">Dropoff</div>
                                                <div className="mt-1 font-medium text-sm">{t.dropoff_location}</div>
                                            </div>
                                        )}
                                    </div>
                                </CardContent>
