...
const STEPS: readonly WizardStep[] = [
    {
        key: 'vehicle',
        label: 'Vehicle & when',
        blurb: 'Pick a vehicle and dates',
        icon: Car,
    },
    {
        key: 'details',
        label: 'Purpose & passengers',
        blurb: 'Trip details',
        icon: Users,
    },
    {
...
function AvailabilityCalendar({
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        const isSelected = !!(
            selectedStart &&
            selectedEnd &&
            dateStr >= selectedStart.split('T')[0] &&
            dateStr <= selectedEnd.split('T')[0]
        );
        ...
        <div>
                ...
                {days.map((cell, i) => {
                    ...
                    return (
                        ...
                        >
                            {cell.day}
                            {hasBooking && (
                                <span className="absolute bottom-0.5 left-1/2 h-1 w-1 -translate-x-1/2 rounded-full bg-status-critical" />
                            )}
                        </div>
                    );
                })}
            ...
            <div className="mt-2 flex gap-3 text-[10px] text-muted-foreground">
                <span className="flex items-center gap-1">
                    <span className="h-2 w-2 rounded-full bg-primary" /> Your
                    selection
                </span>
                <span className="flex items-center gap-1">
                ...
                <span className="flex items-center gap-1">
                    <span className="h-2 w-2 rounded-full bg-white ring-1 ring-ring" />{' '}
                    Today
                </span>
            </div>
        </div>
...
export function BookVehicleWizard({
    ...
    const selectedClient =
        safeClients.find((c) => String(c.id) === form.data.client_id) ?? null;
    const selectedVehicle =
        safeVehicles.find((v) => String(v.id) === form.data.asset_id) ?? null;
    ...
    if (selectedClient?.transport_needs && selectedVehicle) {
        ...
        if (needs.wheelchair_ramp && !selectedVehicle.has_wheelchair_ramp) {
            compatibilityWarnings.push(
                `${clientSingular} requires wheelchair ramp, but this vehicle does not have one.`,
            );
        }
        if (needs.hoist && !selectedVehicle.has_hoist) {
            compatibilityWarnings.push(
                `${clientSingular} requires a hoist, but this vehicle does not have one.`,
            );
        }
        if (needs.child_seat && !selectedVehicle.has_child_seat_anchors) {
            compatibilityWarnings.push(
                `${clientSingular} requires child seat anchors, but this vehicle does not have them.`,
            );
        }
        if (needs.medical_storage && !selectedVehicle.has_medical_storage) {
            compatibilityWarnings.push(
                `${clientSingular} requires medical storage, but this vehicle does not have it.`,
            );
        }
    }
        ...
        form.post('/fleet-assets/bookings', {
            ...
            onError: (errors) => {
                if (errors.asset_id || errors.starts_at || errors.ends_at)
                    ...
                    errors.destination ||
                    errors.passengers ||
                    errors.pickup_site_id ||
                    errors.return_site_id ||
                    errors.notes
                )
                    setStep(1);
            },
    ...
    const siteName = (id: string) =>
        safeSites.find((s) => String(s.id) === id)?.name ?? '';
            ...
            footerStart={
                step === 0 && stepOneComplete && safeConflicts.length === 0 ? (
                    <span className="inline-flex items-center gap-1.5 text-[13px] font-medium text-status-success">
                        <Calendar className="h-3.5 w-3.5" /> Vehicle available
                        for these dates
                    </span>
                    ...
                    <span className="inline-flex items-center gap-1.5 text-[13px] font-medium text-status-warning">
                        ...
                        {safeConflicts.length} conflicting booking
                        {safeConflicts.length !== 1 ? 's' : ''}
                    </span>
                ) : null
            }
            footerEnd={
                    ...
                    <div className="grid gap-6 lg:grid-cols-[3fr_2fr]">
                            ...
                            {safeClients.length > 0 && (
                                <Field label={`${clientSingular} (optional)`}>
                                    ...
                                    >
                                        <SelectTrigger>
                                            <SelectValue
                                                placeholder={`Select ${clientSingular.toLowerCase()} for accessibility matching`}
                                            />
                                        </SelectTrigger>
                                        <SelectContent>
                                            {safeClients.map((c) => (
                                    ...
                                    <p className="mt-1 text-[10px] text-muted-foreground">
                                        Selecting a client checks vehicle
                            ...
                            {compatibilityWarnings.length > 0 && (
                                <div className="rounded-lg border border-status-warning/30 bg-status-warning-bg px-4 py-3 dark:border-status-warning/30">
                                    <div className="mb-2 flex items-center gap-2">
                                        ...
                                        <span className="text-sm font-medium text-status-warning dark:text-status-warning">
                                    ...
                                    <ul className="space-y-1">
                                        {compatibilityWarnings.map((w, i) => (
                                            <li
                                                key={i}
                                                className="text-xs text-status-warning dark:text-status-warning"
                                            >
                                                - {w}
                                            </li>
                                        ))}
                            ...
                            {safeConflicts.length > 0 && (
                                <div className="rounded-lg border border-status-warning/30 p-4 dark:border-status-warning/30">
                                    ...
                                    <p className="mb-3 text-xs text-muted-foreground">
                                        The following existing bookings overlap
                                    ...
                                    <div className="space-y-2">
                                        {safeConflicts.map((c) => (
                                            <div
                                                key={c.id}
                                                className="flex items-center justify-between rounded-md border border-status-warning/30 bg-status-warning-bg px-3 py-2 dark:border-status-warning/30"
                                            >
                                                <div>
                        ...
                        <div>
                            {form.data.asset_id ? (
                                    ...
                                    {safeVehicleBookings.length > 0 && (
                                        <div className="mt-3 space-y-1">
                                            ...
                                            {safeVehicleBookings.map((b) => (
                                                <div
                                                    key={b.id}
                                                    className="flex justify-between rounded border px-2 py-1 text-[10px]"
                                                >
                                                    <span>
                                                        {b.purpose} —{' '}
                                                        {b.user_name}
                                ...
                                <GuardrailCard
                                    unstyled
                                    className="flex flex-col items-center justify-center rounded-xl border border-border bg-card/70 py-12 text-center"
                                >
                                    <Calendar className="mb-3 h-10 w-10 text-muted-foreground/30" />
                                    <p className="px-4 text-sm text-muted-foreground">
                                        Select a vehicle and dates to check
                                        availability
                                    </p>
                                </GuardrailCard>
                            )}
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                        ...
                        >
                            <Input
                                ...
                                onChange={(e) =>
                                    form.setData('passengers', e.target.value)
                                }
                        ...
                        <Field
                            ...
                            error={form.errors.pickup_site_id}
                        >
                            <Select
                                value={form.data.pickup_site_id}
                                onValueChange={(v) =>
                                    form.setData('pickup_site_id', v)
                                }
                            >
                                <SelectTrigger>
                        ...
                        <Field
                            ...
                            error={form.errors.return_site_id}
                        >
                            <Select
                                value={form.data.return_site_id}
                                onValueChange={(v) =>
                                    form.setData('return_site_id', v)
                                }
                            >
                                <SelectTrigger>
                ...
                <WizardStepPane>
                        ...
                        <div className="mb-4 rounded-lg border border-status-critical/30 bg-status-critical-bg px-4 py-3">
                            ...
                            <ul className="space-y-0.5">
                                {serverErrors.map(([key, message]) => (
                                    <li
                                        key={key}
                                        className="text-xs text-status-critical dark:text-status-critical"
                                    >
                                        - {message}
                                    </li>
                                ))}
                    ...
                    {safeConflicts.length > 0 && (
                        <div className="mb-4 flex items-center gap-2 rounded-lg border border-status-warning/30 bg-status-warning-bg px-4 py-3 text-sm text-status-warning dark:border-status-warning/30 dark:text-status-warning">
                            ...
                            {safeConflicts.length} existing booking
                            {safeConflicts.length !== 1
                                ? 's overlap'
                                : ' overlaps'}{' '}
                            your dates — the request may be declined.
                        </div>
                    )}
                    <div className="grid gap-4 sm:grid-cols-2">
                        <ReviewCard
                        ...
                        >
                            ...
                            <ReviewRow
                                ...
                                value={
                                    form.data.starts_at
                                        ? formatDateTime(form.data.starts_at)
                                        : undefined
                                }
                            />
                            <ReviewRow
                                ...
                                value={
                                    form.data.ends_at
                                        ? formatDateTime(form.data.ends_at)
                                        : undefined
                                }
                            />
                        </ReviewCard>
                        ...
                        >
                            ...
                            <ReviewRow
                                label="Pickup"
                                value={siteName(form.data.pickup_site_id)}
                            />
                            <ReviewRow
                                label="Return"
                                value={siteName(form.data.return_site_id)}
                            />
                            <ReviewRow label="Notes" value={form.data.notes} />
                        </ReviewCard>
                    </div>
                    <p className="mt-4 text-xs text-muted-foreground">
                        Your request is submitted for approval — you'll be
                        notified when it's approved or declined.
                    </p>
                </WizardStepPane>
            )}
        </WizardShell>
