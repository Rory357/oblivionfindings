...
} from '@/components/fleet/fleet-incident-dialog';
import {
...
export default function FleetIncidentsIndex({
    ...
    const [reportMode, setReportMode] = useState<ReportMode | null>(
        report && REPORT_MODES.includes(report as ReportMode)
            ? (report as ReportMode)
            : null,
    );
    ...
    const TABS: RosterTabItem[] = [
        ...
        },
        {
            ...
            icon: Search,
            tone: 'primary',
            badge: tabCounts.under_investigation || undefined,
        },
        {
            id: 'police_report_due',
            label: 'Police report due',
            icon: ShieldAlert,
            tone: 'critical',
            badge: tabCounts.police_report_due || undefined,
        },
        {
            id: 'injury_acc',
            label: 'Injury / ACC',
            icon: AlertTriangle,
            tone: 'critical',
            badge: tabCounts.injury_acc || undefined,
        },
        {
            id: 'insurance_claims',
            ...
            icon: CreditCard,
            tone: 'warning',
            badge: tabCounts.insurance_claims || undefined,
        },
        {
            id: 'off_road',
            label: 'Off-road (VOR)',
            icon: CircleSlash,
            tone: 'warning',
            badge: tabCounts.off_road || undefined,
        },
        {
            id: 'near_misses',
            label: 'Near misses',
                ...
                <div className="space-y-2">
                    <label className="block text-xs font-medium text-foreground">
                        ...
                        <input
                            ...
                            onChange={(e) =>
                                go({ date_from: e.target.value || null })
                            }
                    ...
                    <label className="block text-xs font-medium text-foreground">
                        ...
                        <input
                            ...
                            onChange={(e) =>
                                go({ date_to: e.target.value || null })
                            }
                            className="mt-1 block w-full rounded-md border border-border bg-background px-2 py-1 text-sm"
                        />
                    </label>
    ...
    const openRowCtx = (e: ReactMouseEvent, i: IncidentRow) => {
        ...
        const items: ShiftCtxItem[] = [
            {
                ...
                label: 'View incident',
                sub: `${i.reference} · ${titleCase(i.incident_type)}`,
                tone: 'primary',
                ...
                ? [
                      {
                          ...
                          label: 'Log insurance claim',
                          onClick: () => openDetail(i.id),
                      } satisfies ShiftCtxItem,
                      {
                          icon: <CircleSlash className="h-3.5 w-3.5" />,
                          label: i.flags.off_road
                ...
                ? [
                      {
                          ...
                          label: 'View vehicle / asset',
                          sub: i.asset.name,
                          onClick: () =>
                              router.visit(
                                  `/fleet-assets/assets/${i.asset!.id}`,
                              ),
                      } satisfies ShiftCtxItem,
                  ]
                : []),
                ...
                ? [
                      {
                          ...
                          label: i.hr_asset.holder_name
                              ? `HR: assigned to ${i.hr_asset.holder_name}`
                              : 'HR asset record',
                          sub: can.view_hr_assets
                              ? 'Open in HR Asset Register'
                              : undefined,
                          onClick: can.view_hr_assets
                              ? () =>
                                    router.visit(`/hr/assets/${i.hr_asset!.id}`)
                              : undefined,
                      } satisfies ShiftCtxItem,
                  ]
                : []),
                ...
                ? [
                      {
                          ...
                          onClick: () =>
                              router.visit(
                                  `/fleet-assets/drivers/${i.driver!.id}`,
                              ),
                      } satisfies ShiftCtxItem,
                  ]
                : []),
            ...(i.flags.alert_linked
                ? [
                      {
        ...
        setCtx({
            ...
            tag: i.reference,
            meta:
                [i.asset?.name, i.asset?.registration_number]
                    .filter(Boolean)
                    .join(' · ') || titleCase(i.incident_type),
            items,
        });
    };
        ...
        >
            ...
            <div className="flex flex-col gap-6 p-6">
                        ...
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
                            ...
                            {formOptions.assets?.length ? (
                                <EntityFilter
                                    ...
                                    items={formOptions.assets.map((a) => ({
                                        id: a.id,
                                        name: a.name,
                                        description: a.registration_number,
                                    }))}
                            ...
                            {formOptions.users?.length ? (
                                <EntityFilter
                                    ...
                                    items={formOptions.users}
                                    value={filters.driver_id}
                                    onChange={(id) => go({ driver_id: id })}
                                    onDark
                                />
                            ) : null}
                            <HeroSegmented
                ...
                <TabStrip
                    ...
                    onChange={setTab}
                    items={TABS}
                    ariaLabel="Fleet incident views"
                />
            ...
            <FleetTelematicsStoryboard
                open={telematicsOpen}
                onClose={() => setTelematicsOpen(false)}
            />
        </AppLayout>
    );
}
    ...
    if (!dueAt) {
        return (
            ...
            >
                ...
                <style>
                    {'@keyframes fiPulse{0%,100%{opacity:1}50%{opacity:.5}}'}
                </style>
            </span>
        );
    }
...
function FlagChip({
    ...
    return (
        <span
            title={title}
            aria-label={title}
            className={`inline-flex h-[22px] w-[22px] items-center justify-center rounded-md ${cls}`}
        >
            {icon}
        </span>
    );
                                        ...
                                        <div className="flex items-center gap-1.5">
                                            ...
                                            {i.flags.alert_linked ? (
                                                <FlagChip
                                                    ...
                                                    title="Linked Control Room alert"
                                                />
                                            ) : null}
