...
function eventIcon(type: string) {
    switch (type) {
        ...
        case 'speeding':
            return <Gauge className="h-4 w-4 text-status-critical" />;
        case 'idle':
            return <Timer className="h-4 w-4 text-status-info" />;
        default:
...
export default function DriverShow({
    ...
    const changeScorecardPeriod = (newPeriod: string) => {
        router.reload({
            only: ['scorecard'],
            data: { tab: 'scorecard', period: newPeriod },
        });
    };
    ...
    // A missing metric is unknown, not a failed safety score.
    const aggregatedMetrics = {
        score:
            safeDrivingMetrics.length > 0
                ? (safeDrivingMetrics[0]?.score ?? null)
                : null,
        harsh_brakes: safeDrivingMetrics.reduce(
            (sum, m) => sum + (m.harsh_brake_count ?? 0),
            0,
        ),
        speeding_events: safeDrivingMetrics.reduce(
            (sum, m) => sum + (m.speeding_events ?? 0),
            0,
        ),
        idle_minutes: safeDrivingMetrics.reduce(
            (sum, m) => sum + (m.idle_minutes ?? 0),
            0,
        ),
    };
                                    ...
                                    <CardContent>
                                        {safeAssignedVehicles.length > 0 ? (
                                            <div className="grid gap-2 sm:grid-cols-2">
                                                {safeAssignedVehicles.map(
                                                    (v) => (
                                                        <Link
                                                            key={v.id}
                                                            href={`/fleet-assets/vehicles/${v.id}`}
                                                            className="flex items-center gap-3 rounded-lg border p-3 transition-colors hover:border-primary/30 hover:bg-muted/50"
                                                        >
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                            ...
                                            <div className="rounded-lg bg-status-warning-bg p-3 text-center">
                                                <div className="text-2xl font-bold text-status-warning">
                                                    {
                                                        aggregatedMetrics.harsh_brakes
                                                    }
                                                </div>
                                                <div className="mt-1 text-xs text-muted-foreground">
                                                    Harsh Brakes
                                            ...
                                            <div className="rounded-lg bg-status-critical-bg p-3 text-center">
                                                <div className="text-2xl font-bold text-status-critical">
                                                    {
                                                        aggregatedMetrics.speeding_events
                                                    }
                                                </div>
                                                <div className="mt-1 text-xs text-muted-foreground">
                                                    Speeding Events
                                    ...
                                    <CardContent>
                                        <div className="space-y-3">
                                            ...
                                            <div className="flex items-center justify-between rounded-md bg-muted/30 px-3 py-2">
                                                ...
                                                <span className="font-semibold">
                                                    {
                                                        aggregatedMetrics.idle_minutes
                                                    }
                                                </span>
                                            </div>
                                            <div className="flex items-center justify-between rounded-md bg-muted/30 px-3 py-2">
                        ...
                        {!scorecard ? (
                            ...
                            <>
                                <div className="flex items-center justify-between">
                                    <p className="text-sm text-muted-foreground">
                                        Driver-session trips and recorded safety
                                        events for the selected period.
                                    </p>
                                    <Select
                                        value={scorecard.period}
                                        onValueChange={changeScorecardPeriod}
                                    >
                                    ...
                                    <Card className="flex flex-col items-center justify-center px-8 py-6">
                                        ...
                                        <CardContent className="flex flex-col items-center justify-center pb-2">
                                            {scorecard.score === null ? (
                                                <div className="flex min-h-48 max-w-xs flex-col items-center justify-center text-center">
                                                    <Gauge className="mb-3 h-10 w-10 text-muted-foreground/50" />
                                                    <p className="font-medium">
                                                        Driver score unavailable
                                                    </p>
                                                ...
                                                <>
                                                    ...
                                                    null ? (
                                                        <div className="mt-4 flex items-center gap-2">
                                                            ...
                                                            0 ? (
                                                                <Badge
                                                                    variant="default"
                                                                    className="bg-primary/10 text-primary dark:bg-primary dark:text-primary/70"
                                                                >
                                                    ...
                                                    <div className="mt-3 text-sm text-muted-foreground">
                                                        {scorecard.fleet_avg_score ===
                                                        null ? (
                                                            'No accessible Fleet average for this period.'
                                                        ) : (
                                                            <>
                                                                Fleet average:{' '}
                                        ...
                                        <Card>
                                            ...
                                            <CardContent>
                                                ...
                                                <p className="mt-1 text-xs text-muted-foreground">
                                                    {scorecard.metrics
                                                        .harsh_brakes === null
                                                        ? 'Vehicle aggregate withheld'
                                                        : 'recorded events this period'}
                                                </p>
                                            </CardContent>
                                        </Card>
                                            ...
                                            <CardContent>
                                                ...
                                                <p className="mt-1 text-xs text-muted-foreground">
                                                    {scorecard.metrics
                                                        .idle_minutes === null
                                                        ? 'Vehicle aggregate withheld'
                                                        : `${scorecard.metrics.idle_minutes} minutes total`}
                                                </p>
                                            </CardContent>
                                        </Card>
                                ...
                                <Card>
                                    ...
                                    <CardContent>
                                        {(scorecard.recent_events ?? [])
                                            .length > 0 ? (
                                            <div className="space-y-2">
                                                {scorecard.recent_events.map(
                                                    (event) => (
                                                        <div
                                                            key={event.id}
