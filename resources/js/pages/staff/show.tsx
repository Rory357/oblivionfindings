...
import { formatDateTime, formatDuration, formatDistance } from '@/lib/fleet-utils';
...
export default function StaffShow({ user, myDayItems, todayShifts, upcomingShifts, fleet }: Props) {
                    ...
                    <Card className="border-l-4 border-l-purple-500">
                        ...
                        <CardContent className="space-y-5">
                            ...
                            {fleet.eligibility ? (
                                <div className="rounded-md border p-4 space-y-3">
                                    <div className="flex items-center gap-2 text-xs font-semibold uppercase text-muted-foreground">
                                        <IdCard className="h-3.5 w-3.5" /> Driver Eligibility
                                    </div>
                                    <div className="grid gap-3 sm:grid-cols-3">
                                        <div>
                                            <div className="text-xs text-muted-foreground">Licence Class</div>
                                            <div className="text-sm font-medium">{fleet.eligibility.licence_class ?? '—'}</div>
                                        </div>
                                        <div>
                                            <div className="text-xs text-muted-foreground">Licence Expires</div>
                                            <div className="text-sm font-medium flex items-center gap-1.5">
                                                {fleet.eligibility.licence_expires_at ?? '—'}
                                                {fleet.eligibility.licence_expires_at && (() => {
                                ...
                                <div className="rounded-md border border-dashed p-4 text-center">
                                    <IdCard className="mx-auto h-6 w-6 text-muted-foreground/40 mb-1" />
                                    <div className="text-sm text-muted-foreground">No driver eligibility record</div>
                                    <div className="text-xs text-muted-foreground">This staff member has not been set up as a driver.</div>
                                </div>
                            )}
                            ...
                            {fleet.recent_trips.length > 0 ? (
                                <div>
                                    <div className="text-xs font-semibold uppercase text-muted-foreground mb-2">Recent Trips</div>
                                    <div className="space-y-1.5">
                                        {fleet.recent_trips.map((trip) => (
                                            <div key={trip.id} className="flex items-center justify-between rounded-md border px-3 py-2 text-sm">
                                                <div className="flex items-center gap-3 min-w-0">
                                                    <Truck className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
                                                    <div className="min-w-0">
                                                        <span className="font-medium">{trip.vehicle?.name ?? 'Unknown vehicle'}</span>
                                                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                                            {trip.started_at && <span>{formatDateTime(trip.started_at)}</span>}
                                                            {trip.distance_km != null && <span>· {formatDistance(Number(trip.distance_km))}</span>}
                                                            {trip.duration_s != null && <span>· {formatDuration(trip.duration_s)}</span>}
                                                        </div>
                                                    </div>
                                                </div>
                                                <Badge variant={trip.status === 'closed' ? 'default' : 'outline'} className="text-[10px] shrink-0">{trip.status}</Badge>
                                            </div>
                                        ))}
                                    </div>
                                    <Link href={`/fleet-assets/drivers/${user.id}`} className="mt-2 inline-block text-xs text-primary hover:underline">
                                        View full driving profile →
                                    </Link>
                                </div>
                            ) : (
                                <div className="text-center py-4 text-sm text-muted-foreground">
                                    <Truck className="mx-auto h-6 w-6 text-muted-foreground/40 mb-1" />
