...
type Props = {
    ...
    hero: {
        clusters: {
            participation: {
                active_reps: number;
                sites_without_rep: number;
                committees: number;
                meetings_quarter: number;
            };
            consultation: {
                open: number;
        ...
        badges: {
            reps_coverage_pct: number;
            sites_total: number;
            sites_covered: number;
            minutes_overdue: number;
            consultations_awaiting: number;
            training_below_minimum: number;
        };
    };
    detail: {
        kind: 'representative' | 'meeting' | 'consultation';
        data: any;
    } | null;
    can: { manage: boolean };
...
export default function WorkerParticipationIndex({
    ...
    const [wizard, setWizard] = useState<
        'rep' | 'meeting' | 'consultation' | null
    >(null);
    const [newOpen, setNewOpen] = useState(false);
    // Tracks which sub-action a detail dialog should open straight onto (right-click → action).
    const [pendingAction, setPendingAction] = useState<WpDetailAction | null>(
        null,
    );
    ...
    const setTab = (id: string) =>
        router.get(
            BASE,
            { ...filters, tab: id, status: null },
            { preserveState: true, preserveScroll: true },
        );
    const openDetail = (
        param: 'representative' | 'meeting' | 'consultation',
        id: number,
        action: WpDetailAction | null = null,
    ) => {
    ...
    const hasFilters = !!(
        ...
        filters.status ||
        filters.q ||
        (filters.period && filters.period !== 'quarter')
    );
    ...
    const TABS: RosterTabItem[] = [
        {
            ...
            icon: Users,
            tone: 'info',
            badge: tabCounts.representatives || undefined,
        },
        {
            id: 'meetings',
            ...
            icon: Calendar,
            tone: 'info',
            badge: tabCounts.meetings || undefined,
        },
        {
            id: 'consultations',
            label: 'Consultations',
            icon: MessageSquare,
            tone: 'warning',
            badge: tabCounts.consultations || undefined,
        },
    ];
            ...
            const items: ShiftCtxItem[] = [
                {
                    ...
                    sub: row.user?.name,
                    tone: 'primary',
                    onClick: () => openDetail('representative', row.id),
                },
            ];
            if (can.manage) {
                items.push(
                    {
                        ...
                        label: 'Edit details',
                        onClick: () =>
                            openDetail('representative', row.id, 'edit'),
                    },
                    {
                        icon: <GraduationCap className="h-3.5 w-3.5" />,
                        label: 'Record training days',
                        onClick: () =>
                            openDetail('representative', row.id, 'training'),
                    },
                    { sep: true },
                    active
                        ? {
                              icon: <Slash className="h-3.5 w-3.5" />,
                              label: 'Mark stood-down',
                              tone: 'critical',
                              onClick: () =>
                                  router.put(
                                      `${BASE}/representatives/${row.id}`,
                                      { status: 'inactive' },
                                      { preserveScroll: true },
                                  ),
                          }
                ...
                { sep: true },
                {
                    ...
                    onClick: () =>
                        navigator.clipboard?.writeText(
                            `${location.origin}${BASE}?tab=representatives&representative=${row.id}`,
                        ),
                },
            );
            return items;
        ...
        if (kind === 'meeting') {
            ...
            const items: ShiftCtxItem[] = [
                {
                    icon: <Eye className="h-3.5 w-3.5" />,
                    label: 'View meeting',
                    sub: row.committee?.name,
                    tone: 'primary',
                    onClick: () => openDetail('meeting', row.id),
                },
            ];
            if (can.manage && row.status !== 'cancelled') {
                items.push({
                    icon: <Pencil className="h-3.5 w-3.5" />,
                    label: 'Edit meeting',
                    onClick: () => openDetail('meeting', row.id, 'edit'),
                });
            }
            if (can.manage && sched) {
                items.push(
                    {
                        ...
                        label: 'Add attendees',
                        onClick: () =>
                            openDetail('meeting', row.id, 'attendees'),
                    },
                    {
                        icon: <ClipboardCheck className="h-3.5 w-3.5" />,
                        label: 'Complete meeting',
                        sub: 'Minutes + action items',
                        onClick: () =>
                            openDetail('meeting', row.id, 'complete'),
                    },
                    {
            ...
            if (row.minutes_document_path) {
                items.push({
                    ...
                    onClick: () =>
                        window.open(
                            `${BASE}/meetings/${row.id}/minutes/download`,
                        ),
                });
            } else if (can.manage) {
                items.push({
                    ...
                    onClick: () => openDetail('meeting', row.id, 'minutes'),
                });
            }
            return items;
        }
        ...
        const items: ShiftCtxItem[] = [
            {
                ...
                sub: row.title,
                tone: 'primary',
                onClick: () => openDetail('consultation', row.id),
            },
        ];
            ...
            items.push(
                {
                    icon: <Pencil className="h-3.5 w-3.5" />,
                    label: 'Edit details',
                    onClick: () => openDetail('consultation', row.id, 'edit'),
                },
                {
                    icon: <MessageSquare className="h-3.5 w-3.5" />,
                    label: 'Record feedback',
                    onClick: () =>
                        openDetail('consultation', row.id, 'feedback'),
                },
                {
                    icon: <FileCheck2 className="h-3.5 w-3.5" />,
                    label: 'Record outcome',
                    onClick: () =>
                        openDetail('consultation', row.id, 'outcome'),
                },
            );
        }
        if (can.manage) {
            items.push(
                { sep: true },
                {
                    ...
                    onClick: () => openDetail('consultation', row.id, 'upload'),
                },
            );
            if (!closed) {
                items.push({
                    ...
                    tone: 'critical',
                    onClick: () => openDetail('consultation', row.id, 'close'),
                });
            }
        }
        return items;
            ...
            items: [
                {
                    ...
                    tone: 'primary',
                    onClick: () => setWizard('rep'),
                },
                {
                    icon: <Calendar className="h-3.5 w-3.5" />,
                    label: 'Schedule meeting',
                    onClick: () => setWizard('meeting'),
                },
                {
                    icon: <MessageSquare className="h-3.5 w-3.5" />,
                    label: 'New consultation',
                    onClick: () => setWizard('consultation'),
                },
                { sep: true },
                {
                    icon: <Download className="h-3.5 w-3.5" />,
                    ...
                    onClick: () =>
                        window.open(
                            `${BASE}/export${filters.site_id ? `?site_id=${filters.site_id}` : ''}`,
                        ),
                },
            ],
        });
    ...
    const coverageTone: Tone =
        b.reps_coverage_pct >= 100
            ? 'success'
            : b.reps_coverage_pct >= 60
              ? 'warning'
              : 'critical';
    ...
    return (
        <AppLayout
            breadcrumbs={[
                ...
                { title: 'Worker Participation', href: BASE },
            ]}
        >
            ...
            <div className="flex flex-col gap-6 p-6">
                ...
                >
                    <div onContextMenu={openHeroCtx}>
                        ...
                        <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
                            <div className="flex items-start gap-4">
                                ...
                                <div className="flex flex-col gap-1.5">
                                    <HeroStatusPill>
                                        Worker participation · HSWA 2015
                                    </HeroStatusPill>
                                    ...
                                    <p className="max-w-xl text-sm text-primary-foreground/70">
                                        Elect and support H&amp;S
                                        representatives, run committee meetings,
                                        and consult kaimahi on work that affects
                                        them — the HSWA 2015 participation
                                        duties, in one register.
                                    </p>
                                </div>
                            </div>
                            {can.manage ? (
                        ...
                        <div className="mt-4 grid gap-3 lg:grid-cols-2">
                            <HeroCluster
                                title="Representatives & committees"
                                icon={Users}
                            >
                                <HeroClusterTile
                                    href={`${BASE}?tab=representatives&status=active`}
                                    label="Active reps"
                                    value={fmt(c.participation.active_reps)}
                                    caption="currently serving"
                                    tone="success"
                                />
                                <HeroClusterTile
                                    href={`${BASE}?tab=representatives`}
                                    label="Sites w/o rep"
                                    value={fmt(
                                        c.participation.sites_without_rep,
                                    )}
                                    caption="coverage gap"
                                    tone={
                                        c.participation.sites_without_rep > 0
                                            ? 'warning'
                                            : 'success'
                                    }
                                />
                                <HeroClusterTile
                                    ...
                                    label="Committees"
                                    value={fmt(c.participation.committees)}
                                ...
                                <HeroClusterTile
                                    ...
                                    label="Meetings · qtr"
                                    value={fmt(
                                        c.participation.meetings_quarter,
                                    )}
                            ...
                            >
                                <HeroClusterTile
                                    href={`${BASE}?tab=consultations&status=open`}
                                    label="Open consults"
                                    value={fmt(c.consultation.open)}
                                    caption="not yet closed"
                                    tone="warning"
                                />
                                <HeroClusterTile
                                    href={`${BASE}?tab=consultations`}
                                    label="Awaiting feedback"
                                    value={fmt(
                                        c.consultation.awaiting_feedback,
                                    )}
                                    caption="kaimahi to respond"
                                    tone="warning"
                                />
                                <HeroClusterTile
                                    href={`${BASE}?tab=meetings`}
                                    label="Overdue actions"
                                    value={fmt(c.consultation.overdue_actions)}
                                    caption="from meetings"
                                    tone={
                                        c.consultation.overdue_actions > 0
                                            ? 'critical'
                                            : 'success'
                                    }
                                />
                                <HeroClusterTile
                                    ...
                                    label="Minutes out"
                                    value={fmt(
                                        c.consultation.minutes_outstanding,
                                    )}
                                    caption="to be filed"
                                    tone={
                                        c.consultation.minutes_outstanding > 0
                                            ? 'warning'
                                            : 'success'
                                    }
                                />
                        ...
                        <div className="mt-4 flex flex-wrap items-center gap-2">
                            <HeroChip
                                icon={ShieldCheck}
                                tone={coverageTone}
                                label={`Reps coverage · ${b.reps_coverage_pct}% (${b.sites_covered}/${b.sites_total} sites)`}
                            />
                            <HeroChip
                                icon={FileText}
                                tone={
                                ...
                                label={
                                    b.minutes_overdue > 0
                                        ? `Minutes · ${b.minutes_overdue} outstanding`
                                        : 'Minutes · all filed'
                                }
                            />
                            <HeroChip
                                icon={MessageSquare}
                                tone={
                                    b.consultations_awaiting > 0
                                        ? 'warning'
                                        : 'success'
                                }
                                label={
                                    b.consultations_awaiting > 0
                                        ? `Consultations · ${b.consultations_awaiting} awaiting feedback`
                                        : 'Consultations · up to date'
                                }
                            />
                            <HeroChip
                                icon={GraduationCap}
                                tone={
                                    b.training_below_minimum > 0
                                        ? 'warning'
                                        : 'success'
                                }
                                label={
                                    b.training_below_minimum > 0
                                        ? `Rep training · ${b.training_below_minimum} below 2-day min`
                                        : 'Rep training · minimum met'
                                }
                            />
                        </div>
                    </div>
                ...
                <Card>
                    <CardContent className="p-0">
                        ...
                        {tab === 'representatives' && (
                            <RepTable
                                rows={rows.data as RepRow[]}
                                onOpen={(id) =>
                                    openDetail('representative', id)
                                }
                                onCtx={openRowCtx}
                            />
                        )}
                        {tab === 'meetings' && (
                            <MeetingTable
                                rows={rows.data as MeetingRow[]}
                                onOpen={(id) => openDetail('meeting', id)}
                                onCtx={openRowCtx}
                            />
                        )}
                        {tab === 'consultations' && (
                            <ConsultationTable
                                rows={rows.data as ConsultationRow[]}
                                onOpen={(id) => openDetail('consultation', id)}
                                onCtx={openRowCtx}
                            />
                        )}
                    </CardContent>
...
const Pill = ({ tone, children }: { tone: Tone; children: ReactNode }) => (
    <span
        className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[11px] font-medium ${TONE_BG[tone]}`}
    >
        <span className={`h-1.5 w-1.5 rounded-full ${TONE_DOT[tone]}`} />
        {children}
    </span>
                            ...
                            >
                                <td className="px-4 py-3">
                                    <div className="flex items-center gap-2.5">
                                        <span
                                            className={`flex h-8 w-8 items-center justify-center rounded-full text-[11px] font-semibold ${entityTone(r.id)}`}
                                        >
                                            {initials(name)}
                                        </span>
                                ...
                                <td className="px-4 py-3">
                                    <span className="font-medium">
                                        {titleCase(r.election_method ?? '—')}
                                    </span>
                                    <span className="block text-xs text-muted-foreground">
                                        {fmtDate(r.elected_at)}
                                    </span>
                                </td>
                                <td className="px-4 py-3">
                                    <FlagBadge
                                        icon={GraduationCap}
                                        tone={below ? 'warning' : 'neutral'}
                                        title="HSWA paid training"
                                    >
                                        {r.training_days_completed}{' '}
                                        {r.training_days_completed === 1
                                            ? 'day'
                                            : 'days'}
                                    </FlagBadge>
                                </td>
                                <td className="px-4 py-3">
                                    <Pill
                                        tone={REP_STATUS[r.status] ?? 'neutral'}
                                    >
                                        {titleCase(r.status)}
                                    </Pill>
                                </td>
...
function MeetingTable({
    ...
    return (
        <div className="overflow-x-auto">
            <table className="w-full text-sm">
                <HeadRow
                    cols={['Committee', 'Date', 'Status', 'Attendees', 'Flags']}
                />
                <tbody className="divide-y">
                    {rows.map((m) => (
                        <Row
                        ...
                        >
                            ...
                            <td className="px-4 py-3">
                                <Pill
                                    tone={MEETING_STATUS[m.status] ?? 'neutral'}
                                >
                                    {titleCase(m.status)}
                                </Pill>
                            </td>
                            <td className="px-4 py-3">
                                <span className="inline-flex items-center gap-1.5 text-muted-foreground">
                                    ...
                                    {m.attendees_count > 0
                                        ? `${m.attendees_count} confirmed`
                                        : '—'}
                                </span>
                            </td>
                            <td className="px-4 py-3">
                                <div className="flex flex-wrap gap-1.5">
                                    {m.actions_due_count > 0 ? (
                                        <FlagBadge
                                            icon={Wrench}
                                            tone="critical"
                                            title="Action items due"
                                        >
                                            {m.actions_due_count} due
                                        </FlagBadge>
                                    ) : null}
                                    {m.status === 'completed' &&
...
function ConsultationTable({
    ...
    if (!rows.length)
        return <Empty icon={MessageSquare} label="No consultations" />;
    return (
        <div className="overflow-x-auto">
            <table className="w-full text-sm">
                <HeadRow
                    cols={['Topic', 'Type', 'Status', 'Progress', 'Docs']}
                />
                <tbody className="divide-y">
                    {rows.map((c) => {
                        const st =
                            CONSULT_STATUS[c.status] ?? CONSULT_STATUS.open;
                        ...
                        return (
                            ...
                            >
                                <td className="px-4 py-3">
                                    <div className="flex items-center gap-2.5">
                                        <span
                                            className={`h-1.5 w-1.5 rounded-full ${TONE_DOT[st.tone]}`}
                                        />
                                        <span>
                                ...
                                <td className="px-4 py-3">
                                    <span
                                        className={`inline-flex rounded-md px-2 py-0.5 text-[11px] font-medium ${TONE_BG.neutral}`}
                                    >
                                        {titleCase(c.consultation_type)}
                                    </span>
                                </td>
                                <td className="px-4 py-3">
                                    <span
                                        className={`inline-flex rounded-full px-2.5 py-0.5 text-[11px] font-medium ${TONE_BG[st.tone]}`}
                                    >
                                        {st.label}
                                    </span>
                                </td>
                                <td className="px-4 py-3">
                                    <div className="flex items-center gap-1">
                                        {[1, 2, 3, 4].map((i) => (
                                            <span
                                                key={i}
                                                className={`h-1.5 w-6 rounded ${i <= ord ? TONE_DOT[st.tone] : 'bg-muted'}`}
                                            />
                                        ))}
                                    </div>
                                </td>
                                <td className="px-4 py-3">
                                    <div className="flex gap-2 text-muted-foreground">
                                        <FileText
                                            className={`h-4 w-4 ${c.document_path ? 'text-primary' : 'opacity-30'}`}
                                        />
                                        <FileCheck2
                                            className={`h-4 w-4 ${c.outcome_document_path ? 'text-status-success' : 'opacity-30'}`}
                                        />
                                    </div>
                                </td>
                            </Row>
