...
type Props = {
    ...
    hero: {
        ...
        nz: {
            worksafe_approved: number;
            nga_paerewa_documented: boolean;
            review_overdue: number;
            coverage_gaps: number;
            high_risk_covered: boolean;
        };
    };
    filters: Filters;
...
type WizardState =
    | { mode: 'create' }
    | { mode: 'edit'; form: ProcedureFormData }
    | null;
...
export default function ProceduresIndex({
    ...
    const [pendingSection, setPendingSection] =
        useState<ProcedureSectionKey>('overview');
    const [pendingAction, setPendingAction] =
        useState<ProcedureActionKey | null>(null);
    const [wizard, setWizard] = useState<WizardState>(null);

    const wizardOptions: ProcedureWizardOptions = {
        sites,
        roles,
        trainingCourses,
        owners,
    ...
    useEffect(() => {
        ...
        else if (params.get('edit') === '1' && detail)
            setWizard({ mode: 'edit', form: detail.form });
    ...
    const setTab = (id: string) =>
        router.get(URL, { ...filters, tab: id }, { preserveScroll: true });

    const openProcedure = (
        id: number,
        opts?: { section?: ProcedureSectionKey; action?: ProcedureActionKey },
    ) => {
        setPendingSection(opts?.section ?? 'overview');
        setPendingAction(opts?.action ?? null);
        router.get(
            URL,
            { ...filters, procedure: id },
    ...
    const openEdit = (id: number) =>
        router.get(
            ...
            { ...filters, procedure: id },
            {
                ...
                onSuccess: (page) => {
                    const det = (
                        page.props as { detail?: ProcedureDetail | null }
                    ).detail;
    ...
    const TABS: RosterTabItem[] = [
        ...
        },
        {
            ...
            icon: Pencil,
            tone: 'info',
            badge: tabCounts.draft || undefined,
        },
        {
            id: 'under_review',
            label: 'Under review',
            icon: Clock,
            tone: 'warning',
            badge: tabCounts.under_review || undefined,
        },
        {
            id: 'approved',
            label: 'Approved',
            icon: CheckCircle2,
            tone: 'success',
            badge: tabCounts.approved || undefined,
        },
        {
            id: 'review_due',
            label: 'Review due',
            icon: CalendarCheck,
            tone: 'critical',
            badge: tabCounts.review_due || undefined,
        },
        {
            id: 'archived',
            label: 'Archived',
            icon: Archive,
            tone: 'info',
            badge: tabCounts.archived || undefined,
        },
    ];

    const activeReview = filters.review_state ?? 'all';
    const onReview = (key: string) =>
        go({ review_state: key === 'all' ? null : key });
    ...
    const openHeroCtx = (e: ReactMouseEvent) => {
        ...
        items.push(
            ...
            },
            {
                ...
                label: 'Review-due list',
                onClick: () => setTab('review_due'),
            },
        );
        setCtx({
            x: e.clientX,
            y: e.clientY,
            tag: 'LIBRARY',
            meta: `${tabCounts.all ?? 0} procedures`,
            items,
        });
    };
    ...
    const openRowCtx = (e: ReactMouseEvent, p: ProcedureRow) => {
        ...
        if (can.manage && p.status !== 'archived') {
            items.push({
                ...
                label: p.status === 'approved' ? 'New version' : 'Edit',
                onClick: () => openEdit(p.id),
            });
        }
        if (can.manage && p.status === 'draft')
            items.push({
                icon: <Send className="h-3.5 w-3.5" />,
                label: 'Submit for review',
                onClick: () => openProcedure(p.id, { action: 'submit_review' }),
            });
        if (can.approve && p.status === 'under_review')
            items.push({
                icon: <CheckCircle2 className="h-3.5 w-3.5" />,
                label: 'Approve',
                tone: 'primary',
                onClick: () => openProcedure(p.id, { action: 'approve' }),
            });
        if (
            can.manage &&
            ...
            items.push({
                ...
                onClick: () =>
                    openProcedure(p.id, { action: 'request_changes' }),
            });
        if (can.manage && p.status === 'approved')
            items.push({
                icon: <CalendarCheck className="h-3.5 w-3.5" />,
                label: 'Record review',
                onClick: () => openProcedure(p.id, { action: 'record_review' }),
            });
        if (can.manage && p.status !== 'archived')
            items.push({
                ...
                label: 'Archive',
                tone: 'critical',
                onClick: () => openProcedure(p.id, { action: 'archive' }),
            });
        if (can.manage && p.status === 'archived')
            items.push({
                ...
                label: 'Restore',
                tone: 'primary',
                onClick: () => openProcedure(p.id, { action: 'restore' }),
            });
        items.push(
            { sep: true },
            {
                ...
                onClick: () => router.visit(`${URL}/${p.id}`),
            },
        );
        setCtx({
            x: e.clientX,
            y: e.clientY,
            tag: st.label.toUpperCase(),
            meta: `${p.reference_number} · ${categoryMeta(p.category).label}`,
            items,
        });
    };
    ...
    const complianceItems: HeroComplianceBadge[] = [
        {
            icon: nz.worksafe_approved > 0 ? CheckCircle2 : AlertTriangle,
            tone: nz.worksafe_approved > 0 ? 'success' : 'warning',
            label: `WorkSafe-aligned · ${nz.worksafe_approved} approved`,
        },
        {
            icon: ShieldCheck,
            tone: nz.nga_paerewa_documented ? 'success' : 'warning',
            label: `Ngā Paerewa NZS 8134:2021 · ${nz.nga_paerewa_documented ? 'Documented' : 'Review due'}`,
        },
        {
            icon: nz.review_overdue > 0 ? AlertTriangle : CheckCircle2,
            tone: nz.review_overdue > 0 ? 'critical' : 'success',
            label:
                nz.review_overdue > 0
                    ? `Review cycle · ${nz.review_overdue} overdue`
                    : 'Review cycle · On track',
        },
        {
            icon: nz.coverage_gaps > 0 ? AlertTriangle : CheckCircle2,
            tone: nz.coverage_gaps > 0 ? 'warning' : 'success',
            label:
                nz.coverage_gaps > 0
                    ? `Coverage gaps · ${nz.coverage_gaps}`
                    : 'Coverage · Complete',
        },
        {
            icon: HardHat,
            tone: nz.high_risk_covered ? 'success' : 'warning',
            label: `High-risk categories · ${nz.high_risk_covered ? 'Covered' : 'Gaps'}`,
        },
    ];
    ...
    return (
        <AppLayout
            breadcrumbs={[
                ...
                { title: 'Safe Work Procedures', href: URL },
            ]}
        >
            ...
            <div className="flex flex-col gap-6 p-6">
                ...
                >
                    ...
                    >
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex flex-col gap-1.5">
                                <HeroStatusPill>
                                    Procedure library · synced just now
                                </HeroStatusPill>
                                ...
                                <p className="max-w-xl text-sm text-primary-foreground/70">
                                    The controlled library of safe-work
                                    documents — drafted, reviewed, approved and
                                    kept on a recurring review cycle. Every
                                    version is traceable; every procedure maps
                                    to the roles, sites, training and hazards it
                                    governs.
                                </p>
                            </div>
                        </div>
                    ...
                    <div className="grid gap-3 lg:grid-cols-2">
                        <HeroCluster title="Library status" icon={FileText}>
                            <HeroClusterTile
                                href={`${URL}?tab=approved`}
                                label="Approved"
                                value={fmt(lib.approved)}
                                caption="in force"
                                tone="success"
                            />
                            <HeroClusterTile
                                href={`${URL}?tab=under_review`}
                                label="Under review"
                                value={fmt(lib.under_review)}
                                caption="in progress"
                                tone="warning"
                            />
                            <HeroClusterTile
                            ...
                            />
                            <HeroClusterTile
                                href={`${URL}?tab=archived`}
                                label="Archived"
                                value={fmt(lib.archived)}
                                caption="superseded"
                                tone="neutral"
                            />
                        </HeroCluster>
                        ...
                        >
                            <HeroClusterTile
                                href={`${URL}?tab=review_due`}
                                label="Review due"
                                value={fmt(at.review_due_soon)}
                            ...
                            <HeroClusterTile
                                href={`${URL}?tab=draft`}
                                label="Unapproved"
                                value={fmt(at.unapproved)}
                                caption="awaiting sign-off"
                                tone="neutral"
                            />
                            <HeroClusterTile
                                label="Coverage gaps"
                                value={fmt(at.coverage_gaps)}
                                caption="categories at risk"
                                tone="critical"
                            />
                        </HeroCluster>
                    </div>
                ...
                <section className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
                    <RegisterTableHeader
                        icon={FileText}
                        title={tableTitle}
                        subtitle="the procedure library"
                ...
                {procedures.last_page > 1 ? (
                    <LaravelPagination links={procedures.links} />
                ) : null}
            </div>
            ...
            {detail && !wizard ? (
                <ProcedureDetailDialog
                    ...
                    open
                    onClose={closeDetail}
                    onEdit={(form) => setWizard({ mode: 'edit', form })}
...
function ProcedureTable({
    ...
    rows: ProcedureRow[];
    onRowCtx: (e: ReactMouseEvent, p: ProcedureRow) => void;
            ...
            <table className="w-full min-w-[1040px] text-sm">
                ...
                <tbody className="divide-y divide-border">
                    {rows.map((p) => {
                        ...
                        const ownerName =
                            p.owner?.name ?? p.approved_by?.name ?? null;
                        return (
                            <tr
                                key={p.id}
                                onClick={() => onOpen(p.id)}
                            ...
                            >
                                <td className="px-4 py-3 align-top font-mono text-xs font-semibold whitespace-nowrap text-muted-foreground">
                                    {p.reference_number}
                                </td>
                                <td className="max-w-[300px] px-4 py-3 align-top">
                                    <div className="flex items-start gap-2">
                                        <span
                                            className={`mt-1 h-2 w-2 shrink-0 rounded-full ${cat.dot}`}
                                        />
                                ...
                                <td className="px-4 py-3 align-top">
                                    <span
                                        className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[11px] font-medium ${cat.chip}`}
                                    >
                                        {cat.label}
                                    </span>
                                </td>
                                <td className="px-4 py-3 align-top">
                                    <span
                                        className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-medium ${TONE_BG[st.tone]}`}
                                    >
                                        <StatusIcon className="h-3 w-3" />
                                        {st.label}
                                    </span>
                                </td>
                                <td className="px-4 py-3 align-top text-xs font-bold text-muted-foreground tabular-nums">
                                    v{p.version}
                                </td>
                                <td className="px-4 py-3 align-top">
                                    {ownerName ? (
                                        <span className="flex items-center gap-2">
                                            <span
                                                className={`grid h-7 w-7 shrink-0 place-items-center rounded-md text-[10px] font-bold ${entityTone(p.id)}`}
                                            >
                                                {initials(ownerName)}
                                            </span>
                                            <span className="min-w-0">
                                                ...
                                                <span className="block truncate text-[11px] text-muted-foreground">
                                                    {p.approved_by
                                                        ? `Appr. ${p.approved_by.name.split(' ')[0]}`
                                                        : 'Owner'}
                                                </span>
                                            </span>
                                        </span>
                                ...
                                <td className="px-4 py-3 align-top">
                                    <span className="inline-flex items-center gap-1.5 rounded-md bg-muted px-2 py-1 text-[11px] font-medium text-muted-foreground">
                                        ...
                                        {p.roles_count > 0
                                            ...
                                            : 'All roles'}
                                        {p.documents_count > 0
                                            ? ` · ${p.documents_count} doc${p.documents_count === 1 ? '' : 's'}`
                                            : ''}
                                    </span>
                                </td>
                            </tr>
