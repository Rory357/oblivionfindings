...
import {
    RestraintEventDetailDialog,
    type RestraintEventActionKey,
    type RestraintSectionKey,
} from '@/components/health-safety/restraint-event-detail-dialog';
import {
...
export default function RestraintsIndex({
    ...
    const [pendingSection, setPendingSection] =
        useState<RestraintSectionKey>('overview');
    const [pendingAction, setPendingAction] =
        useState<RestraintEventActionKey | null>(null);
    const [planSection, setPlanSection] = useState<
        'overview' | 'content' | 'lifecycle' | 'reviews'
    >('overview');
    const [planAction, setPlanAction] = useState<'review' | null>(null);

    const go = (
        next: Partial<RestraintFilters & { lens: string; tab: string }>,
    ) =>
        router.get(
            '/health-safety/restraints',
            { ...filters, lens, tab, ...next },
            { preserveState: true, preserveScroll: true, replace: true },
        );

    const setLens = (next: 'events' | 'plans') =>
        router.get(
            '/health-safety/restraints',
            { ...filters, lens: next, tab: 'all' },
            { preserveScroll: true },
        );
    ...
    const openEvent = (
        id: number,
        opts?: {
            section?: RestraintSectionKey;
            action?: RestraintEventActionKey;
        },
    ) => {
    ...
    const openPlan = (
        id: number,
        opts?: {
            section?: 'overview' | 'content' | 'lifecycle' | 'reviews';
            action?: 'review' | null;
        },
    ) => {
    ...
    const exportHref = `/health-safety/restraints/export?${new URLSearchParams({
        ...
        ...(filters.restraint_type
            ? { restraint_type: filters.restraint_type }
            : {}),
    }).toString()}`;
    ...
    const EVENT_TABS: RosterTabItem[] = [
        {
            ...
            icon: LayoutList,
            tone: 'primary',
            badge: tabCounts.events.all || undefined,
        },
        {
            id: 'unreviewed',
            label: 'Unreviewed',
            icon: ClipboardCheck,
            tone: 'warning',
            badge: tabCounts.events.unreviewed || undefined,
        },
        {
            id: 'out_of_plan',
            label: 'Out of plan',
            icon: AlertTriangle,
            tone: 'critical',
            badge: tabCounts.events.out_of_plan || undefined,
        },
        {
            id: 'injury',
            label: 'Injury',
            icon: HeartPulse,
            tone: 'critical',
            badge: tabCounts.events.injury || undefined,
        },
        {
            id: 'critical',
            label: 'Critical',
            icon: ShieldAlert,
            tone: 'critical',
            badge: tabCounts.events.critical || undefined,
        },
        {
            id: '30d',
            label: '30 days',
            icon: CalendarClock,
            tone: 'info',
            badge: tabCounts.events['30d'] || undefined,
        },
    ];
    const PLAN_TABS: RosterTabItem[] = [
        {
            ...
            icon: LayoutList,
            tone: 'primary',
            badge: tabCounts.plans.all || undefined,
        },
        {
            id: 'active',
            label: 'Active',
            icon: CheckCircle2,
            tone: 'success',
            badge: tabCounts.plans.active || undefined,
        },
        {
            id: 'draft',
            label: 'Draft',
            icon: FileEdit,
            tone: 'violet',
            badge: tabCounts.plans.draft || undefined,
        },
        {
            id: 'review_due',
            label: 'Review due',
            icon: CalendarClock,
            tone: 'warning',
            badge: tabCounts.plans.review_due || undefined,
        },
        {
            id: 'under_review',
            label: 'Under review',
            icon: Eye,
            tone: 'warning',
            badge: tabCounts.plans.under_review || undefined,
        },
        {
            id: 'archived',
            label: 'Archived',
            icon: Archive,
            tone: 'info',
            badge: tabCounts.plans.archived || undefined,
        },
    ];
    ...
    const openEventRowCtx = (e: ReactMouseEvent, row: EventRow) => {
        ...
        if (can.review && row.flags.unreviewed)
            items.push({
                icon: <ClipboardCheck className="h-3.5 w-3.5" />,
                label: 'Review event',
                sub: 'confirm & close out',
                onClick: () =>
                    openEvent(row.id, { section: 'review', action: 'review' }),
            });
        if (row.behaviour_support_plan_id)
            items.push({
                icon: <BookOpen className="h-3.5 w-3.5" />,
                label: 'Open linked plan',
                onClick: () => openPlan(row.behaviour_support_plan_id!),
            });
        ...
        setCtx({
            x: e.clientX,
            y: e.clientY,
            tag: severityMeta(row.severity).label.toUpperCase(),
            meta: `${row.reference} · ${typeMeta(row.restraint_type).label}`,
            items,
        });
    };
    ...
    const openPlanRowCtx = (e: ReactMouseEvent, row: PlanRow) => {
        ...
        if (can.create && row.client)
            items.push({
                icon: <Plus className="h-3.5 w-3.5" />,
                label: 'Record event under this plan',
                onClick: () =>
                    openEventWizard({
                        client_id: row.client!.id,
                        client_name: row.client!.name,
                        behaviour_support_plan_id: row.id,
                    }),
            });
        if (can.manage && row.status === 'draft')
            items.push({
                ...
                onClick: () =>
                    router.post(
                        `/health-safety/restraints/plans/${row.id}/activate`,
                        {},
                        { preserveScroll: true },
                    ),
            });
        if (can.manage && row.status === 'active')
            items.push({
                icon: <ClipboardCheck className="h-3.5 w-3.5" />,
                label: 'Submit for review',
                onClick: () =>
                    router.post(
                        `/health-safety/restraints/plans/${row.id}/submit-review`,
                        {},
                        { preserveScroll: true },
                    ),
            });
        if (can.review)
            items.push({
                ...
                label: 'Record review',
                onClick: () =>
                    openPlan(row.id, { section: 'reviews', action: 'review' }),
            });
        if (can.manage && row.status !== 'archived')
            items.push({
                ...
                onClick: () =>
                    router.post(
                        `/health-safety/restraints/plans/${row.id}/archive`,
                        {},
                        { preserveScroll: true },
                    ),
            });
        ...
        setCtx({
            x: e.clientX,
            y: e.clientY,
            tag: planStatusMeta(row.status).label.toUpperCase(),
            meta: row.reference,
            items,
        });
    };
    ...
    const openHeroCtx = (e: ReactMouseEvent) => {
        ...
        items.push({
            ...
            onClick: () =>
                router.get(
                    '/health-safety/restraints',
                    { ...filters, lens: 'events', tab: 'unreviewed' },
                    { preserveScroll: true },
                ),
        });
        items.push({ sep: true });
    ...
    const b = hero.badges;
    const reductionTone: ChipTone =
        b.reduction_trend_pct < 0
            ? 'success'
            : b.reduction_trend_pct > 0
              ? 'warning'
              : 'neutral';
    const reductionLabel =
        b.reduction_trend_pct === 0
            ? 'no change'
            : `${b.reduction_trend_pct < 0 ? '↓' : '↑'} ${Math.abs(b.reduction_trend_pct)}%`;
            ...
            <div className="flex flex-col gap-6 p-6">
                            ...
                            <div className="flex items-start gap-4">
                                ...
                                <div className="flex flex-col gap-1.5">
                                    ...
                                    <p className="max-w-xl text-sm text-primary-foreground/70">
                                        Every restrictive-practice episode and
                                        behaviour support plan in one register —
                                        recorded against Ngā Paerewa NZS
                                        8134:2021, reviewed, and driven toward
                                    ...
                                    <div className="mt-1.5 flex flex-wrap gap-2">
                                        ...
                                        >
                                            {b.plans_overdue > 0
                                                ? `Plans overdue review · ${b.plans_overdue}`
                                                : 'Plan reviews current'}
                                        </HeroBadge>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="grid gap-3 lg:grid-cols-2">
                            ...
                            >
                                ...
                                <HeroClusterTile
                                    ...
                                    label="Injuries"
                                    value={fmt(live.injuries)}
                                    caption="with harm"
                                    tone="critical"
                                />
                                <HeroClusterTile
                                    href="/health-safety/restraints?lens=events&tab=critical"
                                    label="Critical"
                                    value={fmt(live.critical)}
                                    caption="severity"
                                    tone="critical"
                                />
                            </HeroCluster>
                            ...
                            >
                                ...
                                <HeroClusterTile
                                    ...
                                    label="Review due"
                                    value={fmt(at.plans_review_due)}
                                    caption="plans"
                                    tone="warning"
                                />
                                <HeroClusterTile
                                    href="/health-safety/restraints?lens=plans&tab=under_review"
                                    label="Under review"
                                    value={fmt(at.plans_under_review)}
                                    caption="plans"
                                    tone="neutral"
                                />
                                <HeroClusterTile
                                    href="/health-safety/restraints?lens=plans&tab=all"
                                    label="No active BSP"
                                    value={fmt(at.clients_no_active_bsp)}
                                    caption="clients"
                                    tone="critical"
                                />
                            </HeroCluster>
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <div className="inline-flex gap-1 rounded-xl bg-muted p-1">
                        ...
                        <LensButton
                            ...
                            icon={BookOpen}
                            label="Plans"
                            onClick={() => setLens('plans')}
                        />
                    </div>
                    <div className="min-w-0 flex-1">
                        <TabStrip
                            value={tab}
                            items={lens === 'events' ? EVENT_TABS : PLAN_TABS}
                            onChange={setTab}
                            ariaLabel={`${lens === 'events' ? 'Event' : 'Plan'} views`}
                        />
                    </div>
                </div>
                ...
                {lens === 'events' ? (
                    <section className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
                        <RegisterTableHeader
                            icon={ShieldAlert}
                            title="All restraint events"
                            subtitle="restrictive-practice episodes"
                            hint="Right-click a row for the full workflow"
                            hintIcon={MousePointer2}
                        />
                        <EventTable
                            rows={events.data}
                            onRowCtx={openEventRowCtx}
                            onOpen={openEvent}
                        />
                    </section>
                ) : (
                    <PlanGrid
                        rows={plans.data}
                        onRowCtx={openPlanRowCtx}
                        onOpen={openPlan}
                    />
                )}

                {lens === 'events' && events.last_page > 1 ? (
                    <LaravelPagination links={events.links} />
                ) : null}
                {lens === 'plans' && plans.last_page > 1 ? (
                        ...
                        can.create && detail.client
                            ? () => {
                                  ...
                                  openEventWizard({
                                      ...
                                      behaviour_support_plan_id: detail.id,
                                  });
                              }
                            : undefined
                    }
...
function HeroBadge({
    ...
    return (
        <span
            className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-semibold ${BADGE_TONE[tone]}`}
        >
...
function EventTable({
    ...
    rows: EventRow[];
    onRowCtx: (e: ReactMouseEvent, row: EventRow) => void;
    ...
    return (
        <div className="overflow-x-auto">
            <table className="w-full min-w-[1040px] text-sm">
                <thead>
                    <tr className="border-b border-border bg-muted/70 text-left text-[11px] font-semibold tracking-wide text-muted-foreground uppercase">
        ...
        >
            ...
            <td className="px-4 py-3 align-top">
                {row.client ? (
                    <div className="flex items-center gap-2">
                        <span
                            className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-[11px] font-bold ${entityTone(row.client.id)}`}
                        >
            ...
            <td className="px-4 py-3 align-top">
                <span className="inline-flex items-center gap-1.5">
                    <span
                        className={`h-2 w-2 rounded-full ${DOT[type.tone]}`}
                    />
                    <type.icon className={`h-4 w-4 ${ICON_TEXT[type.tone]}`} />{' '}
                    {type.label}
                </span>
            </td>
            ...
            <td className="px-4 py-3 align-top">
                <span
                    className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${CHIP[sev.tone]}`}
                >
                    {sev.label}
                </span>
            </td>
                ...
                <div className="flex flex-wrap gap-1.5">
                    ...
                    {row.flags.out_of_plan ? (
                        <FlagBadge
                            icon={AlertTriangle}
                            tone="critical"
                            title="Outside the behaviour support plan"
                        >
                            Out of plan
                        </FlagBadge>
                    ) : null}
...
function PlanGrid({
    ...
    rows: PlanRow[];
    onRowCtx: (e: ReactMouseEvent, row: PlanRow) => void;
    onOpen: (id: number) => void;
}) {
    if (rows.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center gap-2 rounded-2xl border border-border bg-card px-6 py-16 text-center shadow-sm">
                ...
                <div className="text-sm font-semibold">
                    No behaviour support plans here
                </div>
                <p className="text-xs text-muted-foreground">
                    Nothing matches this tab and filters.
                </p>
    ...
    return (
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {rows.map((row) => {
                ...
                const reviewState =
                    REVIEW_STATE_META[row.review_state] ?? REVIEW_STATE_META.ok;
                return (
                    <div
                        key={row.id}
                        role="button"
                    ...
                    >
                        <div className="flex items-start justify-between gap-2">
                            <div className="flex items-center gap-2">
                                <span
                                    className={`grid h-9 w-9 shrink-0 place-items-center rounded-full text-xs font-bold ${row.client ? entityTone(row.client.id) : 'bg-muted text-muted-foreground'}`}
                                >
                                    {initials(row.client?.name)}
                                </span>
                                <div className="min-w-0">
                            ...
                            <span
                                className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ${CHIP[status.tone]}`}
                            >
                        ...
                        <div className="mt-auto flex flex-wrap items-center gap-1.5">
                            {row.restrictive_practice_type ? (
                                <span className="inline-flex items-center rounded-md bg-muted px-2 py-0.5 text-[11px] font-medium text-muted-foreground">
                                    {titleCase(row.restrictive_practice_type)}
                                </span>
                            ) : null}
                            {row.status === 'active' &&
                            row.review_state !== 'ok' ? (
