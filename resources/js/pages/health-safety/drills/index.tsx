...
export default function DrillsIndex({
    ...
    const [pendingAction, setPendingAction] = useState<DrillActionKey | null>(
        null,
    );
    const [scheduleOpen, setScheduleOpen] = useState(false);
    const [completeDrill, setCompleteDrill] = useState<{
        id: number;
        reference: string;
        type_label: string;
    } | null>(null);
    ...
    const openDrill = (
        id: number,
        opts?: { section?: DrillSectionKey; action?: DrillActionKey },
    ) => {
    ...
    const hasFilters = !!(
        ...
        filters.outcome ||
        filters.site_id ||
        (filters.period && filters.period !== '6mo')
    );

    const launchComplete = (
        d: { id: number; reference: string; type_label: string },
        fromDetail: boolean,
    ) => {
        setCompleteDrill(d);
    ...
    const TABS: RosterTabItem[] = [
        ...
        },
        {
            ...
            icon: CheckCircle2,
            tone: 'success',
            badge: tabCounts.completed || undefined,
        },
        {
            id: 'findings',
            label: 'Findings open',
            icon: ClipboardList,
            tone: 'warning',
            badge: tabCounts.findings || undefined,
        },
    ];
    ...
    const openRowCtx = (e: ReactMouseEvent, row: DrillRow) => {
        ...
        if (row.findings_open > 0) {
            items.push({
                ...
                onClick: () => openDrill(row.id, { section: 'findings' }),
            });
        }
        if (can.manage && row.raw_status === 'scheduled') {
            items.push({
                ...
                onClick: () => openDrill(row.id, { action: 'edit' }),
            });
        }
        if (
            can.manage &&
            row.raw_status !== 'completed' &&
            row.raw_status !== 'cancelled'
        ) {
            items.push({
                icon: <XCircle className="h-3.5 w-3.5" />,
                label: 'Cancel drill',
                tone: 'critical',
                onClick: () => openDrill(row.id, { action: 'cancel' }),
            });
        }
        items.push({ sep: true });
        items.push({
            ...
            sub: `/health-safety/drills/${row.id}`,
            onClick: () => router.visit(`/health-safety/drills/${row.id}`),
        });

        setCtx({
            x: e.clientX,
            y: e.clientY,
            tag: status.label.toUpperCase(),
            meta: `${row.reference} · ${type.label}`,
            items,
        });
    };
    ...
    const openHeroCtx = (e: ReactMouseEvent) => {
        ...
        items.push({
            ...
            label: 'Jump to overdue',
            onClick: () => setTab('overdue'),
        });
        items.push({ sep: true });
        items.push({
            icon: <BarChart3 className="h-3.5 w-3.5" />,
            label: 'Drill analytics',
    ...
    const b = hero.badges;
    const pctTone: ChipTone =
        b.sites_drilled_pct >= 80
            ? 'success'
            : b.sites_drilled_pct >= 50
              ? 'warning'
              : 'critical';
    const tableTitle =
        {
            all: 'All drills',
            scheduled: 'Scheduled drills',
            overdue: 'Overdue drills',
            in_progress: 'In progress',
            completed: 'Completed drills',
            findings: 'Drills with open findings',
        }[tab] ?? 'Emergency drills';
        ...
        >
            ...
            <div className="flex flex-col gap-6 p-6">
                ...
                <div onContextMenu={openHeroCtx}>
                        ...
                        footer={
                            <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
                                <HeroSegmented
                                    ...
                                    items={PERIOD_ITEMS}
                                    value={filters.period || '6mo'}
                                    onChange={(key) => go({ period: key })}
                                />
                                {sites?.length ? (
                                    <EntityFilter
                                        label="Site"
                    ...
                    >
                            ...
                            <div className="flex items-start gap-4">
                                ...
                                <div className="flex flex-col gap-1.5">
                                    ...
                                    <p className="max-w-xl text-sm text-primary-foreground/70">
                                        Schedule, run and close out evacuation,
                                        fire and lockdown drills across every
                                        site — then drive the findings to
                                        verified action. Each drill carries its
                                        own lifecycle: schedule → run → record
                                        findings → close out.
                                    </p>
                                    <div className="mt-1.5 flex flex-wrap gap-2">
                                        ...
                                        >
                                            {b.drills_overdue > 0
                                                ? `Fire · ${b.drills_overdue} drill${b.drills_overdue === 1 ? '' : 's'} overdue`
                                                : 'Fire · drills current'}
                                        </HeroBadge>
                                        <HeroBadge
                                            icon={nzsBadge.icon}
                                        ...
                                        <HeroBadge
                                            icon={AlertTriangle}
                                            tone={
                                                b.fenz_reviews_due > 0
                                                    ? 'critical'
                                                    : 'success'
                                            }
                                        >
                                            {b.fenz_reviews_due > 0
                                                ? `FENZ scheme · ${b.fenz_reviews_due} review due`
                                                : 'FENZ scheme · current'}
                                        </HeroBadge>
                                    </div>
                                </div>
                            ...
                            <div className="flex items-center gap-2">
                                <Popover>
                                    ...
                                    >
                                        {[
                                            {
                                                label: 'Drill & compliance analytics',
                        ...
                        <div className="grid gap-3 lg:grid-cols-2">
                            ...
                            >
                                <HeroClusterTile
                                    href="/health-safety/drills?tab=scheduled"
                                    label="Scheduled"
                                    value={fmt(live.scheduled)}
                                    caption="upcoming"
                                    tone="neutral"
                                />
                                <HeroClusterTile
                                    href="/health-safety/drills?tab=overdue"
                                    label="Overdue"
                                    value={fmt(live.overdue)}
                                    caption="past due date"
                                    tone="critical"
                                />
                                <HeroClusterTile
                                    href="/health-safety/drills?tab=in_progress"
                                    label="In progress"
                                ...
                                />
                                <HeroClusterTile
                                    href="/health-safety/drills?tab=completed"
                                    label="Completed"
                                    value={fmt(live.completed)}
                                    caption="this period"
                                    tone="success"
                                />
                            </HeroCluster>
                            ...
                            >
                                <HeroClusterTile
                                    href="/health-safety/drills?tab=overdue"
                                    label="Sites overdue"
                                    value={fmt(at.sites_overdue)}
                                    caption="drill 6mo+ ago"
                                    tone="critical"
                                />
                                <HeroClusterTile
                                    href="/health-safety/drills?tab=findings"
                                    label="Findings open"
                                    value={fmt(at.findings_open)}
                                    caption="across drills"
                                    tone="warning"
                                />
                                <HeroClusterTile
                                    href="/health-safety/drills?tab=findings"
                                    label="Findings overdue"
                                    value={fmt(at.findings_overdue)}
                                    caption="corrective late"
                                    tone="critical"
                                />
                                <HeroClusterTile
                                    href="/health-safety/drills?tab=in_progress"
                                    label="Await write-up"
                                    value={fmt(at.awaiting_writeup)}
                                    caption="ran, not recorded"
                                    tone="warning"
                                />
                            </HeroCluster>
                        </div>
                ...
                <TabStrip
                    ...
                    items={TABS}
                    onChange={setTab}
                    ariaLabel="Emergency drill views"
                />
                ...
                <section className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
                    ...
                    <DrillTable
                        rows={drills.data}
                        onRowCtx={openRowCtx}
                        onOpen={(id) => openDrill(id)}
                    />
                </section>

                {drills.last_page > 1 ? (
                    <LaravelPagination links={drills.links} />
                ) : null}
            </div>
            ...
            <DrillScheduleDialog
                ...
                sites={sites}
                staff={staff}
                defaultSiteId={filters.site_id}
            />

            {completeDrill ? (
                <DrillCompleteDialog
                    open
                    onClose={() => setCompleteDrill(null)}
                    drill={completeDrill}
                />
            ) : null}
        </AppLayout>
...
function DrillTable({
    ...
    rows: DrillRow[];
    onRowCtx: (e: ReactMouseEvent, row: DrillRow) => void;
        ...
        >
            ...
            <td className="px-4 py-3 align-top">
                <div className="flex items-start gap-2">
                    <TypeIcon
                        className={`mt-0.5 h-4 w-4 shrink-0 ${ICON_TEXT[type.tone]}`}
                    />
            ...
            <td className="px-4 py-3 align-top">
                {row.site ? (
                    <div className="flex items-center gap-2">
                        <span
                            className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-[11px] font-bold ${entityTone(row.site.id)}`}
                        >
                            {initials(row.site.name)}
                        </span>
                        <div className="min-w-0">
            ...
            <td className="px-4 py-3 align-top">
                {row.findings_open > 0 ? (
                    <span className="inline-flex items-center gap-1.5 font-medium text-status-warning">
                        ...
                        {row.findings_open} open
                    </span>
                ) : (
                    <span className="text-muted-foreground">
                        {row.findings_count > 0
                            ? `${row.findings_count} closed`
                            : '0'}
                    </span>
                )}
            </td>
