...
export default function ShiftShow({ shift, handover, notes, incidents, incidentTemplates, can }: Props) {
  ...
  async function toggleTask(task: Task, next: boolean) {
    ...
    try {
      const res = await fetch(`/operations/shifts/${shift.id}/tasks/${task.id}`, {
        method: 'PATCH',
  ...
  return (
    <AppLayout
      breadcrumbs={[
        ...
        { title: `${name} (${new Date(shift.starts_at).toLocaleDateString()})`, href: `/operations/shifts/${shift.id}` },
      ]}
    >
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <div className="mt-1 text-sm">
              Client:{' '}
              <Link className="underline" href={`/operations/clients/${shift.client.id}`}>
                {name}
              </Link>
          ...
          <div className="flex items-center gap-2">
            {canStartShift ? (
              <Button
                size="sm"
                onClick={() => router.patch(`/operations/shifts/${shift.id}/start`, {}, { preserveScroll: true })}
              >
                Start
              </Button>
            ) : null}
            ...
            {auth?.can?.shifts?.update ? (
              <Link className="rounded-md border px-3 py-2 text-xs hover:bg-muted" href={`/operations/shifts/${shift.id}/edit`}>
                Edit
              </Link>
            ) : null}
          </div>
          ...
          <DialogContent className="sm:max-w-2xl">
            ...
            <DialogFooter>
              ...
              <Button
                ...
                onClick={() =>
                  completeForm.patch(`/operations/shifts/${shift.id}/complete`, {
                    preserveScroll: true,
                    onSuccess: () => {
          ...
          <DialogContent className="sm:max-w-2xl">
            ...
            <DialogFooter>
              <Button
                ...
                onClick={() =>
                  incidentForm.post(`/operations/shifts/${shift.id}/incidents`, {
                    onSuccess: () => {
                      incidentForm.reset();
            ...
            {can.add_note ? (
              <div className="rounded-md border p-3">
                ...
                <div className="mt-3 flex flex-wrap items-center gap-3">
                  ...
                  <Button
                    onClick={() =>
                      noteForm.post(`/operations/clients/${shift.client.id}/notes`, {
                        preserveScroll: true,
