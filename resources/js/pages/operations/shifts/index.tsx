            ...
            <PageShell>
                <PageHeader
                    ...
                    actions={
                        canCreate ? (
                            <Button asChild>
                                <Link href="/operations/shifts/create">Create</Link>
                            </Button>
                        ) : null
                    }
                />
                ...
                <div className="flex flex-col gap-3">
                    <div className="grid grid-cols-1 gap-3 md:grid-cols-6">
                        <Input
                            ...
                            value={filters.from}
                            onChange={(e) =>
                                router.get('/operations/shifts', { ...filters, from: e.target.value }, { preserveState: true, replace: true })
                            }
                        />
                        <Input
                            type="date"
                            value={filters.to}
                            onChange={(e) =>
                                router.get('/operations/shifts', { ...filters, to: e.target.value }, { preserveState: true, replace: true })
                            }
                        />
                        <Input
                        ...
                        <Select
                            ...
                            onValueChange={(v) =>
                                router.get('/operations/shifts', { ...filters, status: v === ANY ? null : v }, { preserveState: true, replace: true })
                            }
                        >
                        ...
                        <Select
                            ...
                            onValueChange={(v) =>
                                router.get('/operations/shifts', { ...filters, user_id: v === ANY ? null : v }, { preserveState: true, replace: true })
                            }
                        >
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2">
                        <Select
                            ...
                            onValueChange={(v) =>
                                router.get('/operations/shifts', { ...filters, assigned: v === ANY ? null : v }, { preserveState: true, replace: true })
                            }
                        >
                                ...
                                <tr key={s.id} className="border-t">
                                    ...
                                    <td className="p-3">
                                        <div className="flex justify-end gap-2">
                                            <Link className="text-xs underline" href={`/operations/shifts/${s.id}`}>View</Link>
                                            {canEdit ? (
                                                <Link className="text-xs underline" href={`/operations/shifts/${s.id}/edit`}>Edit</Link>
                                            ) : null}
                                            <Link className="text-xs underline" href={`/operations/timesheets/create?shift_id=${s.id}`}>Timesheet</Link>
                                        </div>
                                    </td>
                                </tr>
                            ))}
