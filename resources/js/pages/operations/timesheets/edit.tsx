    ...
    return (
        <AppLayout
            breadcrumbs={[
                ...
                {
                    title: `${clientName} — ${timesheet.work_date}`,
                    href: editTimesheet.url(timesheet.id),
                },
            ]}
        >
            ...
            <PageShell>
                <PageHero
                    ...
                    stats={[
                        ...
                        {
                            label: 'Break',
                            value: `${timesheet.break_minutes ?? 0}m`,
                        },
                    ]}
                    actions={
                ...
                timesheet.decision_notes ? (
                    <div
                        className={`flex items-start gap-3 rounded-xl border p-4 ${status === 'approved' ? 'border-status-success/30 bg-status-success' : 'border-status-critical/30 bg-status-critical'}`}
                    >
                        {status === 'approved' ? (
                        ...
                        <div>
                            <div
                                className={`text-xs font-medium ${status === 'approved' ? 'text-status-success dark:text-status-success' : 'text-status-critical dark:text-status-critical'}`}
                            >
                                {status === 'approved'
                                    ? 'Approved'
                                    : 'Rejected'}{' '}
                            ...
                            <div
                                className={`mt-1 text-sm whitespace-pre-wrap ${status === 'approved' ? 'text-status-success dark:text-status-success' : 'text-status-critical dark:text-status-critical'}`}
                            >
                                {timesheet.decision_notes}
                            </div>
                        </div>
                ...
                {status === 'draft' ? (
                    <div className="flex items-center gap-3 rounded-xl border border-status-info/30 bg-status-info p-4">
                        ...
                        <span className="text-sm text-status-info dark:text-status-info">
                            This timesheet is a draft. Fill in the details and
                    ...
                    <div className="flex items-center gap-3 rounded-xl border border-status-info/30 bg-status-info p-4">
                        <ArrowRight className="h-4 w-4 shrink-0 text-status-info" />
                        <span className="text-sm text-status-info dark:text-status-info">
                            This timesheet has been submitted and is awaiting
                            manager review.
                        </span>
                    </div>
                ) : null}
                    ...
                    <Card className="border-primary/10">
                        ...
                        <CardContent>
                            <div className="flex flex-wrap items-center gap-3 text-sm">
                                <Link
                                    href={showShift.url(timesheet.shift.id)}
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="grid gap-3 md:grid-cols-4">
                                ...
                                <div className="flex items-center gap-2 rounded-lg border p-3">
                                    <Checkbox
                                        checked={
                                            form.data.is_residential_billable
                                        }
                                        onCheckedChange={(v) =>
                                            form.setData(
                    ...
                    <Card className="max-w-2xl border-status-success/20">
                        <CardContent className="p-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <div className="text-sm font-medium text-status-success dark:text-status-success">
                                        Exported to payroll
                                    </div>
                                    <div className="text-xs text-muted-foreground">
                                        ...
                                        {timesheet.payroll_reference
                                            ? ` · Ref: ${timesheet.payroll_reference}`
                                            : ''}
                                    </div>
                                </div>
                            </div>
                ...
                {timesheet.reconciliation_status &&
                timesheet.reconciliation_status !== 'clear' ? (
                    <Card
                        className={`max-w-2xl ${timesheet.reconciliation_status === 'blocked' ? 'border-status-critical/20' : 'border-status-warning/20'}`}
                    >
                        <CardContent className="p-4">
                            <div className="flex items-center gap-3">
                                <AlertTriangle
                                    className={`h-4 w-4 shrink-0 ${timesheet.reconciliation_status === 'blocked' ? 'text-status-critical' : 'text-status-warning'}`}
                                />
                                <div>
                                    <div
                                        className={`text-sm font-medium ${timesheet.reconciliation_status === 'blocked' ? 'text-status-critical dark:text-status-critical' : 'text-status-warning dark:text-status-warning'}`}
                                    >
                                        Reconciliation:{' '}
                                        {timesheet.reconciliation_status ===
                                        'blocked'
                                    ...
                                    {timesheet.reconciliation_findings
                                        ?.summary ? (
