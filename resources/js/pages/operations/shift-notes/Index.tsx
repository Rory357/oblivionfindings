...
type Props = {
    stats: { total: number; today: number; this_week: number; flagged: number; shifts_without_notes: number };
    chart_by_type: Record<string, number>;
    chart_daily: Array<{ label: string; value: number }>;
    notes: { data: ClientNote[]; links: any[]; current_page: number; last_page: number; total: number };
    clients: Array<{ id: number; first_name: string; last_name: string }>;
    staff: Array<{ id: number; name: string }>;
    filters: { q?: string; type?: string; client_id?: string; author_id?: string; date_from?: string; date_to?: string; flagged?: string; private?: string };
};
...
function formatShiftTime(shift: { starts_at: string; ends_at: string }): string {
...
function ExpandableText({ text, maxLength = 300 }: { text: string; maxLength?: number }) {
    ...
    if (!text || text.length <= maxLength) return <p className="text-sm whitespace-pre-wrap">{text}</p>;
    return (
        <div>
...
export default function ShiftNotesIndex({
    ...
    const hasActiveFilters =
        filters?.q || filters?.type || filters?.client_id || filters?.author_id || filters?.date_from || filters?.date_to || filters?.flagged || filters?.private;

    const donutSegments = Object.entries(chart_by_type ?? {}).map(([key, value]) => ({
    ...
    const exportUrl = '/operations/shift-notes/export?' + new URLSearchParams(cleanFilters).toString();

    return (
        <AppLayout>
            <Head title="Shift Notes" />
            <PageHeader title="Shift Notes" description="Manager dashboard for shift documentation and audit trail." backHref="/operations" />
            <PageShell>
                {/* Stats Row */}
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-5">
                    <OpsStatCard label="Total Notes" value={stats?.total ?? 0} icon={FileText} color="indigo" />
                    <OpsStatCard label="Today" value={stats?.today ?? 0} icon={Calendar} color="blue" />
                    <OpsStatCard label="This Week" value={stats?.this_week ?? 0} icon={BarChart3} color="emerald" />
                    <OpsStatCard label="Flagged" value={stats?.flagged ?? 0} icon={Flag} color="amber" />
                    <OpsStatCard label="Shifts w/o Notes" value={stats?.shifts_without_notes ?? 0} icon={AlertTriangle} color="red" />
                </div>
                ...
                <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-3">
                    <Card>
                        <CardHeader className="pb-2">
                            <CardTitle className="text-sm font-medium">Notes by Type</CardTitle>
                        </CardHeader>
                        <CardContent className="flex justify-center pb-4">
                            <DonutChart segments={donutSegments} centerLabel="Total" centerValue={stats?.total ?? 0} size={140} />
                        </CardContent>
                    </Card>
                    <Card className="lg:col-span-2">
                        <CardHeader className="pb-2">
                            <CardTitle className="text-sm font-medium">Daily Trend (7 days)</CardTitle>
                        </CardHeader>
                        <CardContent>
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2">
                    ...
                    <Select value={filters?.author_id ?? ANY} onValueChange={(v) => updateFilters('author_id', v === ANY ? null : v)}>
                    ...
                    <Button
                        ...
                        onClick={() => updateFilters('flagged', filters?.flagged ? null : '1')}
                    >
                        <Flag className="h-3.5 w-3.5" />
                        Flagged
                    </Button>
                    <Button asChild size="sm" variant="outline" className="h-9 gap-1 text-xs">
                        <a href={exportUrl}>
                    ...
                    {(notes?.data ?? []).length === 0 && (
                        <Card>
                            <CardContent className="flex flex-col items-center justify-center py-16">
                                ...
                                <h2 className="text-lg font-semibold text-muted-foreground">No Shift Notes Found</h2>
                                <p className="mt-1 text-sm text-muted-foreground/80">
                                    {hasActiveFilters ? 'Try adjusting your filters.' : 'Notes will appear here as support workers document their shifts.'}
                                </p>
                            </CardContent>
                        </Card>
                    )}
                        ...
                        return (
                            ...
                            >
                                <CardContent className="p-4">
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <Badge variant={(TYPE_BADGE_VARIANTS[note.type] as any) ?? 'outline'}>
                                                ...
                                                <div className="flex gap-3 text-xs text-muted-foreground">
                                                    {note.shift && <span>Shift: {formatShiftTime(note.shift)}</span>}
                                                    {note.client && (
                                                        <span>
                                                            {clientLabel}: {note.client.first_name} {note.client.last_name}
                                                        </span>
                                                    )}
                                                </div>
                                            </div>
                                    ...
                                    {/* Note body */}
                                    <div className="mt-3">
                                        <ExpandableText text={note.body} maxLength={300} />
                                    </div>
                                        ...
                                        <div className="flex gap-1">
                                            {!note.reviewed_at && (
                                                <Button
                                                    ...
                                                    onClick={() =>
                                                        router.patch(`/operations/shift-notes/${note.id}/review`, {}, { preserveScroll: true })
                                                    }
                                                >
                                                    Mark Reviewed
                                                </Button>
                                            ...
                                            <Button
                                                ...
                                                className="h-7 text-xs"
                                                onClick={() =>
                                                    router.patch(`/operations/shift-notes/${note.id}/flag`, {}, { preserveScroll: true })
                                                }
                                            >
                                            ...
                                            {note.shift && (
                                                <Button asChild size="sm" variant="ghost" className="h-7 text-xs">
                                                    <Link href={`/operations/shifts/${note.shift.id}`}>View Shift &rarr;</Link>
                                                </Button>
                                            )}
                                        </div>
                                    </div>
                ...
                {(notes?.last_page ?? 1) > 1 && (
                    <div className="mt-4 flex flex-col items-center gap-2">
                        ...
                        <p className="text-xs text-muted-foreground">
                            Showing page {notes?.current_page ?? 1} of {notes?.last_page ?? 1} ({notes?.total ?? 0} notes)
                        </p>
                    </div>
                )}
            </PageShell>
