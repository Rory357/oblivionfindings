...
const snapshot: ShiftMedSnapshot = {
    window: {
        start: '2026-06-16T07:00:00+12:00',
        end: '2026-06-16T15:00:00+12:00',
    },
    counts: {
        due: 3,
        given: 5,
        ...
        cd_due: 2,
        prn_given: 4,
        reviews_outstanding: 6,
        omissions: 7,
    },
    due: [
        { name: 'Quetiapine', time: '08:00', state: 'due', controlled: true },
    ],
    alerts: [
        {
            kind: 'stock',
            tone: 'warning',
            message: 'Quetiapine low stock — 2 on hand (reorder at 5).',
        },
    ],
...
describe('ShiftMedSummary', () => {
    it('prompts to pick a shift when none is linked', () => {
        render(
            <ShiftMedSummary
                ...
                loading={false}
                hasShift={false}
                noShiftHint="Pick the outgoing shift"
            />,
        );
    ...
    it('shows a loading state while the snapshot is fetching', () => {
        render(
            <ShiftMedSummary snapshot={null} loading={true} hasShift={true} />,
        );
        expect(
            screen.getByText(/Loading the shift's medication state/),
        ).toBeInTheDocument();
    });
    ...
    it('renders the stat tiles, optional note and alerts from the snapshot', () => {
        render(
            <ShiftMedSummary
                ...
                loading={false}
                hasShift={true}
                note="Due meds pre-filled below"
            />,
        );
        ...
        expect(
            screen.getByText('Due meds pre-filled below'),
        ).toBeInTheDocument();
