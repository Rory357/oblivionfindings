...
export default function EvvIndex({ records = { data: [], links: [], current_page: 1, last_page: 1, total: 0 }, filters = {} as any, stats = {} as any }: Props) {
    ...
    const donutSegments = [
        { label: 'Verified', value: stats?.verified ?? 0, color: OPS_COLORS.success },
        { label: 'Pending', value: stats?.pending ?? 0, color: OPS_COLORS.warning },
        { label: 'Flagged', value: stats?.flagged ?? 0, color: OPS_COLORS.danger },
    ];
        ...
        <AppLayout>
            ...
            <PageShell>
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-[1fr_1fr_1fr_auto]">
                    <OpsStatCard label="Total Records" value={stats?.total ?? 0} icon={Shield} color="indigo" />
                    <OpsStatCard label="Verified" value={stats?.verified ?? 0} icon={CheckCircle2} color="emerald" />
                    ...
                    {(records?.data ?? []).length === 0 && (
                        <Card>
                            <CardContent className="flex flex-col items-center justify-center py-16">
                                ...
                                <h2 className="text-lg font-semibold text-muted-foreground">No EVV Records</h2>
                                <p className="mt-1 text-sm text-muted-foreground/80">Records will appear here when visits are logged.</p>
                            </CardContent>
                        </Card>
                    )}
                            ...
                            <CardContent className="flex items-center gap-4 p-4">
                                ...
                                <div className="min-w-0 flex-1">
                                    <div className="flex items-center gap-2">
                                        <Link href={`/operations/evv/${rec.id}`} className="text-sm font-semibold hover:underline">
                                            Visit #{rec.id}
                                        </Link>
                                        <Badge variant={STATUS_VARIANTS[rec.status] ?? 'outline'} className="h-4 px-1.5 text-[9px] capitalize">
                                    ...
                                    <div className="mt-0.5 flex items-center gap-3 text-xs text-muted-foreground">
                                        {rec.worker && <span>{rec.worker.name}</span>}
                                        {rec.client && <span>{rec.client.first_name} {rec.client.last_name}</span>}
                                        <span>{formatDate(rec.shift_date)}</span>
                                ...
                                <div className="flex shrink-0 gap-1">
                                    <Button asChild size="sm" variant="ghost" className="h-7 w-7 p-0">
                                        <Link href={`/operations/evv/${rec.id}`}>
