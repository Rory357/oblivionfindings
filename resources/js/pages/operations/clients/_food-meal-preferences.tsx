...
import { AlertTriangle, Plus, ShieldAlert, Trash2, Utensils } from 'lucide-react';
...
export function FoodMealPreferences({ clientId, canEdit }: { clientId: number; canEdit: boolean }) {
    const [bundle, setBundle] = useState<Bundle | null>(null);
    const [showAdd, setShowAdd] = useState(false);
    const [busy, setBusy] = useState(false);

    const reload = () => {
        axios.get(`/clients/${clientId}/meal-preferences`).then((res) => setBundle(res.data));
    };
    ...
    if (!bundle) {
        return (
            <Card className="border-primary/30">
                <CardContent className="p-4 text-sm text-muted-foreground">Loading food &amp; meal preferences…</CardContent>
            </Card>
        );
    }
    ...
    function toggleTag(kind: 'allergens' | 'preferences', tagId: number) {
        if (!canEdit) return;
        const current = new Set([...bundle!.allergens.map((t) => t.id), ...bundle!.preferences.map((t) => t.id)]);
        if (current.has(tagId)) current.delete(tagId);
        else current.add(tagId);
        setBusy(true);
        router.put(`/clients/${clientId}/meal-preferences/tags`, { tag_ids: Array.from(current) }, {
            preserveScroll: true,
            onSuccess: () => reload(),
            ...
            <CardContent className="space-y-4 p-4">
                <div className="flex items-center justify-between">
                    <p className="flex items-center gap-2 text-[10px] font-bold tracking-wider text-primary uppercase">
                        <Utensils className="h-3.5 w-3.5" /> Food &amp; Meal Preferences
                    </p>
                    {canEdit && <Badge variant="outline" className="text-[10px]">Used for meal-planner warnings</Badge>}
                </div>
                ...
                >
                    {bundle.dislikes.length === 0 && (
                        <div className="text-xs text-muted-foreground">No dislikes recorded.</div>
                    )}
                    <div className="space-y-1">
                        {bundle.dislikes.map((d) => (
                            <DislikeRow key={d.id} clientId={clientId} dislike={d} canEdit={canEdit} onChanged={reload} />
                        ))}
                    </div>
                    {canEdit && (
                        showAdd ? (
...
function TagPicker({
    ...
    if (!canEdit) {
        if (selected.length === 0) return <div className="text-xs text-muted-foreground">{emptyLabel}</div>;
        return (
...
function DislikeRow({ clientId, dislike, canEdit, onChanged }: { clientId: number; dislike: Dislike; canEdit: boolean; onChanged: () => void }) {
    const name = dislike.product_name ?? dislike.free_text_name ?? 'Unnamed';
    function destroy() {
        router.delete(`/clients/${clientId}/meal-preferences/dislikes/${dislike.id}`, {
            preserveScroll: true,
            onSuccess: () => onChanged(),
        });
    }
    return (
        <div className="flex items-center justify-between gap-2 rounded-md border bg-muted/20 px-2 py-1 text-xs">
            <div className="min-w-0 flex-1">
                ...
                {dislike.notes && <span className="ml-2 text-muted-foreground">— {dislike.notes}</span>}
                {dislike.product_id && <Badge variant="outline" className="ml-1 text-[10px]">linked</Badge>}
            </div>
            {canEdit && (
                <ConfirmAction
...
function AddDislikeRow({
    ...
    function submit(e: React.FormEvent) {
        e.preventDefault();
        setBusy(true);
        router.post(`/clients/${clientId}/meal-preferences/dislikes`, {
    ...
    return (
        <form onSubmit={submit} className="grid grid-cols-12 items-end gap-2 rounded-md border bg-muted/30 p-2">
            <div className="col-span-5">
                ...
                {productId === 'free' && <Input className="mt-1" placeholder="e.g. mushrooms" value={name} onChange={(e) => setName(e.target.value)} required />}
            </div>
            <div className="col-span-5">
                <Label className="text-[10px]">Note (optional)</Label>
                <Input value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="e.g. ok in soup but not whole" />
            </div>
