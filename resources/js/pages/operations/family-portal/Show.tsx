...
export default function FamilyPortalShow({ client }: Props) {
    ...
    return (
        <AppLayout>
            <Head
                title={`Family Portal - ${client.first_name} ${client.last_name}`}
            />
            <PageHero
                variant="compact"
                            ...
                            {VISIBILITY_SETTINGS.map(
                                ({ key, label, icon: Icon }) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <Icon
                                                className={`h-4 w-4 ${enabled ? 'text-status-success' : 'text-muted-foreground/40'}`}
                                            />
                                            <span
                                                className={`text-sm ${enabled ? '' : 'text-muted-foreground/60'}`}
                                            >
                                                {label}
                                            </span>
                                            <Badge
                        ...
                        <CardContent className="space-y-2">
                            {NOTIFICATION_SETTINGS.map(({ key, label }) => {
                                ...
                                return (
                                    ...
                                    >
                                        <Bell
                                            className={`h-4 w-4 ${enabled ? 'text-status-info' : 'text-muted-foreground/40'}`}
                                        />
                                        <span
                                            className={`text-sm ${enabled ? '' : 'text-muted-foreground/60'}`}
                                        >
