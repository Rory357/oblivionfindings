...
import { CalendarDays, Clock, DollarSign, Download, Eye, FileSpreadsheet, Plus } from 'lucide-react';
        ...
        <AppLayout>
            ...
            <PageShell>
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                    <OpsStatCard label="Total Exports" value={stats?.total ?? 0} icon={FileSpreadsheet} color="indigo" />
                    <OpsStatCard label="Hours This Period" value={`${(stats?.hours_this_period ?? 0).toLocaleString('en-NZ')} hrs`} icon={Clock} color="blue" />
                    <OpsStatCard label="Amount This Period" value={nzd.format(stats?.amount_this_period ?? 0)} icon={DollarSign} color="emerald" />
                </div>
                    ...
                    {(payrollExports?.data ?? []).length === 0 && (
                        <Card>
                            <CardContent className="flex flex-col items-center justify-center py-16">
                                ...
                                <h2 className="text-lg font-semibold text-muted-foreground">No Payroll Exports</h2>
                                <p className="mt-1 text-sm text-muted-foreground/80">Generate your first payroll export to get started.</p>
                                <Button asChild size="sm" className="mt-4">
                                    <Link href="/operations/payroll-export/create">Generate Export</Link>
                                </Button>
                            </CardContent>
                        </Card>
                    )}
                            ...
                            <CardContent className="flex items-center gap-4 p-4">
                                ...
                                <div className="min-w-0 flex-1">
                                    <div className="flex items-center gap-2">
                                        <Link href={`/operations/payroll-export/${exp.id}`} className="text-sm font-semibold hover:underline">
                                            {exp.reference}
                                        </Link>
                                        <Badge variant={STATUS_VARIANTS[exp.status] ?? 'outline'} className="h-4 px-1.5 text-[9px] capitalize">
                                            {exp.status}
                                        </Badge>
                                    </div>
                                    <div className="mt-0.5 flex items-center gap-3 text-xs text-muted-foreground">
                                        ...
                                        {exp.exported_at && <span>Exported: {formatDate(exp.exported_at)}</span>}
                                    </div>
                                </div>
                                <div className="flex shrink-0 gap-1">
                                    ...
                                    <Button asChild size="sm" variant="ghost" className="h-7 w-7 p-0">
                                        <Link href={`/operations/payroll-export/${exp.id}`}>
