        ...
        <AppLayout>
            ...
            <PageShell>
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                    <OpsStatCard label="Total Budget" value={formatCurrency(s.total_budget)} icon={DollarSign} color="indigo" subtitle={`${s.active_agreements} agreements`} />
                    <OpsStatCard label="Utilised" value={formatCurrency(s.total_used)} icon={TrendingUp} color="blue" subtitle={`${s.utilisation_percent}% used`} />
                    <OpsStatCard label="Remaining" value={formatCurrency(s.total_remaining)} icon={DollarSign} color="emerald" subtitle="Available budget" />
                    <OpsStatCard label="Pending Claims" value={s.pending_claims} icon={FileText} color={s.pending_claims > 0 ? 'amber' : 'slate'} href="/operations/funding/claims" />
                </div>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="flex flex-col items-center gap-4">
                                <DonutChart
                                    segments={[
                                        { label: 'Used', value: s.total_used, color: OPS_COLORS.primary },
                                        { label: 'Remaining', value: s.total_remaining, color: '#e2e8f0' },
                                    ]}
                                ...
                                <div className="flex gap-6 text-xs text-muted-foreground">
                                    <span className="flex items-center gap-1.5">
                                        <div className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: OPS_COLORS.primary }} />
                    ...
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between pb-2">
                            ...
                            <Button asChild variant="ghost" size="sm" className="h-7 text-xs">
                                <Link href="/operations/funding/claims">View all <ArrowRight className="ml-1 h-3 w-3" /></Link>
                            </Button>
                        </CardHeader>
                        <CardContent>
                            <div className="flex items-center justify-center gap-6">
                                <DonutChart
                                    segments={Object.entries(claims_by_status ?? {}).map(([status, count]) => ({
                                        label: status, value: count, color: CLAIM_COLORS[status] ?? OPS_COLORS.muted,
                                    }))}
                                    centerValue={Object.values(claims_by_status ?? {}).reduce((a, b) => a + b, 0)}
                                    centerLabel="Claims"
                                    size={130}
                                    strokeWidth={16}
                                />
                                <div className="space-y-1.5">
                                    {Object.entries(claims_by_status ?? {}).map(([status, count]) => (
                                        <div key={status} className="flex items-center gap-2">
                                            <div className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: CLAIM_COLORS[status] ?? OPS_COLORS.muted }} />
                ...
                {(top_agreements ?? []).length > 0 && (
                    <Card className="mt-6">
                        <CardHeader className="pb-2">
                            <CardTitle className="text-sm font-medium">Top Agreements by Utilisation</CardTitle>
                        </CardHeader>
                        <CardContent>
                                    ...
                                    <div key={ag.id} className="flex items-center gap-3">
                                        <div className="min-w-0 flex-1">
                                            <div className="flex items-center gap-2">
                                                <Link href={`/operations/service-agreements/${ag.id}`} className="truncate text-xs font-medium hover:underline">
                                                    {ag.title}
                                                </Link>
                                                <span className="text-[10px] text-muted-foreground">{ag.client_name}</span>
                                            </div>
                                            <div className="mt-1 flex items-center gap-2">
                                                <div className="h-1.5 flex-1 rounded-full bg-muted">
                                                    <div className="h-1.5 rounded-full transition-all" style={{
                                                        width: `${Math.min(100, ag.utilisation_percent)}%`,
                                                        backgroundColor: ag.utilisation_percent > 90 ? OPS_COLORS.danger : ag.utilisation_percent > 70 ? OPS_COLORS.warning : OPS_COLORS.primary,
                                                    }} />
                                                </div>
                                                <span className="text-[10px] tabular-nums text-muted-foreground">{ag.utilisation_percent}%</span>
                                            </div>
                                        </div>
                                        <span className="shrink-0 text-xs font-medium tabular-nums">
                                            {formatCurrency(ag.budget_used)} / {formatCurrency(ag.total_budget)}
                                        </span>
                                    </div>
                                ))}
                            </div>
