...
export default function CarePlanShow({
    ...
    const getGoalNotes = (goalId: number) => {
        return (progressNotes ?? []).filter(
            (n: any) => n.care_plan_goal_id === goalId,
        );
    };
    ...
    const saveGoalEdit = () => {
        ...
        router.put(
            `/operations/care-plans/${plan.id}/goals/${editingGoal.id}`,
            {
                title: editTitle,
    ...
    const updateGoalProgress = (
        ...
        router.patch(
            `/operations/care-plans/${plan.id}/goals/${goalId}/progress`,
            {
    ...
    const handleCompleteReview = () => {
        router.post(
            `/operations/care-plans/${plan.id}/complete-review`,
            {},
            { preserveScroll: true },
        );
    };
    ...
    const filteredGoals = goals.filter((g) => {
        if (goalStatusFilter !== ANY && g.status !== goalStatusFilter)
            return false;
        if (goalPriorityFilter !== ANY && g.priority !== goalPriorityFilter)
            return false;
        if (goalCategoryFilter !== ANY && g.category !== goalCategoryFilter)
            return false;
        return true;
    });
    ...
    const filteredNotes = (progressNotes ?? []).filter((n) => {
        if (
            noteVisibilityFilter !== ANY &&
            n.visibility !== noteVisibilityFilter
        )
            return false;
        return true;
    });
        ...
        <div className="space-y-6">
                ...
                <Card>
                    ...
                    <CardContent>
                        <p className="text-sm font-semibold">
                            {plan.client?.first_name ?? ''}{' '}
                            {plan.client?.last_name ?? ''}
                        </p>
                        {plan.client?.date_of_birth && (
                            <p className="mt-1 text-xs text-muted-foreground">
                                DOB: {formatDate(plan.client.date_of_birth)}
                            </p>
                        )}
                        {plan.creator && (
                            <p className="mt-1 text-xs text-muted-foreground">
                    ...
                    <CardContent className="flex justify-center">
                        <DonutChart
                            segments={[
                                ...
                                {
                                    label: 'In Progress',
                                    value: goalStatusCounts.in_progress,
                                    color: OPS_COLORS.primary,
                                },
                                {
                                    label: 'Not Started',
                                    value: goalStatusCounts.not_started,
                                    color: OPS_COLORS.muted,
                                },
                                {
                                    label: 'Discontinued',
                                    value: goalStatusCounts.discontinued,
                    ...
                    <CardContent className="space-y-3">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <div className="text-right">
                                ...
                                {plan.next_review_at && (
                                    <p
                                        className={`text-[10px] ${new Date(plan.next_review_at) <= new Date() ? 'font-medium text-status-warning' : 'text-muted-foreground'}`}
                                    >
            ...
            <div className="flex flex-wrap gap-2">
                <Badge variant="outline" className="gap-1.5 px-3 py-1">
                    ...
                    {safeProgressStats.total_goals} Total Goals
                </Badge>
                <Badge
                    variant="outline"
                    className="gap-1.5 border-status-success/30 bg-status-success-bg px-3 py-1 text-status-success dark:border-status-success/30 dark:bg-status-success-bg dark:text-status-success"
                >
                    <CheckCircle2 className="h-3 w-3" />{' '}
                    {safeProgressStats.completed} Completed
                </Badge>
                <Badge
                    variant="outline"
                    className="gap-1.5 border-primary bg-primary/10 px-3 py-1 text-primary dark:border-primary/30 dark:bg-primary/30 dark:text-primary/70"
                >
                    <Play className="h-3 w-3" /> {safeProgressStats.in_progress}{' '}
                    In Progress
                </Badge>
                <Badge variant="outline" className="gap-1.5 px-3 py-1">
                    <Circle className="h-3 w-3" />{' '}
                    {safeProgressStats.total_goals -
                        safeProgressStats.completed -
                        safeProgressStats.in_progress}{' '}
                    Not Started
                </Badge>
            </div>
                ...
                {content.review_schedule && (
                    <Card>
                        ...
                        <CardContent>
                            <p className="text-sm">
                                Frequency: Every{' '}
                                {content.review_schedule.frequency_months} month
                                {content.review_schedule.frequency_months !== 1
                                    ? 's'
                                    : ''}
                            </p>
                        </CardContent>
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
                ...
                <Select
                    ...
                    onValueChange={setGoalPriorityFilter}
                >
                ...
                <Select
                    ...
                    onValueChange={setGoalCategoryFilter}
                >
                    <SelectTrigger className="h-8 w-[130px] text-xs">
            ...
            {/* Add Goal Form */}
            {showGoalForm && (
                <Card className="border-dashed border-primary bg-primary/10 dark:border-primary/30 dark:bg-primary/20">
                                ...
                                <div className="min-w-0 flex-1">
                                    <div className="flex items-center gap-2">
                                        ...
                                        {goal.status !== 'completed' && (
                                            <Button
                                                variant="outline"
                                                size="sm"
                                                className="h-6 gap-1 border-primary px-2 text-[10px] text-primary hover:bg-primary/10"
                                                onClick={() =>
                                                    openGoalEditor(goal)
                                                }
                                                title="Edit goal"
                                        ...
                                        <span
                                            className={`text-[10px] font-medium capitalize ${PRIORITY_COLORS[goal.priority] ?? ''}`}
                                        >
                                            {goal.priority}
                                        </span>
                                    </div>
                                    ...
                                    {goal.outcome_notes && (
                                        <div className="mt-1.5 rounded-md border border-status-warning/30 bg-status-warning-bg px-2.5 py-1.5">
                                            <p className="text-[10px] font-medium tracking-wide text-status-warning uppercase">
                                                Notes
                                            </p>
                                    ...
                                    <div className="mt-2.5 flex items-center gap-2">
                                        ...
                                        {goal.status === 'not_started' && (
                                            <Button
                                                size="sm"
                                                variant="outline"
                                                className="h-7 gap-1 border-primary px-3 text-xs text-primary hover:bg-primary/10"
                                                onClick={() =>
                                                    updateGoalProgress(
                                                        goal.id,
                                                        10,
                                        ...
                                        {getGoalNotes(goal.id).length > 0 && (
                                            <Button
                                                ...
                                                onClick={() =>
                                                    toggleGoalNotes(goal.id)
                                                }
                                            >
                                                <Clock className="h-3 w-3" />
                                                {expandedGoalNotes[goal.id]
                                                    ? 'Hide'
                                                    : 'Show'}{' '}
                                                Notes (
                                                {getGoalNotes(goal.id).length})
                                            </Button>
                                        )}
                                        {goal.target_date && (
                                    ...
                                    {noteGoalId === goal.id && (
                                        <div className="mt-2 rounded-lg border border-primary bg-primary/10 p-3">
                                            <Textarea
            ...
            >
                <DialogContent className="sm:max-w-lg">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Update progress, add notes, and modify goal details.
                        </DialogDescription>
                    </DialogHeader>
                    {editingGoal && (
                        <div className="space-y-5 py-2">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <Textarea
                                    ...
                                    placeholder="Add notes about progress, observations, or outcomes..."
                                />
                            </div>
                        </div>
                    )}
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
                <Select
                    ...
                    onValueChange={setNoteVisibilityFilter}
                >
                        ...
                        <form onSubmit={handleAddNote} className="space-y-3">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <Select
                                        value={noteForm.data.mood_rating}
                                        onValueChange={(v) =>
                                            noteForm.setData('mood_rating', v)
                                        }
                                    >
                                        <SelectTrigger className="h-8 text-xs">
                        ...
                        <CardContent className="flex items-center gap-4 p-4">
                            ...
                            <div className="min-w-0 flex-1">
                                <div className="flex items-center gap-2">
                                    ...
                                    <Badge
                                        variant={
                                            STATUS_VARIANTS[version.status] ??
                                            'outline'
                                        }
                                        className="h-4 px-1.5 text-[9px] capitalize"
                                    >
                                ...
                                <div className="mt-0.5 flex items-center gap-3 text-xs text-muted-foreground">
                                    {version.reviewer && (
                                        <span>
                                            Reviewed by {version.reviewer.name}
                                        </span>
                                    )}
                                    {version.reviewed_at && (
    ...
    return (
        <AppLayout>
            ...
            <PageShell>
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <Badge
                        variant={STATUS_VARIANTS[plan.status] ?? 'outline'}
                        className="capitalize"
                    >
                        {plan.status ?? 'unknown'}
                    </Badge>
                    ...
                    {plan.next_review_at && (
                        <span
                            className={`text-xs ${new Date(plan.next_review_at) <= new Date() ? 'font-medium text-status-warning' : 'text-muted-foreground'}`}
                        >
