...
export function AddMedicationDialog({
    ...
    const allergyClash = (allergies ?? []).find(
        (a) =>
            ...
            a.allergen
                ?.toLowerCase()
                .includes(
                    form.data.medication_name.toLowerCase().split(' ')[0] ??
                        '__',
                ),
    );
            ...
            onSuccess: () => {
                toast.success(
                    form.data.controlled_drug || form.data.high_risk
                        ? 'Medication added — routed for prescriber verification.'
                        : 'Medication added.',
                );
                onClose();
            },
    ...
    const step0Valid =
        (presetClient || form.data.client_id) &&
        form.data.medication_name &&
        form.data.dose;
            ...
            {step === 0 && (
                <>
                    <StepHead
                        ...
                        blurb="New orders default to awaiting prescriber verification."
                    />
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <Field label="Dose unit">
                            <Input
                                ...
                                onChange={(e) =>
                                    form.setData('dose_unit', e.target.value)
                                }
                        ...
                        <Field label="Indication">
                            <Input
                                ...
                                placeholder="Reason for med"
                            />
                        </Field>
                        <Field
                            label="Instructions"
                            span
                            hint="Shown to workers at the point of administration."
                        >
                            <Input
            ...
            {step === 1 && (
                <>
                    <StepHead
                        ...
                        blurb="Set the frequency; tick PRN for as-needed meds."
                    />
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <Field
                            label="Frequency"
                    ...
                    <div className="mt-3">
                        {doseTimes ? (
                            <InfoCard icon={CheckCircle2}>
                                ...
                                <strong>{doseTimes.join(' · ')}</strong>
                            </InfoCard>
                        ) : (
                            <InfoCard icon={ClipboardList}>
                                No fixed schedule — recorded as required.
                            </InfoCard>
                        )}
                    </div>
                    <div className="mt-4">
                        <ToggleRow
                            ...
                            label="As-needed (PRN)"
                            hint="Recorded on demand with a 24h limit."
                            onChange={(v) => form.setData('is_prn', v)}
                        />
                    </div>
                    {form.data.is_prn && (
                ...
                <>
                    ...
                    <div className="mt-4 grid grid-cols-1 gap-2.5">
                        <ToggleRow
                            ...
                            hint="Requires a witness countersignature at administration."
                            onChange={(v) => form.setData('controlled_drug', v)}
                        />
                        <ToggleRow
                            checked={form.data.high_risk}
                            label="High-risk medication"
                            onChange={(v) => form.setData('high_risk', v)}
                        />
                        <ToggleRow
                            checked={form.data.witness_required}
                            label="Witness required"
                            onChange={(v) =>
                                form.setData('witness_required', v)
                            }
                        />
                    </div>
                    <div className="mt-3">
                    ...
                    {(form.data.controlled_drug || form.data.high_risk) && (
                        <div className="mt-3">
                            <InfoCard icon={ShieldCheck} tone="warn">
                                This order will be added{' '}
                                <strong>awaiting verification</strong> and
                                routed to a prescriber-verifier.
                            </InfoCard>
                        </div>
                    )}
                </>
            ...
            {step === 3 && (
                <>
                    <StepHead
                        icon={CheckCircle2}
                        title="Review"
                        blurb="Confirm the new medication order."
                    />
                    <div className="rounded-lg border px-4">
                        ...
                        <SummaryRow
                            label="Dose"
                            value={[form.data.dose, form.data.dose_unit]
                                .filter(Boolean)
                                .join(' ')}
                        />
                        <SummaryRow
                        ...
                        <SummaryRow
                            ...
                            value={
                                [
                                    form.data.controlled_drug && 'CD',
                                    form.data.high_risk && 'High-risk',
                                    form.data.witness_required && 'Witness',
                                ]
                                    .filter(Boolean)
                                    .join(' · ') || 'None'
                            }
    ...
    return (
        ...
        >
            <StepHead
                ...
                blurb="Update the order details and safety flags."
            />
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <Field
                    label="Medication name"
                ...
                <Field label="Form">
                    <Input
                        value={form.data.form}
                        onChange={(e) => form.setData('form', e.target.value)}
                    />
                </Field>
                <Field label="Route">
                    <Input
                        value={form.data.route}
                        onChange={(e) => form.setData('route', e.target.value)}
                    />
                </Field>
            ...
            <div className="mt-4 grid grid-cols-1 gap-2.5">
                ...
                <ToggleRow
                    checked={form.data.witness_required}
                    label="Witness required"
                    onChange={(v) => form.setData('witness_required', v)}
                />
            </div>
        </MedsWizardDialog>
    );
    ...
    return (
        ...
        >
            <StepHead
                icon={Ban}
                title="Reject order"
                blurb="Document why this order cannot be verified."
            />
            <Field
