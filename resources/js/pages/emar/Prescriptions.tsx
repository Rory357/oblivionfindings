...
function ctxTag(o: PrescriptionOrder): {
    tag: string;
    tagBg: string;
    tagColor: string;
} {
    if (isAwaitingCountersign(o)) {
        ...
        return {
            tag: hrs !== null ? `Sign ${hrs}h` : 'Awaiting',
...
export default function Prescriptions(props: Props) {
    ...
    const onSite = (id: number | null) => {
        setSiteFilter(id);
        router.get('/emar/prescriptions', id ? { site_id: id } : {}, {
            preserveState: true,
            preserveScroll: true,
        });
    };
    ...
    const clientItems = useMemo(
        () =>
            clients.map((c) => ({
                ...
                description: c.site_name ?? undefined,
            })),
        [clients],
    );
    ...
    const matchesFooter = useMemo(() => {
        ...
        return (o: GovernedPrescriptionOrder) => {
            ...
            if (
                q &&
                !`${o.medication_name ?? ''} ${o.client_name} ${o.prescriber_name ?? ''}`
                    .toLowerCase()
                    .includes(q)
            )
                return false;
    ...
    const dispensed = useMemo(
        () =>
            orders.filter((o) => o.status === 'dispensed' && matchesFooter(o)),
        [orders, matchesFooter],
    );
    const activity = useMemo(
        () => orders.filter(matchesFooter).slice(0, 40),
        [orders, matchesFooter],
    );
    const covertFiltered = useMemo(() => {
        const q = search.toLowerCase().trim();
        return covert.filter(
            (c) =>
                (!clientFilter || c.client_id === clientFilter) &&
                (!q ||
                    `${c.medication_name ?? ''} ${c.client_name}`
                        .toLowerCase()
                        .includes(q)),
        );
    ...
    const TABS: RosterTabItem[] = [
        ...
        },
        {
            ...
            icon: Package,
            tone: 'success',
            badge: counts.toDispense || undefined,
        },
        {
            id: 'covert',
            label: 'Covert',
    ...
    const openRowCtx = (e: ReactMouseEvent, o: GovernedPrescriptionOrder) => {
        ...
        const items: ShiftCtxItem[] = [
            {
                ...
                label: 'View details',
                sub: `${o.medication_name ?? 'Order'}${o.order_date ? ` · ${o.order_date}` : ''}`,
                tone: 'primary',
                ...
                ? [
                      {
                          ...
                          label: 'Countersign',
                          sub: 'Prescriber sign-off',
                          onClick: () =>
                              setModal({ type: 'countersign', order: o }),
                ...
                ? [
                      {
                          ...
                          onClick: () =>
                              setModal({ type: 'dispense', order: o }),
                      } satisfies ShiftCtxItem,
                  ]
                : []),
            ...(canLinkOrder(o)
                ? [
                      {
                          ...
                          label: 'Link to MAR',
                          onClick: () => setModal({ type: 'link', order: o }),
                      } satisfies ShiftCtxItem,
                  ]
                : []),
            { sep: true },
            {
                ...
                label: 'View client',
                onClick: () =>
                    router.visit(`/operations/clients/${o.client_id}?tab=mar`),
            },
            {
                icon: <FileText className="h-3.5 w-3.5" />,
        ...
        setCtx({
            ...
            tagBg: t.tagBg,
            tagColor: t.tagColor,
            meta: `${o.client_name} · ${o.medication_name ?? '—'}${o.prescriber_name ? ` · ${o.prescriber_name}` : ''}`,
            items,
        });
    };
    ...
    const openCovertCtx = (e: ReactMouseEvent, c: GovernedCovertAuth) => {
        ...
        const items: ShiftCtxItem[] = [
            {
                ...
                label: 'View client',
                onClick: () =>
                    router.visit(`/operations/clients/${c.client_id}?tab=mar`),
            },
            {
                icon: <FileText className="h-3.5 w-3.5" />,
                label: 'Open on MAR chart',
        ...
        setCtx({
            x: e.clientX,
            y: e.clientY,
            tag: c.review_overdue ? 'Review overdue' : 'Active',
            tagBg: 'var(--status-critical-bg)',
            tagColor: 'var(--status-critical)',
            meta: `${c.client_name} · ${c.medication_name ?? '—'}`,
            items,
        });
    };
    ...
    if (counts.overdue > 0)
        alerts.push({
            ...
            tone: 'critical',
            tab: 'countersign',
            text: `${counts.overdue} verbal/telephone order${counts.overdue === 1 ? ' is' : 's are'} overdue for prescriber countersignature.`,
        });
    if (awaitingNotOverdue > 0)
        alerts.push({
            key: 'awaiting',
            tone: 'warning',
            tab: 'countersign',
            text: `${awaitingNotOverdue} order${awaitingNotOverdue === 1 ? '' : 's'} awaiting prescriber countersignature.`,
        });
    if (counts.toDispense > 0)
        alerts.push({
            key: 'dispense',
            tone: 'info',
            tab: 'dispensing',
            text: `${counts.toDispense} confirmed order${counts.toDispense === 1 ? '' : 's'} ready to dispense.`,
        });
    if (counts.covert > 0)
        alerts.push({
            key: 'covert',
            tone: 'warning',
            tab: 'covert',
            text: `${counts.covert} covert administration authorisation${counts.covert === 1 ? '' : 's'} active.`,
        });
        ...
        >
            ...
            <div className="flex flex-col gap-6 p-6">
                ...
                {visibleAlerts.length > 0 && (
                    <div className="flex flex-col gap-2">
                        {visibleAlerts.map((a) => (
                            <div
                                key={a.key}
                                className={`flex items-center justify-between gap-3 rounded-xl border px-4 py-3 ${ALERT_TONE[a.tone]}`}
                            >
                                ...
                                <span className="flex items-center gap-1.5">
                                    ...
                                    <button
                                        ...
                                        onClick={() =>
                                            setDismissed((prev) =>
                                                new Set(prev).add(a.key),
                                            )
                                        }
                                        className="grid h-7 w-7 place-items-center rounded-md text-current/70 hover:bg-foreground/10"
                                    >
                                        <X className="h-3.5 w-3.5" />
                                    </button>
                                </span>
                    ...
                    <div className="overflow-hidden rounded-2xl border bg-card shadow-sm">
                        <div className="flex flex-wrap items-center gap-2.5 border-b p-3.5">
                            ...
                            <span className="ml-auto text-xs text-muted-foreground">
                                {ordersFiltered.length} of {orders.length}
                            </span>
                        </div>
                                        ...
                                        ordersFiltered.map((o) => {
                                            ...
                                            return (
                                                <tr
                                                    ...
                                                    className="cursor-pointer border-b last:border-b-0 hover:bg-muted/40 focus:bg-muted/40 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-inset"
                                                    onClick={() =>
                                                        openDetail(o)
                                                    }
                                                    onContextMenu={(e) =>
                ...
                {activeTab === 'countersign' && (
                    <div className="flex flex-col gap-4">
                        <div className="flex items-center gap-2 rounded-xl border border-status-warning/30 bg-status-warning-bg/50 px-4 py-2.5 text-sm text-status-warning">
                            ...
                            Verbal &amp; telephone orders must be countersigned
                            by the prescriber within 24 hours.
                        </div>
                        {awaitingOrders.length === 0 ? (
                            <EmptyState
                                icon={PenTool}
                                message="Nothing awaiting countersignature."
                            />
                        ) : (
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="text-xs text-muted-foreground">
                                                ...
                                                · Taken {o.order_date} by{' '}
                                                {o.received_by_name ?? '—'}
                                                {o.read_back_confirmed
                                                    ? ' · read-back ✓'
                                                    : ''}
                                            </div>
                                            <div>
                                                <div
                                                    className={`mb-1 text-[11px] font-medium ${overdue ? 'text-status-critical' : 'text-status-warning'}`}
                                                >
                                                    {overdue
                                                ...
                                                <div className="h-1.5 overflow-hidden rounded-full bg-muted">
                                                    <div
                                                        className={`h-full rounded-full ${overdue ? 'bg-status-critical' : 'bg-status-warning'}`}
                                                        style={{
                                                            width: `${overdue ? 100 : Math.max(5, 100 - ((hrs ?? 0) / 24) * 100)}%`,
                                                        }}
                                                    />
                                                </div>
                                            </div>
                ...
                {activeTab === 'dispensing' && (
                    <div className="flex flex-col gap-4">
                        <div className="overflow-hidden rounded-2xl border bg-card shadow-sm">
                            <div className="border-b px-5 py-3.5 text-[15px] font-bold">
                                Awaiting dispense
                            </div>
                            {toDispense.length === 0 ? (
                                <div className="px-5 py-8 text-center text-sm text-muted-foreground">
                                    No confirmed orders awaiting dispense.
                                </div>
                            ) : (
                                <ul className="divide-y">
                                    {toDispense.map((o) => (
                                        <li
                                            ...
                                            className="flex cursor-pointer items-center justify-between px-5 py-3 hover:bg-muted/40 focus:bg-muted/40 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-inset"
                                            onClick={() => openDetail(o)}
                                            onContextMenu={(e) =>
                                                openRowCtx(e, o)
                                            }
                ...
                {activeTab === 'covert' && (
                    <div className="flex flex-col gap-4">
                        <div className="flex items-start gap-2 rounded-xl border border-status-critical/30 bg-status-critical-bg/50 px-4 py-3 text-sm text-status-critical">
                            ...
                            <span>
                                Covert administration is a restrictive practice
                                requiring capacity assessment, a best-interest
                                MDT decision, pharmacist advice, and regular
                                review. Medication must always be offered
                                overtly first.
                            </span>
                        </div>
                        {covertFiltered.length === 0 ? (
                            <EmptyState
                                icon={ShieldCheck}
                            ...
                            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                                {covertFiltered.map((c) => (
                                    ...
                                    >
                                        ...
                                        <div className="text-xs text-muted-foreground">
                                            Authorised by {c.authorised_by_name}{' '}
                                            · {c.authorised_date}
                                        </div>
                                        <div
                                            className={`text-xs ${c.review_overdue ? 'font-medium text-status-critical' : 'text-muted-foreground'}`}
                                        >
                    ...
                    <div className="overflow-hidden rounded-2xl border bg-card shadow-sm">
                        ...
                        {activity.length === 0 ? (
                            <div className="px-5 py-10 text-center text-sm text-muted-foreground">
                                No order activity matches these filters.
                            </div>
                        ) : (
                            <ul className="divide-y">
                                {activity.map((o) => (
                                    <li
                                        ...
                                        className="flex cursor-pointer items-center justify-between px-5 py-3 text-sm hover:bg-muted/40 focus:bg-muted/40 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-inset"
                                        onClick={() => openDetail(o)}
                                    ...
                                    >
                                        <span className="flex items-center gap-2">
                                            <span
                                                className={`flex h-7 w-7 items-center justify-center rounded-full ${orderStatusTone(o.status)}`}
                                            >
            ...
            {modal?.type === 'detail' && (
                <OrderDetailDialog
                    ...
                    onDispense={() =>
                        setModal({ type: 'dispense', order: modal.order })
                    }
                    onLink={() =>
                        setModal({ type: 'link', order: modal.order })
                    }
                />
            )}
