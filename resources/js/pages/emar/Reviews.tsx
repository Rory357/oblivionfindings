...
type Kpis = {
    ...
    gp_acceptance: number | null;
    in_monitoring: number;
    awaiting_gp: number;
};
...
const quarterOf = (iso: string | null) =>
    iso ? Math.floor(new Date(iso).getMonth() / 3) : -1;
const typeLabel = (t: string | null) =>
    REVIEW_TYPES.find((x) => x.value === t)?.label ?? t ?? '—';

function dueBadge(r: ReviewRow): { label: string; cls: string } | null {
    if (r.status !== 'scheduled' || !r.scheduled_date) return null;
    const days = Math.round(
        (new Date(r.scheduled_date).getTime() - Date.now()) / 86400000,
    );
    if (days < 0)
        return {
...
export default function Reviews({
    ...
    const visible = useMemo(() => {
        ...
        return reviews.filter((r) => {
            ...
            if (
                q &&
                !`${r.client_name} ${r.reviewer_name ?? ''} ${(r.actions ?? []).map((a) => a.drug).join(' ')}`
                    .toLowerCase()
                    .includes(q)
            )
                return false;
    ...
    const onSite = (id: number | null) => {
        setSiteFilter(id);
        router.get('/emar/reviews', id ? { site_id: id } : {}, {
            preserveState: true,
            preserveScroll: true,
        });
    };
    const advanceRec = (p: Pipeline) =>
        router.post(
            `/emar/reviews/${p.review_id}/actions/advance`,
            { index: p.index },
            {
                preserveScroll: true,
                only: ['reviews', 'deprescribing', 'kpis'],
            },
        );
    ...
    const TABS: RosterTabItem[] = [
        ...
        },
        {
            ...
            icon: Calendar,
            tone: 'info',
            badge: scheduledList.length || undefined,
        },
        {
            id: 'completed',
            label: 'Completed',
            icon: CheckCircle,
            tone: 'success',
            badge: completedList.length || undefined,
        },
        {
            id: 'deprescribing',
            label: 'Deprescribing',
            icon: Pill,
            tone: 'info',
            badge: deprescribing.length || undefined,
        },
        {
            id: 'all',
            label: 'All',
    ...
    const heroStats: PageHeroStat[] = [
        ...
        },
        {
            label: 'Due 30d',
            value: kpis.due_30,
            tone: kpis.due_30 > 0 ? 'warning' : 'neutral',
        },
        { label: 'Completed (Q)', value: kpis.completed_quarter },
        {
            label: 'GP accept %',
            value: kpis.gp_acceptance === null ? '—' : `${kpis.gp_acceptance}%`,
        },
    ];
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col gap-6 p-6">
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                                <div className="flex items-center gap-2 rounded-full bg-primary-foreground px-3 py-1.5">
                                    ...
                                    <input
                                        ...
                                        placeholder="Search resident, reviewer or drug…"
                    ...
                    <div className="flex flex-col gap-4">
                        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                            ...
                            <KpiCard
                                ...
                                value={kpis.completed_quarter}
                                tone="success"
                            />
                            <KpiCard
                                icon={Pill}
                                label="Live deprescribing actions"
                                value={
                                    deprescribing.filter(
                                        (d) => d.stage !== 'done',
                                    ).length
                                }
                                tone="info"
                        ...
                        <div className="grid gap-4 lg:grid-cols-[1.5fr_1fr]">
                            <div className="overflow-hidden rounded-2xl border border-status-critical/30 bg-card shadow-sm">
                                <div className="border-b bg-status-critical-bg/40 px-4 py-3 text-sm font-semibold text-status-critical">
                                    Overdue &amp; due now
                                </div>
                                {dueList.length === 0 ? (
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        {due && (
                                                            <span
                                                                className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${due.cls}`}
                                                            >
                                                                {due.label}
                                                            </span>
                                                        )}
                                ...
                                <button
                                    onClick={() =>
                                        setActiveTab('deprescribing')
                                    }
                                    className="flex w-full items-center justify-between border-t px-4 py-3 text-left text-xs text-muted-foreground hover:bg-muted/40"
                                >
                                    <span className="font-medium text-foreground">
                                        Deprescribing pipeline
                                    </span>
                                    <span>
                                        Awaiting GP {kpis.awaiting_gp} ·
                                        Monitoring {kpis.in_monitoring}{' '}
                ...
                {['due', 'scheduled', 'completed', 'all'].includes(
                    activeTab,
                ) && (
                    <ReviewTable
                        rows={
                ...
                {activeTab === 'deprescribing' && (
                    <div className="flex flex-col gap-4">
                        <div className="rounded-xl border border-status-info/30 bg-status-info-bg/50 px-4 py-3 text-sm text-status-info">
                            Deprescribing pipeline — pharmacist recommends → GP
                            accepts → implemented → monitored (HQSC frailty
                            guides). Surfaces the recommendations recorded on
                            completed reviews.
                        </div>
                        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                            {STAGES.map((col) => {
                                const cards = deprescribing.filter(
                                    (d) => d.stage === col.id,
                                );
                                return (
                                    <div
                                        key={col.id}
                                        className="flex flex-col gap-3 rounded-2xl border bg-muted/30 p-3"
                                    >
                                        <div
                                            className={`flex items-center justify-between text-sm font-semibold ${col.tone}`}
                                        >
                                        ...
                                        {cards.map((p) => (
                                            ...
                                            >
                                                <div className="flex items-center justify-between gap-2">
                                                    <span
                                                        className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${actionTone(p.action)}`}
                                                    >
                                                        {p.action}
                                                    </span>
...
function KpiCard({
    ...
    return (
        <div className="flex items-center gap-3 rounded-2xl border bg-card p-4 shadow-sm">
            <span
                className={`flex h-10 w-10 items-center justify-center rounded-xl ${cls}`}
            >
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-3">
                                            ...
                                            {due && (
                                                <span
                                                    className={`ml-2 rounded-full px-2 py-0.5 text-[10px] font-semibold ${due.cls}`}
                                                >
                                                    {due.label}
                                                </span>
                                            )}
                                        ...
                                        <td className="px-4 py-3">
                                            <span
                                                className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${sp.cls}`}
                                            >
                                                {sp.label}
                                            </span>
                                        </td>
                                        <td className="px-4 py-3 text-muted-foreground">
                                            {fmtDate(r.next_review_date)}
                                        </td>
                                        <td className="px-4 py-3">
