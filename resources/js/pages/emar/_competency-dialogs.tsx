...
export const COMPETENCY_AREAS = [
    ...
    {
        key: 'covert_admin_knowledge',
        label: 'Covert administration',
        core: false,
    },
    { key: 'error_reporting', label: 'Error reporting', core: true },
    { key: 'allergy_awareness', label: 'Allergy awareness', core: true },
];
export const CORE_KEYS = COMPETENCY_AREAS.filter((a) => a.core).map(
    (a) => a.key,
);
const MED_TYPES = [
    'oral',
...
export function AssessmentWizardDialog({
    ...
    const seedAreas = (): Record<string, TriState> => {
        ...
        COMPETENCY_AREAS.forEach((ar) => {
            if (assessment)
                o[ar.key] = assessment.not_seen_areas.includes(ar.key)
                    ? 'not_seen'
                    : areaVal(assessment, ar.key)
                      ? 'yes'
                      : 'no';
    ...
    const [assessDate, setAssessDate] = useState(
        mode === 'edit' && assessment?.assessment_date
    ...
    const [rounds, setRounds] = useState<ObservedRound[]>(
        mode === 'edit' && assessment ? assessment.observed_rounds : [],
    );
    const [canUnsup, setCanUnsup] = useState(
        assessment?.can_administer_unsupervised ?? false,
    );
    const [canWitness, setCanWitness] = useState(
        assessment?.can_witness_controlled ?? false,
    );
    const [restricted, setRestricted] = useState(
        mode === 'edit' ? (assessment?.restricted ?? false) : false,
    );
    const [restrictionNotes, setRestrictionNotes] = useState(
        mode === 'edit' ? (assessment?.restriction_notes ?? '') : '',
    );
    const [strengths, setStrengths] = useState(
        mode === 'edit' ? (assessment?.strengths ?? '') : '',
    );
    const [improvements, setImprovements] = useState(
        mode === 'edit' ? (assessment?.areas_for_improvement ?? '') : '',
    );
    const [actionPlan, setActionPlan] = useState(
        mode === 'edit' ? (assessment?.action_plan ?? '') : '',
    );
    const [assessorDeclared, setAssessorDeclared] = useState(false);
    const [staffDeclared, setStaffDeclared] = useState(false);

    const yesCount = COMPETENCY_AREAS.filter(
        (a) => areas[a.key] === 'yes',
    ).length;
    ...
    const submit = () => {
        ...
        const payload: Record<string, unknown> = {
            ...
            expiry_date: expiry,
            not_seen_areas: COMPETENCY_AREAS.filter(
                (a) => areas[a.key] === 'not_seen',
            ).map((a) => a.key),
            ...
            assessor_comments: incident
                ? `Triggering incident: ${incident}`
                : mode === 'edit'
                  ? assessment?.assessor_comments
                  : null,
        ...
        if (mode === 'edit' && assessment)
            router.put(`/emar/competency/${assessment.id}`, data, opts);
    ...
    return (
        <MedsWizardDialog
            ...
            railTitle={titleMode}
            railSubtitle={staffName ?? 'Medication competency'}
            steps={[
                {
                    key: 'ctx',
                    label: 'Staff & context',
        ...
        >
            {step === 0 && (
                <>
                    <StepHead
                        icon={User}
                        title="Staff & context"
                        blurb="Who is being assessed and why."
                    />
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        {type === 'remedial' && (
                            <Field
                                label="Linked medication error / incident"
                                span
                            >
                                <Input
                                    value={incident}
                                    ...
                                    placeholder="Reference the triggering error / incident"
                                />
                            </Field>
                        )}
                    </div>
            ...
            {step === 1 && (
                <>
                    <StepHead
                        icon={ShieldCheck}
                        title="Competencies"
                        blurb="Mark each area Yes / No / Not seen. A No on a CORE area must be resolved before unsupervised practice."
                    />
                        ...
                        {COMPETENCY_AREAS.map((a) => (
                            ...
                            >
                                <span className="text-sm font-medium">
                                    ...
                                    {areas[a.key] === 'no' && a.core && (
                                        <span className="ml-2 text-[10px] font-semibold text-status-critical">
                                            resolve
                                        </span>
                                    )}
                                </span>
                ...
                <>
                    ...
                    <div className="flex flex-col gap-2">
                        {rounds.map((r, i) => (
                            <div
                                key={i}
                                className="grid grid-cols-1 gap-2 rounded-lg border p-3 sm:grid-cols-[1.4fr_1fr_1fr_auto]"
                            >
                                <Input
                                    value={r.resident ?? ''}
                                    onChange={(e) =>
                                ...
                                <SelectInput
                                    ...
                                    placeholder="Med type…"
                                    options={MED_TYPES.map((m) => ({
                                        value: m,
                                        label: m,
                                    }))}
                                />
            ...
            {step === 3 && (
                <>
                    <StepHead
                        icon={Award}
                        title="Outcome & permissions"
                        blurb="Eligibility is computed from the competencies."
                    />
                    <div
                    ...
                    <div className="flex flex-col gap-2">
                        <label
                            className={`flex items-center gap-2 text-sm ${!eligible ? 'opacity-50' : ''}`}
                        >
                            <input
                                type="checkbox"
                                disabled={!eligible}
                                checked={eligible && canUnsup}
                                onChange={(e) => setCanUnsup(e.target.checked)}
                        ...
                        <label className="flex items-center gap-2 text-sm">
                            <input
                                ...
                                checked={canWitness}
                                onChange={(e) =>
                                    setCanWitness(e.target.checked)
                                }
                                className="h-4 w-4 rounded border-border"
                            />
                        ...
                        {restricted && (
                            <Field label="Restriction notes" span>
                                <Input
                                    value={restrictionNotes}
                                    onChange={(e) =>
                                        setRestrictionNotes(e.target.value)
                                    }
                                    placeholder="e.g. patches not yet observed"
                                />
                            </Field>
                        )}
                    </div>
            ...
            {step === 4 && (
                <>
                    <StepHead
                        ...
                        blurb="Development notes and two-party declaration."
                    />
                    ...
                    <div className="mt-4 rounded-lg border px-4">
                        ...
                        <SummaryRow
                            label="Score"
                            value={`${yesCount}/${applicable}`}
                        />
                        <SummaryRow
                            label="Observed rounds"
                            value={
                                rounds.filter((r) => r.resident?.trim()).length
                            }
                        />
                        <SummaryRow
                            label="Unsupervised"
                            value={eligible && canUnsup ? 'Yes' : 'No'}
                        />
                    </div>
                    <div className="mt-4 flex flex-col gap-2">
                        <label className="flex items-center gap-2 text-sm">
                            <input
                                type="checkbox"
                                checked={assessorDeclared}
                                onChange={(e) =>
                                    setAssessorDeclared(e.target.checked)
                                }
                                className="h-4 w-4 rounded border-border"
                            />
                            As assessor, I confirm this assessment is accurate.
                        </label>
                        <label className="flex items-center gap-2 text-sm">
                            <input
                                type="checkbox"
                                checked={staffDeclared}
                                onChange={(e) =>
                                    setStaffDeclared(e.target.checked)
                                }
                                className="h-4 w-4 rounded border-border"
                            />
                            The staff member acknowledges the outcome and any
                            restrictions.
                        </label>
                    </div>
                </>
...
export function ViewAssessmentDialog({
    ...
    const triFor = (key: string): TriState =>
        assessment.not_seen_areas.includes(key)
            ? 'not_seen'
            : areaVal(assessment, key)
              ? 'yes'
              : 'no';
    const chips: string[] = [];
    ...
    const viewStaff =
        onViewStaff ?? (() => router.visit(`/staff/${assessment.user_id}`));
    return (
        <MedsWizardDialog
            open
            onClose={onClose}
            title="Assessment detail"
            description={`${assessment.user_name} · ${assessment.assessment_type ?? 'assessment'}`}
            railIcon={ClipboardCheck}
            railTitle="Assessment"
            railSubtitle={assessment.user_name}
            steps={[
                {
                    key: 'detail',
                    label: 'Summary',
            ...
            footer={
                <>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Button variant="ghost" onClick={viewStaff}>
        ...
        >
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                ...
                <FactTile
                    label="Observed"
                    value={assessment.observed_rounds.length}
                />
                <FactTile
                    label="Expiry"
                    value={assessment.expiry_date ?? '—'}
                />
            </div>
                ...
                {COMPETENCY_AREAS.map((a) => {
                    ...
                    return (
                        ...
                        >
                            <span>{a.label}</span>
                            <span
                                className={`text-xs font-semibold ${v === 'yes' ? 'text-status-success' : v === 'no' ? 'text-status-critical' : 'text-muted-foreground'}`}
                            >
                                {v === 'yes'
                                    ? 'Yes'
                                    : v === 'no'
            ...
            {assessment.observed_rounds.length > 0 && (
                <div className="mt-4">
                    <div className="mb-1.5 text-xs font-semibold tracking-wide text-muted-foreground uppercase">
                        Observed rounds ({assessment.observed_rounds.length})
                    </div>
                        ...
                        {assessment.observed_rounds.map((r, i) => (
                            ...
                            >
                                ...
                                <span className="flex items-center gap-2 text-xs text-muted-foreground">
                                    ...
                                    {r.outcome && (
                                        <span
                                            className={`font-semibold capitalize ${r.outcome === 'safe' ? 'text-status-success' : r.outcome === 'intervened' ? 'text-status-critical' : 'text-status-warning'}`}
                                        >
                                            {r.outcome}
                                        </span>
                                    )}
            ...
            {assessment.restriction_notes && (
                <Note
                    label="Restrictions"
                    value={assessment.restriction_notes}
                />
            )}
            {assessment.strengths && (
                <Note label="Strengths" value={assessment.strengths} />
            )}
            ...
            {assessment.action_plan && (
                <Note label="Action plan" value={assessment.action_plan} />
            )}
            {assessment.assessor_comments && (
                <Note
                    label="Assessor comments"
                    value={assessment.assessor_comments}
                />
            )}
            {(assessment.assessor_declared_at ||
                assessment.staff_acknowledged_at) && (
                <div className="mt-4">
                    <div className="mb-1.5 text-xs font-semibold tracking-wide text-muted-foreground uppercase">
                    ...
                    <div className="flex flex-col gap-1.5">
                        <div className="flex items-center justify-between rounded-lg border px-3 py-1.5 text-sm">
                            <span className="flex items-center gap-2">
                                <CheckCircle2
                                    className={`h-3.5 w-3.5 ${assessment.assessor_declared_at ? 'text-status-success' : 'text-muted-foreground'}`}
                                />
                                Assessor confirmed accuracy
                            </span>
                        ...
                        <div className="flex items-center justify-between rounded-lg border px-3 py-1.5 text-sm">
                            <span className="flex items-center gap-2">
                                <CheckCircle2
                                    className={`h-3.5 w-3.5 ${assessment.staff_acknowledged_at ? 'text-status-success' : 'text-muted-foreground'}`}
                                />
                                Staff member acknowledged outcome
                            </span>
                            <span className="text-xs text-muted-foreground">
                                {assessment.staff_acknowledged_at ??
                                    'Not recorded'}
