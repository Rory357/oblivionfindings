        ...
        >
            {step === 0 && (
                <>
                    <StepHead
                        icon={FileText}
                        title="Order context"
                        blurb="What kind of order, and for whom."
                    />
                    ...
                    {isVerbal && (
                        <div className="mt-3">
                            <InfoCard icon={AlertTriangle} tone="warn">
                                Verbal &amp; telephone orders must be
                                countersigned by the prescriber within{' '}
                                <strong>24 hours</strong>. You&apos;ll record
                                the read-back at the final step.
                            </InfoCard>
                        </div>
                    )}
                </>
            ...
            {step === 1 && (
                <>
                    <StepHead
                        icon={Stethoscope}
                        title="Prescriber"
                        blurb="Who authorised this order."
                    />
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <Field
                            ...
                            required
                            span
                            error={form.errors.prescriber_name}
                        >
                            <Input
                        ...
                        <Field label="Prescriber type">
                            <SelectInput
                                value={form.data.prescriber_type}
                                onChange={(v) =>
                                    form.setData('prescriber_type', v)
                                }
                                placeholder="Select…"
                                options={[
                                    { value: 'gp', label: 'GP' },
            ...
            {step === 3 && (
                <>
                    <StepHead
                        icon={PenTool}
                        title="Read-back & review"
                        blurb={
                            isVerbal
                                ? 'Confirm the read-back, then review.'
                                : 'Review the order.'
                        }
                    />
                    {isVerbal && (
                        <div className="mb-4 flex flex-col gap-3">
                            <CheckRow
                                checked={form.data.read_back_confirmed}
...
export function CountersignDialog({
    ...
    const [method, setMethod] = useState<'in_person' | 'electronic'>(
        'electronic',
    );
    ...
    return (
        ...
        >
            <StepHead
                icon={PenTool}
                title="Prescriber countersignature"
                blurb="Confirm the order details are correct as charted."
            />
            <div className="rounded-lg border px-4">
                <SummaryRow label="Resident" value={order.client_name} />
                <SummaryRow
                    label="Medication"
                    value={`${order.medication_name} ${order.dose ?? ''}`}
                />
                <SummaryRow
                    label="Prescriber"
                    value={`${order.prescriber_name}${order.prescriber_registration ? ` · ${order.prescriber_registration}` : ''}`}
                />
            ...
            <div className="mt-3">
                <CheckRow
                    checked={declared}
                    label="I confirm this order as the prescriber (or on their authority)."
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <Field label="Pharmacy notes" span>
                    <Input
                        ...
                        onChange={(e) =>
                            form.setData('pharmacy_notes', e.target.value)
                        }
                    />
                </Field>
            </div>
...
export function LinkMarDialog({
    ...
    const submit = () => {
        form.transform((d) => ({
            client_medication_id: d.client_medication_id
                ? Number(d.client_medication_id)
                : null,
        }));
    ...
    return (
            ...
            footer={
                <>
                    ...
                    <Button
                        onClick={submit}
                        disabled={
                            !form.data.client_medication_id || form.processing
                        }
                    >
                        Link order
                    </Button>
        ...
        >
            <StepHead
                icon={Link2}
                title="Link to charted medication"
                blurb="Connect this prescriber order to a medication on the resident's MAR."
            />
            {clientMeds.length === 0 ? (
                <InfoCard icon={AlertTriangle} tone="warn">
