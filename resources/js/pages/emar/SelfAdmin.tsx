...
function computeOutcome(scores: Record<string, number>, booleans: Record<string, boolean>): string {
...
export default function SelfAdmin({ assessments, dueReassessments, clients, filters }: Props) {
    ...
    const computedOutcome = useMemo(() => {
        ...
        form.data.cognitive_capacity, form.data.physical_dexterity, form.data.vision_ability,
        form.data.swallowing_ability, form.data.understanding_score,
        form.data.can_identify_medications, form.data.can_read_labels, form.data.can_open_packaging,
        form.data.can_manage_timing, form.data.can_store_safely, form.data.willing_to_self_admin,
    ]);
                    ...
                    <div className="ml-auto">
                        <Dialog open={open} onOpenChange={setOpen}>
                            <DialogTrigger asChild>
                                <Button size="sm"><Plus className="mr-1 h-4 w-4" /> New Assessment</Button>
                            </DialogTrigger>
                            <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
                                <DialogHeader>
                                    <DialogTitle>New Self-Administration Assessment</DialogTitle>
                                </DialogHeader>
                                <form onSubmit={submit} className="space-y-5">
                                    ...
                                    {/* Scored Criteria */}
                                    <div>
                                        <Label className="mb-2 block font-semibold">Scored Criteria (1-5)</Label>
                                        <div className="grid gap-3 sm:grid-cols-2">
                                            {SCORE_CRITERIA.map((c) => (
                                                <div key={c.key}>
                                                    <Label htmlFor={`score-${c.key}`} className="text-xs">{c.label}</Label>
                                                    <Input
                                                        id={`score-${c.key}`}
                                                        type="number"
                                                        min={1}
                                    ...
                                    {/* Boolean Checks */}
                                    <div>
                                        <Label className="mb-2 block font-semibold">Capability Checks</Label>
                                        <div className="grid gap-2 sm:grid-cols-2">
                                            {BOOLEAN_CHECKS.map((c) => (
                                                <label key={c.key} className="flex items-center gap-2 text-sm">
                                                    <Checkbox
                                                        checked={form.data[c.key] as boolean}
                                                        onCheckedChange={(v) => form.setData(c.key as any, !!v)}
                                                    />
                                                    {c.label}
                                                </label>
                                            ))}
                                    ...
                                    <div className="rounded-md border p-3">
                                        <Label className="mb-1 block text-xs font-semibold text-muted-foreground">Computed Outcome</Label>
                                        {outcomeLabels[computedOutcome] && (
                                            <Badge className={`${outcomeLabels[computedOutcome].color}`}>
                                                {outcomeLabels[computedOutcome].label}
                                            </Badge>
                                        )}
                                    </div>
                                    ...
                                    <div className="grid gap-3 sm:grid-cols-2">
                                        <div>
                                            <Label htmlFor="sa-risk">Risk Factors</Label>
                                            <Textarea id="sa-risk" rows={2} value={form.data.risk_factors} onChange={(e) => form.setData('risk_factors', e.target.value)} />
                                        </div>
                                        <div>
                                            <Label htmlFor="sa-support">Support Needed</Label>
                                            <Textarea id="sa-support" rows={2} value={form.data.support_needed} onChange={(e) => form.setData('support_needed', e.target.value)} />
                                        </div>
                                        <div>
                                            <Label htmlFor="sa-storage">Safe Storage Notes</Label>
                                            <Textarea id="sa-storage" rows={2} value={form.data.safe_storage_notes} onChange={(e) => form.setData('safe_storage_notes', e.target.value)} />
                                        </div>
                                        <div>
                                            <Label htmlFor="sa-notes">Assessor Notes</Label>
                                            <Textarea id="sa-notes" rows={2} value={form.data.assessor_notes} onChange={(e) => form.setData('assessor_notes', e.target.value)} />
                                        </div>
                                    </div>
                    ...
                    <CardContent className="p-0">
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                ...
                                {assessments.data.length === 0 && <tr><td colSpan={10} className="p-6 text-center text-muted-foreground">No assessments found.</td></tr>}
                            </tbody>
                        </table>
                    </CardContent>
                </Card>
