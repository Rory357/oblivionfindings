...
function catCtxTag(outcome: string): {
    tag: string;
    tagBg: string;
    tagColor: string;
} {
...
export default function SelfAdmin({
    ...
    const visible = useMemo(() => {
        ...
        return assessments.filter(
            (a) =>
                (!q ||
                    `${a.client_name} ${a.nhi ?? ''} ${a.assessor_name ?? ''}`
                        .toLowerCase()
                        .includes(q)) &&
    ...
    const agreements = visible.filter(
        (a) => a.outcome === 'independent' || a.outcome === 'prompted',
    );
    const selfManaging = visible.filter(
        (a) => a.outcome !== 'administered' && a.client_medications.length > 0,
    );

    const onSite = (id: number | null) => {
        setSiteFilter(id);
        router.get('/emar/self-admin', id ? { site_id: id } : {}, {
            preserveState: true,
            preserveScroll: true,
        });
    };
    ...
    const openView = (a: SelfAdminRow) =>
        setModal({ type: 'view', assessment: a });
    const viewClient = (id: number) =>
        router.visit(`/operations/clients/${id}?tab=mar`);
    ...
    const openRowCtx = (e: ReactMouseEvent, a: SelfAdminRow) => {
        ...
        const items: ShiftCtxItem[] = [
            {
                ...
                sub: categoryMeta(a.outcome).label,
                tone: 'primary',
                onClick: () => openView(a),
            },
            {
                ...
                label: 'Reassess',
                onClick: () => setModal({ type: 'reassess', assessment: a }),
            },
            ...(isSelfManaging && !a.agreement_signed_at
                ? [
                      {
                          ...
                          onClick: () =>
                              setModal({ type: 'agreement', assessment: a }),
                ...
                ? [
                      {
                          ...
                          label: 'Set medication scope',
                          onClick: () =>
                              setModal({ type: 'scope', assessment: a }),
                      } satisfies ShiftCtxItem,
                  ]
                : []),
            { sep: true },
            {
                ...
                label: 'View client',
                onClick: () => viewClient(a.client_id),
            },
            {
                icon: <FileText className="h-3.5 w-3.5" />,
                label: 'Open on MAR chart',
                onClick: () => openMar(a.client_id),
            },
            { sep: true },
            {
        ...
        setCtx({
            ...
            tagBg: t.tagBg,
            tagColor: t.tagColor,
            meta: `${a.client_name}${a.nhi ? ` · NHI ${a.nhi}` : ''}${a.reassessment_date ? ` · review ${fmtDate(a.reassessment_date)}` : ''}`,
            items,
        });
    };
    ...
    const TABS: RosterTabItem[] = [
        ...
        },
        {
            ...
            icon: AlarmClock,
            tone: 'warning',
            badge: dueList.length || undefined,
        },
        {
            id: 'agreements',
            label: 'Agreements',
            icon: FileSignature,
            tone: 'info',
            badge: kpis.unsigned || undefined,
        },
        {
            id: 'permed',
            label: 'Per-medication scope',
            icon: Pill,
            tone: 'primary',
        },
    ...
    const heroStats: PageHeroStat[] = [
        ...
        { label: 'Supervised', value: kpis.supervised },
        {
            label: 'Due now',
            value: kpis.due_now,
            tone: kpis.due_now > 0 ? 'critical' : 'neutral',
        },
    ...
    const alerts: SaAlert[] = [
        kpis.due_now > 0 && {
            ...
            icon: AlarmClock,
            message: `${kpis.due_now} self-administration assessment${kpis.due_now === 1 ? '' : 's'} due for reassessment.`,
            tab: 'reassess',
        },
        kpis.unsigned > 0 && {
            kind: 'unsigned',
            ...
            icon: FileSignature,
            message: `${kpis.unsigned} self-managing client${kpis.unsigned === 1 ? '' : 's'} awaiting a signed self-administration agreement.`,
            tab: 'agreements',
        },
    ].filter(
        (a): a is SaAlert =>
            Boolean(a) && !dismissed.includes((a as SaAlert).kind),
    );
        ...
        >
            ...
            <div className="flex flex-col gap-6 p-6">
                ...
                <TabStrip
                    ...
                    items={TABS}
                    ariaLabel="Self-administration views"
                />
                                ...
                                <table className="w-full min-w-[920px] text-sm">
                                    ...
                                    <tbody>
                                        {visible.map((a) => {
                                            ...
                                            const capPct =
                                                (a.total_score / 25) * 100;
                                            const capCount = [
                                                a.can_identify_medications,
                                                a.can_read_labels,
                                                a.can_open_packaging,
                                                a.can_manage_timing,
                                                a.can_store_safely,
                                                a.willing_to_self_admin,
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <td className="px-4 py-3">
                                                        <div className="flex items-center gap-2">
                                                            <div className="h-1.5 w-16 overflow-hidden rounded-full bg-muted">
                                                                <div
                                                                    className={`h-full rounded-full ${capPct >= 80 ? 'bg-status-success' : capPct >= 55 ? 'bg-status-warning' : 'bg-status-critical'}`}
                                                                    style={{
                                                                        width: `${capPct}%`,
                                                                    }}
                                                                />
                                                    ...
                                                    <td className="px-4 py-3">
                                                        <span
                                                            className={`text-xs font-semibold ${capCount >= 5 ? 'text-status-success' : capCount >= 3 ? 'text-status-warning' : 'text-status-critical'}`}
                                                        >
                                                            {capCount}/6
                                                        </span>
                                                    </td>
                                                    <td className="px-4 py-3 text-xs">
                                                        {a.wishes_to_self_administer ? (
                                                    ...
                                                    <td className="px-4 py-3 text-muted-foreground">
                                                        ...
                                                        {a.reassessment_due && (
                                                            <span className="ml-1 rounded-full bg-status-critical-bg px-1.5 py-0.5 text-[10px] font-semibold text-status-critical">
                                                                Due
                                                            </span>
                                                        )}
                                                    </td>
                ...
                {activeTab === 'reassess' &&
                    (dueList.length === 0 ? (
                        <Empty text="No reassessments due." />
                    ) : (
                            ...
                            {dueList.map((a) => {
                                ...
                                return (
                                    <div
                                        key={a.id}
                                        {...rowInteract(a)}
                                        className="cursor-pointer rounded-2xl border border-l-4 border-l-status-critical bg-card p-4 shadow-sm transition-colors hover:bg-muted/30 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary"
                                    >
                                        <div className="flex items-center gap-3">
                                            <span className="flex h-9 w-9 items-center justify-center rounded-full bg-primary/10 text-xs font-bold text-primary">
                                            ...
                                            <div>
                                                ...
                                                <span
                                                    className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${cat.cls}`}
                                                >
                                                    {cat.label}
                                                </span>
                                            </div>
                                        ...
                                        <div className="mt-3 text-xs text-muted-foreground">
                                            ...
                                            {fmtDate(a.reassessment_date)}.
                                            {a.reassessment_trigger
                                                ? ` Trigger: ${a.reassessment_trigger}.`
                                                : ''}
                                        </div>
                ...
                {activeTab === 'agreements' &&
                    (agreements.length === 0 ? (
                        <Empty text="No self-managing clients yet." />
                    ) : (
                        <div className="overflow-hidden rounded-2xl border bg-card shadow-sm">
                            {agreements.map((a) => {
                                ...
                                return (
                                    <div
                                        key={a.id}
                                        {...rowInteract(a)}
                                        className="flex cursor-pointer items-center justify-between gap-3 border-b px-4 py-3 transition-colors last:border-b-0 hover:bg-muted/30 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-inset"
                                    >
                                        <div className="flex items-center gap-3">
                                            <span className="flex h-9 w-9 items-center justify-center rounded-full bg-primary/10 text-xs font-bold text-primary">
                                                {initials(a.client_name)}
                ...
                {activeTab === 'permed' &&
                    (selfManaging.length === 0 ? (
                        <Empty text="No self-managing clients with active medications." />
                    ) : (
                        <div className="flex flex-col gap-4">
                            {selfManaging.map((a) => (
                                ...
                                >
                                    <div
                                        {...rowInteract(a)}
                                        className="flex cursor-pointer items-center justify-between gap-3 border-b bg-muted/40 px-4 py-3 transition-colors hover:bg-muted/60 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-inset"
                                    >
                                        <div className="flex items-center gap-3">
                                            <span className="flex h-9 w-9 items-center justify-center rounded-full bg-primary/10 text-xs font-bold text-primary">
                                                {initials(a.client_name)}
                                            ...
                                            <div>
                                                ...
                                                <span
                                                    className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${categoryMeta(a.outcome).cls}`}
                                                >
                                                    {
                                                        categoryMeta(a.outcome)
                                                            .label
                ...
                {activeTab === 'activity' && (
                    <div className="overflow-hidden rounded-2xl border bg-card shadow-sm">
                        <div className="border-b bg-muted/40 px-4 py-3 text-xs text-muted-foreground">
                            Entries are amendable but never deleted.
                        </div>
                        {activity.length === 0 ? (
