...
export function ReceiveStockDialog({
    ...
    const safeDefaultMedId = receivableMedications.some(
        ...
        : null;
    const form = useForm({
        client_medication_id: safeDefaultMedId
            ? String(safeDefaultMedId)
            : '',
        quantity: '',
        notes: '',
        batch_number: '',
...
function newControlledDeliveryUuid(): string {
    if (typeof crypto !== 'undefined' && 'randomUUID' in crypto)
        return crypto.randomUUID();

    return 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, (c) => {
        const random = Math.floor(Math.random() * 16);
        const value = c === 'x' ? random : (random % 4) + 8;
...
export function ControlledPharmacyDeliveryDialog({
    ...
    const form = useForm({
        ...
        witness_credential: '',
        delivery_notes: '',
        client_request_uuid: newControlledDeliveryUuid(),
    });
    ...
    const submit = () => {
        form.transform(() =>
            buildControlledPharmacyDeliveryRequest({
                ...
                uuid: form.data.client_request_uuid,
            }),
        );
        form.post(controlledPharmacyDeliveryPath(order.id), {
            ...
            onError: () =>
                toast.error(
                    'Could not receive the controlled drug delivery',
                ),
        });
    };
