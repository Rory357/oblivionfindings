...
import {
    CdDetailDialog,
    type CdDetailSubject,
} from '@/components/emar/cd-detail-dialog';
import {
    statusTone,
    type CdDestruction,
    type CdDiscrepancy,
...
export default function ControlledDrugs(props: Props) {
    ...
    const reload = (over: Record<string, string | number | undefined>) => {
        ...
        router.get('/emar/controlled', params, {
            preserveState: true,
            preserveScroll: true,
        });
    };
    ...
    const medsF = useMemo(
        () => medications.filter((m) => matchq(q, m.name, m.client_name)),
        [medications, q],
    );
    const entriesF = useMemo(
        () =>
            recentEntries.filter((e) =>
                matchq(q, e.medication_name, e.client_name),
            ),
        [recentEntries, q],
    );
    const discF = useMemo(
    ...
    const destF = useMemo(
        () =>
            destructions.filter((d) =>
                matchq(q, d.medication_name, d.client_name),
            ),
        [destructions, q],
    );
    const lossF = useMemo(
        () =>
            lossReports.filter((l) =>
                matchq(
                    q,
                    l.medication_name,
                    l.client
    ...
    const reconciliation = useMemo(
        () =>
            medsF.map((m) => ({
                med: m,
                lastAt: m.last_balance_check_at ?? null,
                days: m.days_since_check ?? null,
                overdue: !!m.overdue_check,
            })),
        [medsF],
    );

    const openLosses = lossReports.filter((l) =>
        ['reported', 'investigating'].includes(l.investigation_status),
    );
    ...
    const sync: { label: string; dotClass: string; pingClass: string | null } =
        !online
            ? {
                  label:
                      pendingCount > 0
            ...
            : syncing
              ? {
                    ...
                    pingClass: 'bg-status-info/70',
                }
              : pendingCount > 0
                ? {
                      label: `${pendingCount} queued to sync`,
                      dotClass: 'bg-status-info',
                      pingClass: null,
                  }
                : {
                      ...
                      pingClass: 'bg-status-success/70',
                  };
        ...
        () =>
            medications.filter((m) => {
                ...
                const lowStock =
                    ...
                    onHand != null &&
                    onHand <= Number(s.reorder_level);
                const expiring =
                    !!s.expiry_date &&
                    (parseYmd(s.expiry_date).getTime() - Date.now()) /
                        86_400_000 <=
                        30;
    ...
    const alerts: CdAlert[] = [
        discrepancies.length > 0 && {
            ...
            icon: AlertTriangle,
            message: `${discrepancies.length} open controlled-drug discrepanc${discrepancies.length === 1 ? 'y' : 'ies'} — investigate and resolve.`,
            tab: 'discrepancies',
        },
        openLosses.length > 0 && {
            kind: 'loss',
            tone: 'critical' as const,
            icon: FileWarning,
            message: `${openLosses.length} open loss investigation${openLosses.length === 1 ? '' : 's'} awaiting follow-up.`,
            tab: 'loss',
        },
        overdueChecks > 0 && {
            kind: 'overdue',
            ...
            icon: ShieldCheck,
            message: `${overdueChecks} controlled drug${overdueChecks === 1 ? '' : 's'} overdue a balance check (≥ 7 days).`,
            tab: 'reconciliation',
        },
        stockRisks.length > 0 && {
            kind: 'stock',
            tone: 'warning' as const,
            icon: Package,
            message: `${stockRisks.length} controlled drug${stockRisks.length === 1 ? '' : 's'} at/below reorder level or expiring within 30 days.`,
            tab: 'register',
        },
    ].filter(
        (a): a is CdAlert =>
            Boolean(a) && !dismissed.includes((a as CdAlert).kind),
    );
    ...
    const medForEntry = (e: CdEntry) =>
        medications.find(
            (m) => m.client_id === e.client_id && m.name === e.medication_name,
        );
    const openDetail = (subject: CdDetailSubject) =>
        setModal({ type: 'detail', subject });
    const viewClient = (id: number | null | undefined) =>
        id && router.visit(`/operations/clients/${id}?tab=mar`);
    const exportRegister = () =>
        window.open('/emar/pdf/controlled-register', '_blank');
    ...
    const openRowCtx = (
        ...
        const clientId =
            subject.kind === 'medication'
                ? subject.med.client_id
                : subject.kind === 'entry'
                  ? subject.entry.client_id
                  : subject.kind === 'discrepancy'
                    ? (subject.disc.client?.id ?? null)
                    : subject.kind === 'destruction'
                      ? (subject.destruction.client_id ?? null)
                      : (subject.loss.client?.id ?? null);
        const view: ShiftCtxItem = {
        ...
        if (subject.kind === 'medication' || subject.kind === 'entry') {
            const med =
                subject.kind === 'medication'
                    ? subject.med
                    : medForEntry(subject.entry);
            const drug =
                subject.kind === 'medication'
                    ? subject.med.name
                    : (subject.entry.medication_name ?? 'CD');
            const client =
                subject.kind === 'medication'
                    ? subject.med.client_name
                    : subject.entry.client_name;
            ...
            if (med)
                actions.push({
                    ...
                    label: 'Check balance',
                    onClick: () =>
                        setModal({ type: 'balanceMed', medId: med.id }),
                });
            actions.push({
            ...
            actions.push({
                ...
                label: 'View full register for this CD',
                onClick: () => {
                    setSearch(drug);
                    setActiveTab('recent');
                },
            ...
            if (d.status !== 'closed' && d.status !== 'resolved')
                actions.push({
                    ...
                    onClick: () => setModal({ type: 'resolveDisc', disc: d }),
                });
            ...
            tag = 'Destroyed';
            meta =
                `${d.client_name} · ${d.medication_name ?? 'CD'} · ${d.quantity ?? '—'} ${d.unit ?? ''}`.trimEnd();
            actions.push({
                icon: <Trash2 className="h-3.5 w-3.5" />,
                label: 'Record destruction',
                onClick: () => setModal({ type: 'destruction' }),
            });
        } else {
            ...
            if (l.investigation_status === 'reported')
                actions.push({
                    icon: <FileWarning className="h-3.5 w-3.5" />,
                    label: 'Investigate',
                    onClick: () =>
            ...
            if (l.investigation_status !== 'resolved')
                actions.push({
                    icon: <ShieldCheck className="h-3.5 w-3.5" />,
                    label: 'Resolve',
                    onClick: () =>
    ...
    const TABS: RosterTabItem[] = [
        {
            ...
            icon: Lock,
            tone: 'primary',
            badge: medications.length || undefined,
        },
        {
            id: 'recent',
            ...
            icon: Package,
            tone: 'info',
            badge: recentEntries.length || undefined,
        },
        {
            id: 'reconciliation',
        ...
        },
        {
            ...
            icon: AlertTriangle,
            tone: 'critical',
            badge: discrepancies.length || undefined,
        },
        {
            id: 'destructions',
            label: 'Destructions',
            icon: Trash2,
            tone: 'warning',
            badge: destructions.length || undefined,
        },
        {
            id: 'loss',
            label: 'Loss Reports',
            icon: FileWarning,
            tone: 'critical',
            badge: openLosses.length || undefined,
        },
    ...
    const heroStats: PageHeroStat[] = [
        { label: 'Active CDs', value: medications.length },
        {
            ...
            tone: discrepancies.length > 0 ? 'critical' : 'neutral',
        },
        {
            label: 'Overdue checks',
            value: overdueChecks,
            tone: overdueChecks > 0 ? 'warning' : 'neutral',
        },
        {
            label: 'Loss investigations',
            value: openLosses.length,
            tone: openLosses.length > 0 ? 'critical' : 'neutral',
        },
    ];
        ...
        >
            ...
            <div className="flex flex-col gap-6 p-6">
                        ...
                        <span>
                            <span className="flex items-center gap-2 text-[10.5px] font-semibold tracking-wide text-primary-foreground/80 uppercase">
                                ...
                                >
                                    {sync.pingClass ? (
                                        <span
                                            className={`absolute inset-0 animate-ping rounded-full ${sync.pingClass}`}
                                        />
                                    ) : null}
                                    <span
                                        className={`relative inline-flex h-2 w-2 rounded-full ${sync.dotClass}`}
                                    />
                                </span>
                        ...
                        <div className="flex flex-col items-stretch gap-2 py-3 md:flex-row md:items-center md:justify-between">
                            <div className="flex flex-wrap items-center gap-1.5">
                                ...
                                <DayPickerChip
                                    ...
                                    isToday={isToday}
                                    onPick={goDate}
                                    caption="Register & movements are for the selected day; stock & CD balance checks always show today."
                                />
                                <button
                                    type="button"
                                    className="inline-flex items-center gap-1 rounded-md border border-primary-foreground/20 bg-primary-foreground/10 px-3 py-1.5 text-xs font-semibold text-primary-foreground hover:bg-primary-foreground/20"
                ...
                <TabStrip
                    ...
                    items={TABS}
                    ariaLabel="Controlled drug views"
                />
                ...
                {activeTab === 'register' && (
                    <TableCard
                        ...
                        title="Active controlled drugs"
                        count={medications.length}
                        action={btnEntry}
                        cta={medications.length === 0 ? btnEntry : undefined}
                        empty={
                            medsF.length === 0
                                ? medications.length === 0
                                    ? 'No active controlled drugs.'
                                    : 'No controlled drugs match your search.'
                                : null
                        }
                    >
                        {medsF.map((m) => {
                ...
                {activeTab === 'recent' && (
                    <TableCard
                        ...
                        title={`Movements · ${props.date_label}`}
                        ...
                        empty={
                            entriesF.length === 0
                                ? recentEntries.length === 0
                                    ? `No register movements on ${props.date_label}.`
                            ...
                            >
                                ...
                                <td className="px-4 py-3 font-medium">
                                    <DrugCell
                                        ...
                                        controlled={e.controlled_drug ?? true}
                                        schedule={medForEntry(e)?.schedule}
                                    />
                                    <ExpiryNote value={e.expiry_date} />
                                </td>
                ...
                {activeTab === 'reconciliation' && (
                    <TableCard
                        ...
                        count={medications.length}
                        action={btnBalance}
                        cta={medications.length === 0 ? btnBalance : undefined}
                        empty={
                            reconciliation.length === 0
                                ? medications.length === 0
                                    ? 'No controlled drugs to reconcile.'
                                    : 'No controlled drugs match your search.'
                                : null
                        }
                    >
                        {reconciliation.map((r) => (
                            ...
                            >
                                ...
                                <td className="px-4 py-3 font-medium">
                                    <DrugCell
                                        name={r.med.name}
                                        controlled={r.med.controlled_drug}
                                        schedule={r.med.schedule}
                                    />
                                </td>
                                <td className="px-4 py-3 tabular-nums">
                                    <div>
                                        {r.med.stock
                                            ? `${r.med.stock.on_hand ?? '—'} ${r.med.stock.unit ?? ''}`
                                            : '—'}
                                    </div>
                                    <ExpiryNote
                                        value={r.med.stock?.expiry_date}
                                    />
                                </td>
                                <td className="px-4 py-3 text-muted-foreground">
                                    {r.lastAt
                        ...
                        {discF.length === 0 ? (
                            ...
                            discF.map((d) => (
                                <div
                                    ...
                                    className="flex cursor-pointer flex-wrap items-center justify-between gap-3 rounded-2xl border bg-card p-4 shadow-sm transition-colors hover:bg-muted/30 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary"
                                >
                                    <div>
                                        <div className="font-medium">
                                            {d.medication?.name ?? 'CD'}{' '}
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border bg-card p-4 shadow-sm">
                            <div className="flex items-start gap-3">
                                <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-status-warning-bg text-status-warning">
                                ...
                                <div>
                                    ...
                                    <div className="text-xs text-muted-foreground">
                                        {destructions.length} controlled-drug
                                        destruction
                                        {destructions.length === 1
                                            ? ''
                                            : 's'}{' '}
                                        on {props.date_label}. The full disposal
                                        register — all medications, reports
                                        &amp; export — is the canonical record.
                                    </div>
                                </div>
                            </div>
                            <div className="flex items-center gap-2">
                                ...
                                <Button
                                    ...
                                    variant="outline"
                                    onClick={() =>
                                        router.visit('/emar/destructions')
                                    }
                                >
                                    <ArrowUpRight className="h-4 w-4" />
                                    Open destructions register
                                </Button>
                            </div>
                        </div>
                        <TableCard
                            ...
                            title={`CD destructions · ${props.date_label}`}
                            count={destructions.length}
                            cta={
                            ...
                            empty={
                                destF.length === 0
                                    ? destructions.length === 0
                                        ? `No CD destructions on ${props.date_label}.`
                                        : 'No destructions match your search.'
                                    : null
                            }
                        >
                            {destF.map((d) => (
                            ...
                            <EmptyCard
                                text={
                                    lossReports.length === 0
                                        ? 'No loss reports.'
                                        : 'No loss reports match your search.'
                                }
                                cta={
                                    lossReports.length === 0
                                        ? btnLoss
                                ...
                                >
                                    <div>
                                        <div className="font-medium">
                                            {l.medication_name}{' '}
                                            <span className="text-sm text-muted-foreground">
                                                · {l.quantity_lost} {l.unit}{' '}
                                                lost
                                            </span>
                                        </div>
                                        <div className="text-xs text-muted-foreground">
                                            {l.circumstances}
                                            {l.reported_to_police
                                                ? ` · Police ${l.police_reference ?? 'ref'}`
                                                : ''}
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <CdPill
                                            label={l.investigation_status}
                                            tone={statusTone(
            ...
            {modal?.type === 'entry' && (
                <RecordCdEntryDialog
                    ...
                    staff={staff}
                    currentUserId={props.current_user?.id ?? null}
...
function DrugCell({
    ...
    return (
        <span className="inline-flex items-center gap-1.5">
            {controlled ? (
                <Lock
                    className="h-3 w-3 shrink-0 text-status-critical"
                    aria-label="Controlled drug"
                />
            ) : null}
            <span>{name ?? '—'}</span>
            {controlled ? (
            ...
            {schedule ? (
                <span
                    className="rounded bg-muted px-1 py-0.5 text-[9px] font-bold text-muted-foreground"
                    title={`Controlled Drug Schedule ${schedule}`}
                >
                    S{schedule}
                </span>
            ) : null}
...
function ExpiryNote({ value }: { value?: string | null }) {
    ...
    return (
        <span
            className={
                warn
                    ? 'mt-0.5 block text-[11px] text-status-warning'
                    : 'mt-0.5 block text-[11px] text-muted-foreground'
            }
        >
            Exp {exp.toLocaleDateString('en-NZ')}
        </span>
    );
}
