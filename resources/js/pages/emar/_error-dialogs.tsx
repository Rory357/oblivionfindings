...
export const typeLabel = (v: string) =>
    ERROR_TYPES.find((t) => t.value === v)?.label ?? v;
...
export function TriageDialog({
    ...
    const stageIdx = Math.max(
        0,
        STAGES.findIndex((s) => s.id === error.status),
    );
    const sev = severityMeta(error.severity);
    const [linking, setLinking] = useState(false);
    const [linkAction, setLinkAction] = useState(error.immediate_action ?? '');
    const seriousIncidentNeedsAction =
        ['major', 'critical'].includes(error.severity) && !linkAction.trim();
    const viewClient = () =>
        error.client_id &&
        router.visit(`/operations/clients/${error.client_id}?tab=mar`);
    const viewIncident = () =>
        error.incident && router.visit(`/incidents/${error.incident.id}`);
    ...
    const createIncident = () => {
        setLinking(true);
        router.post(
            `/emar/errors/${error.id}/link-incident`,
            { immediate_action: linkAction.trim() || null },
    ...
    return (
        <MedsWizardDialog
            ...
            description={`${typeLabel(error.error_type)} · ${error.client ? `${error.client.first_name} ${error.client.last_name}` : 'Unknown client'}`}
            railIcon={FileWarning}
            railTitle="Error triage"
            railSubtitle={error.ref}
            steps={[
                {
                    key: 'detail',
                    label: 'Triage',
            ...
            footer={
                <>
                    ...
                    <div className="flex flex-wrap items-center justify-end gap-2">
                        ...
                        {error.client_id && (
                            <Button variant="outline" onClick={viewClient}>
                                <User className="h-4 w-4" />
                                Client
                            </Button>
                        )}
                        {error.incident ? (
                            <Button variant="outline" onClick={viewIncident}>
                          ...
                          (error.status === 'reported' ||
                            ...
                            <Button
                                ...
                                disabled={linking || seriousIncidentNeedsAction}
                            >
                                <Link2 className="h-4 w-4" />
                        ...
                        {error.mar_url && (
                            <Button
                                variant="ghost"
                                onClick={() => router.visit(error.mar_url!)}
                            >
        ...
        >
            <div className="flex flex-wrap items-center gap-2">
                ...
                <span
                    className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${statusMeta(error.status).cls}`}
                >
                    {statusMeta(error.status).label}
                </span>
                {error.reached_client && (
                    <span className="rounded-full border px-2 py-0.5 text-[11px] text-muted-foreground">
                        Reached client: {error.reached_client}
                    </span>
                )}
                {error.harm_level && (
                    <span className="rounded-full border px-2 py-0.5 text-[11px] text-muted-foreground">
                        Harm {error.harm_level.toUpperCase().replace('_', '–')}
                    </span>
                )}
                {error.open_disclosure && error.open_disclosure !== 'na' && (
                    <span className="rounded-full border px-2 py-0.5 text-[11px] text-muted-foreground">
                        Open disclosure: {error.open_disclosure}
                    </span>
                )}
            </div>
            <div className="mt-4 rounded-lg border px-4">
                <SummaryRow
                    label="Medication"
                    value={error.medication?.name ?? '—'}
                />
                <SummaryRow
                    label="Reported"
                    value={`${fmt(error.reported_at)}${error.reported_by_user ? ` · ${error.reported_by_user.name}` : ''}`}
                />
                <SummaryRow
                    label="Reviewer"
                    value={error.reviewed_by_user?.name ?? '—'}
                />
                ...
                {STAGES.map((s, i) => (
                    ...
                    >
                        <div className="flex flex-col items-center gap-1">
                            <span
                                className={`h-2.5 w-2.5 rounded-full ${i <= stageIdx ? 'bg-primary' : 'bg-border'}`}
                            />
                            <span
                                className={`text-[10px] ${i === stageIdx ? 'font-semibold text-foreground' : 'text-muted-foreground'}`}
                            >
                                {s.label}
                            </span>
                        </div>
                        {i < STAGES.length - 1 && (
                            <div
                                className={`mx-1 h-0.5 flex-1 ${i < stageIdx ? 'bg-primary' : 'bg-border'}`}
                            />
                        )}
                    </div>
                ))}
            ...
            {error.incident && (
                <div className="mt-4 flex items-center justify-between gap-3 rounded-lg border border-status-critical/30 bg-status-critical-bg/50 px-3 py-2">
                    <span className="flex items-center gap-2 text-sm text-status-critical">
                        <Link2 className="h-4 w-4" />
                        Linked incident {error.incident.ref}
                    </span>
                    <button
                        type="button"
                ...
                <div className="mt-4">
                    ...
                    >
                        <Textarea
                            value={linkAction}
                            onChange={(event) =>
                                setLinkAction(event.target.value)
                            }
                            rows={3}
                            placeholder="Record the clinical or safety action actually taken before creating the incident."
                        />
                    </Field>
                </div>
            ) : null}
            {error.attachments.length > 0 && (
                <div className="mt-4">
                    <div className="mb-1 text-xs font-semibold tracking-wide text-muted-foreground uppercase">
                        Supporting evidence ({error.attachments.length})
                    </div>
                    <div className="flex flex-col gap-1">
                        {error.attachments.map((a) => (
                            <div
                                key={a.id}
...
// ── Review (2-step) ──────────────────────────────────────────────────────────
export function ReviewErrorDialog({
    error,
    onClose,
}: {
    error: ErrorRow;
    ...
    const submit = () => {
        setBusy(true);
        router.post(
            `/emar/errors/${error.id}/review`,
            { review_notes: notes, status },
            {
                preserveScroll: true,
                onSuccess: () => {
                    toast.success('Error reviewed');
                    onClose();
                },
                onError: () => toast.error('Review notes are required'),
    ...
    return (
        <MedsWizardDialog
            ...
            description="Record the review findings and move the error forward."
            railIcon={ClipboardList}
            railTitle="Review error"
            railSubtitle={error.ref}
            steps={[
                {
                    key: 'findings',
            ...
            footer={
                <>
                    ...
                    {step < 1 ? (
                        <Button onClick={() => setStep(1)} disabled={!valid[0]}>
                            Continue
                        </Button>
                    ) : (
            ...
            {step === 0 ? (
                <>
                    <StepHead
                        icon={ClipboardList}
                        title="Findings"
                        blurb="What did the review establish?"
                    />
                    <Field label="Review notes" required span>
                        <Textarea
                            value={notes}
                            onChange={(e) => setNotes(e.target.value)}
                            rows={5}
                            placeholder="Findings, contributing factors confirmed, immediate learning."
                        />
                    </Field>
                </>
            ) : (
                <>
                    <StepHead
                        icon={ShieldCheck}
                        title="Status & sign"
                        blurb="Where does this error go next?"
                    />
                    <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                        {[
                            { v: 'investigating', l: 'Keep investigating' },
                            { v: 'resolved', l: 'Mark resolved' },
                            { v: 'reported', l: 'Back to reported' },
                        ].map((o) => (
                            <button
                                key={o.v}
                                type="button"
                                onClick={() => setStatus(o.v)}
                                className={`rounded-xl border-2 p-3 text-left text-sm ${status === o.v ? 'border-primary bg-primary/5' : 'border-border'}`}
                            >
                                {o.l}
                            </button>
                        ))}
...
export function ResolveErrorDialog({
    ...
    const submit = () => {
        setBusy(true);
        router.post(
            `/emar/errors/${error.id}/resolve`,
            {
                outcome,
                preventive_actions: preventive,
                harm_level: harm || null,
            },
            {
                preserveScroll: true,
                onSuccess: () => {
                ...
                onError: () =>
                    toast.error('Outcome and preventive actions are required'),
                onFinish: () => setBusy(false),
            },
        );
    };
    ...
    return (
        <MedsWizardDialog
            ...
            description="Record the outcome, harm band and preventive actions."
            railIcon={ShieldCheck}
            railTitle="Resolve error"
            railSubtitle={error.ref}
            steps={[
                {
                    key: 'outcome',
            ...
            footer={
                <>
                    ...
                    {step < 1 ? (
                        ...
                        <Button onClick={submit} disabled={busy || !valid[1]}>
                            Resolve error
                        </Button>
                    )}
                </>
            ...
            {step === 0 ? (
                <>
                    <StepHead
                        icon={ShieldCheck}
                        title="Outcome"
                        blurb="What was the outcome, and what harm band (NCC-MERP)?"
                    />
                    <Field label="Outcome" required span>
                        <Textarea
                            value={outcome}
                            onChange={(e) => setOutcome(e.target.value)}
                            rows={4}
                            placeholder="Clinical outcome for the client."
                        />
                    </Field>
                    <Field label="Harm band" span>
                        <SelectInput
                            value={harm}
                            onChange={setHarm}
                            placeholder="NCC-MERP band…"
                            options={HARM_BANDS}
                        />
                    </Field>
                </>
                ...
                <>
                    <StepHead
                        ...
                        blurb="What changes will stop this recurring?"
                    />
                    <Field label="Preventive actions" required span>
                        <Textarea
                            value={preventive}
                            onChange={(e) => setPreventive(e.target.value)}
                            rows={5}
                            placeholder="System changes, process updates, training."
                        />
                    </Field>
                </>
            )}
...
export function CloseErrorDialog({
    ...
    const submit = () => {
        setBusy(true);
        router.post(
            `/emar/errors/${error.id}/close`,
            { close_note: note || null },
            {
                preserveScroll: true,
                onSuccess: () => {
                    toast.success('Error closed out');
                    onClose();
                },
                onError: () => toast.error('Could not close the error'),
                onFinish: () => setBusy(false),
            },
        );
    };
    return (
        <MedsWizardDialog
            ...
            description="Final governance sign-off for this medication error."
            railIcon={Lock}
            railTitle="Close out"
            railSubtitle={error.ref}
            steps={[
        ...
        >
            <StepHead
                icon={AlertTriangle}
                title="Close out"
                blurb="Confirm the error is fully resolved and learning is embedded."
            />
            <Field label="Close-out note" span>
                <Textarea
                    value={note}
                    onChange={(e) => setNote(e.target.value)}
                    rows={4}
                    placeholder="Confirm actions complete and shared (optional)."
                />
            </Field>
            <label className="mt-3 flex items-center gap-2 text-sm">
                ...
                />
                I confirm the outcome and preventive actions are complete and
                this error can be closed.
            </label>
        </MedsWizardDialog>
    );
}
