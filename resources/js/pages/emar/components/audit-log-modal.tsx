...
import { SelectInput, StepHead } from '@/components/wizard/primitives';
...
export function AuditLogModal({
    ...
    const rows = activity.filter((a) => {
        ...
        return (
            name.includes(q) ||
            (a.medication?.name ?? '').toLowerCase().includes(q)
        );
    });
        ...
        <>
            ...
            <div className="flex items-center gap-2">
                <Button
                    variant="outline"
                    onClick={() =>
                        (window.location.href = '/emar/audit/export')
                    }
                >
    ...
    return (
            ...
            steps={[
                {
                    key: 'log',
                    label: 'Audit log',
                    blurb: 'Recent medication activity',
                    icon: History,
                },
            ]}
            stepIndex={0}
        ...
        >
            <StepHead
                ...
                blurb="Immutable record of recent administrations. Filter below; the full history is on the audit page."
            />
            ...
            <div className="overflow-hidden rounded-lg border border-border">
                {rows.length === 0 ? (
                    ...
                    <ul className="divide-y divide-border">
                        {rows.map((a) => {
                            const meta =
                                STATUS_META[a.status] ?? STATUS_META.pending;
                            return (
                                <li
                                    key={a.id}
