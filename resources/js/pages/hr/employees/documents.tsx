                                                ...
                                                <div className="absolute inset-x-0 bottom-0 flex items-center justify-center gap-1 rounded-b-xl bg-gradient-to-t from-white via-white to-transparent pb-2 pt-6 opacity-0 transition-opacity group-hover:opacity-100">
                                                    ...
                                                    {can.manage && (
                                                        <>
                                                            ...
                                                            <button onClick={() => setDeletingDoc(d)} className="flex h-7 w-7 items-center justify-center rounded-full bg-status-critical-bg text-status-critical hover:bg-status-critical-bg">
                                        ...
                                        return (
                                            <tr key={d.id} className="border-b last:border-0 hover:bg-muted">
                                                ...
                                                <td className="hidden px-4 py-2.5 lg:table-cell">
                                                    {d.expires_at ? (
                                                        <span className={expired ? 'font-medium text-status-critical' : expiring ? 'font-medium text-status-warning' : 'text-muted-foreground'}>{formatDate(d.expires_at)}</span>
                                                    ) : <span className="text-muted-foreground">{'\u2014'}</span>}
                                                </td>
                                                    ...
                                                    <div className="flex items-center justify-end gap-1">
                                                        ...
                                                        {can.manage && (
                                                            <>
                                                                ...
                                                                <button onClick={() => setDeletingDoc(d)} className="flex h-7 w-7 items-center justify-center rounded-lg hover:bg-status-critical-bg">
            ...
            <Dialog open={showUpload} onOpenChange={v => !v && setShowUpload(false)}>
                <DialogContent className="sm:max-w-lg">
                    ...
                    <form onSubmit={e => { e.preventDefault(); uploadForm.post(`/hr/people/${profile.id}/documents`, { forceFormData: true, preserveScroll: true, onSuccess: () => { uploadForm.reset(); setShowUpload(false); } }); }} className="space-y-4">
                        ...
                        {uploadForm.errors.file && <p className="text-xs text-status-critical">{uploadForm.errors.file}</p>}
