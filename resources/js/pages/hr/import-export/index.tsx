...
import { Download, Upload, FileText, CheckCircle2, XCircle, AlertTriangle } from 'lucide-react';
...
export default function ImportExportIndex() {
    ...
    function handleImport() {
        ...
        router.post('/hr/import-export/import', formData, {
            forceFormData: true,
            onFinish: () => { setImporting(false); setFile(null); setPreview([]); if (fileInputRef.current) fileInputRef.current.value = ''; },
        });
    }
    ...
    return (
        <AppLayout breadcrumbs={[{ title: 'HR', href: '/hr/people' }, { title: 'Import / Export', href: '/hr/import-export' }]}>
            <Head title="Employee Import / Export" />
            <PageShell>
                <PageHeader title="Employee Import / Export" description="Bulk import or export employee records via CSV." />
                <div className="grid gap-6 lg:grid-cols-2">
                    <Card>
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2"><Download className="h-5 w-5" /> Export Employees</CardTitle>
                            <CardDescription>Download a CSV of all active employee records.</CardDescription>
                        </CardHeader>
                        <CardContent className="flex flex-col gap-4">
                            <Button onClick={handleExport}><Download className="mr-2 h-4 w-4" /> Export to CSV</Button>
                            <a href="/hr/import-export/template" className="inline-flex items-center text-sm text-muted-foreground hover:underline">
                                <FileText className="mr-1 h-4 w-4" /> Download blank template
                            </a>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Upload a CSV to create or update employee records.</CardDescription>
                        </CardHeader>
                        <CardContent className="flex flex-col gap-4">
                                ...
                                <div className="overflow-x-auto rounded-lg border">
                                    <table className="w-full text-xs">
                                        <thead className="bg-muted/50">
                                            <tr>{preview[0].map((h, i) => <th key={i} className="px-3 py-2 text-left font-medium">{h}</th>)}</tr>
                                        </thead>
                                        <tbody>
                                            {preview.slice(1, 6).map((row, ri) => (
                                                <tr key={ri} className="border-t">
                                                    {row.map((c, ci) => <td key={ci} className="px-3 py-1.5 text-muted-foreground">{c || '-'}</td>)}
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
                        ...
                        <CardContent className="space-y-3">
                            <div className="flex flex-wrap gap-3">
                                <Badge variant="outline" className="border-status-success/30 text-status-success bg-status-success">
                                    <CheckCircle2 className="mr-1 h-3 w-3" /> {importResult.created} created
                                </Badge>
                                <Badge variant="outline" className="border-status-info/30 text-status-info bg-status-info">
                                    <CheckCircle2 className="mr-1 h-3 w-3" /> {importResult.updated} updated
                                </Badge>
                                {importResult.errors.length > 0 && (
                                    <Badge variant="outline" className="border-status-critical/30 text-status-critical bg-status-critical">
                                        <XCircle className="mr-1 h-3 w-3" /> {importResult.errors.length} errors
                                    </Badge>
                                )}
                            </div>
                            {importResult.errors.length > 0 && (
                                <div className="rounded-lg border border-status-critical/20 bg-status-critical p-4">
                                    <div className="flex items-center gap-2 mb-2 text-sm font-medium text-status-critical">
