...
const formatDate = (value?: string | null) => {
    ...
    return Number.isNaN(d.getTime()) ? value : d.toLocaleString('en-GB', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' });
};
...
function ChangesViewer({ oldValues, newValues }: { oldValues: Record<string, any> | null; newValues: Record<string, any> | null }) {
...
export default function AuditLogIndex({ logs, actions, modelTypes, users, filters }: Props) {
    const NONE = '__none__';
    ...
    const onFilter = (next: Partial<typeof filters>) => {
        ...
        router.get('/hr/settings/audit-log', params, { preserveState: true, preserveScroll: true });
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <PageShell>
                <PageHeader title="Audit Log" description="View all HR module activity and changes." />
                ...
                <Card className="mb-6">
                    <CardContent className="pt-6">
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                            ...
                            <div>
                                <Label>Action</Label>
                                <Select value={filters.action ?? NONE} onValueChange={(v) => onFilter({ action: v === NONE ? null : v })}>
                            ...
                            <div>
                                ...
                                <Select value={filters.model_type ?? NONE} onValueChange={(v) => onFilter({ model_type: v === NONE ? null : v })}>
                                    <SelectTrigger><SelectValue placeholder="All types" /></SelectTrigger>
                                    <SelectContent>
                                        <SelectItem value={NONE}>All types</SelectItem>
                                        {modelTypes.map((t) => (
                                            <SelectItem key={t} value={t}>{shortModelType(t)}</SelectItem>
                                        ))}
                                    </SelectContent>
                                </Select>
                            </div>
                            <div>
                                ...
                                <Input type="date" value={filters.date_from ?? ''} onChange={(e) => onFilter({ date_from: e.target.value || null })} />
                            </div>
                            <div>
                                <Label>Date To</Label>
                                <Input type="date" value={filters.date_to ?? ''} onChange={(e) => onFilter({ date_to: e.target.value || null })} />
                            </div>
                        </div>
                    </CardContent>
                </Card>
                                            ...
                                            <>
                                                <CollapsibleTrigger asChild>
                                                    <TableRow className="cursor-pointer hover:bg-muted/50" onClick={() => toggleRow(entry.id)}>
                                                        ...
                                                        <TableCell className="text-sm">{shortModelType(entry.auditable_type)}</TableCell>
                                                        <TableCell className="font-mono text-xs">{entry.auditable_id}</TableCell>
                                                ...
                                                <CollapsibleContent asChild>
                                                    <TableRow>
                                                        <TableCell colSpan={7} className="bg-muted/30 p-4">
                                                            ...
                                                            <ChangesViewer oldValues={entry.old_values} newValues={entry.new_values} />
                                                        </TableCell>
                                                    </TableRow>
                                                </CollapsibleContent>
                                            </>
