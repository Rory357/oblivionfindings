...
import type { ScreeningQuestion, JobPostingFormData } from '@/types/job-postings';
...
export default function CreateJobPosting({ positions, users, posting }: Props) {
    ...
    const { data, setData, post, put, processing, errors } = useForm({
        ...
        screening_questions: posting?.screening_questions || ([] as ScreeningQuestion[]),
    ...
    const addEmail = useCallback(() => {
        ...
        if (email && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) && !data.notification_emails.includes(email)) {
            setData('notification_emails', [...data.notification_emails, email]);
            setEmailInput('');
        }
    ...
    const removeQuestion = (index: number) => {
        setData('screening_questions', data.screening_questions.filter((_, i) => i !== index));
    };
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-col gap-6 p-6 max-w-3xl mx-auto">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="col-span-2 sm:col-span-1">
                                    ...
                                    <Select value={data.position_id} onValueChange={v => setData('position_id', v)}>
                                        <SelectTrigger className="mt-1"><SelectValue placeholder="Select position (optional)" /></SelectTrigger>
                                        <SelectContent>
                                            {positions.map(p => (
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="flex items-end gap-6 pb-1">
                                    <div className="flex items-center gap-2">
                                        <Switch id="is_remote" checked={data.is_remote} onCheckedChange={v => setData('is_remote', v)} />
                                        <Label htmlFor="is_remote">Remote</Label>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <Switch id="is_internal" checked={data.is_internal} onCheckedChange={v => setData('is_internal', v)} />
                                        <Label htmlFor="is_internal">Internal Only</Label>
                                    </div>
                                </div>
                            </div>
                        </CardContent>
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            <div>
                                ...
                                <Textarea id="summary" value={data.summary} onChange={e => setData('summary', e.target.value)} rows={2} className="mt-1" placeholder="Brief overview shown on listing cards..." />
                                <p className="text-xs text-muted-foreground mt-1">{data.summary.length}/1000</p>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <Textarea id="responsibilities" value={data.responsibilities} onChange={e => setData('responsibilities', e.target.value)} rows={5} className="mt-1" />
                            </div>
                            <div>
                                <Label htmlFor="requirements">Requirements</Label>
                                <Textarea id="requirements" value={data.requirements} onChange={e => setData('requirements', e.target.value)} rows={5} className="mt-1" />
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        <CardHeader>
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <CardDescription>Optional questions candidates must answer when applying</CardDescription>
                                </div>
                        ...
                        <CardContent>
                            {data.screening_questions.length === 0 ? (
                                <p className="text-sm text-muted-foreground text-center py-4">No screening questions added yet.</p>
                            ) : (
                                            ...
                                            <div className="grid grid-cols-3 gap-3">
                                                <div>
                                                    ...
                                                    <Select value={q.type} onValueChange={v => updateQuestion(qi, 'type', v)}>
                                                        <SelectTrigger className="mt-1"><SelectValue /></SelectTrigger>
                                                        <SelectContent>
                                                            {Object.entries(questionTypeLabels).map(([k, l]) => (
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle>Compensation, Closing & Notifications</CardTitle>
                            <CardDescription>Salary, closing date, approval, and notification settings</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <Label htmlFor="salary_range_min">Salary Range Min (NZD)</Label>
                                    <Input id="salary_range_min" type="number" step="0.01" value={data.salary_range_min} onChange={e => setData('salary_range_min', e.target.value)} className="mt-1" />
                                </div>
                                <div>
                                    <Label htmlFor="salary_range_max">Salary Range Max (NZD)</Label>
                                    <Input id="salary_range_max" type="number" step="0.01" value={data.salary_range_max} onChange={e => setData('salary_range_max', e.target.value)} className="mt-1" />
                                </div>
                            </div>

                            <div className="flex items-center gap-6">
                                <div className="flex items-center gap-2">
                                    <Switch id="show_salary" checked={data.show_salary} onCheckedChange={v => setData('show_salary', v)} />
                                    <Label htmlFor="show_salary">Show salary on public listing</Label>
                                </div>
                                <div className="flex items-center gap-2">
                                    <Switch id="requires_approval" checked={data.requires_approval} onCheckedChange={v => setData('requires_approval', v)} />
                                    <Label htmlFor="requires_approval">Require approval before publishing</Label>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <Input id="closes_at" type="date" value={data.closes_at} onChange={e => setData('closes_at', e.target.value)} className="mt-1" />
                                </div>
                                <div>
                                    <Label htmlFor="hiring_manager_id">Hiring Manager</Label>
                                    <Select value={data.hiring_manager_id} onValueChange={v => setData('hiring_manager_id', v)}>
                                        <SelectTrigger className="mt-1"><SelectValue placeholder="Select hiring manager..." /></SelectTrigger>
                                        <SelectContent>
                                            {users.map(u => (
                            ...
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground mb-2">These email addresses will be notified when a candidate applies</p>
                                ...
                                {errors.notification_emails && <p className="text-sm text-destructive mt-1">{errors.notification_emails}</p>}
                            </div>
                        </CardContent>
                    </Card>
