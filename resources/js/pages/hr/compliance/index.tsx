...
export default function ComplianceOverview({ staffStatuses, summary, requirements, hero, wizard, filters, can }: Props) {
    ...
    const pct = summary.total_staff > 0 ? Math.round((summary.fully_compliant / summary.total_staff) * 100) : 0;
    ...
    useEffect(() => {
        const t = setTimeout(() => {
            ...
            router.get('/hr/compliance', { ...cleanFilters(filters), q: search || undefined }, { preserveState: true, preserveScroll: true, replace: true });
        }, 350);
    ...
    const applyFilter = (key: string, value: string | undefined) => {
        router.get('/hr/compliance', { ...cleanFilters(filters), [key]: value || undefined }, { preserveState: true, preserveScroll: true, replace: true });
    };

    const toggleRow = (id: number) => setSelected((s) => ({ ...s, [id]: !s[id] }));
    const allSelected = staffStatuses.data.length > 0 && staffStatuses.data.every((r) => selected[r.user_id]);
    const toggleAll = () =>
        setSelected(allSelected ? {} : Object.fromEntries(staffStatuses.data.map((r) => [r.user_id, true])));
    const clearSelection = () => setSelected({});

    const sendReminder = useCallback((userIds: number[], label: string) => {
        router.post('/hr/compliance/bulk-remind', { user_ids: userIds }, {
            preserveScroll: true,
    ...
    const rowMenu = (r: StaffRow): CtxItem[] => [
        { icon: ShieldCheck, label: 'Open', onClick: () => router.visit(`/hr/compliance/staff/${r.user_id}`) },
        ...(can.manage
            ? [
                  { icon: ClipboardCheck, label: 'Record compliance', kbd: 'R', onClick: () => setWz({ type: 'record', preset: { person: String(r.user_id) } }) },
                  { icon: Ban, label: 'Waive a requirement', onClick: () => setWz({ type: 'waive', preset: { person: String(r.user_id) } }) },
                  { icon: Bell, label: 'Send reminder', onClick: () => sendReminder([r.user_id], `Reminder sent to ${r.user_name}.`) },
              ]
            : []),
        ...(can.vetting ? [{ icon: UserCheck, label: 'View vetting', onClick: () => router.visit('/hr/compliance/vetting') }] : []),
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="space-y-4 px-4 py-4 lg:px-6">
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
                    <StatTile label="Fully compliant" value={`${pct}%`} icon={CheckCircle2} tone="success" subtitle={`${summary.fully_compliant} of ${summary.total_staff}`} />
                    <StatTile label="Expiring" value={summary.expiring_total} icon={Clock} tone="warning" subtitle="next 30 days" />
                    <StatTile label="Expired" value={summary.expired_total} icon={AlertTriangle} tone="critical" subtitle={`${summary.has_expired} staff`} />
                    <StatTile label="Hard-stops" value={summary.hard_stops} icon={Ban} tone="critical" subtitle="blocked from shifts" />
                    <StatTile label="Shifts affected" value={summary.shifts_affected} icon={Car} tone="info" subtitle="upcoming" />
                </div>
                ...
                <GuardrailCard unstyled className="flex flex-wrap items-center gap-2.5 rounded-xl border border-border bg-card p-2.5">
                    ...
                    <Select value={filters.status || 'all'} onValueChange={(v) => applyFilter('status', v === 'all' ? undefined : v)}>
                        ...
                        <SelectContent>
                            ...
                            <SelectItem value="has_expiring">Expiring</SelectItem>
                            <SelectItem value="incomplete">Incomplete</SelectItem>
                        </SelectContent>
                    </Select>
                    <Select value={filters.requirement_id || 'all'} onValueChange={(v) => applyFilter('requirement_id', v === 'all' ? undefined : v)}>
                        <SelectTrigger className="h-9 w-[180px]" aria-label="Requirement filter">
                ...
                {selectedIds.length > 0 && (
                    <div className="flex flex-wrap items-center gap-3 rounded-xl border border-primary/30 bg-accent p-2.5 motion-safe:animate-in motion-safe:zoom-in-95">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {can.manage && (
                                <>
                                    <BulkBtn onClick={() => setWz({ type: 'record', userIds: selectedIds })}>Record compliance</BulkBtn>
                                    <BulkBtn onClick={() => sendReminder(selectedIds, `Reminders sent to ${selectedIds.length} staff.`)}>Send reminders</BulkBtn>
                                    <BulkBtn onClick={() => setWz({ type: 'waive', userIds: selectedIds })}>Waive…</BulkBtn>
                                </>
                            )}
                ...
                <GuardrailCard unstyled className="overflow-hidden rounded-xl border border-border bg-card">
                            ...
                            {staffStatuses.data.length === 0 ? (
                                ...
                                staffStatuses.data.map((r) => {
                                    ...
                                    return (
                                        <tr
                                            key={r.user_id}
                                            onContextMenu={(e) => openCtx(e, rowMenu(r))}
                                            className={`border-b border-border transition-colors last:border-0 hover:bg-muted/60 ${sel ? 'bg-accent/50' : ''}`}
                                        >
                                            <td className="px-3 py-2.5">
                                                <Checkbox checked={sel} onChange={() => toggleRow(r.user_id)} />
                                            </td>
                                            <td className="px-3 py-2.5">
                                                <div className="flex items-center gap-2.5">
                                                    <AvatarBubble name={r.user_name} />
                                                    <div className="min-w-0">
                                                        <Link href={`/hr/compliance/staff/${r.user_id}`} className="font-semibold text-primary hover:underline">
                                                            {r.user_name}
                                                        </Link>
                                                        <div className="truncate text-[11px] text-muted-foreground">{r.user_email}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-3 py-2.5">
                                            ...
                                            <td className="px-3 py-2.5">
                                                <VettingChip status={r.vetting_status} />
                                            </td>
                                            <td className="px-3 py-2.5">
                                                <DriverChip status={r.driver_status} />
                                            </td>
                                            <td className="px-3 py-2.5">
                                                {r.future_shifts_affected ? (
                                                    <StatusBadge variant="critical">
                                                        {r.future_shifts_affected} shift{r.future_shifts_affected > 1 ? 's' : ''}
                                                    </StatusBadge>
                                                ) : (
                                                    <span className="text-muted-foreground">—</span>
                                                )}
                    ...
                    <div className="flex items-center justify-between border-t border-border px-4 py-3 text-[12.5px] text-muted-foreground">
                        <span>
                            Showing {staffStatuses.from ?? 0}–{staffStatuses.to ?? 0} of {staffStatuses.total} staff
                        </span>
