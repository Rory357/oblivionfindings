...
import {
    ...
    HeroInitials,
    HeroSolidButton,
} from './components/compliance-hero';
import {
    ComplianceWizards,
    type ReqOption,
    type RoleOption,
...
export default function StaffDetail({
    ...
    const record = (preset: Record<string, unknown> = {}) =>
        setWz({
            type: 'record',
            preset: { person: String(staff.id), ...preset },
        });
    const waive = (preset: Record<string, unknown> = {}) =>
        setWz({
            type: 'waive',
            preset: { person: String(staff.id), ...preset },
        });
    const remind = () =>
        router.post(
            ...
            { user_ids: [staff.id] },
            {
                preserveScroll: true,
                onSuccess: () =>
                    toast.success(`Reminder sent to ${staff.name}.`),
            },
        );
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="space-y-4 px-4 py-4 lg:px-6">
                <Link
                    href="/hr/compliance"
                    className="inline-flex w-max items-center gap-1.5 text-[12.5px] font-semibold text-muted-foreground hover:text-foreground"
                >
                ...
                <CompactHeroBand>
                    ...
                    <div className="flex items-center gap-5">
                        <div className="text-center">
                            ...
                            <div className="text-[10px] font-bold tracking-wide text-primary-foreground/60 uppercase">
                                Compliant
                            </div>
                        </div>
                        {can.manage && (
                            <div className="flex flex-col gap-2">
                                ...
                                <div className="flex gap-2">
                                    <HeroGhostButton onClick={() => waive()}>
                                        Waive
                                    </HeroGhostButton>
                                    <HeroGhostButton onClick={remind}>
                                        Remind
                                    </HeroGhostButton>
                                </div>
                            </div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 rounded-xl border border-status-critical/35 bg-status-critical-bg p-3.5">
                        ...
                        <div className="flex-1">
                            <div className="font-bold text-status-critical-foreground">
                                Blocked from shifts
                                {futureShiftsAffected > 0
                                    ? ` — ${futureShiftsAffected} upcoming affected`
                                    : ''}
                            </div>
                            <div className="text-xs text-status-critical-foreground">
                                Expired hard-stop requirement(s):{' '}
                                {hardStopFailures
                                    .map((f) => f.requirement)
                                    .join(', ')}
                            </div>
                        ...
                        {can.manage && (
                            <div className="flex gap-2">
                                <GuardrailButton
                                    unstyled
                                    onClick={() => record()}
                                    className="rounded-lg bg-status-critical px-3 py-1.5 text-[12.5px] font-semibold text-white"
                                >
                                    Resolve
                                </GuardrailButton>
                                <GuardrailButton
                                    unstyled
                                    onClick={() => waive()}
                                    className="rounded-lg border border-border bg-card px-3 py-1.5 text-[12.5px] font-semibold"
                                >
                                    Waive
                                </GuardrailButton>
                            </div>
                                ...
                                >
                                    ...
                                    {items.map((it) => (
                                        <div
                                            ...
                                            className="flex items-center gap-3 border-b border-border px-3.5 py-3 last:border-0 hover:bg-muted/60"
                                        >
                                            <div className="min-w-0 flex-1">
                                                ...
                                                <div className="text-[11.5px] text-muted-foreground">
                                                    {it.requirement_type.replace(
                                                        /_/g,
                                                        ' ',
                                                    )}
                                                    {it.status === 'expired' &&
                                                    it.expiry_date
                                                        ? ` · Expired ${fmtDate(it.expiry_date)}`
                                                        : null}
                                                    {it.status ===
                                                        'expiring_soon' &&
                                                    it.expiry_date
                                                        ? ` · Expires ${fmtDate(it.expiry_date)}`
                                                        : null}
                                                    {it.status ===
                                                        'compliant' &&
                                                    it.expiry_date
                                                        ? ` · Valid to ${fmtDate(it.expiry_date)}`
                                                        : null}
                    ...
                    <div className="flex flex-col gap-3.5">
                        <SidePanel
                            ...
                            onView={
                                can.vetting
                                    ? () =>
                                          router.visit('/hr/compliance/vetting')
                                    : undefined
                            }
                        >
                            {!can.vetting ? (
                                <p className="text-xs text-muted-foreground">
                                    Vetting access is required to view this
                                    panel.
                                </p>
                            ) : vetting ? (
                                <>
                                    ...
                                    <p className="mt-2 text-xs text-muted-foreground">
                                        ...
                                        {vetting.expires_at
                                            ? ` · valid to ${fmtDate(vetting.expires_at)}`
                                            : ''}
                                    </p>
                                </>
                            ) : (
                        ...
                        <SidePanel
                            ...
                            onView={
                                can.driver && driver
                                    ? () =>
                                          router.visit(
                                              `/hr/compliance/drivers/${driver.id}`,
                                          )
                                    : undefined
                            }
                        >
                            {!can.driver ? (
                                <p className="text-xs text-muted-foreground">
                                    Driver access is required to view this
                                    panel.
                                </p>
                            ) : driver ? (
                                <>
                                    ...
                                    <p className="mt-2 text-xs text-muted-foreground">
                                        {driver.licence_class
                                            ...
                                            : 'Licence'}
                                        {driver.expires_at
                                            ? ` · expires ${fmtDate(driver.expires_at)}`
                                            : ''}
                                    </p>
                                    {driver.at_risk_shifts.length > 0 && (
                                        <div className="mt-3 rounded-lg border border-status-critical/30 bg-status-critical/5 p-2.5">
                                            <p className="flex items-center gap-1.5 text-[11.5px] font-semibold text-status-critical">
                                                ...
                                                {driver.at_risk_shifts.length}{' '}
                                                rostered shift
                                                {driver.at_risk_shifts
                                                    .length === 1
                                                    ? ''
                                                    : 's'}{' '}
                                            ...
                                            <p className="mt-1.5 text-[11px] text-muted-foreground">
                                                Re-roster or renew the licence
                                                before these dates.
                                            </p>
                                        </div>
                                    )}
