...
export default function ComplianceRenewals({
    hero,
    events,
    wizard,
    can,
    ...
    const groups = useMemo(() => {
        ...
        return Array.from(map.entries()).sort(([a], [b]) => {
            ...
            return (
                new Date(map.get(a)![0].start).getTime() -
                new Date(map.get(b)![0].start).getTime()
            );
        });
    }, [filtered]);
    ...
    const snooze = (e: RenewalEvent) => {
        router.post(
            '/hr/compliance/renewals/snooze',
            { type: e.entity_type, id: e.entity_id, days: 7 },
            {
                preserveScroll: true,
    ...
    const recordRenewal = (e: RenewalEvent) =>
        setWz({
            type: 'record',
            preset: e.user_id ? { person: String(e.user_id) } : {},
        });
    ...
    const rowMenu = (e: RenewalEvent): CtxItem[] => [
        {
            ...
            label: 'Open person',
            onClick: () =>
                e.user_id && router.visit(`/hr/compliance/staff/${e.user_id}`),
        },
        ...(can.manage
            ? [
                  {
        ...
        ...(can.manage
            ? [{ icon: Ban, label: 'Waive', onClick: () => waive(e) }]
            : []),
        ...(can.manage
            ? [{ icon: Clock, label: 'Snooze', onClick: () => snooze(e) }]
            : []),
    ];
            ...
            <div className="space-y-4 px-4 py-4 lg:px-6">
                ...
                {groups.length === 0 ? (
                    ...
                    groups.map(([month, items]) => (
                        <div key={month}>
                            <div className="mt-1.5 mb-2 flex items-center gap-2.5 px-0.5">
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="text-right">
                                                ...
                                                <div
                                                    className={`text-[11px] font-semibold ${e.urgency === 'over' ? 'text-status-critical' : e.urgency === 'soon' ? 'text-status-warning' : 'text-muted-foreground'}`}
                                                >
                                                    {e.days}
                                                </div>
                                            </div>
