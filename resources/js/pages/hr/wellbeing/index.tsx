...
const createDefaultQuestions = (): QuestionDraft[] => ([
    ...
    { ...initialQuestion, question_type: 'text', question_text: 'What should we improve first?' },
]);
...
export default function WellbeingIndex({
    ...
    const [questions, setQuestions] = useState<QuestionDraft[]>(createDefaultQuestions());
    ...
    function startEditSurvey(survey: Survey) {
        ...
        const questionDrafts = survey.questions.length > 0
            ? survey.questions
    ...
    function submitSurvey(event: FormEvent<HTMLFormElement>) {
        ...
        const payloadQuestions = questions
            .map((question, index) => ({
                ...
                options: question.question_type === 'choice'
    ...
    function publishSurvey(id: number) {
        router.post(`/hr/wellbeing/surveys/${id}/publish`, {}, { preserveScroll: true });
    }

    function closeSurvey(id: number) {
        router.post(`/hr/wellbeing/surveys/${id}/close`, {}, { preserveScroll: true });
    }

    function applyActionPlanFilters(next: Partial<{ status: string; owner: string }>) {
        const status = next.status ?? filters.status;
        const owner = next.owner ?? (filters.owner ? String(filters.owner) : 'all');
    ...
    function updateActionPlanStatus(planId: number, status: 'in_progress' | 'completed') {
        const payload: { status: 'in_progress' | 'completed'; progress_percent?: number } = { status };
        if (status === 'completed') {
            payload.progress_percent = 100;
        }

        router.put(`/hr/wellbeing/action-plans/${planId}`, payload, { preserveScroll: true });
    }
        ...
        >
            ...
            <PageShell>
                ...
                <div className="grid gap-4 md:grid-cols-4">
                    <Card>
                        <CardHeader className="pb-3">
                            <CardTitle className="text-sm font-medium text-muted-foreground">Staff Assessed</CardTitle>
                        </CardHeader>
                        <CardContent className="text-2xl font-semibold">{wellbeingSummary.total_staff}</CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="pb-3">
                            <CardTitle className="text-sm font-medium text-muted-foreground">Red Flags</CardTitle>
                        </CardHeader>
                        <CardContent className="text-2xl font-semibold text-destructive">{wellbeingSummary.flagged_red}</CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="pb-3">
                    ...
                    <Card>
                        <CardHeader className="pb-3">
                            <CardTitle className="text-sm font-medium text-muted-foreground">Open Surveys</CardTitle>
                        </CardHeader>
                        <CardContent className="text-2xl font-semibold">{openSurveys}</CardContent>
                    </Card>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-6">
                    <Card>
                        <CardHeader className="pb-3">
                            <CardTitle className="text-sm font-medium text-muted-foreground">Open Plans</CardTitle>
                        </CardHeader>
                        <CardContent className="text-xl font-semibold">{slaSummary.open_total}</CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="pb-3">
                            <CardTitle className="text-sm font-medium text-muted-foreground">Overdue Plans</CardTitle>
                        </CardHeader>
                        <CardContent className="text-xl font-semibold text-destructive">{slaSummary.overdue}</CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="pb-3">
                            <CardTitle className="text-sm font-medium text-muted-foreground">Due Today</CardTitle>
                        </CardHeader>
                        <CardContent className="text-xl font-semibold">{slaSummary.due_today}</CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="pb-3">
                            <CardTitle className="text-sm font-medium text-muted-foreground">Due Next 7 Days</CardTitle>
                        </CardHeader>
                        <CardContent className="text-xl font-semibold">{slaSummary.due_next_7_days}</CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="pb-3">
                            <CardTitle className="text-sm font-medium text-muted-foreground">Avg Progress</CardTitle>
                        </CardHeader>
                        <CardContent className="text-xl font-semibold">{slaSummary.avg_progress_open}%</CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="pb-3">
                            <CardTitle className="text-sm font-medium text-muted-foreground">Completed (30d)</CardTitle>
                        </CardHeader>
                        <CardContent className="text-xl font-semibold">{slaSummary.completed_last_30_days}</CardContent>
                    </Card>
                </div>
                        ...
                        <CardHeader>
                            <div className="flex items-center justify-between">
                                <CardTitle className="flex items-center gap-2">
                                    ...
                                    {editingSurveyId ? 'Edit Engagement Survey' : 'Create Engagement Survey'}
                                </CardTitle>
                                {editingSurveyId && (
                                    <Button type="button" size="sm" variant="outline" onClick={resetSurveyComposer}>
                                        Cancel Edit
                                    </Button>
                                )}
                            </div>
                            ...
                            <form onSubmit={submitSurvey} className="space-y-4">
                                <div className="grid gap-4 md:grid-cols-2">
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            ...
                                            onChange={(event) => surveyForm.setData('title', event.target.value)}
                                            placeholder="Quarterly Pulse Check"
                                        />
                                        {surveyForm.errors.title && <p className="text-sm text-destructive">{surveyForm.errors.title}</p>}
                                    </div>
                                    <div className="space-y-2">
                                        <Label>Survey Type</Label>
                                        <Select
                                            value={surveyForm.data.survey_type}
                                            onValueChange={(value: 'pulse' | 'enps' | 'engagement') => surveyForm.setData('survey_type', value)}
                                        >
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Textarea
                                        ...
                                        onChange={(event) => surveyForm.setData('description', event.target.value)}
                                ...
                                <div className="grid gap-4 md:grid-cols-3">
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            ...
                                            onChange={(event) => surveyForm.setData('starts_at', event.target.value)}
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            ...
                                            onChange={(event) => surveyForm.setData('ends_at', event.target.value)}
                                        />
                                    </div>
                                    <div className="flex items-center gap-2 pt-8">
                                        <Checkbox
                                            ...
                                            onCheckedChange={(checked) => surveyForm.setData('is_anonymous', Boolean(checked))}
                                        />
                                        <Label htmlFor="anonymous" className="font-normal">Anonymous responses</Label>
                                    </div>
                                </div>
                                            ...
                                            <div className="grid gap-3 md:grid-cols-4">
                                                <div className="md:col-span-1">
                                                    <Select
                                                        ...
                                                        onValueChange={(value: QuestionDraft['question_type']) => updateQuestion(index, { question_type: value })}
                                                    >
                                                        <SelectTrigger><SelectValue /></SelectTrigger>
                                                        <SelectContent>
                                            ...
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-2">
                                                    <Checkbox
                                                        ...
                                                        onCheckedChange={(checked) => updateQuestion(index, { is_required: Boolean(checked) })}
                                                    />
                                                    <Label htmlFor={`required-${index}`} className="font-normal">Required</Label>
                                                </div>
                                                {questions.length > 1 && (
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                                ...
                                <div key={survey.id} className="rounded-lg border p-3">
                                    <div className="flex items-start justify-between gap-2">
                                        <div>
                                            ...
                                            <p className="text-xs text-muted-foreground">
                                                {survey.survey_type.toUpperCase()} - {survey.question_count} questions - {survey.response_count} responses
                                            </p>
                                        </div>
                                    ...
                                    <div className="mt-2 flex flex-wrap gap-2">
                                        <Button size="sm" variant="outline" asChild>
                                            <Link href={`/hr/wellbeing/surveys/${survey.id}`}>Open</Link>
                                        </Button>
                                        {can.manage && (
                                            <Button size="sm" variant="outline" onClick={() => startEditSurvey(survey)}>
                                                Edit
                                            </Button>
                                        )}
                                        {can.manage && survey.status === 'draft' && (
                                            <Button size="sm" onClick={() => publishSurvey(survey.id)}>Publish</Button>
                                        )}
                                        {can.manage && survey.status === 'published' && (
                                            <Button size="sm" variant="secondary" onClick={() => closeSurvey(survey.id)}>Close</Button>
                                        )}
                                    </div>
                                </div>
                            ))}
                            {surveys.length === 0 && <p className="text-sm text-muted-foreground">No surveys available.</p>}
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-3">
                            <div className="grid gap-2 md:grid-cols-2">
                                <div className="space-y-1">
                                    ...
                                    <Select
                                        ...
                                        onValueChange={(value) => applyActionPlanFilters({ status: value })}
                                    >
                                        <SelectTrigger>
                                            <SelectValue />
                                        </SelectTrigger>
                                ...
                                {can.manage && (
                                    <div className="space-y-1">
                                        <Label className="text-xs text-muted-foreground">Owner filter</Label>
                                        <Select
                                            value={filters.owner ? String(filters.owner) : 'all'}
                                            onValueChange={(value) => applyActionPlanFilters({ owner: value })}
                                        >
                                            <SelectTrigger>
                                                <SelectValue />
                                            </SelectTrigger>
                                ...
                                <div key={plan.id} className="rounded-lg border p-3">
                                    <div className="flex flex-wrap items-center justify-between gap-2">
                                        ...
                                        <div className="flex flex-wrap items-center gap-2">
                                            ...
                                            {!plan.is_overdue && plan.is_due_soon && plan.days_until_due !== 0 && (
                                                <Badge variant="secondary">Due in {plan.days_until_due}d</Badge>
                                            )}
                                        </div>
                                    </div>
                                    <p className="mt-1 text-xs text-muted-foreground">
                                        {plan.survey?.title ?? 'No survey'} - Owner: {plan.owner?.name ?? 'Unassigned'} - Progress: {plan.progress_percent}% - Due:{' '}
                                        {plan.due_date ?? 'Not set'}
                                    </p>
                                    {plan.can_update && (plan.status === 'open' || plan.status === 'in_progress') && (
                                        <div className="mt-2 flex flex-wrap gap-2">
                                            {plan.status === 'open' && (
                                                <Button
                                                    type="button"
                                                    size="sm"
                                                    variant="outline"
                                                    onClick={() => updateActionPlanStatus(plan.id, 'in_progress')}
                                                >
                                                    Mark in progress
                                                </Button>
                                            )}
                                            <Button
                                                type="button"
                                                size="sm"
                                                onClick={() => updateActionPlanStatus(plan.id, 'completed')}
                                            >
                                                Mark completed
                                            </Button>
                                        </div>
                            ...
                            {actionPlans.length === 0 && <p className="text-sm text-muted-foreground">No action plans yet.</p>}

                            {can.manage && ownerWorkload.length > 0 && (
                                <div className="rounded-lg border p-3">
                                    <p className="text-sm font-medium">Owner workload snapshot</p>
                                    <div className="mt-2 space-y-2">
                                        {ownerWorkload.slice(0, 5).map((row) => (
                                            <div key={row.owner_user_id} className="flex items-center justify-between text-xs text-muted-foreground">
                                                <span>{row.owner_name ?? `User #${row.owner_user_id}`}</span>
                                                <span>
                                                    Open {row.open_count} - Overdue {row.overdue_count} - Due soon {row.due_next_7_days}
                                                </span>
                                            </div>
                                        ))}
                                    </div>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-3">
                        {sortedFlaggedStaff.map((entry) => (
                            <div key={entry.user_id} className="rounded-lg border p-3">
                                <div className="flex items-center justify-between gap-2">
                                    ...
                                    <Badge variant={entry.flag_level === 'red' ? 'destructive' : 'secondary'}>{entry.flag_level.toUpperCase()}</Badge>
                                </div>
                                <p className="mt-1 text-xs text-muted-foreground">
                                    {entry.position_title ?? 'Role not set'} - OT {entry.metrics.overtime_hours}h - Consecutive days {entry.metrics.consecutive_days_worked}
                                </p>
                                {entry.triggered_rules.length > 0 && (
                                    <ul className="mt-2 list-disc space-y-1 pl-4 text-xs text-muted-foreground">
                                        {entry.triggered_rules.map((rule, index) => (
                        ...
                        {flaggedStaff.length === 0 && <p className="text-sm text-muted-foreground">No active wellbeing flags.</p>}
                    </CardContent>
                </Card>
            </PageShell>
        </AppLayout>
