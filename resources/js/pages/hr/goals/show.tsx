...
export default function GoalShow({
    ...
    const deleteKr = (id: number) => {
        if (confirm('Delete this key result?'))
            router.delete(`/hr/goals/key-results/${id}`, {
                preserveScroll: true,
            });
    };
    const archive = () => {
    ...
    const exportCsv = () => {
        window.location.href = goal.cycle_id
            ? `/hr/goals/export?cycle=${goal.cycle_id}`
            : '/hr/goals/export';
    };
    ...
    const subTabs: { key: SubTab; label: string; count: number }[] = [
        ...
        { key: 'children', label: 'Children', count: goal.child_goals.length },
        {
            key: 'plans',
            label: 'Linked plans',
            count: goal.development_goals.length,
        },
    ];
            ...
            <div className="mx-auto flex w-full max-w-[1100px] flex-col gap-4 p-6">
                ...
                <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
                    <div className="flex flex-wrap items-start gap-6">
                        <div
                            ...
                            style={{
                                background: `conic-gradient(${ragVar(goal.confidence)} ${goal.progress_percentage}%, var(--muted) 0)`,
                            }}
                        >
                            <div className="absolute inset-[9px] grid place-items-center rounded-full bg-card">
                                ...
                                <span className="text-[8.5px] font-bold tracking-wide text-muted-foreground uppercase">
                        ...
                        <div className="min-w-0 flex-1">
                            <div className="mb-2 flex flex-wrap items-center gap-2">
                                ...
                                <span
                                    className={cn(
                                        'inline-flex items-center rounded-md px-1.5 py-0.5 text-[10px] font-bold capitalize',
                                ...
                                <span
                                    className={cn(
                                        'inline-flex items-center gap-1.5 rounded-md px-1.5 py-0.5 text-[10px] font-bold',
                                        rag.chip,
                                    )}
                                >
                                    <span
                            ...
                            <div className="mt-3 flex flex-wrap items-center gap-4 text-[12px] text-muted-foreground">
                                ...
                                <span>
                                    · {formatDate(goal.start_date, true)} →{' '}
                                    {formatDate(goal.due_date, true)}
                                </span>
                            </div>
                            {goal.tags.length > 0 && (
                                <div className="mt-2.5 flex flex-wrap items-center gap-1.5">
                                    {goal.tags.map((t) => (
                                        <span
                                            key={t}
                                            className="inline-flex items-center rounded-md bg-muted px-1.5 py-0.5 text-[11px] font-semibold text-muted-foreground"
                                        >
                                            {t}
                                        </span>
                                    ))}
                    ...
                    <div className="mt-5 flex flex-wrap gap-2">
                        ...
                        {can.manage && (
                            <button
                                type="button"
                                onClick={archive}
                                className="inline-flex items-center gap-1.5 rounded-lg border border-status-critical/30 bg-card px-3 py-2 text-[12.5px] font-semibold text-status-critical hover:bg-status-critical-bg"
                            >
                ...
                <div className="inline-flex gap-1 self-start rounded-xl border border-border bg-card p-1.5 shadow-sm">
                    {subTabs.map((t) => (
                        ...
                        >
                            {t.label}
                            <span className="inline-flex items-center rounded-full bg-muted/70 px-1.5 text-[10px] font-bold tabular-nums">
                                {t.count}
                            </span>
                        </button>
                    ))}
                    ...
                    <div className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
                        {goal.key_results.length === 0 ? (
                            <p className="px-5 py-10 text-center text-[13px] text-muted-foreground">
                                No key results — this objective uses manual
                                progress.
                            </p>
                        ) : (
                            goal.key_results.map((k) => (
                                <div
                                    key={k.id}
                                    className="flex items-center gap-3.5 border-b border-border/60 px-4 py-3.5 last:border-0"
                                >
                                    <div className="min-w-0 flex-1">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="inline-flex items-center rounded-md bg-muted px-1.5 py-0.5 text-[10px] font-bold text-muted-foreground capitalize">
                                                {k.kr_type}
                                            </span>
                                    ...
                                    {can.updateProgress && (
                                        <button
                                            type="button"
                                            onClick={() => setCheckinOpen(true)}
                                            className="rounded-md border border-border bg-card px-2.5 py-1.5 text-[11px] font-semibold hover:bg-muted"
                                        >
                                            Check in
                                        </button>
                                    )}
                                    {can.manage && (
                                        <button
                                            type="button"
                                            onClick={() => deleteKr(k.id)}
                                            aria-label="Delete key result"
                                            className="grid h-7 w-7 place-items-center rounded-md border border-border bg-card text-muted-foreground hover:bg-muted"
                                        >
                        ...
                        {can.manage && (
                            <button
                                type="button"
                                onClick={() => setKrDialog(true)}
                                className="flex w-full items-center gap-2 px-4 py-3.5 text-[12.5px] font-semibold text-primary hover:bg-muted/40"
                            >
                    ...
                    <div className="rounded-xl border border-border bg-card px-5 py-2 shadow-sm">
                        {goal.updates.length === 0 ? (
                            ...
                            goal.updates.map((h) => (
                                <div
                                    key={h.id}
                                    className="flex gap-3.5 border-b border-border/60 py-3.5 last:border-0"
                                >
                                    <Avatar
                                        name={h.user_name}
                                ...
                                >
                                    <div className="mb-2 flex items-center gap-2">
                                        ...
                                        <span
                                            className={cn(
                                                'h-1.5 w-1.5 rounded-full',
                                                RAG[c.confidence].dot,
                                            )}
                                        />
                                    </div>
                                    <div className="mb-2.5 text-[13.5px] leading-snug font-semibold">
                                        {c.title}
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <ProgressBar
...
function AddKrDialog({
    ...
    const submit = () => {
        ...
        form.post(`/hr/goals/${goalId}/key-results`, {
            preserveScroll: true,
            onSuccess: onClose,
        });
    };
        ...
        >
            ...
            >
                <h3 className="flex items-center gap-2 text-base font-bold">
                    <ListChecks className="h-4 w-4 text-primary" /> Add key
                    result
                </h3>
                <div className="mt-4 flex flex-col gap-3">
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                        <Select
                            value={form.data.kr_type}
                            onValueChange={(v) => form.setData('kr_type', v)}
                        >
                            <SelectTrigger>
                                <SelectValue />
                            </SelectTrigger>
                        ...
                        <Input
                            value={form.data.start_value}
                            onChange={(e) =>
                                form.setData('start_value', e.target.value)
                            }
                        ...
                        <Select
                            ...
                            onValueChange={(v) => form.setData('owner_id', v)}
                        >
                            <SelectTrigger>
