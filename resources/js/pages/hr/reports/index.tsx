...
import { BarChart3, Users, TrendingDown, ShieldCheck, CalendarDays, GraduationCap, Download, Clock3 } from 'lucide-react';
...
export default function ReportsIndex({ availableReports, recentExports, subscriptions, recipientOptions, defaultFilters, can }: Props) {
    ...
    const buildDefaultSubscription = () => ({
        ...
        recipient_user_id: recipientOptions[0] ? String(recipientOptions[0].id) : '',
        date_from: defaultFilters.date_from || '',
        date_to: defaultFilters.date_to || '',
    });

    const [newSubscription, setNewSubscription] = useState(buildDefaultSubscription);
    ...
    }, [newSubscription.cadence, newSubscription.day_of_week, newSubscription.day_of_month]);
    ...
    function submitSubscription(e: React.FormEvent) {
        ...
        const payload = {
            ...
            day_of_week: newSubscription.cadence === 'weekly' ? Number(newSubscription.day_of_week) : null,
            day_of_month: newSubscription.cadence === 'monthly' ? Number(newSubscription.day_of_month) : null,
            run_at: newSubscription.run_at,
            timezone: newSubscription.timezone || 'Pacific/Auckland',
            recipient_user_ids: newSubscription.recipient_user_id ? [Number(newSubscription.recipient_user_id)] : [],
        ...
        if (editingSubscriptionId) {
            router.put(`/hr/reports/subscriptions/${editingSubscriptionId}`, payload, {
                preserveScroll: true,
                onSuccess,
            });
            return;
    ...
    function toggleSubscription(id: number) {
        router.post(`/hr/reports/subscriptions/${id}/toggle-active`, {}, { preserveScroll: true });
    }
    ...
    function startEditSubscription(subscription: ReportSubscription) {
        ...
        setNewSubscription({
            ...
            recipient_user_id: subscription.recipient_user_ids[0] ? String(subscription.recipient_user_ids[0]) : '',
            date_from: subscription.filters.date_from || '',
            date_to: subscription.filters.date_to || '',
        });
    }

    function subscriptionScheduleLabel(subscription: ReportSubscription): string {
        if (subscription.cadence === 'daily') {
            return `Daily at ${subscription.run_at}`;
        }
        if (subscription.cadence === 'weekly') {
            const label = subscription.day_of_week !== null ? weekdayLabels[subscription.day_of_week] : 'Weekly';
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-col gap-6 p-6">
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="text-base">Report Date Range</CardTitle>
                    </CardHeader>
                    <CardContent className="grid gap-3 md:grid-cols-3">
                        ...
                        <div className="flex items-end text-xs text-muted-foreground">
                            These filters apply to quick generate/export actions below.
                        </div>
                    </CardContent>
                </Card>
                            ...
                            return (
                                <Card key={report.key} className="transition-shadow hover:shadow-md">
                                    ...
                                    <CardContent>
                                        ...
                                        <div className="flex items-center gap-2">
                                            <Button variant="outline" size="sm" onClick={() => generateReport(report.key)}>
                                                Generate Report
                                            </Button>
                                            {can.export_data && (
                                                <Button variant="ghost" size="sm" asChild>
                                                    <a href={`/hr/reports/export?${buildFilterParams(report.key)}`}>
                                ...
                                <tbody className="divide-y">
                                    {recentExports.map((report) => (
                                        <tr key={report.id} className="hover:bg-muted/30">
                                            ...
                                            <td className="px-4 py-3 text-muted-foreground">{report.row_count}</td>
                                            <td className="px-4 py-3 text-muted-foreground">
                                                {report.generated_at || '\u2014'}
                                            ...
                                            <td className="px-4 py-3 text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    <Button variant="ghost" size="sm" asChild>
                                                        <Link href={`/hr/reports/exports/${report.id}`}>View</Link>
                                                    </Button>
                                                    {can.export_data && (
                                                        <Button variant="outline" size="sm" asChild>
                                                            <a href={`/hr/reports/exports/${report.id}/download`} target="_blank" rel="noreferrer">
                                                                <Download className="mr-1 h-3 w-3" />
                                                                Download
                                                            </a>
                                                        </Button>
                    ...
                    <>
                        <div>
                            ...
                            <Card>
                                <CardHeader>
                                    <div className="flex items-center justify-between">
                                        <CardTitle className="text-base">{editingSubscriptionId ? 'Edit Schedule' : 'Create Schedule'}</CardTitle>
                                ...
                                <CardContent>
                                    <form className="grid gap-4 md:grid-cols-4" onSubmit={submitSubscription}>
                                        <div className="space-y-2">
                                            <Label>Report</Label>
                                            <Select
                                                value={newSubscription.report_type}
                                                onValueChange={(value) => setNewSubscription((prev) => ({ ...prev, report_type: value }))}
                                            >
                                                <SelectTrigger><SelectValue /></SelectTrigger>
                                                <SelectContent>
                                                    {availableReports.map((report) => (
                                                        <SelectItem key={report.key} value={report.key}>{report.title}</SelectItem>
                                                    ))}
                                                </SelectContent>
                                            </Select>
                                        </div>
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Select
                                                ...
                                                onValueChange={(value) => setNewSubscription((prev) => ({ ...prev, cadence: value as 'daily' | 'weekly' | 'monthly' }))}
                                            >
                                                <SelectTrigger><SelectValue /></SelectTrigger>
                                                <SelectContent>
                                        ...
                                        {newSubscription.cadence === 'weekly' && (
                                            <div className="space-y-2">
                                                <Label>Weekday</Label>
                                                <Select
                                                    value={newSubscription.day_of_week}
                                                    onValueChange={(value) => setNewSubscription((prev) => ({ ...prev, day_of_week: value }))}
                                                >
                                                    <SelectTrigger><SelectValue /></SelectTrigger>
                                                    <SelectContent>
                                        ...
                                        {newSubscription.cadence === 'monthly' && (
                                            <div className="space-y-2">
                                                ...
                                                <Input
                                                    ...
                                                    min={1}
                                                    max={28}
                                                    value={newSubscription.day_of_month}
                                                    onChange={(event) => setNewSubscription((prev) => ({ ...prev, day_of_month: event.target.value }))}
                                                />
                                            </div>
                                        )}
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Input
                                                ...
                                                onChange={(event) => setNewSubscription((prev) => ({ ...prev, run_at: event.target.value }))}
                                            />
                                        </div>
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Input
                                                ...
                                                onChange={(event) => setNewSubscription((prev) => ({ ...prev, timezone: event.target.value }))}
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Select
                                                value={newSubscription.recipient_user_id || '__none__'}
                                                onValueChange={(value) => setNewSubscription((prev) => ({ ...prev, recipient_user_id: value === '__none__' ? '' : value }))}
                                            >
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Input
                                                type="date"
                                                value={newSubscription.date_from}
                                                onChange={(event) => setNewSubscription((prev) => ({ ...prev, date_from: event.target.value }))}
                                            />
                                        </div>
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Input
                                                ...
                                                onChange={(event) => setNewSubscription((prev) => ({ ...prev, date_to: event.target.value }))}
                                            />
                                        </div>

                                        <div className="flex items-end">
                                            <Button type="submit">{editingSubscriptionId ? 'Update Schedule' : 'Create Schedule'}</Button>
                                        </div>
                                    </form>
                                    <p className="mt-3 text-xs text-muted-foreground">{cadenceHint}</p>
                                </CardContent>
                            </Card>
                        </div>
                                    ...
                                    <tbody className="divide-y">
                                        {subscriptions.map((subscription) => (
                                            <tr key={subscription.id} className="hover:bg-muted/30">
                                                <td className="px-4 py-3 font-medium capitalize">
                                                    {subscription.report_type.replace(/_/g, ' ')}
                                                </td>
                                                <td className="px-4 py-3 text-muted-foreground">
                                                    <div className="flex items-center gap-1.5">
                                                        <Clock3 className="h-3.5 w-3.5" />
                                                        {subscriptionScheduleLabel(subscription)} ({subscription.timezone})
                                                    </div>
                                                </td>
                                                <td className="px-4 py-3 text-muted-foreground">
                                                    {subscription.recipient_names.length > 0 ? subscription.recipient_names.join(', ') : '\u2014'}
                                                </td>
                                                ...
                                                <td className="px-4 py-3 text-muted-foreground">
                                                    ...
                                                    {subscription.last_run_at ? ` (last: ${subscription.last_run_at})` : ''}
                                                </td>
                                                <td className="px-4 py-3 text-right">
                                                    <div className="flex items-center justify-end gap-2">
                                                        <Button
