...
export default function HrAutomationsPage({
    ...
    const hydrateFormFromRule = (rule: RuleRow) => {
        const equalsConditions = (rule.conditions as { equals?: Record<string, string> })?.equals ?? {};
        ...
        const actionType = String(action?.type ?? actionOptions[0]?.value ?? 'notify_role_group');
        const recipientIdsRaw = actionType === 'queue_report_export'
    ...
    const toggleRule = (id: number) => {
        router.post(`/hr/reports/automations/${id}/toggle-active`, {}, { preserveScroll: true });
    };
    ...
    const removeRecipient = (id: number) => {
        setData('recipient_user_ids', data.recipient_user_ids.filter((userId) => userId !== id));
    };
            ...
            <div className="flex flex-col gap-6 p-6">
                ...
                {can.manage && (
                    <Card>
                        <CardHeader>
                            <div className="flex items-center justify-between">
                                <CardTitle className="text-base">{editingRuleId ? 'Edit Automation Rule' : 'Create Automation Rule'}</CardTitle>
                                {editingRuleId && (
                                    <Button
                                        type="button"
                                        variant="outline"
                        ...
                        <CardContent>
                            <form className="grid gap-4 md:grid-cols-2" onSubmit={submit}>
                                    ...
                                    <Select value={data.event_type} onValueChange={(value) => setData('event_type', value)}>
                                        ...
                                        <SelectContent>
                                            {eventOptions.map((eventOption) => (
                                                <SelectItem key={eventOption.value} value={eventOption.value}>{eventOption.label}</SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input value={data.condition_field} onChange={(e) => setData('condition_field', e.target.value)} placeholder="leave_type" />
                                </div>
                                    ...
                                    <Select value={data.action_type} onValueChange={(value) => setData('action_type', value)}>
                                        ...
                                        <SelectContent>
                                            {actionOptions.map((actionOption) => (
                                                <SelectItem key={actionOption.value} value={actionOption.value}>{actionOption.label}</SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input value={data.action_title} onChange={(e) => setData('action_title', e.target.value)} placeholder="Leave request approved" />
                                </div>

                                <div className="space-y-2 md:col-span-2">
                                    <Label>Action Body</Label>
                                    <Input value={data.action_body} onChange={(e) => setData('action_body', e.target.value)} placeholder="Automation rule fired for leave approval." />
                                </div>

                                <div className="space-y-2 md:col-span-2">
                                    <Label>Action URL (optional)</Label>
                                    <Input value={data.action_url} onChange={(e) => setData('action_url', e.target.value)} placeholder="/hr/leave" />
                                </div>

                                {data.action_type === 'notify_role_group' && (
                                    <div className="space-y-2">
                                        <Label>Role Group</Label>
                                        <Select value={data.role_group} onValueChange={(value) => setData('role_group', value)}>
                                            <SelectTrigger><SelectValue /></SelectTrigger>
                                            <SelectContent>
                                ...
                                {(data.action_type === 'notify_users' || data.action_type === 'queue_report_export') && (
                                    <div className="space-y-2 md:col-span-2">
                                        <Label>Recipients</Label>
                                        <Select value="__pick__" onValueChange={updateRecipients}>
                                            <SelectTrigger><SelectValue placeholder="Add recipient" /></SelectTrigger>
                                            <SelectContent>
                                                <SelectItem value="__pick__">Pick recipient</SelectItem>
                                        ...
                                        {data.recipient_user_ids.length > 0 && (
                                            <div className="flex flex-wrap gap-2 pt-1">
                                                {data.recipient_user_ids.map((id) => {
                                                    const recipient = recipientOptions.find((option) => option.id === id);
                                                    return (
                                                        <Badge key={id} variant="outline" className="cursor-pointer" onClick={() => removeRecipient(id)}>
                                                            {recipient?.name ?? `User ${id}`} x
                                                        </Badge>
                                                    );
                                                })}
                                            </div>
                                ...
                                {data.action_type === 'queue_report_export' && (
                                    <>
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Input type="date" value={data.report_date_from} onChange={(e) => setData('report_date_from', e.target.value)} />
                                        </div>
                                        <div className="space-y-2">
                                ...
                                <div className="md:col-span-2 flex flex-wrap items-center gap-6 rounded-md border p-3">
                                    <label className="flex items-center gap-2 text-sm">
                                        <Checkbox checked={data.is_active} onCheckedChange={(checked) => setData('is_active', Boolean(checked))} />
                                        <span>Active immediately</span>
                                    </label>
                                    <label className="flex items-center gap-2 text-sm">
                                        <Checkbox checked={data.stop_on_match} onCheckedChange={(checked) => setData('stop_on_match', Boolean(checked))} />
                                        <span>Stop processing after this rule matches</span>
                                    </label>
                                </div>
                    ...
                    <CardContent className="p-0">
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y">
                                {rules.map((rule) => (
                                    <tr key={rule.id} className="hover:bg-muted/30">
                                        ...
                                        <td className="px-4 py-3 text-muted-foreground">
                                            {rule.last_ran_at || '-'}
                                            {rule.last_status && (
                                                <Badge variant="outline" className={`ml-2 ${statusClass[rule.last_status] || statusClass.skipped}`}>
                                                    {rule.last_status}
                                                </Badge>
                                            )}
                                        </td>
                                        <td className="px-4 py-3 text-muted-foreground">
                                            {rule.runs_count} runs, {rule.failed_runs_count} failed
                                        </td>
                                ...
                                {rules.length === 0 && (
                                    <tr>
                                        <td colSpan={5} className="px-4 py-8 text-center text-muted-foreground">No automation rules configured.</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
                            ...
                            <tbody className="divide-y">
                                {recentRuns.map((run) => (
                                    <tr key={run.id} className="hover:bg-muted/30">
                                        <td className="px-4 py-3 font-medium">{run.rule_name || `Rule #${run.rule_id}`}</td>
                                        <td className="px-4 py-3 text-muted-foreground">{run.event_type}</td>
                                        <td className="px-4 py-3">
                                            <Badge variant="outline" className={statusClass[run.status] || statusClass.skipped}>{run.status}</Badge>
                                        </td>
                                        <td className="px-4 py-3 text-muted-foreground">{run.message || '-'}</td>
                                        <td className="px-4 py-3 text-muted-foreground">{run.executed_at || '-'}</td>
                                    </tr>
                                ))}
                                {recentRuns.length === 0 && (
                                    <tr>
                                        <td colSpan={5} className="px-4 py-8 text-center text-muted-foreground">No automation runs recorded yet.</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
