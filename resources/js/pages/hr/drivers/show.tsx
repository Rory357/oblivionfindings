...
import {
    ...
    HeroInitials,
    HeroSolidButton,
} from '@/pages/hr/compliance/components/compliance-hero';
...
export default function DriverShow({ driver, history, can }: Props) {
    ...
    const approve = () =>
        router.post(
            `/hr/compliance/drivers/${driver.id}/approve`,
            {},
            {
                preserveScroll: true,
    ...
    const doSuspend = () => {
        router.post(
            `/hr/compliance/drivers/${driver.id}/suspend`,
    ...
    const facts: { label: string; value: string }[] = [
        ...
        {
            label: 'Class',
            value: driver.licence_class ? `Class ${driver.licence_class}` : '—',
        },
        {
            label: 'Endorsements',
            value: driver.licence_endorsements.length
                ? driver.licence_endorsements.join(', ')
                : 'None',
        },
        { label: 'Expiry', value: fmtDate(driver.licence_expires_at) },
        {
            label: 'Incident-free since',
            value: fmtDate(driver.incident_free_since),
        },
        {
            label: 'Can drive clients',
            value: driver.can_drive_clients ? 'Yes' : 'No',
        },
            ...
            <AlertDialog open={suspendOpen} onOpenChange={setSuspendOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Record a reason. This removes client-transport
                            eligibility.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <Textarea
