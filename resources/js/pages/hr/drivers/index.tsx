...
import {
    ComplianceHubHeader,
    type HeroPayload,
} from '@/pages/hr/compliance/components/compliance-hub-header';
import {
    ComplianceWizards,
    type ReqOption,
    type RoleOption,
    type WizardState,
} from '@/pages/hr/compliance/components/compliance-wizards';
...
export default function DriversIndex({
    ...
    useEffect(() => {
        const t = setTimeout(() => {
            ...
            router.get(
                ...
                { q: search || undefined, status: filters.status || undefined },
    ...
    const approve = (d: DriverRecord) =>
        router.post(
            `/hr/compliance/drivers/${d.id}/approve`,
            {},
            {
                preserveScroll: true,
                onSuccess: () => toast.success(`${d.user?.name} approved.`),
    ...
    const doSuspend = () => {
        if (!suspend) return;
        router.post(
            `/hr/compliance/drivers/${suspend.id}/suspend`,
    ...
    const rowMenu = (d: DriverRecord): CtxItem[] => [
        {
            icon: FileText,
            label: 'Open',
            onClick: () => router.visit(`/hr/compliance/drivers/${d.id}`),
        },
        ...(can.manage
            ? [
                  {
                      ...
                      label: 'Edit',
                      onClick: () =>
                          router.visit(`/hr/compliance/drivers/${d.id}`),
                  },
                  {
                      icon: CheckCircle2,
                      label: 'Approve',
            ...
            <div className="space-y-4 px-4 py-4 lg:px-6">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <p className="text-[12.5px] text-muted-foreground">
                        NZTA licence classes &amp; endorsements ·
                                        ...
                                        >
                                            <td className="px-3 py-2.5">
                                                <div className="flex items-center gap-2.5">
                                                    ...
                                                    />
                                                    <Link
                                                        href={`/hr/compliance/drivers/${d.id}`}
                                            ...
                                            <td className="px-3 py-2.5">
                                                {d.licence_endorsements &&
                                                d.licence_endorsements.length >
                                                    0 ? (
                                                    <span className="flex flex-wrap gap-1">
                                                        {d.licence_endorsements.map(
                                                            (e) => (
                                                                <span
                                                                    key={e}
                                                                    className="inline-flex h-[22px] min-w-[22px] items-center justify-center rounded-md bg-accent px-1.5 text-[11px] font-bold text-primary"
                                                                >
                                                                    {e}
                                                                </span>
                                                            ),
            ...
            >
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Record a reason. This removes the driver's
                            client-transport eligibility.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <Textarea
                        value={reason}
