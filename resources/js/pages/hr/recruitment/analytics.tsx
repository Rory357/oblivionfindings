...
function EmptyState({ message }: { message: string }) {
    return <div className="flex items-center justify-center h-48 text-sm text-muted-foreground">{message}</div>;
}

export default function RecruitmentAnalytics({ timeToHire, sourceEffectiveness, pipelineConversion, openPositions, hiringVelocity, stageBottlenecks, monthlyTrend }: Props) {
    const totalCandidates = pipelineConversion.reduce((sum, p) => sum + p.count, 0);
    const avgTimeToHire = timeToHire.length > 0
        ? Math.round(timeToHire.reduce((sum, t) => sum + t.avg_days, 0) / timeToHire.length)
        : 0;
    const totalHired = pipelineConversion.find(p => p.stage === 'hired')?.count ?? 0;
    ...
    const activePipeline = pipelineConversion.filter(p => !['withdrawn', 'rejected'].includes(p.stage));
            ...
            <PageShell>
                <PageHeader
                    ...
                    actions={
                        <Button variant="outline" size="sm" asChild>
                            <Link href="/hr/recruitment"><ArrowLeft className="mr-2 h-4 w-4" />Back to Pipeline</Link>
                        </Button>
                    }
                />
                ...
                <Tabs defaultValue="overview" className="space-y-6">
                    ...
                    <TabsContent value="overview" className="space-y-6">
                        <div className="grid gap-6 lg:grid-cols-2">
                            <Card>
                                ...
                                <CardContent>
                                    {monthlyTrend.length === 0 ? <EmptyState message="No application data yet." /> : (
                                        <div className="h-64">
                                            <ResponsiveContainer width="100%" height="100%">
                                                <AreaChart data={monthlyTrend}>
                                                    ...
                                                    <Area type="monotone" dataKey="count" stroke="#3b82f6" fillOpacity={1} fill="url(#colorApps)" name="Applications" />
                                                </AreaChart>
                                            </ResponsiveContainer>
                                        </div>
                                    )}
                            ...
                            <Card>
                                <CardHeader><CardTitle className="text-base">Hiring Velocity</CardTitle></CardHeader>
                                <CardContent>
                                    {hiringVelocity.length === 0 ? <EmptyState message="No hiring data yet." /> : (
                                        <div className="h-64">
                                            <ResponsiveContainer width="100%" height="100%">
                                                <BarChart data={hiringVelocity}>
                                                    ...
                                                    <YAxis className="text-xs" tick={{ fill: 'currentColor' }} />
                                                    <Tooltip />
                                                    <Bar dataKey="count" fill="#10b981" radius={[4, 4, 0, 0]} name="Hires" />
                                                </BarChart>
                                            </ResponsiveContainer>
                                        </div>
                                    )}
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    {activePipeline.length === 0 ? <EmptyState message="No pipeline data." /> : (
                                        <div className="space-y-2">
                                            {activePipeline.map((entry) => {
                                                const config = statusConfig[entry.stage];
                                                return (
                                                    <div key={entry.stage} className="flex items-center gap-3">
                                                        <span className="text-xs w-24 truncate">{stageLabels[entry.stage] ?? entry.stage}</span>
                                                        <div className="flex-1 bg-muted/30 rounded-full h-6 overflow-hidden">
                                                            <div
                                                                className={`h-full rounded-full flex items-center px-2.5 ${config?.bgClass ?? 'bg-slate-500'}/30`}
                                                                style={{ width: `${Math.max((entry.count / maxPipelineCount) * 100, 5)}%` }}
                                                            >
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    {sourceDonutData.length === 0 ? <EmptyState message="No source data." /> : (
                                        <>
                                            <div className="h-48">
                                                <ResponsiveContainer width="100%" height="100%">
                                                    <PieChart>
                                                        <Pie data={sourceDonutData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={40} outerRadius={65} paddingAngle={2}>
                                                            {sourceDonutData.map((entry, i) => <Cell key={i} fill={entry.fill} />)}
                                                        </Pie>
                                                        <Tooltip />
                                                    </PieChart>
                                                </ResponsiveContainer>
                    ...
                    <TabsContent value="pipeline" className="space-y-6">
                        <div className="grid gap-6 lg:grid-cols-2">
                            <Card>
                                <CardHeader><CardTitle className="text-base">Pipeline Conversion</CardTitle></CardHeader>
                                <CardContent className="space-y-2">
                                    {pipelineConversion.map((entry, i) => {
                                        const prev = i > 0 ? pipelineConversion[i - 1] : null;
                                        const dropOff = prev && prev.count > 0 ? Math.round(((prev.count - entry.count) / prev.count) * 100) : null;
                                        return (
                                            <div key={entry.stage} className="flex items-center gap-3">
                                                <span className="text-xs w-28 truncate font-medium">{stageLabels[entry.stage] ?? entry.stage}</span>
                                                <div className="flex-1 bg-muted/30 rounded-full h-6 overflow-hidden">
                                                    <div
                                                        className="bg-primary/30 h-full rounded-full flex items-center px-2.5"
                            ...
                            <Card>
                                <CardHeader><CardTitle className="text-base">Stage Bottlenecks</CardTitle></CardHeader>
                                <CardContent>
                                    {stageBottlenecks.length === 0 ? <EmptyState message="No bottleneck data." /> : (
                                        <div className="h-64">
                                            <ResponsiveContainer width="100%" height="100%">
                                                <BarChart data={stageBottlenecks.map(b => ({ ...b, label: stageLabels[b.stage] ?? b.stage }))} layout="vertical">
                                                    <CartesianGrid strokeDasharray="3 3" className="stroke-muted" />
                    ...
                    <TabsContent value="sources" className="space-y-6">
                        <Card>
                            <CardHeader><CardTitle className="text-base">Source Comparison</CardTitle></CardHeader>
                            <CardContent>
                                {sourceChartData.length === 0 ? <EmptyState message="No source data." /> : (
                                    <div className="h-72">
                                        <ResponsiveContainer width="100%" height="100%">
                        ...
                        <Card>
                            <CardHeader><CardTitle className="text-base">Source Conversion Rates</CardTitle></CardHeader>
                            <CardContent>
                                {sourceEffectiveness.length === 0 ? <EmptyState message="No source data." /> : (
                                        ...
                                        <table className="w-full text-sm">
                                            ...
                                            <tbody>
                                                {sourceEffectiveness.map((source) => (
                                                    <tr key={source.source} className="border-t hover:bg-muted/30">
                                                        <td className="px-4 py-2 capitalize font-medium">{source.source.replace(/_/g, ' ')}</td>
                                                        <td className="px-4 py-2 text-right">{source.total}</td>
                                                        <td className="px-4 py-2 text-right">{source.active}</td>
                                                        <td className="px-4 py-2 text-right">{source.hired}</td>
                    ...
                    <TabsContent value="time" className="space-y-6">
                        <Card>
                            <CardHeader><CardTitle className="text-base">Time to Hire Trend</CardTitle></CardHeader>
                            <CardContent>
                                {timeToHire.length === 0 ? <EmptyState message="No hire data yet." /> : (
                                    <div className="h-64">
                                        <ResponsiveContainer width="100%" height="100%">
                        ...
                        <Card>
                            ...
                            <CardContent>
                                {openPositions.length === 0 ? <EmptyState message="No open positions." /> : (
                                    <div className="overflow-hidden rounded-lg border">
                                        <table className="w-full text-sm">
                                            <thead className="bg-muted/50"><tr>
