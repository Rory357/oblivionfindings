...
export default function RecruitmentJobs({
    ...
    function publishJob(jobId: number) {
        router.post(
            `/hr/recruitment/jobs/${jobId}/publish`,
            {},
            { preserveScroll: true },
        );
    }
    function closeJob(jobId: number) {
        router.post(
            `/hr/recruitment/jobs/${jobId}/close`,
            {},
            { preserveScroll: true },
        );
    }
    function syncPosting(jobId: number) {
        router.post(
            `/hr/recruitment/jobs/${jobId}/sync-posting`,
            {},
            { preserveScroll: true },
        );
    }
    ...
    function startEdit(job: Job) {
        ...
        createForm.setData({
            ...
            employment_type:
                job.employment_type || employmentTypes[0] || 'full_time',
            openings: String(job.openings || 1),
            summary: job.summary || '',
            description: job.description || '',
            requirements: job.requirements || '',
            responsibilities: job.responsibilities || '',
            default_interview_kit_id: job.default_interview_kit
                ? String(job.default_interview_kit.id)
                : '',
            hiring_manager_user_id: job.hiring_manager
                ? String(job.hiring_manager.id)
                : '',
    ...
    function togglePostingChannel(channel: string, checked: boolean) {
        createForm.setData(
            ...
            checked
                ? Array.from(
                      new Set([...createForm.data.posting_channels, channel]),
                  )
            ...
            <PageShell>
                                                        ...
                                                        (ch) => (
                                                            <Button
                                                                ...
                                                                className={`h-8 gap-1.5 px-3 text-xs ${
                                                                    createForm.data.posting_channels.includes(
                                                                        ch,
                                                                    )
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
                    <KpiCard
                        ...
                        value={summary.open_requisitions}
                ...
                {jobs.data.length === 0 ? (
                    <Card>
                        <CardContent className="py-12 text-center text-muted-foreground">
                            ...
                            <p className="text-sm">
                                Create a job requisition to start recruiting.
                            </p>
                        </CardContent>
                    </Card>
                ) : viewMode === 'cards' ? (
                    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                        {jobs.data.map((job) => {
                            const total =
                                job.metrics.active_candidates +
                                job.metrics.stale_candidates +
                                job.metrics.hired_candidates;
                            const activePct =
                                total > 0
                                    ? (job.metrics.active_candidates / total) *
                                      100
                                    : 0;
                            const stalePct =
                                total > 0
                                    ? (job.metrics.stale_candidates / total) *
                                      100
                                    : 0;
                            const hiredPct =
                                total > 0
                                    ? (job.metrics.hired_candidates / total) *
                                      100
                                    : 0;
                            return (
                                <Card
                                ...
                                >
                                    <CardContent className="p-5">
                                        ...
                                        <div className="mb-3 flex items-center justify-between text-xs text-muted-foreground">
                                            <span>
                                                {job.metrics.total_applications}{' '}
                                                applications
                                            </span>
                                            <span>
                                                {job.metrics.conversion_rate}%
                                                conversion
                                            </span>
                                        </div>
