...
const reviewTypeLabels: Record<string, { label: string; desc: string }> = {
    peer: {
        label: 'Peer Review',
        desc: 'Feedback from colleagues at the same level',
    },
    manager: {
        label: 'Manager Review',
        desc: 'Feedback from a direct manager',
    },
    direct_report: {
        label: 'Direct Report',
        desc: 'Feedback from people they manage',
    },
    self: {
        label: 'Self Assessment',
        desc: 'Self-reflection on own performance',
    },
};
...
export default function FeedbackRequest({
    ...
    const [newTemplate, setNewTemplate] = useState({
        name: '',
        description: '',
        questions: [{ key: '', question: '' }] as TemplateQuestion[],
    });
    ...
    useEffect(() => {
        ...
        // eslint-disable-next-line react-hooks/exhaustive-deps -- Inertia form helpers are stable; sync only when these selected fields change.
    ...
    const availableReviewers = employees
        ...
        .filter(
            (emp) =>
                !reviewerSearch ||
                emp.name.toLowerCase().includes(reviewerSearch.toLowerCase()),
        );

    const selectedSubject = employees.find(
        (e) => String(e.id) === form.data.subject_user_id,
    );
    const selectedTemplate = templates.find(
        (t) => String(t.id) === form.data.template_id,
    );
    ...
    const addQuestion = () =>
        setNewTemplate((prev) => ({
            ...prev,
            questions: [...prev.questions, { key: '', question: '' }],
        }));
                ...
                <form onSubmit={submit} className="mx-auto max-w-3xl space-y-6">
                            ...
                            <div className="grid gap-2 sm:grid-cols-2">
                                {templates.map((t) => (
                                    <Button
                                        ...
                                        className={`h-auto items-start justify-start gap-3 rounded-xl border p-3 text-left transition-all ${String(t.id) === form.data.template_id ? 'bg-primary/10/50 border-primary shadow-sm ring-1 ring-ring' : 'hover:border-muted-foreground/20 hover:bg-muted/50'}`}
                                    >
                                        <div
                                            className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${String(t.id) === form.data.template_id ? 'bg-primary/10' : 'bg-muted'}`}
                                        >
                                            <FileText
                                                className={`h-4 w-4 ${String(t.id) === form.data.template_id ? 'text-primary' : 'text-muted-foreground'}`}
                                            />
                                        </div>
                                        <div className="min-w-0 flex-1">
                                            <div className="flex items-center gap-1.5">
                                                ...
                                                {t.is_default && (
                                                    <Badge className="border-0 bg-primary/10 text-[8px] text-primary">
                                                        Default
                                                    </Badge>
                                                )}
                                            </div>
                            ...
                            {templates.length === 0 && (
                                <div className="py-6 text-center">
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                        No templates yet. The default questions
                                        will be used.
                                    </p>
                                </div>
                            )}
                            ...
                            <div className="grid gap-4 sm:grid-cols-2">
                                <div className="space-y-1.5">
                                    ...
                                    <Select
                                        ...
                                        onValueChange={(v) =>
                                            form.setData('subject_user_id', v)
                                        }
                                    >
                                        <SelectTrigger>
                    ...
                    <Card className="overflow-hidden">
                        <CardHeader className="border-b bg-gradient-to-r from-status-info-bg to-transparent pb-3">
                            <div className="flex items-center justify-between">
                                <CardTitle className="flex items-center gap-2 text-base">
                                    <div className="flex h-7 w-7 items-center justify-center rounded-full bg-status-info text-[11px] font-bold text-white">
                                        3
                                    </div>
                                    {isSelfAssessment
                                        ? 'Reviewer'
                                ...
                                {!isSelfAssessment &&
                                    form.data.reviewer_user_ids.length > 0 && (
                                        <Badge className="border-0 bg-status-info-bg text-status-info">
                            ...
                            {isSelfAssessment ? (
                                ...
                                <div className="flex items-center gap-3 rounded-xl border border-status-info/30 bg-status-info-bg p-4">
                                    {selectedSubject ? (
                                        <>
                                            <div
                                                className={`flex h-10 w-10 items-center justify-center rounded-full text-xs font-bold text-white ${avatarColor(parseInt(form.data.subject_user_id) || 0)}`}
                                            >
                                                {getInitials(
                                        ...
                                        <p className="text-sm text-muted-foreground">
                                            Select an employee above — they will
                                            be assigned as their own reviewer
                                        </p>
                                    )}
                                </div>
                                ...
                                <div className="flex flex-col items-center gap-2 py-8">
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                        Select an employee first to see
                                        available reviewers
                                    </p>
                                </div>
                            ) : (
                                        ...
                                        {availableReviewers.map((emp) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    />
                                                    <div
                                                        className={`flex h-8 w-8 items-center justify-center rounded-full text-[10px] font-bold text-white ${avatarColor(emp.id)}`}
                                                    >
                                                    ...
                                                    {selected && (
                                                        <CheckCircle2 className="ml-auto h-4 w-4 text-status-info" />
                                                    )}
                                                </label>
                                            );
                                        })}
                    ...
                    <Card className="overflow-hidden">
                        <CardHeader className="border-b bg-gradient-to-r from-status-success-bg to-transparent pb-3">
                            <CardTitle className="flex items-center gap-2 text-base">
                                <div className="flex h-7 w-7 items-center justify-center rounded-full bg-status-success text-[11px] font-bold text-white">
                                    4
                                </div>
                                Review Questions
                                {selectedTemplate && (
                        ...
                        <CardContent className="pt-4">
                            <p className="mb-3 text-xs text-muted-foreground">
                                Reviewers will rate and comment on the following{' '}
                                {displayQuestions.length} areas:
                            </p>
                            <div className="space-y-2">
                                {displayQuestions.map((q, i) => (
                                    ...
                                    >
                                        <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-status-success-bg text-[10px] font-bold text-status-success">
                                            {i + 1}
                                        </div>
                                        <div>
                        ...
                        <div className="text-sm">
                            {selectedSubject ? (
                                <p>
                                    ...
                                    <strong>{selectedSubject.name}</strong> from{' '}
                                    <strong>
                                        {form.data.reviewer_user_ids.length}
                                    </strong>{' '}
                                    reviewer
                                ...
                                <p className="text-muted-foreground">
                                    Complete the form above to send feedback
                                    requests
                                </p>
                            )}
                        </div>
                ...
                <DialogContent className="max-h-[80vh] overflow-y-auto sm:max-w-lg">
                                    ...
                                    >
                                        ...
                                        {newTemplate.questions.length > 1 && (
                                            <Button
                                                ...
                                                variant="ghost"
                                                size="sm"
                                                className="h-7 w-7 shrink-0 p-0 text-status-critical"
                                                onClick={() =>
                                                    removeQuestion(i)
                                                }
                                            >
                    ...
                    <DialogFooter>
                        ...
                        <Button
                            ...
                            disabled={
                                !newTemplate.name ||
                                newTemplate.questions.some((q) => !q.question)
                            }
