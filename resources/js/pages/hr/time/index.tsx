...
interface Props {
    ...
    filters: { status?: string; pay_type?: string; q?: string; tab?: string; scope?: string };
    can: { manage?: boolean; approveTeam?: boolean; approveAny?: boolean; editEntry?: boolean; clockOnBehalf?: boolean };
}
...
export default function TimeIndex({
    entries, timesheets, approvalTimesheets, pendingApprovalCount, teamMembers,
    activeClock, weeklySummary, kpiStats, recentActivity, filters, can,
}: Props) {
    ...
    const [editForm, setEditForm] = useState({ clock_in: '', clock_out: '', break_minutes: 0, pay_type: 'standard', notes: '', amendment_reason: '' });

    // Clock on behalf dialog state
    const [showClockOnBehalf, setShowClockOnBehalf] = useState(false);
    const [cobForm, setCobForm] = useState({ target_user_id: '', clock_in: '', clock_out: '', break_minutes: 0, pay_type: 'standard', notes: '', reason: '' });

    // Return / reject dialog state
    const [returnTimesheetId, setReturnTimesheetId] = useState<number | null>(null);
    ...
    function updateFilter(key: string, value: string | null) {
        ...
        if (value === null || value === NONE) delete newFilters[key as keyof typeof newFilters];
        router.get('/hr/time', newFilters, { preserveState: true, replace: true });
    }

    function handleClockIn() { router.post('/hr/time/clock-in', {}, { preserveScroll: true }); }
    function handleClockOut() { router.post('/hr/time/clock-out', {}, { preserveScroll: true }); }
    ...
    function submitEditEntry() {
        ...
        router.put(`/hr/time/entries/${editEntry.id}`, editForm, {
            preserveScroll: true,
            onSuccess: () => { setEditEntry(null); setProcessing(null); },
    ...
    function submitClockOnBehalf() {
        ...
        router.post('/hr/time/clock-on-behalf', cobForm, {
            preserveScroll: true,
            onSuccess: () => { setShowClockOnBehalf(false); setProcessing(null); setCobForm({ target_user_id: '', clock_in: '', clock_out: '', break_minutes: 0, pay_type: 'standard', notes: '', reason: '' }); },
            onError: () => setProcessing(null),
        });
    }
    ...
    function handleApprove(id: number) {
        setProcessing(id);
        router.post(`/hr/time/timesheets/${id}/approve`, {}, { preserveScroll: true, onFinish: () => setProcessing(null) });
    }
    function submitReturn() {
        if (!returnTimesheetId || !returnNotes.trim()) return;
        setProcessing(returnTimesheetId);
        router.post(`/hr/time/timesheets/${returnTimesheetId}/return`, { notes: returnNotes }, {
            preserveScroll: true,
            onSuccess: () => { setReturnTimesheetId(null); setReturnNotes(''); setProcessing(null); },
            onError: () => setProcessing(null),
        });
    }
    function submitReject() {
        ...
        router.post(`/hr/time/timesheets/${rejectTimesheetId}/reject`, { rejection_reason: rejectReason }, {
            preserveScroll: true,
            onSuccess: () => { setRejectTimesheetId(null); setRejectReason(''); setProcessing(null); },
            onError: () => setProcessing(null),
        });
    }
    ...
    function bulkApprove() {
        ...
        setProcessing('bulk');
        router.post('/hr/time/timesheets/bulk-approve', { ids: selectedApprovalIds }, {
            preserveScroll: true,
            onSuccess: () => { setSelectedApprovalIds([]); setProcessing(null); },
            onError: () => setProcessing(null),
        });
    }
    function submitBulkReject() {
        ...
        setProcessing('bulk');
        router.post('/hr/time/timesheets/bulk-reject', { ids: selectedApprovalIds, reason: bulkNotes }, {
            preserveScroll: true,
            onSuccess: () => { setSelectedApprovalIds([]); setBulkNotes(''); setShowBulkReject(false); setProcessing(null); },
            onError: () => setProcessing(null),
        });
    }
    function submitBulkReturn() {
        if (!bulkNotes.trim()) return;
        setProcessing('bulk');
        router.post('/hr/time/timesheets/bulk-return', { ids: selectedApprovalIds, notes: bulkNotes }, {
            preserveScroll: true,
            onSuccess: () => { setSelectedApprovalIds([]); setBulkNotes(''); setShowBulkReturn(false); setProcessing(null); },
            onError: () => setProcessing(null),
        });
    }
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <PageShell>
                <PageHeader
                    ...
                    actions={
                        <div className="flex items-center gap-2">
                            {can.clockOnBehalf && (
                                <Button variant="outline" size="sm" onClick={() => setShowClockOnBehalf(true)}>
                                    <UserPlus className="mr-1.5 h-4 w-4" /> Clock On Behalf
                                </Button>
                            )}
                        </div>
                    }
                ...
                {/* Tabs */}
                <Tabs defaultValue={filters.tab ?? 'dashboard'} className="space-y-4">
                    <TabsList className="flex h-auto flex-wrap gap-1">
                        <TabsTrigger value="dashboard"><LayoutDashboard className="mr-1.5 h-3.5 w-3.5" /> Dashboard</TabsTrigger>
                        <TabsTrigger value="entries"><List className="mr-1.5 h-3.5 w-3.5" /> Time Entries</TabsTrigger>
                        <TabsTrigger value="timesheets"><ClipboardCheck className="mr-1.5 h-3.5 w-3.5" /> Timesheets</TabsTrigger>
                        {can.approveAny && (
                        ...
                        <div className="grid gap-6 lg:grid-cols-3">
                            <div className="space-y-4 lg:col-span-2">
                                <div className="grid gap-4 sm:grid-cols-2">
                                    <Card>
                                        <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-base"><Clock className="h-4 w-4" /> Clock Status</CardTitle></CardHeader>
                                        <CardContent>
                                            {activeClock ? (
                                    ...
                                    <Card>
                                        <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-base"><Timer className="h-4 w-4" /> This Week</CardTitle></CardHeader>
                                        <CardContent>
                                            <div className="mb-3 text-center">
                                                <p className="text-3xl font-bold">{weeklySummary.total_hours}h</p>
                                                <p className="text-xs text-muted-foreground">{weeklySummary.week_start} to {weeklySummary.week_end}</p>
                                            </div>
                                            <div className="flex items-end justify-between gap-1">
                                                {Object.entries(weeklySummary.daily_hours).map(([date, hours], i) => {
                                                    const h = Number(hours) || 0;
                                                    const max = Math.max(10, ...Object.values(weeklySummary.daily_hours).map((v) => Number(v) || 0));
                                                    const bar = max > 0 ? (h / max) * 50 : 0;
                                                    return (<div key={date} className="flex flex-1 flex-col items-center gap-1"><div className="w-full rounded bg-primary/20" style={{ height: `${Math.max(4, bar)}px` }}><div className="w-full rounded bg-primary" style={{ height: `${bar}px` }} /></div><span className="text-[10px] text-muted-foreground">{dayLabels[i] ?? date.slice(5)}</span></div>);
                                                })}
                                            </div>
                                        </CardContent>
                                    </Card>
                            ...
                            {can.approveAny && recentActivity.length > 0 && (
                                <Card>
                                    <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-base"><Activity className="h-4 w-4" /> Recent Activity</CardTitle></CardHeader>
                                        ...
                                        {recentActivity.map((item) => (
                                            <div key={item.id} className="flex items-start gap-2">
                                                ...
                                                {item.pay_type !== 'standard' && (
                                                    <Badge variant="outline" className={`shrink-0 text-[10px] ${payTypeConfig[item.pay_type]?.className ?? ''}`}>{payTypeConfig[item.pay_type]?.label ?? item.pay_type}</Badge>
                                                )}
                                            </div>
                                        ))}
                                    </CardContent>
                    ...
                    <TabsContent value="entries" className="space-y-4">
                        <div className="flex flex-col gap-3 sm:flex-row">
                            <div className="relative flex-1">
                                ...
                                <Input placeholder="Search by staff name..." className="pl-9" value={searchValue} onChange={(e) => setSearchValue(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && updateFilter('q', searchValue.trim() || null)} />
                            </div>
                            <div className="flex flex-wrap gap-2">
                                {can.approveAny && (
                                    <Select value={filters.scope ?? 'team'} onValueChange={(v) => updateFilter('scope', v)}>
                                        <SelectTrigger className="h-9 w-28 text-xs"><SelectValue placeholder="Scope" /></SelectTrigger>
                                        <SelectContent>
                                            <SelectItem value="mine">My Entries</SelectItem>
                                            <SelectItem value="team">My Team</SelectItem>
                                            {can.manage && <SelectItem value="all">All Staff</SelectItem>}
                                        </SelectContent>
                                    </Select>
                                )}
                                <Select value={filters.status ?? NONE} onValueChange={(v) => updateFilter('status', v === NONE ? null : v)}>
                                ...
                                <Select value={filters.pay_type ?? NONE} onValueChange={(v) => updateFilter('pay_type', v === NONE ? null : v)}>
                                    <SelectTrigger className="h-9 w-32 text-xs"><SelectValue placeholder="All Pay Types" /></SelectTrigger>
                                    <SelectContent>
                                        <SelectItem value={NONE}>All Pay Types</SelectItem>
                                        {Object.entries(payTypeConfig).map(([k, v]) => <SelectItem key={k} value={k}>{v.label}</SelectItem>)}
                                    </SelectContent>
                                </Select>
                            </div>
                        </div>
                        ...
                        {entries.data.length === 0 ? (
                            <div className="py-16 text-center"><Clock className="mx-auto mb-4 h-12 w-12 text-muted-foreground/40" /><p className="font-medium text-muted-foreground">No time entries found</p></div>
                        ) : (
                            <>
                                <div className="overflow-hidden rounded-xl border">
                                    <table className="w-full text-sm">
                                        <thead className="border-b bg-muted/50">
                                            <tr>
                                                {can.approveAny && <th className="px-4 py-3 text-left font-medium">Staff</th>}
                                                <th className="px-4 py-3 text-left font-medium">Date</th>
                                                <th className="px-4 py-3 text-left font-medium">Shift / Client</th>
                                                ...
                                                <th className="px-4 py-3 text-left font-medium">Pay Type</th>
                                                <th className="px-4 py-3 text-left font-medium">Status</th>
                                                {can.editEntry && <th className="px-4 py-3 text-right font-medium">Actions</th>}
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {entries.data.map((entry) => {
                                                const sc = statusConfig[entry.status] || statusConfig.active;
                                                ...
                                                return (
                                                    <tr key={entry.id} className="border-b last:border-b-0 hover:bg-muted/50">
                                                        ...
                                                        <td className="px-4 py-3"><Badge variant="outline" className={sc.className}>{sc.label}</Badge>{entry.amended_by && <span className="ml-1 text-[10px] text-muted-foreground">(amended)</span>}</td>
                                                        {can.editEntry && (
                                                            <td className="px-4 py-3 text-right">
                                                                {entry.status !== 'approved' && (
                                                                    <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => openEditDialog(entry)}><Pencil className="h-3.5 w-3.5" /></Button>
                                                                )}
                                                            </td>
                                                        )}
                                                    </tr>
                                ...
                                {entries.last_page > 1 && (
                                    <div className="flex items-center justify-between">
                                        <p className="text-sm text-muted-foreground">Showing {(entries.current_page - 1) * entries.per_page + 1} to {Math.min(entries.current_page * entries.per_page, entries.total)} of {entries.total}</p>
                    ...
                    <TabsContent value="timesheets" className="space-y-4">
                        {timesheets.data.length === 0 ? (
                            <div className="py-16 text-center"><ClipboardCheck className="mx-auto mb-4 h-12 w-12 text-muted-foreground/40" /><p className="font-medium text-muted-foreground">No timesheets found</p></div>
                        ) : (
                            <div className="overflow-hidden rounded-xl border">
                                <table className="w-full text-sm">
                                    <thead className="border-b bg-muted/50">
                                        <tr>
                                            ...
                                            {can.approveAny && <th className="px-4 py-3 text-right font-medium">Actions</th>}
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {timesheets.data.map((ts) => {
                                            ...
                                            return (
                                                <tr key={ts.id} className="border-b last:border-b-0 hover:bg-muted/50">
                                                    {can.approveAny && <td className="px-4 py-3 font-medium">{ts.user_name}</td>}
                                                    ...
                                                    <td className="px-4 py-3 text-muted-foreground">{ts.submitted_at?.slice(0, 10) ?? '-'}</td>
                                                    {can.approveAny && (
                                                        <td className="px-4 py-3 text-right">
                                                            {ts.status === 'submitted' && (
                    ...
                    {can.approveAny && (
                        <TabsContent value="approvals" className="space-y-4">
                            {approvalTimesheets.length === 0 ? (
                                <div className="py-16 text-center"><CheckCircle className="mx-auto mb-4 h-12 w-12 text-muted-foreground/40" /><p className="font-medium text-muted-foreground">No timesheets awaiting approval</p><p className="mt-1 text-sm text-muted-foreground">All caught up!</p></div>
                            ) : (
                                <>
                                    {/* Bulk actions */}
            ...
            <Dialog open={!!editEntry} onOpenChange={(open) => !open && setEditEntry(null)}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>Amend {editEntry?.user_name}'s time entry for {editEntry?.entry_date}. A reason is required for audit.</DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div><Label>Clock In</Label><Input type="datetime-local" value={editForm.clock_in} onChange={(e) => setEditForm({ ...editForm, clock_in: e.target.value })} /></div>
                            <div><Label>Clock Out</Label><Input type="datetime-local" value={editForm.clock_out} onChange={(e) => setEditForm({ ...editForm, clock_out: e.target.value })} /></div>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div><Label>Break (minutes)</Label><Input type="number" min={0} max={480} value={editForm.break_minutes} onChange={(e) => setEditForm({ ...editForm, break_minutes: Number(e.target.value) })} /></div>
                            <div><Label>Pay Type</Label>
                                <Select value={editForm.pay_type} onValueChange={(v) => setEditForm({ ...editForm, pay_type: v })}>
                                    <SelectTrigger><SelectValue /></SelectTrigger>
                                    <SelectContent>{Object.entries(payTypeConfig).map(([k, v]) => <SelectItem key={k} value={k}>{v.label}</SelectItem>)}</SelectContent>
                                </Select>
                            </div>
                        </div>
                        <div><Label>Notes</Label><Input value={editForm.notes} onChange={(e) => setEditForm({ ...editForm, notes: e.target.value })} /></div>
                        <div><Label>Amendment Reason *</Label><Textarea placeholder="Why is this entry being amended?" value={editForm.amendment_reason} onChange={(e) => setEditForm({ ...editForm, amendment_reason: e.target.value })} /></div>
                    </div>
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setEditEntry(null)}>Cancel</Button>
                        <Button onClick={submitEditEntry} disabled={!editForm.amendment_reason.trim() || processing === 'edit'}>
                            {processing === 'edit' ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null} Save Changes
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
            ...
            <Dialog open={showClockOnBehalf} onOpenChange={setShowClockOnBehalf}>
                <DialogContent className="sm:max-w-lg">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Create a time entry for a team member who forgot to clock in or out. This will be recorded as an admin entry with your name as the creator.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="rounded-lg border bg-muted/30 p-4">
                            ...
                            <Select value={cobForm.target_user_id} onValueChange={(v) => setCobForm({ ...cobForm, target_user_id: v })}>
                        ...
                        <div>
                            ...
                            <div className="mt-2 grid grid-cols-2 gap-3">
                                ...
                                <div>
                                    ...
                                    <Input type="datetime-local" className="mt-1" value={cobForm.clock_out} onChange={(e) => setCobForm({ ...cobForm, clock_out: e.target.value })} />
                                    <p className="mt-1 text-[10px] text-muted-foreground">Leave blank if still on shift</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <Input type="number" className="mt-1" min={0} max={480} value={cobForm.break_minutes} onChange={(e) => setCobForm({ ...cobForm, break_minutes: Number(e.target.value) })} />
                            </div>
                            <div>
                                <Label className="text-sm">Pay Type</Label>
                                <Select value={cobForm.pay_type} onValueChange={(v) => setCobForm({ ...cobForm, pay_type: v })}>
                        ...
                        <div>
                            ...
                            <Textarea
                                ...
                                onChange={(e) => setCobForm({ ...cobForm, reason: e.target.value })}
                                rows={2}
                            />
                        </div>
                        <div>
                            <Label className="text-sm">Additional Notes</Label>
                            <Input className="mt-1" placeholder="Optional notes for the time entry" value={cobForm.notes} onChange={(e) => setCobForm({ ...cobForm, notes: e.target.value })} />
                        </div>

                        {/* Summary preview */}
                        {cobForm.target_user_id && cobForm.clock_in && (
                            <div className="rounded-lg border border-primary/20 bg-primary/5 p-3">
                                <p className="text-xs font-medium text-primary">Entry Preview</p>
                                <div className="mt-1.5 grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                                    <span className="text-muted-foreground">Staff:</span>
                                    <span className="font-medium">{teamMembers.find((m) => String(m.id) === cobForm.target_user_id)?.name}</span>
                                    <span className="text-muted-foreground">Clock In:</span>
                                    <span>{cobForm.clock_in.replace('T', ' ')}</span>
                                    {cobForm.clock_out && (
                                        <>
                                            <span className="text-muted-foreground">Clock Out:</span>
                                            <span>{cobForm.clock_out.replace('T', ' ')}</span>
                                        </>
                                    )}
                                    <span className="text-muted-foreground">Pay Type:</span>
                                    <span><Badge variant="outline" className={`text-[10px] ${payTypeConfig[cobForm.pay_type]?.className}`}>{payTypeConfig[cobForm.pay_type]?.label}</Badge></span>
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <DialogFooter className="gap-2 sm:gap-0">
                        <Button variant="outline" onClick={() => setShowClockOnBehalf(false)}>Cancel</Button>
                        <Button
                            onClick={submitClockOnBehalf}
                            disabled={!cobForm.target_user_id || !cobForm.clock_in || !cobForm.reason.trim() || processing === 'cob'}
                        >
                            {processing === 'cob' ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <UserPlus className="mr-2 h-4 w-4" />}
                            Create Time Entry
                        </Button>
                    </DialogFooter>
                </DialogContent>
            ...
            {/* Return Dialog */}
            <Dialog open={!!returnTimesheetId} onOpenChange={(open) => !open && setReturnTimesheetId(null)}>
                <DialogContent>
                    <DialogHeader><DialogTitle>Return Timesheet for Changes</DialogTitle><DialogDescription>The timesheet will be sent back to draft for the staff member to correct.</DialogDescription></DialogHeader>
            ...
            {/* Reject Dialog */}
            <Dialog open={!!rejectTimesheetId} onOpenChange={(open) => !open && setRejectTimesheetId(null)}>
                <DialogContent>
                    <DialogHeader><DialogTitle>Reject Timesheet</DialogTitle><DialogDescription>This timesheet will be permanently rejected.</DialogDescription></DialogHeader>
                    <div><Label>Rejection Reason *</Label><Textarea placeholder="Why is this timesheet being rejected?" value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} /></div>
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setRejectTimesheetId(null)}>Cancel</Button>
                        <Button variant="destructive" onClick={submitReject} disabled={!rejectReason.trim()}><XCircle className="mr-2 h-4 w-4" /> Reject Timesheet</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
            ...
            <Dialog open={showBulkReject} onOpenChange={setShowBulkReject}>
                <DialogContent>
                    <DialogHeader><DialogTitle>Reject {selectedApprovalIds.length} Timesheet(s)</DialogTitle></DialogHeader>
                    <div><Label>Rejection Reason *</Label><Textarea value={bulkNotes} onChange={(e) => setBulkNotes(e.target.value)} /></div>
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setShowBulkReject(false)}>Cancel</Button>
                        <Button variant="destructive" onClick={submitBulkReject} disabled={!bulkNotes.trim() || processing === 'bulk'}>Reject All</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
            ...
            <Dialog open={showBulkReturn} onOpenChange={setShowBulkReturn}>
                <DialogContent>
                    <DialogHeader><DialogTitle>Return {selectedApprovalIds.length} Timesheet(s) for Changes</DialogTitle></DialogHeader>
