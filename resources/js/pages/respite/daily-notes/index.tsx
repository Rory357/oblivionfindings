...
export default function DailyNotesIndex({
    ...
    const applyFilter = (key: string, value: string) => {
        ...
        router.get('/respite/daily-notes', updated, {
            preserveState: true,
            preserveScroll: true,
        });
    };
                ...
                hero={
                    <PageHero
                        ...
                        stats={[
                            ...
                            {
                                label: 'With concerns',
                                value: notes.data.filter(
                                    (n: any) => n.has_concerns,
                                ).length,
                            },
                            {
                                label: 'Incidents',
                                value: notes.data.filter(
                                    (n: any) => n.incident_occurred,
                                ).length,
                            },
                        ]}
                        actions={
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid gap-4 sm:grid-cols-3">
                            ...
                            <div>
                                ...
                                <Select
                                    value={localFilters.shift_period || ANY}
                                    onValueChange={(v) =>
                                        applyFilter(
                                            'shift_period',
                                            v === ANY ? '' : v,
                        ...
                        <div className="flex flex-wrap gap-4">
                            ...
                            <label className="flex items-center gap-2 text-sm">
                                <input
                                    type="checkbox"
                                    checked={
                                        localFilters.with_incidents === '1'
                                    }
                                    onChange={() =>
                                        toggleFilter('with_incidents')
                                    }
                                    className="rounded border-border"
                                />
                                With incidents
                                    ...
                                    <div className="flex items-start justify-between gap-3">
                                        <div className="flex-1">
                                            <div className="font-semibold">
                                                {note.stay?.client?.first_name}{' '}
                                                {note.stay?.client?.last_name}
                                            </div>
