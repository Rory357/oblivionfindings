            ...
            >
                        ...
                        <div className="flex flex-wrap gap-3">
                            {note.mood && (
                                <div className="text-sm">
                                    <span className="text-muted-foreground">Mood:</span> <Badge variant="outline">{note.mood}</Badge>
                                </div>
                            )}
                            {note.appetite && (
                                <div className="text-sm">
                                    <span className="text-muted-foreground">Appetite:</span> <Badge variant="outline">{note.appetite}</Badge>
                                </div>
                            )}
                            {note.sleep_quality && (
                                <div className="text-sm">
                                    <span className="text-muted-foreground">Sleep:</span> <Badge variant="outline">{note.sleep_quality}</Badge>
                                </div>
                            )}
                            {note.engagement && (
                                <div className="text-sm">
                                    <span className="text-muted-foreground">Engagement:</span> <Badge variant="outline">{note.engagement}</Badge>
                                </div>
                            )}
                            {note.mobility && (
                                <div className="text-sm">
                                    <span className="text-muted-foreground">Mobility:</span> <Badge variant="outline">{note.mobility}</Badge>
                                </div>
                            )}
                        </div>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4 text-sm text-muted-foreground">
                        {note.activities && (
                            <div>
                                <div className="font-medium text-foreground">Activities</div>
                                <div className="mt-1 whitespace-pre-wrap">{note.activities}</div>
                            </div>
                        )}
                        {note.observations && (
                            <div>
                                <div className="font-medium text-foreground">Observations</div>
                                <div className="mt-1 whitespace-pre-wrap">{note.observations}</div>
                            </div>
                        )}
                        {note.concerns && (
                            <div>
                                <div className="font-medium text-foreground">Concerns</div>
                                <div className="mt-1 whitespace-pre-wrap">{note.concerns}</div>
                            </div>
                        )}
                        {note.goals_progress && (
                            <div>
                                <div className="font-medium text-foreground">Goals Progress</div>
                        ...
                        {!note.activities && !note.observations && !note.concerns && !note.goals_progress && (
                            <div className="py-4 text-center text-sm text-muted-foreground">No content recorded.</div>
                        )}
                    </CardContent>
                </Card>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-2 text-sm text-muted-foreground">
                        ...
                        {note.incident_occurred && note.linked_incident && (
                            <div>
                                Linked Incident:{' '}
                                <Link href={`/incidents/${note.linked_incident.id}`} className="text-primary hover:text-primary">
