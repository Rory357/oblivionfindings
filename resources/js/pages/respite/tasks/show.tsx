            ...
            >
                ...
                <Card>
                    ...
                    <CardContent className="space-y-3 text-sm text-muted-foreground">
                        ...
                        {task.due_at && (
                            <div>Due: {formatDateTimeLong(task.due_at)}</div>
                        )}
                        {task.assigned_to && (
                            <div>Assigned to: {task.assigned_to?.name}</div>
                        )}
                        {task.assigned_by && (
                            <div>Assigned by: {task.assigned_by?.name}</div>
                        )}
                        {task.completed_by && (
                            <div>Completed by: {task.completed_by?.name}</div>
                        )}
                        {task.approved_by && (
                            <div>Approved by: {task.approved_by?.name}</div>
                        )}
                        {task.procedure_run && (
                            <div>
                                Procedure Run:{' '}
                                <Link
                                    href={`/respite/procedure-runs/${task.procedure_run.id}`}
                                    className="text-primary hover:text-primary"
                                >
                                    {task.procedure_run.template?.name ||
                                        `Run #${task.procedure_run.id}`}
                                </Link>
                            </div>
                        )}
                    </CardContent>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        {task.status === 'assigned' && (
                            <Button
                                size="sm"
                                onClick={() => router.post(`${base}/start`)}
                            >
                                Start
                            </Button>
                        )}
