...
export default function CateringRecipeShow({
    recipe,
    canManage,
    impact,
}: Props) {
    return (
        <AppLayout
            breadcrumbs={[
                ...
                { title: recipe.name, href: `/catering/recipes/${recipe.id}` },
            ]}
        >
                    ...
                    <PageHero
                        ...
                        actions={
                            canManage && (
                                <Button asChild>
                                    <Link
                                        href={`/catering/recipes/${recipe.id}/edit`}
                                    >
                    ...
                    >
                        <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                            <span className="flex items-center gap-1">
                                <Utensils className="h-4 w-4" /> Serves{' '}
                                {recipe.serves_default}
                            </span>
                            {recipe.prep_minutes !== null && (
                                <span className="flex items-center gap-1">
                                    <Clock className="h-4 w-4" /> Prep{' '}
                                    {recipe.prep_minutes}m
                                </span>
                            )}
                            {recipe.cook_minutes !== null && (
                                <span className="flex items-center gap-1">
                                    <Clock className="h-4 w-4" /> Cook{' '}
            ...
            >
                ...
                {impact &&
                    (impact.report.has_hard_blocks ||
                        ...
                        <div
                            className={`rounded-md border-2 p-4 ${impact.report.has_hard_blocks ? 'border-status-critical/40 bg-status-critical-bg' : 'border-status-warning/30 bg-status-warning-bg'}`}
                        >
                            <div
                                className={`mb-2 flex items-center gap-2 font-semibold ${impact.report.has_hard_blocks ? 'text-status-critical' : 'text-status-warning'}`}
                            >
                            ...
                            {impact.report.has_hard_blocks && (
                                <div className="mb-2">
                                    <div className="text-xs font-semibold tracking-wider text-status-critical uppercase">
                                        Allergy alerts (hard block)
                                    </div>
                                    <ul className="mt-1 space-y-1 text-sm text-status-critical">
                                        {impact.report.hard_blocks.map((p) => (
                                            <li key={`h-${p.client_id}`}>
                            ...
                            {impact.report.has_soft_warnings && (
                                <div>
                                    <div className="text-xs font-semibold tracking-wider text-status-warning uppercase">
                                        Soft warnings
                                    </div>
                                    <ul className="mt-1 space-y-1 text-sm text-status-warning">
                                        {impact.report.soft_warnings.map(
                                            (p) => (
                                                <li key={`s-${p.client_id}`}>
                                                    <strong>
                                                        {p.client_name}
                                                    </strong>
                                                    :{' '}
                ...
                {impact &&
                    ...
                    !impact.report.has_soft_warnings && (
                        <div className="rounded-md border border-status-success/30 bg-status-success-bg p-3 text-sm text-status-success">
                            ✓ No dietary conflicts for current residents at{' '}
                            {impact.site.name}.
                        </div>
                    )}
                            ...
                            {(recipe.ingredients ?? []).map((ing) => (
                                ...
                                >
                                    <span className="font-medium">
                                        ...
                                        ).product?.name ?? ing.free_text_name}
                                    </span>
