...
export default function PlaybooksIndex({ playbooks, filters, categories, stepTypes, can }: Props) {
    ...
    const applyFilter = (key: string, value: string) => {
        ...
        router.get('/control-room/playbooks', newFilters as Record<string, string>, {
            preserveState: true,
            preserveScroll: true,
        });
    };
    ...
    const toggleActive = (playbook: PlaybookSummary) => {
        router.post(`/control-room/playbooks/${playbook.id}/toggle-active`, {}, {
            preserveScroll: true,
        });
    };
    ...
    const handleCreate = (e: FormEvent) => {
        ...
        router.post('/control-room/playbooks', {
            name,
            ...
            sla_response_minutes: slaResponse ? parseInt(slaResponse) : null,
            sla_resolution_minutes: slaResolution ? parseInt(slaResolution) : null,
                ...
                <PageShell>
                                    ...
                                    <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
                                        <form onSubmit={handleCreate}>
                                            <DialogHeader>
                                                ...
                                                <DialogDescription>
                                                    Define a response procedure with ordered steps.
                                                </DialogDescription>
                                            </DialogHeader>
                                            ...
                                            <div className="mt-4 space-y-6">
                                                ...
                                                <div className="grid gap-4 sm:grid-cols-2">
                                                    ...
                                                    <div className="flex items-end gap-6">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            />
                                                            <Label htmlFor="pb-auto">Auto-attach</Label>
                                                        </div>
                                                        <div className="flex items-center gap-2">
                                                            <Switch
                                                                id="pb-approval"
                                                            ...
                                                            />
                                                            <Label htmlFor="pb-approval">Requires Approval</Label>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                {/* SLA Targets */}
                                                <div>
                                                    <h4 className="mb-2 text-sm font-medium">SLA Targets (minutes)</h4>
                                                    <div className="grid gap-3 sm:grid-cols-3">
                                                        <div className="space-y-1">
                                                            <Label htmlFor="sla-ack" className="text-xs text-muted-foreground">Acknowledge</Label>
                                                            <Input
                                                                id="sla-ack"
                                                                type="number"
                                                                min={1}
                                                        ...
                                                        <div className="space-y-1">
                                                            <Label htmlFor="sla-resp" className="text-xs text-muted-foreground">Response</Label>
                                                            <Input
                                                                id="sla-resp"
                                                                type="number"
                                                                min={1}
                                                        ...
                                                        <div className="space-y-1">
                                                            <Label htmlFor="sla-res" className="text-xs text-muted-foreground">Resolution</Label>
                                                            <Input
                                                                id="sla-res"
                                                                type="number"
                                                                min={1}
                                                ...
                                                {/* Required Evidence */}
                                                <div>
                                                    <h4 className="mb-2 text-sm font-medium">Required Evidence</h4>
                                                        ...
                                                        {evidenceOptions.map((type) => (
                                                            <button
                                                                ...
                                                                className={`rounded-full border px-3 py-1 text-xs transition-colors ${
                                                                    requiredEvidence.includes(type)
                                                                            ...
                                                                            <div className="flex flex-wrap items-center gap-4">
                                                                                ...
                                                                                <div className="flex items-center gap-2">
                                                                                    <Switch
                                                                                        ...
                                                                                        onCheckedChange={(v) => updateStep(index, 'is_blocking', v)}
                                                                                    />
                                                                                ...
                                                                                <div className="flex items-center gap-2">
                                                                                    <Input
                                                                                        ...
                                                                                        onChange={(e) => updateStep(index, 'time_limit_minutes', e.target.value)}
                    ...
                    {playbooks.length === 0 ? (
                        ...
                        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            {playbooks.map((pb) => {
                                const catConfig = categoryConfig[pb.category] ?? categoryConfig.maintenance;
                                            ...
                                            <div className="flex items-start justify-between gap-2">
                                                ...
                                                {can.manage && (
                                                    <Switch
                                                        ...
                                                        onCheckedChange={() => toggleActive(pb)}
                                        ...
                                        <CardContent>
                                            ...
                                            <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                                                <span className="flex items-center gap-1">
                                                    ...
                                                    {pb.steps_count} step{pb.steps_count !== 1 ? 's' : ''}
                                                </span>
                                                <span className="flex items-center gap-1">
                                                    <Play className="h-3 w-3" />
                                                    {pb.runs_count} run{pb.runs_count !== 1 ? 's' : ''}
                                                </span>
                                                <span className="flex items-center gap-1">
                                                    <Clock className="h-3 w-3" />
                                                    {formatRelativeTime(pb.last_run_at)}
                                                </span>
                                            </div>
                                            {(pb.sla_acknowledge_minutes || pb.sla_response_minutes || pb.sla_resolution_minutes) && (
                                                <div className="mt-2 flex flex-wrap gap-2">
                                                    {pb.sla_acknowledge_minutes && (
                                                        <span className="rounded bg-muted px-1.5 py-0.5 text-[10px] font-medium">
                                                            ACK {pb.sla_acknowledge_minutes}m
                                                        </span>
                                                    )}
                                                    {pb.sla_response_minutes && (
                                                        <span className="rounded bg-muted px-1.5 py-0.5 text-[10px] font-medium">
                                                            RESP {pb.sla_response_minutes}m
                                                        </span>
                                                    )}
                                                    {pb.sla_resolution_minutes && (
                                                        <span className="rounded bg-muted px-1.5 py-0.5 text-[10px] font-medium">
                                                            RES {pb.sla_resolution_minutes}m
                                                        </span>
                                                    )}
                                                </div>
                                            )}
