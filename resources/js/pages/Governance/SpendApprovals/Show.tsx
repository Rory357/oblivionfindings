      ...
      <PageLayout
        hero={
          <PageHero
            ...
            stats={[
              { label: 'Amount', value: formatNzd(approval.amount) },
              { label: 'Threshold', value: formatNzd(threshold) },
              { label: 'Status', value: approval.status },
            ]}
            badges={approval.requires_board ? [{ label: 'Board sign-off' }] : undefined}
            actions={
      ...
      >
        <div className="grid gap-4 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-4">
              ...
              <CardContent className="space-y-3 text-sm">
                {approval.description && (
                  <div>
                    ...
                    <p className="mt-1 whitespace-pre-wrap">{approval.description}</p>
                  </div>
                )}
                <div className="grid gap-3 sm:grid-cols-2">
                  <div>
                    <p className="text-xs text-muted-foreground">Requested by</p>
                    <p className="mt-1 font-medium">{approval.requestedBy?.name ?? '—'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">Submitted at</p>
                    <p className="mt-1 font-medium">{approval.submitted_at ?? 'Not yet submitted'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">Valid until</p>
                    <p className="mt-1 font-medium">{approval.valid_until ?? '—'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">Currency</p>
                    <p className="mt-1 font-medium">{approval.currency}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
                ...
                <CardContent className="space-y-3 text-sm">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-muted-foreground">
                      {approval.decided_at} by {approval.decidedBy?.name ?? 'unknown'}
                    </span>
                  </div>
                  {approval.decision_notes && (
                    <p className="whitespace-pre-wrap text-sm">{approval.decision_notes}</p>
                  )}
                  {approval.resolution && (
                    <p className="text-xs text-muted-foreground">
                      Linked resolution: {approval.resolution.title} ({approval.resolution.outcome})
                    </p>
                  )}
                </CardContent>
              </Card>
            ...
            {isSubmitted && can.approve && (showApprove || showReject) && (
              <Card>
                <CardHeader>
                  <CardTitle>{showApprove ? 'Approve request' : 'Reject request'}</CardTitle>
                </CardHeader>
                <CardContent>
                  <form onSubmit={showApprove ? handleApprove : handleReject} className="space-y-3">
                    <Textarea
                      rows={4}
                      placeholder={showApprove ? 'Optional decision notes' : 'Reason for rejection (required)'}
                      value={showApprove ? approveForm.data.decision_notes : rejectForm.data.decision_notes}
                      onChange={(e) =>
                        showApprove
                          ? approveForm.setData('decision_notes', e.target.value)
                          : rejectForm.setData('decision_notes', e.target.value)
                      }
                      required={showReject}
                    />
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <Button type="submit" disabled={showApprove ? approveForm.processing : rejectForm.processing}>
                        {showApprove ? 'Confirm approval' : 'Confirm rejection'}
                      </Button>
                    </div>
                  </form>
                </CardContent>
          ...
          <div className="space-y-3">
            <Card>
              ...
              <CardContent className="space-y-2">
                {isDraft && (
                  <Button onClick={handleSubmit} disabled={submitForm.processing} className="w-full">
                    <Send className="mr-2 h-4 w-4" /> Submit for sign-off
                  </Button>
                )}
                {isSubmitted && can.approve && !showApprove && !showReject && (
                  <>
            ...
            {approval.budget && (
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">Linked Budget</CardTitle>
                </CardHeader>
                <CardContent className="text-sm">
                  <p className="font-medium">{approval.budget.title}</p>
                  <p className="text-xs text-muted-foreground">FY {approval.budget.fiscal_year}</p>
                </CardContent>
              </Card>
            )}
          </div>
