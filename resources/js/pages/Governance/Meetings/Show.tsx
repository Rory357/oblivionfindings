...
export default function MeetingShow({
    ...
    const parsedTab = new URLSearchParams(page.url.split('?')[1] ?? '').get(
        'tab',
    );
    const defaultTab =
        parsedTab && validTabs.includes(parsedTab) ? parsedTab : 'agenda';
                ...
                hero={
                    <PageHero
                        ...
                        meta={[
                            {
                                icon: Users,
                                label: `Chair: ${meeting.chair?.user.name ?? 'Unassigned'}`,
                            },
                            {
                                icon: Users,
                                label: `Secretary: ${meeting.secretary?.user.name ?? 'Unassigned'}`,
                            },
                        ]}
                        stats={[
                            {
                                label: 'Workflow',
                                value: `${workflowChecklist.counts.done}/${workflowChecklist.counts.done + workflowChecklist.counts.remaining + workflowChecklist.counts.blocked}`,
                            },
                            {
                                label: 'Quorum',
                            ...
                            <Card>
                                <CardHeader className="flex flex-row items-center justify-between">
                                    <div>
                                        ...
                                        <CardDescription>
                                            {meeting.minutes
                                                ? `Version ${meeting.minutes.version_number} - ${meeting.minutes.status}`
                                                : 'No minutes recorded'}
                                        </CardDescription>
                                    </div>
                                ...
                                <CardContent>
                                    {meeting.minutes ? (
                                            ...
                                            {meeting.minutes.content_blocks &&
                                                Array.isArray(
                                                    ...
                                                    <div className="prose max-w-none">
                                                        {meeting.minutes.content_blocks.map(
                                                            (
                                                                block: {
                                                                    heading: string;
                                                                    content: string;
                                        ...
                                        <p className="py-8 text-center text-muted-foreground">
                                            No minutes have been recorded for
                                            this meeting yet.
                                        </p>
                                    )}
                                </CardContent>
