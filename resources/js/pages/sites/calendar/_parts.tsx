...
export const fmtTimeRange = (s: Date, e: Date | null): string =>
    e ? `${fmtTime(s)} – ${fmtTime(e)}` : fmtTime(s);
...
export function MiniMonth({
    ...
    return (
        <div className="w-60 p-1">
            ...
            <div className="grid grid-cols-7 gap-0.5 px-1 text-center text-[10px] font-medium text-muted-foreground uppercase">
                {WD.map((d) => (
...
export function decorate(item: CalendarItem): Decorated {
    ...
    return {
        ...item,
        _start: start,
        _end: item.end ? parseDT(item.end) : null,
    };
}
...
export function CalendarUIProvider({
    ...
    return (
        <CalendarUICtx.Provider value={value}>
            {children}
        </CalendarUICtx.Provider>
    );
}
...
function RecurGlyph({
    ...
    return ev.isOccurrence || ev.recurrence ? (
...
function MiniChip({
    ...
    return (
        ...
        >
            ...
            <span
                className={`truncate ${overdue ? 'font-semibold text-status-critical' : 'text-foreground/90'}`}
            >
                {ev.title}
            </span>
            <RecurGlyph
                ev={ev}
                className="ml-auto h-2.5 w-2.5 shrink-0 opacity-45"
            />
        </button>
    );
}
...
function TimeBlock({ ev, compact }: { ev: Packed; compact?: boolean }) {
    ...
    const liveH = Math.max(
        baseH + (drag && drag.mode === 'resize' ? dy : 0),
        22,
    );
    const w = 100 / ev._cols;
    const srcLbl = srcByKey[ev.source]?.label ?? ev.source;
    const ariaLabel = `${ev.title}, ${ev.allDay ? 'all day' : fmtTimeRange(ev._start, ev._end)}, ${srcLbl}`;
    return (
        <div
            role="button"
            tabIndex={0}
        ...
        >
            ...
            <div
                className={`truncate text-[12px] leading-tight font-medium text-foreground ${compact ? '' : 'mt-0.5'}`}
            >
                {ev.title}
            </div>
...
function AllDayRow({
    ...
    return (
        ...
        >
            <div className="flex w-14 shrink-0 items-center justify-end pr-2 text-[10px] font-medium tracking-wide text-muted-foreground uppercase">
                All-day
            </div>
            <div
                        ...
                        {events
                            ...
                            .map((e) => (
                                ...
                                >
                                    ...
                                    />
                                    <span
                                        className={`truncate ${e.status === 'overdue' ? 'text-status-critical' : 'text-foreground/90'}`}
                                    >
                                        {e.title}
                                    </span>
                                    <RecurGlyph
...
export function MonthView({
    ...
    const { density, onCreateAt, onContext, onMove, onSelect, onMore } =
        useCalUI();
    const TODAY = new Date();
    ...
    const shown = Array.from({ length: rows * 7 }, (_, i) =>
        addDays(gridStart, i),
    );
    ...
    // Roving tabindex (G-3): exactly one day cell is in the tab order; arrow keys
    // move focus across the grid so empty days are reachable too. Seed to today when
    // the browsed month contains it, otherwise the 1st of the month.
    const cellRefs = useRef<(HTMLDivElement | null)[]>([]);
    const seedIndex = (() => {
        const t = shown.findIndex((d) => sameDay(d, TODAY));
        if (t >= 0) return t;
        const f = shown.findIndex(
            (d) => d.getMonth() === navDate.getMonth() && d.getDate() === 1,
        );
        return f >= 0 ? f : 0;
    })();
    const [focusIdx, setFocusIdx] = useState(seedIndex);
    useEffect(() => {
        setFocusIdx(seedIndex);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [navDate]);

    const focusCell = (i: number) => {
        const clamped = Math.max(0, Math.min(shown.length - 1, i));
        setFocusIdx(clamped);
    ...
    return (
        <div className="flex h-full flex-col overflow-hidden rounded-xl border bg-card">
            ...
            <div
                role="grid"
                aria-label={`${MO[navDate.getMonth()]} ${navDate.getFullYear()}`}
                aria-rowcount={rows}
                aria-colcount={7}
                className="scroll-pretty grid flex-1 grid-cols-7 overflow-y-auto"
                style={{
                    gridTemplateRows: `repeat(${rows}, minmax(118px, 1fr))`,
                }}
            >
                {weeks.map((week, wi) => (
                        ...
                        {week.map((d, di) => {
                            ...
                            return (
                                <div
                                    ...
                                    aria-current={isToday ? 'date' : undefined}
                                    aria-label={`${WD_FULL[d.getDay()]} ${d.getDate()} ${MO[d.getMonth()]} ${d.getFullYear()}${countLabel}`}
                                    onFocus={() => setFocusIdx(i)}
                                    onKeyDown={(e) => onCellKey(e, i, d)}
                                    onClick={() => {
                                        setFocusIdx(i);
                                        onCreateAt?.(d);
                                    }}
                                    ...
                                    className={`group relative flex min-h-0 flex-col overflow-hidden border-r border-b p-1 transition-colors outline-none focus-visible:z-10 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-inset ${cellBg} ${(i + 1) % 7 === 0 ? 'border-r-0' : ''}`}
                                >
                                    {isToday && (
                                        <span
                                            aria-hidden="true"
                                    ...
                                    <div className="space-y-[3px]">
                                        ...
                                        {dayEvents.length > cap && (
                                            <button
                                                ...
                                                aria-label={`Show all ${dayEvents.length} entries on ${WD_FULL[d.getDay()]} ${d.getDate()} ${MO[d.getMonth()]}`}
                                                className="rounded px-1 text-[11px] font-medium text-muted-foreground transition-colors hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none"
                                            >
...
export function WeekView({
    ...
    const ctxAt = (e: React.MouseEvent, d: Date) => {
        ...
        const h = Math.min(
            Math.max(
                GRID_START + Math.floor((e.clientY - r.top) / HOUR_H),
                GRID_START,
            ),
            GRID_END - 1,
        );
    ...
    return (
        <div className="flex h-full flex-col overflow-hidden rounded-xl border bg-card">
            ...
            <div
                ref={scrollRef}
                data-view-scroll
                className="scroll-pretty relative flex-1 overflow-y-auto pt-3"
            >
                    ...
                    <div className="w-14 shrink-0">
                        {HOURS.map((h) => (
                            <div
                                ...
                                style={{ height: HOUR_H }}
                            >
                                <span className="tnum absolute -top-2 right-2 text-[10.5px] font-medium text-muted-foreground">
                                    {h % 12 || 12}
                                    {h >= 12 ? 'pm' : 'am'}
                                </span>
                            </div>
                    ...
                    {days.map((d, i) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <NowLine day={d} />
                                {packed.map((e) => (
                                    <TimeBlock key={e.id} ev={e} compact />
                                ))}
                            </div>
                        );
                    })}
...
export function AgendaView({
    ...
    events: Decorated[];
    navDate: Date;
    /** Every source layer is toggled off (vs a genuinely empty month). */
    sourcesOff?: boolean;
                ...
                <div className="flex h-full flex-col items-center justify-center gap-3 py-16 text-center text-muted-foreground">
                    ...
                    {sourcesOff ? (
                        <p className="max-w-xs text-sm">
                            All source layers are hidden — turn one on in the
                            legend to see entries.
                        </p>
                    ) : filtersActive ? (
                        <p className="max-w-xs text-sm">
                            No entries match your active filters this month. Try
            ...
            {groups.map((g) => {
                ...
                return (
                    <div
                        ...
                        onContextMenu={
                            onContext ? (e) => onContext(e, g.date) : undefined
                        }
                    >
                        <div
                            className={`sticky top-0 flex w-24 shrink-0 flex-col items-center gap-0.5 border-r py-4 ${isToday ? 'bg-primary/5' : 'bg-muted/30'}`}
                        >
                            <span className="text-[11px] font-medium tracking-wide text-muted-foreground uppercase">
                                {WD[g.date.getDay()]}
                            </span>
                            <span
                                className={`tnum text-2xl font-bold ${isToday ? 'text-primary' : 'text-foreground'}`}
                            >
                                {g.date.getDate()}
                            </span>
                            <span className="text-[11px] text-muted-foreground">
                                {MO[g.date.getMonth()].slice(0, 3)}
                            </span>
                        </div>
                            ...
                            {g.items.map((e) => (
                                ...
                                >
                                    ...
                                    <span className="min-w-0 flex-1">
                                        <span
                                            className={`flex items-center gap-1.5 truncate text-sm font-medium ${e.status === 'overdue' ? 'text-status-critical' : 'text-foreground'}`}
                                        >
                                            {e.title}
                                            <RecurGlyph
                                                ev={e}
                                        ...
                                        <span className="flex items-center gap-1.5 text-[12px] text-muted-foreground">
                                            <span
                                                style={{
                                                    color: `var(--src-${e.source})`,
                                                }}
                                            >
                                                {e.typeLabel ||
                                                    srcByKey[e.source]?.label ||
                                                    e.source}
                                            </span>
                                            {e.room && (
                                                <>
                        ...
                        {days.map((d, i) => {
                            ...
                            return (
                                <div
                                    key={i}
                                    style={{ width: COLW }}
                                    className={`flex flex-col items-center justify-center border-l py-1 ${wknd ? 'bg-muted/40' : ''}`}
                                >
                                ...
                                {days.map((d, i) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`relative min-h-[44px] border-l p-0.5 ${wknd ? 'bg-muted/30' : ''} ${sameDay(d, TODAY) ? 'bg-primary/[0.04]' : ''}`}
                                        >
                                                    ...
                                                    .map((e) => {
                                                        ...
                                                        return (
                                                            <button
                                                                ...
                                                                aria-label={`${e.title}, ${e._start.toLocaleDateString('en-NZ', { weekday: 'short', day: 'numeric', month: 'short' })}, ${s.label}`}
                                                                className="h-3 w-7 rounded-full ring-2 ring-card transition-transform hover:scale-125 focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none"
                                                                style={{
                                                                    background:
                                                                        vars[
                                                                            '--c'
...
function RailRow({
    ...
    return (
        ...
        >
            ...
            <span className="min-w-0 flex-1">
                <span
                    className={`block truncate text-[12.5px] font-medium ${ev.status === 'overdue' ? 'text-status-critical' : 'text-foreground'}`}
                >
                    {ev.title}
                </span>
                <span
...
export function TodayRail({
    ...
    const happening = todayItems.find(
        (e) => !e.allDay && e._end && e._start <= now && e._end > now,
    );
    ...
    const firstFutureIdx = todayItems.findIndex(
        (e) =>
            !e.allDay &&
            e._start.getHours() * 60 + e._start.getMinutes() > nowMin,
    );
                ...
                <div className="flex items-start justify-between gap-2 border-b bg-gradient-to-br from-primary/[0.07] to-transparent p-4">
                    <div>
                        ...
                        <div className="mt-0.5 flex items-baseline gap-2">
                            <span className="tnum text-[34px] leading-none font-bold tracking-tight">
                                {now.getDate()}
                            </span>
                ...
                <div className="p-3">
                    {focus ? (
                        ...
                        <div className="flex items-center gap-2.5 rounded-xl border border-dashed p-3 text-[12.5px] text-muted-foreground">
                            ...
                            No more entries scheduled today.
                        </div>
                    )}
                </div>
            </div>
            ...
            {attention > 0 && (
                <div className="overflow-hidden rounded-2xl border border-status-warning/30 bg-card shadow-sm">
                    <div className="flex items-center gap-2 border-b border-status-warning/20 bg-status-warning-bg/50 px-4 py-2.5">
                        ...
                        <span className="tnum ml-auto rounded-full bg-status-warning px-1.5 py-0.5 text-[10px] font-bold text-white">
                            {attention}
                        </span>
                    </div>
            ...
            <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border bg-card shadow-sm">
                <div className="flex items-center justify-between border-b px-4 py-2.5">
                    ...
                    <span className="tnum text-[11px] text-muted-foreground">
                        {todayItems.length}{' '}
                        {todayItems.length === 1 ? 'entry' : 'entries'}
                    </span>
                </div>
                <div className="scroll-pretty min-h-0 flex-1 overflow-y-auto p-1.5">
                    ...
                    {todayItems.map((e, i) => {
                        // The "Now" divider belongs exactly once — immediately before the
                        // first still-upcoming timed item. Deriving it from the single
                        // firstFutureIdx avoids the all-day→timed mis-placement (G-27).
                        const showNow = i === firstFutureIdx;
                        return (
                            <Fragment key={`${e.id}-${e.start}`}>
                                {showNow && (
                                    <div className="my-0.5 flex items-center gap-1.5 px-2">
                                        ...
                                        <span className="text-[10px] font-bold tracking-wide text-status-critical uppercase">
                                            Now · {fmtTime(now)}
                                        </span>
                    ...
                    {upcoming.length > 0 && (
                        <>
                            <div className="mt-1 px-3 pt-3 pb-1 text-[10.5px] font-semibold tracking-wider text-muted-foreground uppercase">
                                Up next
                            </div>
                            {upcoming.map((e) => (
                                <RailRow
