...
export default function PlanInspector({
    ...
    return (
        <div className="space-y-4 text-sm">
                ...
                <div className="grid grid-cols-2 items-center gap-2">
                    ...
                    <Input
                        ...
                        onChange={(event) => {
                            ...
                            dispatch({ type: 'set_canvas_meters_per_unit', metersPerUnit: value });
                        }}
                        className="h-7 text-xs"
                    />
                </div>
                <p className="mt-2 text-[11px] text-muted-foreground">
                    Use the <strong>Set scale</strong> tool on the canvas for a click-and-measure calibration. Walls show their length live.
                </p>
                <div className="mt-3 flex items-center justify-between rounded-md border bg-slate-50 px-2 py-2">
                    <div>
                        <Label className="text-xs">Snap to grid</Label>
                        <p className="text-[11px] text-muted-foreground">Hold Alt to place without snap.</p>
                    </div>
                    <Switch
                        checked={layout.grid?.snap !== false}
                        onCheckedChange={(checked) => dispatch({ type: 'set_grid_snap', snap: checked })}
                    />
                </div>
            </section>
            ...
            {/* Layers ------------------------------------------------------------ */}
            {mode === 'emergency' ? (
                <EmergencyChecklist pins={pins} emergencyKinds={emergencyKinds} taxonomy={taxonomy} dispatch={dispatch} />
            ) : (
                <section className="rounded-md border p-3">
                                ...
                                <div key={group.id} className="flex items-center justify-between">
                                    ...
                                    <Switch
                                        ...
                                        onCheckedChange={(checked) =>
                                            dispatch({ type: 'set_layer_visibility', layer: layerKey, visible: checked })
                                        }
                                    />
                                </div>
                            );
            ...
            <section className="rounded-md border p-3">
                <div className="mb-2 flex items-center justify-between">
                    <h3 className="text-sm font-medium">Rooms ({rooms.length})</h3>
                ...
                <div className="space-y-2">
                    {rooms.length === 0 ? (
                        <p className="text-xs text-muted-foreground">No rooms placed yet.</p>
                    ) : (
                        rooms.map((room) => {
                            const linkedRoom = room.room_ref_id
                                ? inventory?.rooms.find(
                                      (r) => r.id === room.room_ref_id && r.type === room.room_ref_type,
                                  )
                                : null;
                            const selected = selection.some((s) => s.type === 'room' && s.id === room.id);
                            return (
                                <div
                                    key={room.id}
                                ...
                                >
                                    <button
                                        ...
                                        onClick={() => dispatch({ type: 'select', ref: { type: 'room', id: room.id } })}
                                    >
                                        <div className="text-xs font-medium">{room.label ?? 'Unnamed room'}</div>
                                        {linkedRoom ? (
                                    ...
                                    <div className="mt-2 flex flex-wrap gap-1">
                                        <RoomLinkPopover
                                            roomId={room.id}
                                            currentRefId={room.room_ref_id ?? null}
                                            currentRefType={room.room_ref_type ?? null}
                                            inventory={inventory}
                                            dispatch={dispatch}
                                        />
                                        {!linkedRoom && (
            ...
            {/* Pins -------------------------------------------------------------- */}
            <section className="rounded-md border p-3">
                <h3 className="mb-2 text-sm font-medium">Pins ({pins.length})</h3>
                <div className="space-y-2">
                    {pins.length === 0 ? (
                        <p className="text-xs text-muted-foreground">No pins placed yet.</p>
                    ) : (
                        pins
                            .map((pin, index) => ({ pin, index }))
                            .filter(({ pin }) => mode === 'full' || isEmergencyPlanKind(pin.kind, emergencyKinds))
                            .map(({ pin, index }) => {
                            const id = pinIdOf(pin, index);
                            const selected = selection.some((s) => s.type === 'pin' && String(s.id) === id);
                                ...
                                >
                                    <div className="flex items-start justify-between gap-2">
                                        <button
                                            ...
                                            onClick={() => dispatch({ type: 'select', ref: { type: 'pin', id } })}
                                        >
                                        ...
                                        <Button
                                            ...
                                            onClick={() => {
                                                ...
                                                dispatch({ type: 'select', ref: { type: 'pin', id } });
...
function SelectionDetails({
    ...
    if (selection.length === 0) {
        return (
            <div className="rounded-md border border-dashed bg-slate-50 p-3 text-xs text-muted-foreground">
                Select an item on the canvas to edit it. Drag on empty canvas to select multiple items.
            </div>
        );
    }
    ...
    if (selection.length > 1) {
        return (
            <div className="rounded-md border bg-blue-50 p-3 text-xs">
                <div className="mb-2 font-medium text-blue-900">{selection.length} items selected</div>
                <p className="mb-2 text-blue-900/80">Drag any selected item on the canvas to move the whole group.</p>
                <Button
                    type="button"
                    variant="outline"
                    size="sm"
    ...
    if (single.type === 'room') {
        ...
        return (
            <div className="rounded-md border p-3">
                <h3 className="mb-2 text-sm font-medium">Room — {room.label ?? 'Unnamed'}</h3>
                <dl className="space-y-1 text-xs text-slate-700">
                    ...
                    <div className="flex justify-between">
                        ...
                        <dd>
                            {(
                                room.width * canvasWidth * mpu * room.height * canvasHeight * mpu
                            ).toFixed(1)}{' '}
                            m²
                        </dd>
                    </div>
        ...
        return (
            <div className="rounded-md border p-3">
                ...
                <dl className="space-y-1 text-xs text-slate-700">
                    ...
                    <div className="flex justify-between">
                        <dt>Angle</dt>
                        <dd>{((Math.atan2(dy, dx) * 180) / Math.PI).toFixed(1)}°</dd>
                    </div>
                </dl>
                <Button
                    type="button"
    ...
    if (single.type === 'pin') {
        const index = pins.findIndex((pin, i) => pinIdOf(pin, i) === String(single.id));
        if (index === -1) return null;
        const pin = pins[index];
        if (mode === 'emergency' && !isEmergencyPlanKind(pin.kind, emergencyKinds)) return null;
        const kindMeta = taxonomy?.kinds?.[pin.kind];
        const linkedDevice = pin.device_id ? inventory?.devices.find((d) => d.id === pin.device_id) : null;
        const validationError = validationErrors[`pin:${single.id}`];
        return (
            <div className="space-y-2 rounded-md border p-3">
                <h3 className="text-sm font-medium">{kindMeta?.label ?? pin.kind.replaceAll('_', ' ')}</h3>
                {validationError && (
                ...
                <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-3 items-center gap-2">
                        ...
                        <Input
                            value={pin.label ?? ''}
                            onChange={(event) =>
                                dispatch({ type: 'update_pin', pinId: single.id, patch: { label: event.target.value } })
                            }
                        ...
                        <div className="grid grid-cols-3 items-center gap-2">
                            ...
                            <select
                                ...
                                onChange={(event) =>
                                    dispatch({
                                        type: 'update_pin',
                                        pinId: single.id,
                                        patch: { subkind: event.target.value || null },
                                    })
                                }
                            ...
                            {pin.subkind && (
                                <Button
                                    ...
                                    variant="ghost"
                                    size="sm"
                                    className="col-start-2 col-span-2 h-6 justify-start px-1 text-xs"
                                    onClick={() =>
                                        dispatch({
                                            type: 'update_pin',
                                            pinId: single.id,
                            ...
                            <div className="col-span-2">
                                ...
                                {linkedDevice && (
                                    <div className="mt-1 text-[10px] text-muted-foreground">
                                        {linkedDevice.manufacturer ? `${linkedDevice.manufacturer} ` : ''}
                                        {linkedDevice.model}
                                    </div>
                                )}
                            </div>
                ...
                {single.type === 'pin' && (
                    <div className="grid grid-cols-2 gap-1">
                        <Button
                            ...
                            onClick={() => dispatch({ type: 'bring_to_front', ref: single })}
                        >
                        ...
                        <Button
                            ...
                            size="sm"
                            className="h-7 gap-1 text-xs"
                            onClick={() => dispatch({ type: 'send_to_back', ref: single })}
                        >
...
function RoomLinkPopover({
    ...
    const grouped = (inventory?.rooms ?? []).reduce<Record<string, Inventory['rooms']>>((acc, room) => {
                        ...
                        {currentRefId !== null && (
                            <CommandGroup heading="Linked">
                                <CommandItem
                                    onSelect={() => {
                                        dispatch({ type: 'commit' });
                                        dispatch({ type: 'link_room', roomId, ref: null });
                                    }}
                                >
                                    ...
                                    <CommandItem
                                        ...
                                        onSelect={() => {
                                            ...
                                            dispatch({
                                                type: 'link_room',
                                                roomId,
                                                ref: { type: room.type, id: room.id, name: room.name },
                                            });
                                        }}
                                    >
...
function DeviceLinkPopover({
    ...
    const current = currentDeviceId ? devices.find((d) => d.id === currentDeviceId) : null;

    return (
        <Popover>
            <PopoverTrigger asChild>
                <Button type="button" variant="outline" size="sm" className="h-7 w-full justify-start gap-1 text-xs">
                    <Link2 className="h-3 w-3" />
            ...
            <PopoverContent className="w-72 p-0" align="start">
                <Command>
                    ...
                    <CommandList>
                        ...
                        {currentDeviceId !== null && (
                            <CommandGroup heading="Linked">
                                <CommandItem
                                    onSelect={() => {
                                        dispatch({ type: 'commit' });
                                        dispatch({ type: 'link_device', pinId, device: null });
                                    }}
                                >
                                    <Link2Off className="h-4 w-4" />
                                    Unlink
                        ...
                        {devices.length === 0 ? (
                            <div className="px-3 py-4 text-center text-xs text-muted-foreground">
                                No devices assigned to this site. Assign devices in the asset registry.
                            </div>
                        ) : (
