import type { PlanDoor, PlanRoom, PlanWall, PlanWindow } from './_types';

/**
 * Prefix used for walls auto-generated from a room's four edges. Persisted as
 * regular `PlanWall` entries so doors/windows can snap to them like any other
 * wall, but the prefix lets us trace provenance if we ever need to clean up.
 */
export const ROOM_EDGE_WALL_PREFIX = 'wall-room-edge:';

/**
 * Build the four edges of a rectangular room as `PlanWall`s. IDs are
 * deterministic so repeated calls return identical structures and we can
 * detect existing room-edge walls without storing extra metadata.
 */
export function roomEdgeWalls(room: PlanRoom): PlanWall[] {
    const left = room.x;
    const top = room.y;
    ...
    return [
        {
            id: `${ROOM_EDGE_WALL_PREFIX}${room.id}:N`,
            points: [
                { x: left, y: top },
                { x: right, y: top },
            ],
        ...
        {
            id: `${ROOM_EDGE_WALL_PREFIX}${room.id}:E`,
            points: [
                { x: right, y: top },
                { x: right, y: bottom },
            ],
        ...
        {
            id: `${ROOM_EDGE_WALL_PREFIX}${room.id}:S`,
            points: [
                { x: right, y: bottom },
                { x: left, y: bottom },
            ],
        ...
        {
            id: `${ROOM_EDGE_WALL_PREFIX}${room.id}:W`,
            points: [
                { x: left, y: bottom },
                { x: left, y: top },
            ],
...
/**
 * Returns the set of room-edge walls that don't already exist in `walls`.
 * Use this to compute what to dispatch via `add_wall` before placing a door
 * snapped to a room edge.
 */
export function missingRoomEdgeWalls(
    room: PlanRoom,
    walls: PlanWall[],
): PlanWall[] {
    const existing = new Set(walls.map((wall) => wall.id));
    return roomEdgeWalls(room).filter((wall) => !existing.has(wall.id));
}

/**
 * Extract a `room.id` from a wall id auto-generated by `roomEdgeWalls`.
 * Returns null if the id wasn't produced by us.
 */
export function roomIdFromEdgeWallId(wallId: string | null | undefined): string | null {
    if (!wallId || !wallId.startsWith(ROOM_EDGE_WALL_PREFIX)) return null;
    const tail = wallId.slice(ROOM_EDGE_WALL_PREFIX.length);
...
/**
 * Result of an opening snap that includes both the resolved geometry and any
 * walls that need to be created (e.g. when the snap hit a room edge that
 * hasn't been promoted yet).
 */
export type SnapWithFallback = {
    snapped: ResolvedOpening;
    newWalls: PlanWall[];
};

/**
 * Snap an opening to the nearest wall, falling back to room edges if no wall
 * is in range. When the snap hits a room edge, returns the wall objects that
 * need to be persisted before the opening can attach to them.
 */
export function snapOpeningWithRoomFallback(
    point: Point,
    walls: PlanWall[],
    rooms: PlanRoom[],
    canvas: CanvasSize,
    options: { width?: number; maxDistancePx?: number } = {},
): SnapWithFallback | null {
    const initial = snapOpeningToNearestWall(point, walls, canvas, options);
    if (initial) return { snapped: initial, newWalls: [] };

    const virtualWalls = rooms.flatMap(roomEdgeWalls);
    if (virtualWalls.length === 0) return null;

    const fallback = snapOpeningToNearestWall(
        point,
        [...walls, ...virtualWalls],
        canvas,
        options,
    ...
    const roomId = roomIdFromEdgeWallId(fallback.wall_id);
    if (!roomId) return { snapped: fallback, newWalls: [] };

    const room = rooms.find((r) => r.id === roomId);
    if (!room) return { snapped: fallback, newWalls: [] };

    return {
        snapped: fallback,
        newWalls: missingRoomEdgeWalls(room, walls),
    };
}

/**
 * Pick a sensible default `swing_direction` (`in` / `out`) based on which
 * side of the wall the user clicked. The "in" side is the door's local +y
 * direction after the wall's rotation.
 */
export function inferSwingDirection(
    clickPoint: Point,
    snapped: ResolvedOpening,
    canvas: CanvasSize,
): 'in' | 'out' {
    const angle = ((snapped.rotation_deg ?? 0) * Math.PI) / 180;
    const nx = -Math.sin(angle);
    const ny = Math.cos(angle);
    const anchorX = (snapped.x + snapped.width / 2) * canvas.width;
    const anchorY = snapped.y * canvas.height;
    const dx = clickPoint.x * canvas.width - anchorX;
    const dy = clickPoint.y * canvas.height - anchorY;
    return dx * nx + dy * ny >= 0 ? 'in' : 'out';
}
