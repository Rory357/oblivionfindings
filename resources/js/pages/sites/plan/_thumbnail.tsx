...
export { normaliseLayout, type PlanLayout, type PlanPin, type PlanRoom } from './_types';
...
function resolveIcon(name: string): React.ComponentType<{ className?: string }> {
    const candidate = (Lucide as unknown as Record<string, React.ComponentType<{ className?: string }>>)[name];
...
export function PlanThumbnail({ layout, pins = [], className, onCanvasClick, selectedPinId, taxonomy, showScale }: Props) {
            ...
            <svg viewBox={`0 0 ${width} ${height}`} className="h-full min-h-[260px] w-full" onClick={handleClick}>
                ...
                {resolved.grid?.enabled &&
                    Array.from({ length: Math.floor(width / gridSize) + 1 }).map((_, index) => (
                        <line key={`gx-${index}`} x1={index * gridSize} y1={0} x2={index * gridSize} y2={height} stroke="#e2e8f0" strokeWidth={1} />
                    ))}
                {resolved.grid?.enabled &&
                    Array.from({ length: Math.floor(height / gridSize) + 1 }).map((_, index) => (
                        <line key={`gy-${index}`} x1={0} y1={index * gridSize} x2={width} y2={index * gridSize} stroke="#e2e8f0" strokeWidth={1} />
                    ))}
                ...
                {pins.map((pin, index) => {
                    if (pin.kind === 'evacuation_route' && pin.path_points && pin.path_points.length >= 2) {
                        return (
                            <polyline
                                key={`route-${pin.id ?? index}`}
                                points={pin.path_points.map((point) => `${point.x * width},${point.y * height}`).join(' ')}
                                fill="none"
                    ...
                    return (
                        <g key={`${pin.kind}-${pin.id ?? index}`} transform={`translate(${x} ${y})`}>
                            <circle r={selected ? 18 : 14} fill={style.color} stroke="#fff" strokeWidth={4} />
