    ...
    for (const ing of recipe.ingredients) {
        ...
        if (status === 'short' || status === 'out') {
            ...
            if (item?.product.cost_per_unit_cents) {
                ...
                estCents += Math.round(
                    Math.max(0, needed - cur) *
                        item.product.cost_per_unit_cents,
                );
            }
        }
    }
...
export default function RecipesPanel({
    ...
    const filtered = useMemo(() => {
        ...
        return recipes.filter((r) => {
            ...
            if (
                needle &&
                !`${r.name} ${r.tags.map((t) => t.label).join(' ')}`
                    .toLowerCase()
                    .includes(needle)
            )
                return false;
    ...
    return (
        <div className="space-y-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    {canManageTags && (
                        <Button
                            ...
                            className="inline-flex h-9 items-center gap-1.5 rounded-md border border-border bg-card px-3 text-sm font-medium text-foreground transition hover:bg-accent"
                        >
                    ...
                    {canManage && (
                        <Button
                            ...
                            className="inline-flex h-9 items-center gap-1.5 rounded-md bg-sites px-3 text-sm font-semibold text-primary-foreground transition hover:opacity-90"
                        >
            ...
            <p className="text-[12.5px] text-muted-foreground">
                ...
                is your org-wide starting point — nothing is locked. A house can
                use a recipe as-is, or add house-only recipes with{' '}
                <strong className="font-medium text-foreground">
                    Add recipe
                </strong>
                .
                    ...
                    {['all', ...categories].map((c) => (
                        <Button
                            ...
                            className={cn(
                                ...
                                cat === c
                                    ? 'border-sites bg-sites/10 text-sites'
            ...
            {filtered.length === 0 ? (
                filtersActive ? (
                    <GuardrailCard
                        unstyled
                        className="flex flex-col items-center gap-2.5 rounded-xl border border-dashed border-border bg-card px-6 py-12 text-center"
                    >
                    ...
                    >
                        ...
                        <div>
                            ...
                            <p className="mx-auto mt-1 max-w-md text-[13px] text-muted-foreground">
                                Build your house's first recipe, or activate one
                                from the shared library.
                            </p>
                            <p className="mx-auto mt-1 max-w-md text-[12px] text-muted-foreground">
                                Some recipes may be saved as drafts — activate
                                them to plan from them.
                            </p>
                        </div>
                        {canManage && (
                            <Button
                    ...
                    {filtered.map((r) => {
                        ...
                        return (
                            <Button
                                ...
                                type="button"
                                onClick={() => setView(r)}
                                className="group flex flex-col rounded-xl border border-border bg-card p-4 text-left shadow-sm transition-all hover:border-sites/50 hover:shadow-md"
                            >
                                <div className="flex items-start justify-between gap-2">
                                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-sites-bg text-sites-deep">
                                ...
                                <div className="mt-2.5 text-[14.5px] leading-tight font-semibold text-foreground">
                                    {r.name}
                                </div>
                                            ...
                                            <span
                                                ...
                                                className={cn(
                                                    ...
                                                    t.kind === 'allergen'
                                                        ? t.severity ===
                                                          'critical'
                                                            ? 'bg-status-critical font-semibold text-white'
...
function RecipeDetailDialog({
    ...
    const invByProduct = useMemo(
        () => new Map(inventory.map((i) => [i.product_id, i])),
        [inventory],
    );
    const { toBuy, estCents } = recipeStockSummary(recipe, inventory, scale);
    const inStock =
        recipe.ingredients.filter((i) => i.product_id != null).length - toBuy;
    ...
    function buyableIngredients(): RecipeIngredient[] {
        return recipe.ingredients.filter((ing) => {
            ...
            const status = stockStatus(
                needed,
                invByProduct.get(ing.product_id),
            );
            return (
                status === 'short' || status === 'out' || status === 'untracked'
            );
        });
    }
    ...
    async function addToShopping(only?: RecipeIngredient[]) {
        ...
        try {
            const listsRes = await axios.get(
                `/sites/${siteId}/meal-shopping-lists`,
            );
            let draft = (listsRes.data.lists ?? []).find(
                (l: ShoppingDraft) => l.status === 'draft',
            ) as ShoppingDraft | undefined;
            if (!draft) {
                const today = new Date();
                const end = new Date();
                end.setDate(today.getDate() + 6);
                await axios.post(
                    `/sites/${siteId}/meal-shopping-lists/generate`,
                    {
            ...
            for (const ing of items) {
                try {
                    await axios.post(
                        `/sites/${siteId}/meal-shopping-lists/${draft.id}/items`,
                        {
                            product_id: ing.product_id ?? null,
                            free_text_name:
                                ing.product_id == null ? ing.name : null,
            ...
            if (failed.length === 0) {
                ...
                toast.error(
                    `Added ${added} of ${items.length} — ${failed.length} couldn't be added`,
                );
            }
        } catch {
            ...
            <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2">
                        <ChefHat className="h-4 w-4 text-sites" /> {recipe.name}
                    </DialogTitle>
                    <DialogDescription>
                        <span
                            className={cn(
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-1.5 rounded-xl border border-border bg-muted/20 p-3">
                        <div className="flex flex-wrap items-center gap-1.5">
                            <span className="inline-flex items-center gap-1 text-[11px] font-semibold tracking-wide text-status-critical uppercase">
                                <ShieldAlert className="h-3 w-3" /> Contains
                                allergens:
                            </span>
                            {allergenTags.length === 0 ? (
                                ...
                                allergenTags.map((t) => (
                                    <span
                                        key={t.id}
                                        className={cn(
                                            'inline-flex items-center gap-1 rounded-full px-2 py-px text-[11px] font-medium',
                                            t.severity === 'critical'
                                    ...
                                    >
                                        {t.label}
                                        {t.severity === 'critical' && (
                                            <span className="rounded-full bg-white/25 px-1 text-[8.5px] font-bold uppercase">
                                                Critical
                                            </span>
                                        )}
                                    </span>
                        ...
                        {dietaryTags.length > 0 && (
                            <div className="flex flex-wrap items-center gap-1.5">
                                <span className="inline-flex items-center gap-1 text-[11px] font-semibold tracking-wide text-sites-deep uppercase">
                                    <Leaf className="h-3 w-3" /> Dietary:
                                </span>
                                {dietaryTags.map((t) => (
                                    <span
                                        key={t.id}
                                        className="rounded-full bg-sites-bg px-2 py-px text-[11px] font-medium text-sites-deep"
                                    >
                                        {t.label}
                                    </span>
                                ))}
                    ...
                    <div>
                        <div className="mb-1.5 text-[12px] font-semibold tracking-wide text-muted-foreground uppercase">
                            Ingredients &amp; stock check
                        </div>
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <td className="px-3 py-2 text-right text-muted-foreground tabular-nums">
                                                    {item
                                                        ? `${toNum(item.current_qty)} ${item.unit}`
                                                        : '—'}
                                                </td>
                                                <td
                                                    className={cn(
                            ...
                            >
                                {adding ? (
                                    <>
                                        ...
                                        Adding{' '}
                                        {addProgress
                                            ? `${addProgress.current} of ${addProgress.total}`
                                            : ''}
                                        …
                                    </>
                                ) : (
                        ...
                        {addFailures.length > 0 && (
                            <div className="mt-2 rounded-lg border border-status-warning/40 bg-status-warning-bg/40 p-2.5 text-[12px]">
                                <div className="font-medium text-status-warning">
                                    {addFailures.length} item
                                    {addFailures.length === 1 ? '' : 's'}{' '}
                                    couldn't be added:
                                </div>
                                <ul className="mt-1 list-inside list-disc text-muted-foreground">
                                    {addFailures.map((f, i) => (
                                        <li key={i}>{f.name || 'item'}</li>
                                    ))}
                                </ul>
                                <Button
                                    variant="outline"
                                    ...
                                    className="mt-2"
                                    disabled={adding}
                                    onClick={() => addToShopping(addFailures)}
                                >
                    ...
                    {recipe.instructions && (
                        <div>
                            <div className="mb-1 text-[12px] font-semibold tracking-wide text-muted-foreground uppercase">
                                Method
                            </div>
                    ...
                    <div className="flex gap-2">
                        ...
                        {canPlan && (
                            <Button onClick={onPlan}>
                                <CalendarPlus className="mr-1.5 h-4 w-4" /> Plan
                                this meal
                            </Button>
                        )}
                    </div>
