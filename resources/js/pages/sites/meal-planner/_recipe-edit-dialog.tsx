...
function RecipeEditBody({
    ...
    const availability: {
    ...
    }[] = [
        {
            value: 'house',
            icon: Home,
            title: siteName || 'This house',
            description: 'Only this house sees this recipe.',
        },
        {
            value: 'shared',
            icon: Library,
            title: 'Shared library',
            description: 'Added to the org-wide library for every site.',
        },
    ];
    ...
    function updateIngredient(idx: number, p: Partial<IngredientRow>) {
        patch({
            ingredients: data.ingredients.map((row, i) =>
                i === idx ? { ...row, ...p } : row,
            ),
        });
    }
    ...
    const valid =
        ...
        data.ingredients.some(
            (i) => i.product_id != null || i.name.trim() !== '',
        );
    ...
    async function submit(e: React.FormEvent) {
        ...
        const payload = {
            ...
            cook_minutes:
                data.cook_minutes === '' ? null : Number(data.cook_minutes),
    ...
    async function doDelete() {
        ...
        setDeleting(true);
        try {
            await axios.delete(`/catering/recipes/${recipe.id}`, {
        ...
        <form onSubmit={submit}>
            <DialogHeader>
                <DialogTitle className="flex items-center gap-2">
                    {isNew ? (
                        <BookPlus className="h-4 w-4 text-sites" />
                    ) : (
                        <Pencil className="h-4 w-4 text-sites" />
                    )}
                    {isNew ? 'Add recipe' : 'Edit recipe'}
                </DialogTitle>
                <DialogDescription>
                    Recipes power meal planning and the stock check. Link
                    ingredients to inventory products to track stock
                    automatically.
                </DialogDescription>
            </DialogHeader>
            ...
            <div className="mt-3 space-y-4">
                ...
                <div className="grid gap-3 sm:grid-cols-[1fr_160px]">
                    <div>
                        ...
                        <Input
                            ...
                            placeholder="e.g. Creamy chicken & leek pie"
                            autoFocus
                        />
                    </div>
                    <div>
                ...
                <div className="grid grid-cols-3 gap-3">
                    <div>
                        ...
                        <Input
                            ...
                            type="number"
                            min={1}
                            value={data.serves_default}
                            onChange={(e) =>
                                patch({
                                    serves_default: Number(e.target.value),
                                })
                            }
                        />
                    </div>
                    <div>
                        ...
                        <Input
                            ...
                            onChange={(e) =>
                                patch({ prep_minutes: e.target.value })
                            }
                        />
                    </div>
                    <div>
                        ...
                        <Input
                            ...
                            onChange={(e) =>
                                patch({ cook_minutes: e.target.value })
                            }
                        />
                    </div>
                </div>
                ...
                <div className="space-y-3">
                    <div>
                        <Label className="flex items-center gap-1.5 text-status-critical">
                            <ShieldAlert className="h-3.5 w-3.5" /> Contains
                            allergens
                        </Label>
                        <p className="mt-0.5 text-[11px] text-muted-foreground">
                            Allergen tags drive the safety check shown when
                            planning meals.
                        </p>
                        <div
                            role="group"
                        ...
                        >
                            {allergenTags.length === 0 && (
                                <span className="text-xs text-muted-foreground">
                                    No allergen tags are set up yet.
                                </span>
                            )}
                            {allergenTags.map((t) => {
                                ...
                                return (
                                    <Button
                                        ...
                                        className={cn(
                                            'inline-flex min-h-6 items-center gap-1 rounded-full border px-2.5 py-1 text-[12px] font-medium transition-colors focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none',
                                            sel
                                    ...
                                    >
                                        ...
                                        {t.label}
                                        {critical && (
                                            <span className="rounded-full bg-status-critical px-1 text-[8.5px] leading-tight font-bold text-white uppercase">
                                                Critical
                                            </span>
                                        )}
                                    </Button>
                    ...
                    <div>
                        <Label className="flex items-center gap-1.5">
                            <Leaf className="h-3.5 w-3.5 text-sites" /> Dietary
                        </Label>
                        <div
                            role="group"
                            ...
                            className="mt-1.5 flex flex-wrap gap-1.5"
                        >
                            {dietaryTagOpts.length === 0 && (
                                <span className="text-xs text-muted-foreground">
                                    No dietary tags are set up yet.
                                </span>
                            )}
                            {dietaryTagOpts.map((t) => {
                                ...
                                return (
                                    <Button
                                        ...
                                        onClick={() => toggleTag(t.id)}
                                        className={cn(
                                            'inline-flex min-h-6 items-center rounded-full border px-2.5 py-1 text-[12px] font-medium transition-colors focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none',
                                            sel
                ...
                <div>
                    <div className="mb-1.5 flex items-center justify-between">
                        ...
                        <span className="text-[11px] text-muted-foreground">
                            Link to a product to enable stock checks
                        </span>
                    </div>
                ...
                <div>
                    ...
                    <Textarea
                        ...
                        placeholder="Short cooking steps…"
                    />
                </div>
                ...
                <label className="flex items-start gap-2 rounded-lg border border-border bg-muted/30 p-2.5 text-sm">
                    ...
                    <span>
                        ...
                        <span className="ml-1 font-normal text-muted-foreground">
                            — available to plan meals from. Uncheck to keep it
                            as a draft.
                        </span>
                    </span>
                </label>
