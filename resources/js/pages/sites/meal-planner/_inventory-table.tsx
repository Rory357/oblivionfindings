...
function stockState(item: InventoryItem): StockState {
    ...
    if (cur <= 0) return 'out';
    const reorder =
        item.reorder_level == null ? null : toNum(item.reorder_level);
...
function StockGauge({ item }: { item: InventoryItem }) {
    ...
    const reorderPct =
        reorder != null ? Math.min(100, (reorder / scaleMax) * 100) : null;
    const barColor =
        state === 'out'
            ? 'bg-status-critical'
            : state === 'low'
    ...
    return (
        <div className="w-40">
            <div
                ...
                aria-label={`${formatQty(cur, item.unit)} in stock, ${stateLabel}`}
            ...
            >
                ...
                {reorderPct != null && (
                    <div
                        className="absolute top-1/2 h-3.5 w-px -translate-y-1/2 bg-status-critical/70"
                        style={{ left: `${reorderPct}%` }}
                        title={`Reorder at par (${formatQty(reorder, item.unit)})`}
                        aria-hidden="true"
                    />
                )}
            </div>
            <div className="mt-1 text-[11px] text-muted-foreground tabular-nums">
                {formatQty(cur, item.unit)}
                {par != null ? ` / par ${par}` : ''}
            </div>
        </div>
    );
...
export default function InventoryTable({
    ...
    const filtered =
        cat === 'all' ? items : items.filter((i) => i.product.category === cat);

    const itemsTracked = items.length;
    const value = items.reduce(
        (s, i) =>
            s +
            (i.product.cost_per_unit_cents
                ? Math.round(
                      toNum(i.current_qty) * i.product.cost_per_unit_cents,
                  )
                : 0),
        0,
    );
    ...
    async function quickAdjust(item: InventoryItem, sign: 1 | -1) {
        setBusyId(item.id);
        try {
            await axios.post(`/sites/${siteId}/meal-inventory/adjust`, {
    ...
    async function destroy(item: InventoryItem) {
        try {
            await axios.delete(
                `/sites/${siteId}/meal-inventory/items/${item.id}`,
            );
                ...
                >
                    {categories.map((c) => (
                        <Button
                            ...
                            onClick={() => setCat(c)}
                            className={cn(
                                'rounded-full border px-3 py-1 text-[12px] font-medium capitalize transition-colors focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none',
                                c === cat
                ...
                {(canAdjust || canManageProducts) && (
                    <div className="flex gap-2">
                        {canManageProducts && (
                            ...
                            >
                                <Package className="mr-1.5 h-[15px] w-[15px]" />{' '}
                                Manage products
                            </Button>
                        )}
                        {canAdjust && (
                            ...
                            >
                                <ClipboardCheck className="mr-1.5 h-[15px] w-[15px]" />{' '}
                                Stocktake
                            </Button>
                        )}
                        {canAdjust && (
                    ...
                    <table className="w-full min-w-[820px] text-sm">
                        ...
                        <tbody>
                            ...
                            {filtered.map((i) => {
                                ...
                                const val =
                                    i.product.cost_per_unit_cents != null
                                        ? Math.round(
                                              toNum(i.current_qty) *
                                                  i.product.cost_per_unit_cents,
                                          )
                                    ...
                                    >
                                        <td className="px-4 py-2.5">
                                            <div className="flex items-center gap-2">
                                                ...
                                                {state === 'out' && (
                                                    <span className="rounded-full bg-status-critical px-1.5 py-px text-[9px] font-bold text-white uppercase">
                                                        Out
                                                    </span>
                                                )}
                                                {state === 'low' && (
                                                    <span className="rounded-full bg-status-warning px-1.5 py-px text-[9px] font-bold text-white uppercase">
                                                        Low
                                                    </span>
                                                )}
                                            </div>
                                            ...
                                            <td className="px-4 py-2.5">
                                                <div className="flex items-center justify-end gap-1">
                                                    ...
                                                    <Button
                                                        ...
                                                        variant="ghost"
                                                        className="h-7 w-7"
                                                        aria-label={`Adjust ${i.product.name}`}
                                                        onClick={() =>
                                                            onOpenAdjust(i)
                                                        }
                                                    >
                                                    ...
                                                    <ConfirmAction
                                                        ...
                                                        description="Removes the product from this site's inventory list. Movement history is kept for audit."
                                                        confirmLabel="Remove"
                                                        onConfirm={() =>
                                                            destroy(i)
                                                        }
                                                    >
                                                        <Button
                                                            ...
                                                            variant="ghost"
                                                            className="h-7 w-7"
                                                            aria-label={`Delete ${i.product.name}`}
                                                        >
                                                            <Package className="h-3.5 w-3.5 text-status-critical" />
                                                        </Button>
                                                    </ConfirmAction>
                                                </div>
                                            </td>
