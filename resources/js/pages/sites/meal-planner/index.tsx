...
import {
    DietaryTagsManagerDialog,
    ProductsManagerDialog,
} from './_library-dialogs';
...
const PlanEntryDialog = lazy(() =>
    import('./_dialogs').then((m) => ({ default: m.PlanEntryDialog })),
);
const AdjustInventoryDialog = lazy(() =>
    import('./_dialogs').then((m) => ({ default: m.AdjustInventoryDialog })),
);
const StocktakeDialog = lazy(() =>
    import('./_dialogs').then((m) => ({ default: m.StocktakeDialog })),
);
const ShoppingListGenerateDialog = lazy(() =>
    import('./_dialogs').then((m) => ({
        default: m.ShoppingListGenerateDialog,
    })),
);
const SettingsDialog = lazy(() =>
    import('./_dialogs').then((m) => ({ default: m.SettingsDialog })),
);
...
export default function MealPlannerSubTabs({
    site: siteProp,
    mode = 'embedded',
    defaultSiteId,
}: Props) {
    ...
    const [site, setSite] = useState<SiteInfo>({
        id: initialSiteId,
        name: siteProp?.name ?? 'Meal Planner',
        type: siteProp?.type ?? 'house',
        suburb: null,
        region: null,
        weekly_food_budget_cents: null,
    });
    const [recipes, setRecipes] = useState<RecipeFull[]>([]);
    const [products, setProducts] = useState<
        { id: number; name: string; default_unit: string }[]
    >([]);
    const [productCategories, setProductCategories] = useState<string[]>([]);
    const [residents, setResidents] = useState<Resident[]>([]);
    const [templates, setTemplates] = useState<
        import('./_helpers').WeekTemplate[]
    >([]);
    const [sites, setSites] = useState<SiteSearchItem[]>([]);
    const [iddsiLevels, setIddsiLevels] = useState<IddsiLevel[]>([]);
    const [dietaryTags, setDietaryTags] = useState<
        { id: number; label: string; kind: 'allergen' | 'dietary' }[]
    >([]);
    const [perms, setPerms] = useState({
        plan: false,
        inventory_adjust: false,
        shopping_manage: false,
    ...
    const [tab, setTab] = useState<SubTab>(
        (siteProp?.type ?? 'house') === 'house' ? 'calendar' : 'inventory',
    );
    ...
    }>({ open: false, entry: null, date: toIsoDate(weekStart), slot: 'lunch' });
    ...
    const bootstrap = useCallback(async () => {
        ...
        try {
            const res = await axios.get(
                `/sites/${currentSiteId}/meal-planner/bootstrap`,
            );
            ...
            if (nextSite) {
                ...
                if (standalone && nextSite.type !== 'house') {
                    setTab((t) =>
                        t === 'calendar' || t === 'templates' ? 'inventory' : t,
                    );
                }
            }
            ...
            const status = (e as { response?: { status?: number } }).response
                ?.status;
            const kind: 'access' | 'session' | 'generic' =
                status === 403
                    ? 'access'
    ...
    const reloadCalendar = useCallback(async () => {
        ...
        try {
            const [planRes, summaryRes] = await Promise.all([
                axios.get(`/sites/${currentSiteId}/meal-plan`, {
                    params: { week },
                }),
                axios.get(`/sites/${currentSiteId}/meal-plan/week-summary`, {
                    params: { week },
                }),
            ]);
            ...
            markStale('calendar');
            toast.error(
                "Couldn't refresh meals for this week — showing last loaded data.",
            );
        } finally {
    ...
    const reloadInventory = useCallback(async () => {
        ...
        try {
            const res = await axios.get(
                `/sites/${currentSiteId}/meal-inventory`,
            );
            ...
            markStale('inventory');
            toast.error(
                "Couldn't refresh inventory — showing last loaded data.",
            );
        }
    ...
    const reloadLists = useCallback(async () => {
        ...
        try {
            const res = await axios.get(
                `/sites/${currentSiteId}/meal-shopping-lists`,
            );
            ...
            markStale('shopping');
            toast.error(
                "Couldn't refresh shopping lists — showing last loaded data.",
            );
        }
    }, [currentSiteId, markStale, clearStale]);
    ...
    const reloadTemplates = useCallback(async () => {
        ...
        try {
            const res = await axios.get(
                `/sites/${currentSiteId}/meal-templates`,
            );
            setTemplates(res.data.templates ?? []);
            ...
            markStale('templates');
            toast.error(
                "Couldn't refresh templates — showing last loaded data.",
            );
        }
    }, [currentSiteId, markStale, clearStale]);
    ...
    const stats: HeroStats = useMemo(() => {
        ...
        const overrides = entries.filter(
            (e) => !!e.allergen_override_at,
        ).length;
        const itemsTracked = inventory.length;
        const outOfStock = inventory.filter(
            (i) => toNum(i.current_qty) <= 0,
        ).length;
        const lowStock = inventory.filter((i) => {
            const cur = toNum(i.current_qty);
            const reorder =
                i.reorder_level == null ? null : toNum(i.reorder_level);
        ...
        const fillPct = isHouse
            ? Math.round((filled / 21) * 100)
            : itemsTracked
              ? Math.round((atPar / itemsTracked) * 100)
              : 0;
        ...
        const textureModified = residents.filter(
            (r) => r.texture != null && r.texture.level < 7,
        ).length;
    ...
    const notifications: HeroNotification[] = useMemo(() => {
        ...
        if (isHouse && stats.unresolved > 0)
            out.push({
                ...
                icon: ShieldAlert,
                tone: 'critical',
                label: `${stats.unresolved} allergen conflict${stats.unresolved === 1 ? '' : 's'} to resolve`,
                tab: 'calendar',
            });
        if (isHouse && stats.textureEntries > 0)
            out.push({
                ...
                icon: Soup,
                tone: 'warning',
                label: `${stats.textureEntries} meal${stats.textureEntries === 1 ? '' : 's'} need a texture check`,
                tab: 'calendar',
            });
        if (isHouse && stats.softWarnings > 0)
            out.push({
                ...
                icon: TriangleAlert,
                tone: 'warning',
                label: `${stats.softWarnings} meal${stats.softWarnings === 1 ? '' : 's'} with a soft warning`,
                tab: 'calendar',
            });
        if (stats.outOfStock > 0)
            out.push({
                ...
                icon: CircleAlert,
                tone: 'critical',
                label: `${stats.outOfStock} item${stats.outOfStock === 1 ? '' : 's'} out of stock`,
                tab: 'inventory',
            });
        if (stats.lowStock > 0)
            out.push({
        ...
        const expiring = inventory.filter((i) => {
            ...
            const days =
                (new Date(i.expiry_date).getTime() - today.getTime()) /
                86400000;
            return days <= 3;
        });
        if (expiring.length > 0)
            out.push({
                ...
                icon: Clock,
                tone: 'warning',
                label: `${expiring.length} item${expiring.length === 1 ? '' : 's'} expiring soon`,
                sub: expiring
    ...
    /* ── week label ──────────────────────────────────────────────────────── */
    const rangeStart = weekStart.toLocaleDateString('en-NZ', {
        day: 'numeric',
        month: 'short',
    });
    const rangeEnd = addDays(weekStart, 6).toLocaleDateString('en-NZ', {
        day: 'numeric',
        month: 'short',
    });
    ...
    function selectSite(id: number) {
        ...
        if (standalone) {
            ...
            if (typeof window !== 'undefined')
                window.history.replaceState({}, '', `/catering?site=${id}`);
        } else {
    ...
    function planToday() {
        ...
        const slot: MealSlot =
            hour >= 15 ? 'dinner' : hour >= 12 ? 'lunch' : 'breakfast';
        // Don't bounce the user off a week that already contains today.
        if (toIsoDate(weekStart) !== toIsoDate(startOfWeek(today)))
    ...
    return (
        <div className="space-y-5">
            ...
            {isHouse && residents.length === 0 && !onboardingDismissed && (
                <ZeroResidentOnboarding
                    siteId={site.id}
                    onDismiss={() => setOnboardingDismissed(true)}
                    onSetBudget={() => setSettingsOpen(true)}
                    onPlanMeals={() => setTab('calendar')}
                />
            )}
            ...
            <div
                ...
                tabIndex={0}
                aria-busy={tab === 'calendar' && reloadingCalendar}
                className="focus-visible:outline-none"
            >
                {tab === 'calendar' && staleSurfaces.has('calendar') && (
                    <StaleStrip onRetry={reloadCalendar} />
                )}
                {tab === 'inventory' && staleSurfaces.has('inventory') && (
                    <StaleStrip onRetry={reloadInventory} />
                )}
                {tab === 'shopping' && staleSurfaces.has('shopping') && (
                    <StaleStrip onRetry={reloadLists} />
                )}
                {tab === 'templates' && staleSurfaces.has('templates') && (
                    <StaleStrip onRetry={reloadTemplates} />
                )}
                ...
                {tab === 'inventory' && (
                    <InventoryTable
                        ...
                        onOpenAdjust={(i) =>
                            setAdjustDialog({ open: true, item: i })
                        }
            ...
            <DialogErrorBoundary
                onError={() =>
                    toast.error("Couldn't open — please reload the page.")
                }
            >
            ...
            {spendOpen && (
                <SpendReportDialog
                    siteId={site.id}
                    currentWeekCents={weekTotalCents}
                    budgetCents={site.weekly_food_budget_cents}
...
function PlannerLoadError({
    ...
    return (
        <div
            role="alert"
            className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-status-critical/30 bg-status-critical-bg/40 px-6 py-12 text-center"
        >
            ...
            <div>
                <div className="text-[16px] font-semibold text-foreground">
                    We couldn't load the meal planner
                </div>
    ...
    return (
        <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-border bg-card px-6 py-12 text-center">
            ...
            <div>
                ...
                <p className="mx-auto mt-1 max-w-md text-[13.5px] text-muted-foreground">
                    Choose a house or office to plan meals, track inventory and
...
function ZeroResidentOnboarding({
    siteId,
    onDismiss,
    onSetBudget,
    onPlanMeals,
    ...
    const numClass =
        'mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-primary/15 text-[11px] font-bold text-primary';
    return (
        <div className="relative rounded-xl border border-primary/30 bg-primary/5 p-4 pr-9">
            <Button
                ...
                className="absolute top-2 right-2 flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-accent hover:text-foreground"
            >
                <X className="h-4 w-4" />
            </Button>
            <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                    <div className="text-[14.5px] font-semibold text-foreground">
                        Finish setting up this house to plan meals safely.
                    </div>
                    <p className="mt-0.5 text-[12.5px] text-muted-foreground">
                        No residents are linked yet, so allergen and texture
                        checks are paused.
                    </p>
                    <ol className="mt-3 space-y-2 text-[13px]">
                        <li className={stepClass}>
                            ...
                            <span className="min-w-0">
                                ...
                                >
                                    Add residents &amp; their dietary needs{' '}
                                    <ArrowUpRight className="h-3.5 w-3.5" />
                                </a>
                                <span className="ml-1 text-muted-foreground">
                                    — opens the site profile in a new tab
                                </span>
                            </span>
                        </li>
                        <li className={stepClass}>
                            ...
                            <span className="min-w-0">
                                <Button
                                    ...
                                    className="inline-flex items-center gap-1 font-medium text-primary hover:underline"
                                >
                                    <Wallet className="h-3.5 w-3.5" /> Set a
                                    weekly food budget
                                </Button>
                            </span>
                        </li>
                        <li className={stepClass}>
                            ...
                            <span className="min-w-0">
                                ...
                                >
                                    ...
                                    Plan meals or apply a template
                                </Button>
                            </span>
                        </li>
                    </ol>
...
function StaleStrip({ onRetry }: { onRetry: () => void }) {
    return (
        <div
            role="status"
            className="mb-2 flex items-center gap-2 rounded-lg border border-status-warning/40 bg-status-warning-bg/60 px-3 py-1.5 text-[12.5px] font-medium text-status-warning"
        >
            <TriangleAlert
            ...
            <span className="flex-1">
                Showing data from before your last action — it didn't refresh.
            </span>
            <Button
                unstyled
                type="button"
                onClick={onRetry}
                className="shrink-0 rounded-md border border-status-warning/50 px-2 py-0.5 text-[11.5px] font-semibold transition-colors hover:bg-status-warning/10"
            >
                Retry
            </Button>
        </div>
...
function SubTabs({
    ...
    return (
        <GuardrailCard
            ...
            className="flex items-center gap-1 overflow-x-auto rounded-xl border border-border bg-card p-1 shadow-sm"
        >
            {ordered.map((it, i) => {
                ...
                return (
                    <Button
                        ...
                        className={cn(
                            'inline-flex min-h-10 flex-1 items-center justify-center gap-2 rounded-lg px-3 py-2 text-[13.5px] font-medium whitespace-nowrap transition-all focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1 focus-visible:outline-none sm:flex-none',
                            active
