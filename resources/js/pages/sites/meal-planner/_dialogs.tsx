...
import {
    addDays,
    allergenResidentsFor,
    dietaryMismatchesFor,
    firstName,
    fluidsResidentsFor,
    formatQty,
    joinNames,
...
export function PlanEntryDialog({
    ...
    const initialSource: SourceType =
        entry?.source_type ??
        (entry?.takeaway_vendor
            ? 'takeaway'
            : entry?.recipe_id || initialRecipeId
              ? 'recipe'
              : 'ad_hoc');

    const initialTakeawayCost =
        entry?.takeaway_cost_cents != null
    ...
    useEffect(() => {
        ...
        axios
            .get(`/sites/${siteId}/meal-planner/takeaway-vendors`)
            .then((res) => {
    ...
    const selectedRecipe =
        form.data.source_type === 'recipe' && form.data.recipe_id != null
            ? recipes.find((r) => r.id === form.data.recipe_id)
            : undefined;
    const textureResidents = showResidents
        ? textureResidentsFor(residents, selectedClientIds)
        : [];
    const fluidsResidents = showResidents
        ? fluidsResidentsFor(residents, selectedClientIds)
        : [];
    const allergenResidents = showResidents
        ? allergenResidentsFor(residents, selectedClientIds)
        : [];
    const dietaryMismatchList =
        showResidents && selectedRecipe
            ? dietaryMismatchesFor(selectedRecipe, residents, selectedClientIds)
            : [];
    ...
    const careClauses = [
        ...textureResidents.map(
            (r) =>
                `${firstName(r.name)} needs IDDSI ${r.texture!.level} (${r.texture!.label})`,
        ),
        ...fluidsResidents.map(
            (r) => `${firstName(r.name)} needs ${r.fluids} fluids`,
        ),
    ];
    ...
    function submit(e: React.FormEvent) {
        ...
        if (isNew) {
            form.post(`/sites/${siteId}/meal-plan`, {
                onSuccess,
                onError,
                preserveScroll: true,
            });
        } else {
            form.put(`/sites/${siteId}/meal-plan/${entry!.id}`, {
                onSuccess,
                onError,
                preserveScroll: true,
            });
    ...
    function toggleServed() {
        ...
        router.post(
            ...
            {},
            {
                ...
                onSuccess: () => onClose(),
                onError: () => {
                    const msg = entry.served_at
                        ? "Couldn't mark not served — try again"
                        : "Couldn't mark served — try again";
                    toast.error(msg);
                    announce(msg);
                },
            },
        ...
        <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
            <DialogContent className="max-w-xl">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Choose the meal time, source and residents, then record
                        any planning notes.
                    </DialogDescription>
                </DialogHeader>
                <form onSubmit={submit} className="space-y-3">
                            ...
                            <div className="flex items-start gap-2 rounded-md border border-status-critical/30 bg-status-critical-bg/60 p-2 text-xs text-status-critical">
                                ...
                                <div>
                                    <div className="font-medium">
                                        Allergen override on file
                                    </div>
                                    <div className="mt-0.5">
                                        &ldquo;{existingOverrideReason}&rdquo;
                                    </div>
                                    {entry.allergen_override_by && (
                                        <div className="mt-0.5 text-status-critical/80">
                                            by {entry.allergen_override_by.name}{' '}
                                            ·{' '}
                                            {new Date(
                                                entry.allergen_override_at,
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <Input
                                type="date"
                                value={form.data.plan_date}
                                onChange={(e) =>
                                    form.setData('plan_date', e.target.value)
                                }
                                required
                            />
                        </div>
                        <div>
                            ...
                            <Select
                                value={form.data.meal_slot}
                                onValueChange={(v) =>
                                    form.setData('meal_slot', v as MealSlot)
                                }
                            >
                                <SelectTrigger>
                                    <SelectValue />
                                ...
                                [
                                    {
                                        ...
                                        label: 'From recipe',
                                        icon: ChefHat,
                                        desc: 'Cooked from a saved recipe',
                                    },
                                    {
                                        key: 'ad_hoc',
                                        label: 'Ad-hoc cook',
                                        icon: Utensils,
                                        desc: 'Cooked without a recipe',
                                    },
                                    {
                                        key: 'takeaway',
                                        label: 'Takeaway',
                                        icon: ShoppingBag,
                                        desc: 'Ordered in from a vendor',
                                    },
                                ] as {
                                    key: SourceType;
                                    label: string;
                                ...
                                return (
                                    <Button
                                        ...
                                        onClick={() => {
                                            ...
                                            if (m.key !== 'recipe')
                                                form.setData('recipe_id', null);
                                            if (m.key !== 'ad_hoc')
                                                form.setData('ad_hoc_name', '');
                                            if (m.key !== 'takeaway') {
                                                form.setData(
                                                    'takeaway_vendor',
                                                    '',
                        ...
                        <>
                            <div>
                                ...
                                <Input
                                    ...
                                    placeholder="e.g. Cheese toasties"
                                />
                            </div>
                            {allergenReminder ? (
                                <div className="flex items-start gap-2 rounded-md border border-status-warning/30 bg-status-warning-bg p-2 text-xs text-status-warning">
                                    ...
                                    />
                                    <span>
                                        {allergenReminder} Ad-hoc meals aren't
                                        auto-checked — confirm ingredients
                                        before serving.
                                    </span>
                                </div>
                            ) : (
                                <div className="flex items-start gap-2 rounded-md border border-muted bg-muted/30 p-2 text-xs text-muted-foreground">
                                    ...
                                    />
                                    <span>
                                        No allergen check available for ad-hoc
                                        meals — check ingredients carefully
                                        before serving.
                                    </span>
                                </div>
                            )}
                    ...
                    {form.data.source_type === 'takeaway' && (
                        <>
                            ...
                            <div className="flex items-start gap-2 rounded-md border border-status-warning/30 bg-status-warning-bg p-2 text-xs text-status-warning">
                                ...
                                <span>
                                    <strong>Takeaway meal</strong> —{' '}
                                    {allergenReminder
                                        ? `${allergenReminder} Confirm dietary suitability with the vendor before serving.`
                                        : 'allergen check unavailable. Confirm dietary suitability with the vendor before serving.'}
                                </span>
                            </div>
                        </>
                    )}
                    <div>
                        ...
                        <Input
                            ...
                            onChange={(e) =>
                                form.setData('servings', Number(e.target.value))
                            }
                        />
                    </div>
                    <div>
                        ...
                        <Textarea
                            ...
                            rows={2}
                            placeholder="Intake / refusals (e.g. 'Aroha ate half, refused vegetables'; 'Mila — dairy-free portion plated')"
                        />
                        <p className="mt-1 text-[11px] text-muted-foreground">
                            One free-text note for the whole meal — not a
                            per-resident intake record.
                        </p>
                    </div>
                        ...
                        <div>
                            <div className="mb-1 flex items-center justify-between">
                                <Label className="mb-0">
                                    Residents{' '}
                                    <span className="font-normal text-muted-foreground">
                                        (allergens checked against these)
                                    </span>
                                </Label>
                        ...
                        <div className="flex items-start gap-2 rounded-md border border-status-warning/30 bg-status-warning-bg p-2.5 text-xs text-status-warning">
                            ...
                            <div>
                                <div className="font-semibold">
                                    {textureResidents.length > 0
                                        ? 'Texture-modified diet'
                                        : 'Thickened fluids'}
                                </div>
                                <div className="mt-0.5">
                                    {careClauses.join(', ')}. Confirm this meal
                                    is prepared to the right texture and
                                    consistency.
                                </div>
                            </div>
                        </div>
                    ...
                    {dietaryMismatchList.length > 0 && (
                        <div className="flex items-start gap-2 rounded-md border border-status-warning/30 bg-status-warning-bg p-2.5 text-xs text-status-warning">
                            ...
                            <div>
                                ...
                                <div className="mt-0.5">
                                    {joinNames(
                                        dietaryMismatchList.map(
                                            (m) =>
                                                `${firstName(m.resident.name)} is ${m.requirements.join(', ')}`,
                                        ),
                                    )}{' '}
                                    — confirm this meal meets{' '}
                                    {dietaryMismatchList.length === 1
                                        ? 'it'
                                        : 'them'}
                                    .
                                </div>
                            ...
                            <div className="rounded-md border-2 border-status-critical bg-status-critical-bg/60 p-3 text-sm">
                                <div className="mb-1 flex items-center gap-2 font-semibold text-status-critical">
                                    <ShieldAlert
                                        ...
                                        aria-hidden="true"
                                    />{' '}
                                    Couldn't verify allergens for this meal
                                </div>
                                <p className="text-status-critical">
                                    Do not save until allergens are verified.
                                </p>
                                <Button
                                    type="button"
                                    size="sm"
                                    variant="outline"
                                    className="mt-2 border-status-critical/50 text-status-critical hover:bg-status-critical-bg"
                                    onClick={retryCheck}
                                >
                                    <Loader2
                                        className={cn(
                    ...
                    {hasHard && (
                        <div
                            role="alert"
                            className="rounded-md border-2 border-status-critical bg-status-critical-bg/60 p-3 text-sm"
                        >
                            ...
                            <ul className="space-y-1.5 text-status-critical">
                                {report.hard_blocks.map((panel) => {
                                    const sorted = [...panel.matches].sort(
                                        (a, b) =>
                        ...
                        <div className="rounded-md border border-status-warning/30 bg-status-warning-bg p-3 text-sm">
                                ...
                                {report.soft_warnings.map((panel) => (
                                    <li key={panel.client_id}>
                                        ...
                                        {panel.matches
                                            .map(
                                                (m) =>
                                                    `${m.label}${m.kind === 'dislike' ? ' (dislike)' : ''}`,
                                            )
                                            .join(', ')}
                                    </li>
                                ))}
                            ...
                            <label className="mt-2 flex items-center gap-2 text-xs text-status-warning">
                                <input
                                    ...
                                    onChange={(e) =>
                                        setAcknowledgedSoft(e.target.checked)
                                    }
                                />
                                I've reviewed these and want to plan this meal
                                anyway
                            </label>
                        </div>
                    )}
...
> = {
    ...
    set: {
        ...
        reason: 'stocktake',
        reasons: [{ value: 'stocktake', label: 'Counted on hand' }],
    },
};
...
export function AdjustInventoryDialog({
    ...
    const currentQty = item
        ? typeof item.current_qty === 'string'
    ...
    const preview = useMemo(() => {
        if (numericQty === null || Number.isNaN(numericQty) || numericQty < 0)
            return null;
    ...
    function submit(e: React.FormEvent) {
        ...
        if (mode === 'set') {
            router.post(
                `/sites/${siteId}/meal-inventory/stocktake`,
                {
            ...
            router.post(
                `/sites/${siteId}/meal-inventory/adjust`,
                {
    ...
    async function createProduct(e: React.FormEvent) {
        ...
        if (newProductCategory === '__new__') {
            ...
            if (draft.length === 0) {
                setNewProductError(
                    'Type a category name or pick an existing one',
                );
                return;
            }
        ...
        try {
            ...
            const apiMsg =
                axErr.response?.data?.errors?.name?.[0] ??
    ...
    return (
        <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>
                        {item ? item.product.name : 'Update inventory'}
                    </DialogTitle>
                </DialogHeader>
                <form onSubmit={submit} className="space-y-4">
                        ...
                        <div>
                            ...
                            {canCreateProducts && (
                                <p className="mt-1 text-xs text-muted-foreground">
                                    Can't find it? Pick "Create new product" at
                                    the top of the list.
                                </p>
                            )}
                        </div>
                        ...
                        <div className="rounded-md border-2 border-primary/40 bg-primary/5 p-3">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                ...
                                <div>
                                    ...
                                    <Select
                                        value={newProductCategory || '__none__'}
                                        onValueChange={(v) => {
                                            setNewProductCategory(
                                                v === '__none__' ? '' : v,
                                            );
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {(['add', 'remove', 'set'] as AdjustMode[]).map(
                                (m) => (
                                    <Button
                                        unstyled
                                        key={m}
                    ...
                    <div>
                        ...
                        <Input
                            ...
                            placeholder="e.g. delivered by FreshChoice"
                        />
                    </div>
                    ...
                    <DialogFooter>
                        ...
                        <Button
                            type="submit"
                            disabled={submitting || !productId || qty === ''}
    ...
    function submit(e: React.FormEvent) {
        ...
        router.post(
            ...
            { counts: rows, note },
            {
                ...
                onSuccess: () => onClose(),
                onError: () => {
                    toast.error("Stocktake didn't save — try again");
                    announce("Stocktake didn't save");
                },
                onFinish: () => setSubmitting(false),
            },
        );
            ...
            <DialogContent className="max-w-2xl">
                ...
                <form onSubmit={submit} className="space-y-3">
                    <p className="text-sm text-muted-foreground">
                        Set the on-hand quantity for each item you count. Items
                        left blank are skipped.
                    </p>
                                ...
                                {items.map((i) => (
                                    <tr key={i.id} className="border-t">
                                        ...
                                        <td className="px-3 py-2 text-muted-foreground">
                                            {formatQty(i.current_qty, i.unit)}
                                        </td>
        ...
        <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Builds a list from this week's planned meals, plus
                        anything below par if enabled.
                    </DialogDescription>
                </DialogHeader>
                <form onSubmit={submit} className="space-y-3">
                    {noMeals && (
                        <div className="flex items-start gap-2 rounded-md border border-status-warning/30 bg-status-warning-bg p-2.5 text-xs text-status-warning">
                            ...
                            />
                            <span>
                                No planned meals this week to build a list from
                                — plan meals first. You can still top up stock
                                to par.
                            </span>
                        </div>
                    )}
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <Input
                                ...
                                type="date"
                                autoFocus
                                value={form.data.covers_from}
                                onChange={(e) =>
                                    form.setData('covers_from', e.target.value)
                                }
                            />
                            {form.errors.covers_from && (
                        ...
                        <div>
                            ...
                            <Input
                                id="gen-to"
                                type="date"
                                value={form.data.covers_to}
                                onChange={(e) =>
                                    form.setData('covers_to', e.target.value)
                                }
                            />
                            {form.errors.covers_to && (
                                <p className="mt-1 text-xs text-destructive">
                                    {form.errors.covers_to}
                                </p>
                            )}
                    ...
                    <label className="flex items-center gap-2 text-sm">
                        <input
                            type="checkbox"
                            checked={form.data.include_restock_to_par}
                            onChange={(e) =>
                                form.setData(
                                    'include_restock_to_par',
                                    e.target.checked,
                                )
                            }
                        />
                    ...
                    <Badge variant="outline" className="text-xs">
                        Manual items on the current draft are preserved.
                    </Badge>
                    <DialogFooter>
                        <Button type="button" variant="ghost" onClick={onClose}>
                            Cancel
                        ...
                        <Button
                            ...
                            disabled={
                                form.processing ||
                                (noMeals && !form.data.include_restock_to_par)
                            }
                        >
                            Generate
                        </Button>
...
export function SettingsDialog({
    ...
    const [budget, setBudget] = useState(
        budgetCents ? (budgetCents / 100).toFixed(0) : '',
    );
    ...
    const weekMeals = entries.filter(
        (e) =>
            weekDates.includes(e.plan_date.slice(0, 10)) &&
            e.source_type === 'recipe' &&
            e.recipe_id != null,
    );
    ...
    async function saveBudget() {
        ...
        try {
            await axios.put(`/sites/${siteId}/meal-planner/settings`, {
                weekly_food_budget_cents:
                    budget === '' ? null : Math.round(Number(budget) * 100),
            });
            setSavedBudget(true);
    ...
    async function saveCurrentWeek() {
        ...
        setSavingTpl(true);
        try {
            const meals = weekMeals.map((e) => ({
                day: weekDates.indexOf(e.plan_date.slice(0, 10)),
                slot: e.meal_slot,
                recipe_id: e.recipe_id,
                servings: e.servings,
            }));
            await axios.post(`/sites/${siteId}/meal-templates`, {
                name: tplName.trim(),
                description: `${meals.length} meals`,
                meals,
            });
            ...
            <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl">
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2">
                        <Settings className="h-4 w-4 text-primary" /> Meal
                        planner settings
                    </DialogTitle>
                    <DialogDescription>
                        Weekly food budget and reusable week templates.
                    </DialogDescription>
                </DialogHeader>
                <div className="space-y-5">
                    <section>
                        <h3 className="mb-1.5 flex items-center gap-2 text-[13.5px] font-semibold text-foreground">
                            <Wallet className="h-[15px] w-[15px] text-muted-foreground" />{' '}
                            Weekly food budget
                        </h3>
                        <p className="mb-2 text-[12px] text-muted-foreground">
                            Drives the budget bar and spend report for this
                            house.
                        </p>
                    ...
                    <section>
                        ...
                        <p className="mb-2 text-[12px] text-muted-foreground">
                            Reusable meal rotations you can apply to any week.
                        </p>
                            ...
                            >
                                ...
                                <span className="min-w-0 flex-1">
                                    ...
                                    <span className="block text-[11.5px] text-muted-foreground">
                                        Compose a full week on a day × meal grid
                                    </span>
                                </span>
                            </Button>
                        )}
                            ...
                            <details className="mb-3 rounded-xl border border-border bg-muted/40">
                                <summary className="cursor-pointer list-none px-3.5 py-2.5 text-[12.5px] font-medium text-foreground [&::-webkit-details-marker]:hidden">
                                    <span className="inline-flex items-center gap-1.5">
                                        ...
                                        Or save the week you're viewing
                                    </span>
                                </summary>
                                <div className="border-t border-border p-3">
                                    <div className="flex items-end gap-2">
                                        <Input
                                            ...
                                            placeholder="e.g. Winter rotation"
                                            className="flex-1"
                                        />
                                        <Button
                                    ...
                                    <p className="mt-1.5 text-[11.5px] text-muted-foreground">
                                        {weekMeals.length > 0
                                            ? `Captures ${weekMeals.length} recipe meal${weekMeals.length === 1 ? '' : 's'} from the week you're viewing.`
                                            : 'Plan some recipe meals this week first, then save them as a template.'}
                                    </p>
                                </div>
                            </details>
                        )}
                            ...
                            {templates.map((t) => (
                                ...
                                >
                                    ...
                                    {canManage && (
                                        <ConfirmAction
                                            title={`Delete “${t.name}”?`}
                                            description="Removes the template."
                                            confirmLabel="Delete"
                                            onConfirm={() =>
                                                deleteTemplate(t.id)
                                            }
