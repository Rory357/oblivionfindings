...
const SHOWCASE_TABS: PageTabItem[] = [
    ...
    { value: 'overflow-a', label: 'Tab A', icon: Eye, overflowable: true, badge: <Badge variant="outline" className="ml-1 px-1.5 py-0 text-xs">3</Badge> },
...
export default function PageHeroShowcase() {
    ...
    return (
        <AppLayout breadcrumbs={[
            ...
            { title: 'PageHero showcase', href: '/internal/_design/page-hero' },
        ]}>
            ...
            <PageLayout
                hero={
                    <PageHero
                        ...
                        meta={[
                            { icon: MapPin, label: 'resources/js/components/page' },
                            { icon: ShieldAlert, label: 'Admin only' },
                        ]}
                        badges={[
                            { label: 'Variant: hero', tone: 'default' },
                            { icon: AlertTriangle, label: 'Demo', tone: 'warning' },
            ...
            >
                <TabsContent value="hero" className="space-y-6">
                    <Section title="Hero — Site Detail reference (icon + meta + badges + stats + actions)">
                        <PageHero
                            ...
                            meta={[{ icon: MapPin, label: '123 Example St, Wellington 6011' }]}
                            badges={[
                                { icon: Home, label: 'House', tone: 'default' },
                                { icon: AlertTriangle, label: 'High Risk', tone: 'warning' },
                                { label: 'Active', tone: 'success' },
                                { label: 'Wellington', tone: 'default' },
                            ]}
                            stats={[
                    ...
                    <Section title="Hero — avatar (person detail)">
                        <PageHero
                            ...
                            meta={[{ icon: MapPin, label: 'Wellington · NZST' }]}
                            badges={[
                                { label: 'Active', tone: 'success' },
                                { icon: ShieldAlert, label: 'On-call', tone: 'warning' },
                            ]}
                            stats={[
                ...
                <TabsContent value="categories" className="space-y-6">
                    {CATEGORIES.map((cat) => (
                        <Section key={cat.value} title={`Category: ${cat.label} (--category-${cat.value})`}>
                            <PageHero
                                category={cat.value}
                                icon={cat.value === 'fleet' ? Car : LayoutGrid}
                ...
                <TabsContent value="overflow-a">
                    <DemoCard title="Overflow tab A">
                        This tab is marked <code>overflowable: true</code> and appears under the More dropdown below the 2xl breakpoint.
                    </DemoCard>
                </TabsContent>
    ...
    return (
        <Card>
            ...
            <CardContent className="text-sm text-muted-foreground">
                ...
                <div className="mt-3">
                    <Link href="/internal/_design/page-hero" className="text-primary hover:underline">
                        Reload showcase
                    </Link>
                </div>
            </CardContent>
