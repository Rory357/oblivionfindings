...
foreach (array_slice($argv, 1) as $argument) {
    if (! is_string($argument)
        || preg_match('/\A--(central-runtime-evidence|evidence|output-directory|provider-receipt|public-key|signature)=(.+)\z/', $argument, $matches) !== 1
        || isset($arguments[$matches[1]])) {
        $fail('arguments');
    }
    $arguments[$matches[1]] = $matches[2];
}
$expectedArguments = ['central-runtime-evidence', 'evidence', 'output-directory', 'provider-receipt', 'public-key', 'signature'];
...
$authorityVerifier = new CentralRuntimeReleaseAuthorityVerifier;
$authorityBefore = $authorityVerifier->loadInstalled();
$checkoutVerifier = new LoadSoakReleaseCheckoutVerifier('/usr/bin/git');
if (! $checkoutVerifier->verify(
    $applicationPath,
    (string) $authorityBefore['release_revision'],
)) {
...
try {
    $inputs = [
        'evidence' => $readStable($arguments['evidence'], 65_536),
        'signature' => $readStable($arguments['signature'], 512),
        'public_key' => $readStable($arguments['public-key'], 512),
        'central_runtime' => $readStable($arguments['central-runtime-evidence'], 65_536),
        'provider_receipt' => $readStable($arguments['provider-receipt'], 65_536),
    ];
    $report = (new ExternalWatchdogEvidenceVerifier)->verify(
        ...
        rawCentralRuntimeEvidence: $inputs['central_runtime']['raw'],
        rawProviderReceipt: $inputs['provider_receipt']['raw'],
    ...
    $artifactFile = 'monitoring-external-watchdog-release-verification-'
