...
Route::middleware(['auth:sanctum'])->prefix('hr')->name('api.hr.')->group(function () {
    ...
    Route::get('/employees/{id}', [HrApiController::class, 'employee'])->whereNumber('id')->name('employee');
    Route::get('/leave/requests', [HrApiController::class, 'leaveRequests'])->name('leave.requests');
    Route::get('/leave/balances/{userId}', [HrApiController::class, 'leaveBalances'])
        ->whereNumber('userId')->name('leave.balances');
