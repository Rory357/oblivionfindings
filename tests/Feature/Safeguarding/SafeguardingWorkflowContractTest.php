    ...
    public function test_safeguarding_show_serializes_related_records_for_the_frontend(): void
    {
        ...
        $this->actingAs($user)
            ...
            ->assertInertia(fn (Assert $page) => $page
                ...
                ->where('detail.can.update', true)
                ->where('detail.can.investigate', true)
                ->where('detail.can.report_external', true)
                ->where('detail.people.reported_by', $user->name)
                ->where('detail.people.assigned_to', $assignee->name)
                ->where('detail.investigations.0.status', 'paused')
                ->where('detail.external_reports.0.ack_received', true)
                ->where('detail.external_reports.0.ack_reference', 'ACK-123')
                ->where('detail.risk_assessments.0.overall_risk_level', 'high')
                ->where('detail.action_plans.0.assigned_to', $assignee->name)
            );
    }
