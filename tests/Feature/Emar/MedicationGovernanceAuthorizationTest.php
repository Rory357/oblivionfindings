...
class MedicationGovernanceAuthorizationTest extends TestCase
{
    ...
    {
        $context = $this->context();
        $ordinaryMedication = $this->medication(
            $context['local_client'],
            'Local paracetamol',
            'LOCAL-MED-STOCK-001',
            false,
        );
        $ordinaryStock = ClientMedicationStock::create([
    ...
    public function test_explicit_global_site_role_still_requires_and_honours_each_exact_capability(): void
    {
        ...
        $this->actingAs($globalActor)
            ->post(
                route('emar.controlled.entries.store'),
                $this->controlledEntryPayload($context['foreign_client'], $context['foreign_medication'], $context['foreign_witness']),
            )
            ->assertRedirect();
    ...
    public function test_controlled_entry_replay_is_bound_to_the_canonical_action_payload(): void
    {
        ...
        // Resolve the alternative target and its witness before consulting the
        // replay record; a local-only witness remains concealed at another Site.
        $this->actingAs($actor)
            ->postJson(route('emar.controlled.entries.store'), [
                ...$payload,
                'client_id' => $context['foreign_client']->id,
                'client_medication_id' => $context['foreign_medication']->id,
                'medication_name' => $context['foreign_medication']->name,
            ])
            ->assertNotFound();

        // Keep the replay authorized at its effective time so this matrix
        // isolates the durable fingerprint conflict from witness concealment.
        $capturedOfflineAt = now()->subMinutes(5)->toIso8601String();
        $mismatchedPayloads = [
            'entry type' => [...$payload, 'entry_type' => 'receipt'],
            'quantity' => [...$payload, 'quantity' => '0.50'],
            'on hand before' => [...$payload, 'on_hand_before' => '9.50'],
            'on hand after' => [...$payload, 'on_hand_after' => '8.50'],
            'unit' => [...$payload, 'unit' => 'mL'],
            'witness identity' => [
                ...$payload,
                'witnessed_by' => $context['second_witness']->id,
            ],
            'register provenance' => [
                ...$payload,
                'batch_number' => 'CD-BATCH-CHANGED',
        ...
        foreach ($mismatchedPayloads as $description => $mismatchedPayload) {
            $response = $this->actingAs($actor)
                ->postJson(route('emar.controlled.entries.store'), $mismatchedPayload);
        ...
        $this->assertSame(0, ClientControlledDrugEntry::query()
            ->where('client_medication_id', $context['foreign_medication']->id)
            ->count());
    ...
    public function test_controlled_balance_check_replay_is_bound_to_the_canonical_action_payload(): void
    {
        ...
        $first = $this->actingAs($actor)
            ...
            ->assertJsonPath('sync.duplicate', false);
        $entryId = $first->json('entry.id');
        $discrepancyId = $first->json('discrepancy.id');
        ...
        $this->actingAs($actor)
            ...
            ->assertOk()
            ->assertJsonPath('entry.id', $entryId)
            ->assertJsonPath('discrepancy.id', $discrepancyId)
            ->assertJsonPath('sync.duplicate', true);

        // A replay UUID never bypasses the current-Site witness boundary for a
        // different controlled-medication target.
        $this->actingAs($actor)
            ->postJson(route('emar.controlled.balance_check.store'), [
                ...$payload,
                'client_id' => $context['foreign_client']->id,
        ...
        $capturedOfflineAt = now()->subMinutes(5)->toIso8601String();
        $mismatchedPayloads = [
            'expected balance' => [...$payload, 'expected_balance' => '9.75'],
            'actual balance' => [...$payload, 'actual_balance' => '9.25'],
            'witness identity' => [
                ...$payload,
                'witnessed_by' => $context['second_witness']->id,
            ],
        ...
        foreach ($mismatchedPayloads as $description => $mismatchedPayload) {
            $response = $this->actingAs($actor)
                ->postJson(route('emar.controlled.balance_check.store'), $mismatchedPayload);
        ...
        $this->assertSame(0, ClientControlledDrugEntry::query()
            ->where('client_medication_id', $context['foreign_medication']->id)
            ->count());
        $this->assertSame(1, ClientControlledDrugDiscrepancy::query()
            ->where('client_medication_id', $context['local_medication']->id)
            ->count());
        $this->assertSame(1, ClientControlledDrugDiscrepancy::query()
            ->where('client_medication_id', $context['foreign_medication']->id)
    ...
    public function test_stock_receipt_durable_replay_survives_cache_loss_and_rolls_back_with_the_stock_mutation(): void
    {
        $context = $this->context();
        $medication = ClientMedication::create([
            'client_id' => $context['local_client']->id,
            'name' => 'Durable receipt medication',
            'dosage' => '500mg',
            'frequency' => 'PRN',
            ...
            'barcode' => 'DURABLE-RECEIPT-001',
        ]);
        $medication->forceFill(['barcode' => 'DURABLE-RECEIPT-001'])->saveQuietly();
        ...
        $payload = [
            'client_medication_id' => $medication->id,
            'quantity' => '0.50',
            'notes' => 'Sealed pharmacy receipt.',
            'batch_number' => 'RECEIPT-BATCH-001',
            ...
            'client_request_uuid' => $uuid,
            'captured_offline_at' => '2026-08-27T09:20:00+12:00',
            'origin_device_id' => 'stock-receipt-device-1',
            'queued_offline' => true,
        ];
        $injectFailure = true;
        ...
        MedicationIdempotencyResult::creating(
            function (MedicationIdempotencyResult $result) use (&$injectFailure, $uuid): void {
                if ($injectFailure && $result->request_uuid === $uuid) {
                    ...
                    throw new RuntimeException('Injected failure after durable stock receipt mutation.');
                }
            },
        );
        ...
        try {
            $this->actingAs($actor)
                ->postJson(route('emar.stock.receive'), $payload);
            $this->fail('The injected durable receipt failure should escape.');
        } catch (RuntimeException $exception) {
            $this->assertSame(
                'Injected failure after durable stock receipt mutation.',
        ...
        $this->actingAs($actor)
            ...
            ->assertOk()
            ->assertJsonPath('stock.id', $stockId)
            ->assertJsonPath('sync.duplicate', true);
        ...
        $this->assertSame(10.5, (float) $stock->refresh()->on_hand);
        $this->assertSame(1, AuditLog::query()
            ->where('action', 'medications.stock.receive')
            ->count());
        $this->assertSame(1, MedicationIdempotencyResult::query()
            ->where('scope', 'emar-stock-receive')
            ->where('request_uuid', $uuid)
            ->count());
    }
    ...
    public function test_shared_stock_writers_reject_excess_scale_without_any_side_effect(): void
    {
        ...
        $this->assertSame($before, $this->stateSnapshot($context));
        $this->assertSame(10.0, (float) $ordinaryStock->refresh()->on_hand);
    }
    ...
    public function test_idempotency_result_rolls_back_with_the_failed_transaction_and_retry_can_publish_it(): void
    {
        ...
        $this->assertEquals($payload, $stored);
    ...
    public function test_audit_failure_rolls_back_stock_receipt(): void
    {
        ...
        $medication = ClientMedication::create([
            ...
            'state' => 'active',
            'approval_status' => 'verified',
            'barcode' => 'AUDITED-STOCK-001',
        ]);
        $medication->forceFill(['barcode' => 'AUDITED-STOCK-001'])->saveQuietly();
        $stock = ClientMedicationStock::create([
            'client_medication_id' => $medication->id,
            'on_hand' => 10,
            'unit' => 'tablets',
    ...
    public function test_client_medical_discrepancy_conceals_foreign_nested_objects_before_validation(): void
    {
        $context = $this->context();
        $actor = $this->userWithPermissions([
            MedicationGovernanceScopeService::CONTROLLED_CAPABILITY,
        ...
        $foreign = $this->actingAs($actor)->post(
            route('clients.medical.controlled_discrepancies.close', [
                ...
                'discrepancy' => $context['foreign_discrepancy'],
            ]),
            ['resolution_notes' => ['invalid-before-scope']],
        )->assertNotFound();
        $missing = $this->actingAs($actor)->post(
    ...
    public function test_locked_actor_evidence_is_scoped_to_the_legacy_callback(): void
    {
        $site = Site::factory()->create(['type' => 'house', 'is_active' => true]);
        $client = Client::factory()->create(['site_id' => $site->id, 'status' => 'active']);
        $medication = $this->medication($client, 'Projection medication', 'PROJECTION-001', false);
        $actor = $this->userWithPermissions([
            MedicationGovernanceScopeService::STOCK_CAPABILITY,
            'clients.update',
        ], $site);
        $actor->load(['roles.permissions', 'permissionOverrides', 'hrEmployeeProfile']);
        $originalRelations = $actor->getRelations();

        $result = app(MedicationGovernanceScopeService::class)->forMedication(
            $actor,
            $medication->id,
            MedicationGovernanceScopeService::STOCK_CAPABILITY,
            function (
            ...
            ) use ($actor, $client, $medication, $originalRelations): string {
                $this->assertSame($client->id, $lockedClient->id);
                $this->assertSame($medication->id, $lockedMedication->id);
                $this->assertNotSame(
                    $originalRelations['permissionOverrides'],
                    $actor->getRelation('permissionOverrides'),
                );
                $this->assertTrue($actor->canDo(MedicationGovernanceScopeService::STOCK_CAPABILITY));
        ...
        $this->assertSame('callback-complete', $result);
        $this->assertSame($originalRelations, $actor->getRelations());
        $this->assertTrue($actor->canDo('clients.update'));
    }

    public function test_locked_actor_evidence_is_restored_when_the_legacy_callback_throws(): void
    {
        $site = Site::factory()->create(['type' => 'house', 'is_active' => true]);
        $client = Client::factory()->create(['site_id' => $site->id, 'status' => 'active']);
        $medication = $this->medication($client, 'Throwing projection medication', 'PROJECTION-THROW-001', false);
        $actor = $this->userWithPermissions([
            MedicationGovernanceScopeService::STOCK_CAPABILITY,
            'clients.update',
        ], $site);
        ...
        try {
            app(MedicationGovernanceScopeService::class)->forMedication(
                $actor,
                $medication->id,
                MedicationGovernanceScopeService::STOCK_CAPABILITY,
                function () use ($actor, $originalRelations): never {
                    $this->assertNotSame(
                        $originalRelations['permissionOverrides'],
                        $actor->getRelation('permissionOverrides'),
                    );
                    ...
                    throw new RuntimeException('Injected legacy callback failure.');
                },
                expectedClientId: $client->id,
            );
            $this->fail('The injected legacy callback failure should escape the transaction.');
        } catch (RuntimeException $exception) {
            $this->assertSame('Injected legacy callback failure.', $exception->getMessage());
        }
    ...
    private function context(): array
    {
        ...
        $foreignWitness = $this->userWithPermissions(['medications.controlled.witness'], $foreignSite);
        $competencyAssessor = $this->userWithPermissions([], $localSite);
        foreach ([
            [$witness, $localSite, $localClient],
            [$secondWitness, $localSite, $localClient],
            [$foreignWitness, $foreignSite, $foreignClient],
        ] as [$controlledWitness, $witnessSite, $witnessClient]) {
            MedicationCompetencyAssessment::query()->create([
                'user_id' => $controlledWitness->id,
                'assessor_id' => $competencyAssessor->id,
        ...
        $foreignDestruction = MedicationDestruction::create([
            ...
            'disposal_method' => 'denaturing',
            'is_controlled_drug' => true,
            'destroyed_by' => $foreignWitness->id,
            'witness_1_id' => $foreignWitness->id,
            'destroyed_at' => now(),
        ]);
        ...
        $foreignLoss = ControlledDrugLossReport::create([
            ...
            'discovered_by' => $foreignWitness->id,
            'discovered_at' => now(),
        ...
        return compact(
            ...
            'foreign_witness' => $foreignWitness,
        ];
    }
    ...
    private function medication(
        ...
        $medication->forceFill(['barcode' => $barcode])->saveQuietly();
