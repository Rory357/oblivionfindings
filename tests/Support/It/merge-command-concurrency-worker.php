...
if (getenv('APP_ENV') !== 'testing' || getenv('DB_HOST') !== '127.0.0.1'
    ...
    throw new RuntimeException('This worker can only join its exact parent isolated merge schema.');
}
...
if (DB::connection()->getDatabaseName() !== getenv('DB_DATABASE')
    ...
    throw new RuntimeException('Merge worker database or notification isolation differs.');
}
Notification::fake();
...
if (! in_array($operation, ['execute', 'cancel'], true)) {
    throw new RuntimeException('Unexpected merge race operation.');
}
...
foreach ([$ready, $attempt, $release] as $path) {
    ...
    if (! str_starts_with($normalized, $barrierRoot)
        ...
        || ! str_starts_with(basename($normalized), 'it-merge-concurrency-'.$token.'-')) {
        throw new RuntimeException('Merge barriers must stay inside the exact parent test scope.');
    }
}
...
while (! is_file($release)) {
    if (microtime(true) > $deadline) {
        throw new RuntimeException('Parent did not release merge workers.');
    }
    usleep(10_000);
}
...
try {
    ...
    $output = $operation === 'cancel'
        ? $service->recover($source, $target, $actor, $input['request_uuid'], true)
        : $service->execute($source, $target, $actor, $input);
