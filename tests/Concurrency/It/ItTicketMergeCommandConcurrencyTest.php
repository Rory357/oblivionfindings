...
final class ItTicketMergeCommandConcurrencyTest extends TestCase
{
    public function createApplication()
    {
        ...
        if (getenv('APP_ENV') !== 'testing' || ($database !== 'oblivion_it_support_test' && ! $prepared)
            ...
            throw new RuntimeException('Use the isolated wrapper for standalone merge concurrency verification.');
        }

        return parent::createApplication();
    }

    public function test_real_workers_serialize_duplicate_opposite_and_cancelled_merges(): void
    {
        ...
        foreach (['duplicate', 'opposite', 'cancel'] as $round) {
            ...
            $saved = ItTicket::query()->whereKey([$source->id, $target->id])->get();
            $merged = $saved->whereNotNull('merged_into_ticket_id');
            $cancelled = $statuses[0] === 'cancelled';
            $this->assertCount($cancelled ? 0 : 1, $merged);
            $this->assertSame(1, ItTicketCommandReceipt::query()->where('operation', ItTicketCommandReceipt::MERGE_OPERATION)
                ->whereIn('it_ticket_id', [$source->id, $target->id])->count());
            $this->assertSame($cancelled ? 0 : 1, AuditLog::query()->where('action', 'it.ticket.merged')->whereIn('auditable_id', [$source->id, $target->id])->count());
            $this->assertSame($cancelled ? 0 : 2, ItTicketEvent::query()->where('type', 'merged')
                ->where('subject_type', $source->getMorphClass())->whereIn('subject_id', [$source->id, $target->id])->count());
            if ($cancelled) {
                $this->assertSame([1, 1], $saved->pluck('lock_version')->all());
                $destinationId = $source->id;
            } else {
                $original = $merged->sole();
                $survivor = $saved->firstWhere('id', $original->merged_into_ticket_id);
                $this->assertNotNull($survivor);
                $this->assertNull($survivor->merged_into_ticket_id);
                $this->assertSame('closed', $original->status);
                $this->assertSame(3, $original->lock_version);
                $this->assertSame(2, $survivor->lock_version);
                $destinationId = $survivor->id;
            }
            $this->assertSame($destinationId, $public->fresh()->ticket_id);
            $this->assertSame($destinationId, $private->fresh()->ticket_id);
    ...
    {
        $review = app(ItTicketMergeService::class)->preview($source, $target, $actor, 1, 1);

        return ['actor_user_id' => $actor->id, 'source_version' => 1, 'target_version' => 1,
            'request_uuid' => (string) Str::uuid(), 'review_token' => $review['review_token'],
            'reason' => 'Synthetic same-issue merge concurrency verification'];
    }

    private function race(User $actor, ItTicket $source, ItTicket $target, array $first, array $second, string $round): array
    {
        $barrier = storage_path('framework/testing/it-merge-concurrency-'.getenv('TEST_TOKEN').'-'.Str::uuid());
        ...
        try {
            foreach ([$first, $second] as $index => $input) {
                ...
                $processes[] = $worker = new Process([PHP_BINARY, base_path('tests/Support/It/merge-command-concurrency-worker.php'),
            ...
            foreach ($processes as $worker) {
                $this->assertTrue($worker->isRunning(), 'Both merge commands must wait behind the held canonical pair.');
            }
    ...
    private function waitForBarriers(array $paths, array $processes): void
    {
        ...
        while (count(array_filter($paths, 'is_file')) !== count($paths)) {
            if (microtime(true) > $deadline) {
                $this->fail('The isolated merge workers did not reach their barrier.');
            }
            foreach ($processes as $worker) {
                if (! $worker->isRunning()) {
