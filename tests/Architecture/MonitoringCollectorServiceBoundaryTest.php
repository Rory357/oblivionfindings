...
it('binds the complete collector exercise to one protected release authority', function () {
    ...
    expect($verifier)->toContain(
        ...
        "'monitoring_collector_release_evidence_v2'",
        ...
        'array_unique($values, SORT_STRING)',
        "'reviewed_at'",
        ...
        'getTimestamp() - $replacementHeartbeatObservedAt->getTimestamp() > 180',
        'exerciseEvidenceReferencesAreDistinct',
        'count(array_unique($references, SORT_STRING))',
        "'service_restored_at'",
        "'replacement_token_reuse_denial_reference_sha256'",
        "'general_site_token_denial_reference_sha256'",
        "'signed_collector_evidence_sha256'",
    ...
    expect($runbook)->toContain(
        '/etc/oblivion/monitoring-collector-release-authority.json',
        'monitoring_collector_release_evidence_v2',
        'replacement heartbeat requires its own timestamp and immutable evidence reference',
        'Both token-denial timestamps must be at or after replacement',
        'at or before the replacement heartbeat',
        'Every event-specific evidence reference in that exercise must be distinct',
        'configuration hashes must be pairwise',
        'exact raw signed-collector and detached-signature SHA-256 values',
