...
class ShiftSeriesController extends Controller
{
    public function __construct(
        private ShiftSeriesPresenter $presenter,
    ...
    public function store(Request $request)
    {
        ...
        $this->siteAccess->assertCanAccessClientId(
            ...
            (int) $data['client_id'],
            ['reports.viewAny'],
            'You are not authorized to create recurring shifts for that client.',
        );
        $client = Client::query()
            ->whereKey($data['client_id'])
            ->firstOrFail(['id', 'site_id', 'service_context_id']);
        ...
        if (empty($data['service_context_id'])) {
            $data['service_context_id'] = $client->service_context_id;
        }

        // If still not set, apply the application default service context (if configured).
        if (empty($data['service_context_id'])) {
        ...
        if (! empty($data['user_id']) && ! empty($occurrenceWindows)) {
            $this->assertStaffEligibleForShiftSite((int) $data['user_id'], $data['site_id']);
    ...
    private function assertStaffEligibleForShiftSite(int $userId, int $siteId): void
    {
        ...
        if (! $query->exists()) {
            throw ValidationException::withMessages([
                'user_id' => 'This staff member is not currently assigned to the recurring shift Site.',
            ]);
        }
    }
