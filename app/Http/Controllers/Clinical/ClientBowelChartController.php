...
class ClientBowelChartController extends Controller
{
    ...
    public function store(Request $request, Client $client)
    {
        ...
        return back()->with('success', 'Bowel chart entry added.');
    }

    public function update(Request $request, Client $client, ClientBowelEntry $entry)
    {
        $this->authorize('view', $client);
        ...
        return back()->with('success', 'Bowel chart entry updated.');
    }

    public function destroy(Request $request, Client $client, ClientBowelEntry $entry)
    {
        $this->authorize('view', $client);
        abort_unless((int) $entry->client_id === (int) $client->id, 404);
        abort_unless($request->user()?->canDo('medications.administer.record') || $request->user()?->canDo('clients.update'), 403);
        ...
        return back()->with('success', 'Bowel chart entry removed.');
    }
    ...
    private function validatedPayload(Request $request, bool $creating): array
    {
        ...
        return $request->validate([
            ...
            'bristol_type' => [$required, 'integer', 'min:1', 'max:7'],
