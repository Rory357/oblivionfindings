...
class ControlRoomDeviceController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = Device::query()
            ->with(['signalSource:id,name,status']);
        ...
        $devices = $query->paginate(48)->through(fn(Device $device) => [
            'id' => $device->id,
            'name' => $device->name,
    ...
    public function show(Request $request, Device $device)
    {
        ...
        $device->load(['signalSource:id,name,status,vendor']);
