...
final class CollectorReleaseAuthorityVerifier
{
    public const string AUTHORITY_PATH = '/etc/oblivion/monitoring-collector-release-authority.json';
    ...
    public function loadInstalled(?DateTimeImmutable $now = null): array
    {
        ...
        try {
            ...
            throw new RuntimeException('Collector release authority is invalid.', previous: $exception);
        } finally {
            fclose($handle);
        }
    }
    ...
    public function verifyRecord(
        ...
        if (($authority['schema_version'] ?? null) !== 1
            || ($authority['evidence_class'] ?? null) !== 'monitoring_collector_release_authority_v1'
            ...
            || ! $this->sha($authority['remote_site_reference_sha256'] ?? null)
            || ! $this->sha($authority['load_balancer_reference_sha256'] ?? null)
    ...
    {
        throw new RuntimeException('Collector release authority is invalid.');
    }
}
