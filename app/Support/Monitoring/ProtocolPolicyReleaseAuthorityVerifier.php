...
final class ProtocolPolicyReleaseAuthorityVerifier
{
    public const string AUTHORITY_PATH = '/etc/oblivion/monitoring-protocol-policy-release-authority.json';
    ...
    public function loadInstalled(?DateTimeImmutable $now = null): array
    {
        ...
        try {
            ...
            throw new RuntimeException('Protocol-policy release authority is invalid.', previous: $exception);
        } finally {
            fclose($handle);
        }
    }

    /** @param array<string, mixed> $metadata @return array<string, int|string> */
    public function verifyRecord(string $rawAuthority, array $metadata, ?DateTimeImmutable $now = null): array
    {
        try {
            ...
            if (($authority['schema_version'] ?? null) !== 1
                || ($authority['evidence_class'] ?? null) !== 'monitoring_protocol_policy_release_authority_v1'
                ...
                || ! $this->sha($authority['attestation_public_key_sha256'] ?? null)
                || $notBefore === null
                || $notAfter === null
                || $notBefore >= $notAfter
            ...
            return [
                'attestation_public_key_sha256' => $authority['attestation_public_key_sha256'],
    ...
    {
        throw new RuntimeException('Protocol-policy release authority is invalid.');
    }
}
