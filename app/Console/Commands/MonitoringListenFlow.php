<?php

namespace App\Console\Commands;

use App\Domain\Monitoring\Protocols\Flow\FlowIntakeService;
use App\Domain\Monitoring\Services\ListenerHeartbeatReporter;
use App\Domain\Monitoring\Services\UdpListenerLiveness;
use App\Domain\Monitoring\Services\UdpPeerAddress;
use Illuminate\Console\Command;
use Illuminate\Support\Facades\Log;
use Throwable;

final class MonitoringListenFlow extends Command
{
    protected $signature = 'monitoring:listen-flow {--once : Stop after one datagram}';

    protected $description = 'Listen for bounded NetFlow, IPFIX, and sFlow datagrams and enqueue signed telemetry';

    public function handle(
        FlowIntakeService $intake,
        ListenerHeartbeatReporter $heartbeat,
        UdpListenerLiveness $liveness,
    ): int {
        $bind = config('monitoring.inbound.flow.bind', '127.0.0.1');
        $port = config('monitoring.inbound.flow.port', 2055);
        $maximum = config('monitoring.inbound.flow.max_datagram_bytes', 65_507);
        $allowlist = config('monitoring.inbound.bind_allowlist', ['127.0.0.1', '::1']);
        if (! is_string($bind) || filter_var($bind, FILTER_VALIDATE_IP) === false
            || ! is_array($allowlist) || ! in_array($bind, $allowlist, true)
            || ! is_int($port) || $port < 1 || $port > 65_535
            || $maximum !== 65_507) {
            $this->error('Flow listener configuration is invalid.');

            return self::FAILURE;
        }
        $host = str_contains($bind, ':') ? "[{$bind}]" : $bind;
        $socket = @stream_socket_server("udp://{$host}:{$port}", $errorCode, $errorMessage, STREAM_SERVER_BIND);
        unset($errorMessage);
        if (! is_resource($socket)) {
            Log::error('Flow listener could not bind.', ['error_code' => $errorCode]);

            return self::FAILURE;
        }
        $liveness->prepare($socket);
        $heartbeat->started('flow');
        try {
            do {
                if (! $liveness->wait($socket, $heartbeat, 'flow')) {
                    continue;
                }
                $peer = null;
                $datagram = @stream_socket_recvfrom($socket, 65_507 + 1, 0, $peer);
                if (! is_string($datagram) || ! is_string($peer)) {
                    continue;
                }
                $heartbeat->received('flow');
                $sender = UdpPeerAddress::parse($peer);
                if ($sender === null) {
                    $heartbeat->rejected('flow');
                    Log::warning('Flow datagram had an invalid sender address.');

                    continue;
                }
                try {
                    $intake->ingest($datagram, $sender);
                    $heartbeat->accepted('flow');
                } catch (Throwable) {
                    $heartbeat->rejected('flow');
                    Log::warning('Flow datagram was rejected.');
                }
            } while (! $this->option('once'));
        } finally {
            fclose($socket);
        }

        return self::SUCCESS;
    }
}
