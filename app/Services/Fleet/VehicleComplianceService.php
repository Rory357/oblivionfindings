...
class VehicleComplianceService
{
    ...
    public function record(User $actor, int $assetId, string $kind, array $data, string $requestKey, mixed $expectedCurrentVersionId = null): FleetVehicleComplianceVersion
    {
        ...
        return DB::transaction(function () use ($actor, $assetId, $kind, $data, $requestKey, $expectedCurrentVersionId): FleetVehicleComplianceVersion {
            ...
            abort_unless($currentVersionId === $expectedCurrentVersionId, 409, 'Compliance evidence changed. Reload before recording another version.');

            $documentId = isset($data['asset_document_id']) ? (int) $data['asset_document_id'] : null;
            if ($documentId) {
                abort_unless(AssetDocument::query()->whereKey($documentId)->where('asset_id', $asset->id)->exists(), 404);
            }
    ...
    private function validate(string $kind, array $data): void
    {
        Validator::make($data, [
            ...
            'ruc_start_km' => ['nullable', 'numeric', 'min:0'],
            'ruc_end_km' => ['nullable', 'numeric', 'min:0'],
            'observed_at' => ['nullable', 'date'],
            'reason' => ['nullable', 'string', 'max:5000'],
        ])->validate();
        $applicability = $data['applicability'] ?? null;
        $outcome = $data['outcome'] ?? null;
        if (! in_array($applicability, self::APPLICABILITY, true) || ! in_array($outcome, self::OUTCOMES, true)) {
            throw ValidationException::withMessages(['applicability' => 'Choose a supported applicability and outcome.']);
        }
        if ($applicability === 'unknown' && $outcome !== 'needs_assessment') {
            throw ValidationException::withMessages(['outcome' => 'Unknown applicability must remain needs assessment.']);
        }
        if (! empty($data['effective_on']) && ! empty($data['expires_on'])
            && CarbonImmutable::parse($data['effective_on'])->startOfDay()->greaterThan(CarbonImmutable::parse($data['expires_on'])->startOfDay())) {
            throw ValidationException::withMessages(['expires_on' => 'Expiry cannot be before the effective date.']);
        }
        if ($applicability === 'not_applicable' && trim((string) ($data['applicability_basis'] ?? '')) === '') {
            throw ValidationException::withMessages(['applicability_basis' => 'Record the basis for Not applicable.']);
        }
        if ($applicability !== 'applicable' || ! in_array($outcome, ['recorded', 'passed', 'failed'], true)) return;
        // I1 can retain a same-Asset legacy document link, but it cannot treat
        // that unscanned attachment as trusted evidence. A human-readable
        // provenance reference remains required until I2 owns file trust.
        if (trim((string) ($data['evidence_reference'] ?? '')) === '') {
            throw ValidationException::withMessages(['evidence_reference' => 'Record the evidence reference. An attached file is retained as unverified until scanning is implemented.']);
        }
        if ($kind === 'ruc' && in_array($outcome, ['recorded', 'passed'], true)) {
            $low = $data['ruc_start_km'] ?? null; $high = $data['ruc_end_km'] ?? null;
            if (! is_numeric($low) || ! is_numeric($high) || (float) $low < 0 || (float) $high <= (float) $low) {
                throw ValidationException::withMessages(['ruc_end_km' => 'Licence end must exceed its non-negative start.']);
            }
        } elseif ($kind !== 'ruc' && in_array($outcome, ['recorded', 'passed'], true) && empty($data['expires_on'])) {
            throw ValidationException::withMessages(['expires_on' => 'Record the next due or expiry date.']);
        }
    }
}
