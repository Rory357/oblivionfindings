...
- [ ] 6. §E ticket workspace: show route/policy payloads → `TicketThread` (comments API, internal notes) → properties rail (assign/status/priority/category/asset/watchers) → timeline → drawer. *(split 2026-07-07:)*
  ...
  - [ ] 6b. Detail page `/it/tickets/{ticket}`: shared `TicketThread` (thread + composer Reply⇄Internal, Ctrl+Enter), compact header band (reference copy, badges), properties rail (status/priority/category/subcategory, assign + assign-to-me, watchers, linked asset/provisioning, stamps), timeline toggle.
  - [ ] 6c. Quick-peek drawer sharing `TicketThread` + condensed rail; row click → drawer, "Open full page"; My-tickets + queue wiring.
  - [ ] 6d. Attachments (§P.4): `it_attachments` morphs table/model, upload (ticket/comment) + authorized download via `ServesPrivateAttachments`, dropzone in composer + raise modal disclosure.
