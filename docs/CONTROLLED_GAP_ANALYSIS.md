...
- [ ] **C6** `onContextMenu` on every interactive row on all 7 tabs → `ShiftContextMenu` (copy PRN `openRowCtx`). Items: View details · Check balance / Record movement · View full register for this CD · — · View client (`/operations/clients/{id}/care`) · Open on MAR chart · Export CD register (PDF) · — · Report discrepancy / Report loss.
- [ ] **C7** CD record detail modal (read-only, `prn-detail-dialog.tsx` idiom) — `cd-detail-dialog.tsx`. Client (avatar+room+site), drug (+CD lock + Schedule chip, form/strength), movement type, qty, balance before→after, recorded-by + witness, batch, expiry, timestamp, linked incident/discrepancy ref. Footer Options bar opens the relevant wizard in place.
- [ ] **C8** Every tab's rows clickable (cursor-pointer + hover) + keyboard-focusable, like PRN register rows.

## D. Per-tab add affordance + empty-state CTAs (priority 4)

- [ ] **D9** Primary create action in each tab's panel header (Add-Client style): Register/Recent/Audit → Record CD entry; Reconciliation → Balance check; Discrepancies → Report discrepancy; Destructions → Record destruction; Loss Reports → Report loss. Each empty state gets a matching CTA button.
- [ ] **D10** Client/drug-specific rows: per-tab add opens the wizard pre-filled to that client/CD (`presetMedId`/client).
...
- [ ] **BK-detail** Each entry/destruction/loss row carries witness, batch, expiry, balances, linked `incident_id`/`discrepancy_id` for C7.
