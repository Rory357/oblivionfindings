...
6. ~~**Documents tab and Risk Management tab**~~ — **SHIPPED**. Both tabs now follow the Phase 2 component pattern:
   - **Documents:** new [tabs/documents.tsx](../resources/js/pages/operations/clients/tabs/documents.tsx) replaces the inline IIFE in `show.tsx`. Stat strip (total / expired / expiring 30 d / family-portal), filter bar (search + category + visibility + expiry state) with active-filter counter, Card-grouped grid by folder, `EmptyState` for "no docs" vs "no matches". **Bug fixes shipped alongside:** the legacy inline read `d.expires_at` (field is `d.expiry_date`) so expired/expiring badges never fired, and `folder` was missing from the controller's `select` so all docs grouped under "Unfiled". Both fixed — `ClientController::show` now selects `folder` and the new tab uses `expiry_date`.
   - **Risk Management:** new [tabs/risk-management.tsx](../resources/js/pages/operations/clients/tabs/risk-management.tsx) replaces the `ClientProfilePlaceholder`. Inline CRUD via Dialog (create / edit / delete) wired to the existing `ClientRiskController` routes. Stat strip (active / critical / review overdue / review in 30 d), status + severity filters, severity-tinted Cards, review-state badges. Controller now returns all risks (active first, severity-ordered, limit 50) so inactive ones can be managed; the tab nav badge counts active risks only.
