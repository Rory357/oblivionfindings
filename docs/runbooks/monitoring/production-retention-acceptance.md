...
4. At least one otherwise-due series is protected by an active policy legal hold or canonical Site, Device or series legal hold. “Due” uses the same tier boundary as deletion: raw cutoffs align to the hour and hourly cutoffs to the day; a record merely older than the unaligned day count does not prove legal-hold protection.
...
- contain the one-time run UUID, exact protected-authority release revision, strict whole-second UTC `Z` validity window of no more than 24 hours, the three observed endpoint commitments, key reference and detached signature; duplicate object keys at any depth are prohibited;
...
2. Reads the fixed protected release-authority record, rejects an unprotected, unstable, expired or malformed record, and obtains the only eligible release revision and attestation public key from it. It pins the SHA-256 of the exact verified protected-authority bytes in memory; this is a comparison guard only and is never published in the value-free artifact. Before any endpoint probe or retention mutation, the command requires one clean source checkout whose `HEAD` and `origin/main` both equal the protected authority revision; it repeats that exact check after verification and before writing a release-eligible artifact. It then connects to the live endpoints and verifies the MySQL identity and InfluxDB TLS certificate/scope against the signed pins before requiring a healthy InfluxDB response. After retention verification, it re-probes the live MySQL and InfluxDB identities and requires the exact same still-current signed attestation before publication. It also re-reads the fixed authority immediately before artifact publication and fails the evidence if that protected record is now unavailable, invalid, expired, revoked to a different release, or its exact verified bytes changed, including a changed approval window with the same release and attestation key.
...
8. Validates that a `verified` report has non-empty execution semantics and zero integrity/unresolved-intent gaps, then requires an exact service-account-owned mode-`0700` output directory and exclusively creates a mode-`0600` JSON artifact with matching `.sha256` sidecar. UUID filenames plus exclusive-create mode make collisions fail without overwriting prior evidence. Both writes are flushed, synced and checked for complete stable file identity. Before the writer commits the pair, it revalidates the live endpoint identities, clean exact checkout and protected authority inside the artifact writer, confirms the output-directory identity is unchanged, then reopens both output files and requires their exact bytes, private owner/mode and stable identity. Any failed final check removes only the newly created pair; an existing collision remains untouched.
...
Reject the run if the command exits non-zero, no artifact is produced, the checksum differs, the fixed release authority is absent/unprotected/expired/malformed or changes before or during publication, the endpoint attestation is absent/expired/mismatched/invalid, either endpoint identity changes between the preflight, postflight and final pre-commit probes, the clean checkout changes, the output directory changes identity or protection, `a05_release_evidence` is false, any safe error code is present, the verified execution counts are empty, or an unresolved deletion intent remains. A failed artifact is diagnostic evidence only and cannot close A05.
