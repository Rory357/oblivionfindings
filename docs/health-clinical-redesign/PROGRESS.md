...
- [ ] Wire from module hero buttons + register ctx menus; also mount in client profile (`tabs/health-monitoring`, `tabs/incidents-tab`, Behaviour tab) passing the current client.
- [ ] Tests: gating, type-aware payloads validate against `validateDataForType`, profile-context skips step 1. **Dependency:** Steps 1, 3. **Ships:** inline recording from module + profile.
