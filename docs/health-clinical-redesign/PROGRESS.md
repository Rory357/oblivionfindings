...
### 🔧 Step 4 — Record wizards (3, one lifted component, §8) — **IN PROGRESS**
- [x] **4a backend foundations** (the wizards depend on these): `clinical_attachments` polymorphic table (`morphs`, single `mime`, `kind`, `is_sensitive`, soft-deletes — mirrors SafeguardingAttachment) + `ClinicalAttachment` model + `attachments()` MorphMany on `ClinicalEvent` + `BehaviourAbcEntry`; `ClinicalAttachmentService` (attach/attachMany). Create-time evidence wired into event store (module + profile) + ABC store (validated `attachments[]` files, ≤10MB, image/pdf/doc). `is_flagged`/`flagged_reason` added to obs trait validation + `ClinicalObservationService::record()` (sets `flagged_by`). New endpoints: `GET clients/search` (name/preferred/NHI, debounced picker) + `GET clients/{client}/clinical-card` (allergies/baseline vitals+NEWS2/active protocols for the rail; resus deferred — no data source). Commit: `__STEP4A_SHA__`.
- [ ] **4b** Record Observation wizard (WizardShell + primitives + add-client gating; optional-client; type-aware measurements; live NEWS2; `is_flagged`/`recorded_at`/`protocol_schedule_id`; clinical-card rail via `WizardShell` railExtra slot). Wire hero "Record observation" + obs register ctx menu.
- [ ] **4c** Log Clinical Event wizard (+ severity, witnesses, immediate action/outcome, **evidence dropzone** via FileDropzone+StagedFileCard create-time, cross-module HS note). Wire hero "Log clinical event" + events ctx menu.
- [ ] **4d** Record ABC wizard (Context→A·B·C→Response+evidence→Review; posts `clients.behaviour.abc.store`). Wire Behaviour tab + ABC ctx menu.
...
### ☐ Step 5 — Event review/follow-up/escalate + context menus (B2)
- [ ] `PATCH review`, `PATCH follow-up/complete`, `POST escalate` + `ClinicalEventService` methods + `ClinicalEventPolicy::escalate()`; permission `clinical.events.escalate` (B9 partial).
- [ ] Events card-list rework + ctx-menu actions + follow-up panel.
- [ ] Tests: review sets `reviewed_*`; escalate notifies + HS-links. **Dependency:** Steps 1, 2. **Ships:** sign-off/escalate workflow live.
