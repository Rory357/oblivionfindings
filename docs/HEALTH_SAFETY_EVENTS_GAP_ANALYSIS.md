...
- [x] A3 — **Orphan categories** (E-Gap 5): `exposure` / `inspection_failure` / `equipment_fault` are no longer dead filters. `SubstanceExposureRecord` creates `exposure`, failed `SiteInspectionRecord` creates `inspection_failure`, and high/critical `FleetWorkOrder` creates `equipment_fault`; each resolves to its originating record/link. ✅ Step 7 verified (HsEventBackboneTest + HsEventRegisterTest: 35 tests / 167 assertions).
...
- **19 Jun 2026 (Step 7)** — User chose wiring over removal and asked whether a site-inspection module exists. Audit confirmed it does: `/sites/{site}/inspections` and `/sites/inspections` create `SiteInspectionRecord` rows, with failed inspections already emitting facility Control Room signals. Resolved all three orphans by wiring existing modules, without building a new site-profile tab: `SubstanceExposureRecordObserver` → `exposure`; `SiteInspectionRecordObserver` (fail only) → `inspection_failure`; `FleetWorkOrderObserver` (high/critical/urgent priority only) → `equipment_fault`. Added source resolver links for substance exposure parent, site inspections, and fleet work-order detail.
