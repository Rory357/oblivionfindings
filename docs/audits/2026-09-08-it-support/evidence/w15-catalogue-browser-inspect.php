...
try {
    ...
    $queries = [
        ...
        'catalogue_attachments' => "SELECT id, attachable_type, attachable_id, original_name, mime, size, uploaded_by, catalogue_field_key FROM it_attachments WHERE attachable_type = 'it_catalog_submission' ORDER BY id",
        'catalogue_storage_intents' => "SELECT id, parent_type, parent_id, state, attachment_id, actor_user_id, size FROM it_attachment_storage_intents WHERE parent_type = 'it_catalog_submission' ORDER BY id",
        'provisioning' => 'SELECT id, employee_profile_id, created_by, type, status, lock_version, approval_required, approval_status, primary_approver_user_id, cover_approver_user_id, approval_requested_at, approval_expires_at, approved_by_user_id, fulfilled_by, fulfilled_at, evidence_summary, external_ref, fulfilment_mode, assigned_to_user_id, due_date, due_offset_days, failed_at, failure_reason, canonical_target_type, canonical_target_id, reversal_of_request_id, dependency_request_ids FROM it_provisioning_requests ORDER BY id',
        'templates' => 'SELECT id, name, site_id, lock_version, current_version_id, published_version_id, published_at FROM it_provisioning_templates ORDER BY id',
        ...
        'workflows' => 'SELECT id, provisioning_template_id, template_version_id, employee_profile_id, status, lock_version, source_type, owner_user_id, cover_user_id, effective_at, original_effective_at, cancelled_at, changes FROM it_provisioning_workflows ORDER BY id',
        'provisioning_commands' => "SELECT id, actor_user_id, operation, request_uuid, committed_at, result_metadata FROM it_ticket_command_receipts WHERE operation LIKE 'provisioning.%' ORDER BY id",
