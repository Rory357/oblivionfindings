...
const W06_BROWSER_DATABASE_PREFIX = 'oblivion_ticket_work_browser_';
...
{
    $files = ['ticket-workflow-browser-environment.ps1', 'ticket-workflow-browser-runtime.php', 'ticket-workflow-browser-bootstrap.php',
...
function w06BrowserGuard(bool $requireReady = false): array
{
    ...
    $root = $testing.'/it-ticket-work-browser-'.$token;
    ...
    w06BrowserRequire(in_array((int) ($manifest['port'] ?? 0), [8779, 8780], true)
...
function w06BrowserApplication(array $context): Application
{
    ...
    $app->booted(fn () => app(\Illuminate\Foundation\Vite::class)->useBuildDirectory('ticket-workflow-review'));
