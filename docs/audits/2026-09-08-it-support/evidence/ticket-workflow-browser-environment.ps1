...
$itBrowserFiles = @('ticket-workflow-browser-environment.ps1', 'ticket-workflow-browser-runtime.php', 'ticket-workflow-browser-bootstrap.php',
    'ticket-workflow-browser-router.php', 'ticket-workflow-browser-fixtures.php', 'ticket-workflow-browser-teardown.php', 'w11-mailbox-browser-fixture.php', 'w11-merge-browser-scenario.php', 'w12-delivery-browser-fixture.php', 'w13-api-browser-fixture.php', 'w14-monitoring-browser-fixture.php', 'w15-catalogue-browser-fixture.php', 'w21-workspace-browser-fixture.php', 'w23-vendor-vault-browser-fixture.php')
...
$itBrowserAssetHash = (Get-FileHash -LiteralPath (Join-Path $itBrowserRepo 'public/ticket-workflow-review/manifest.json') -Algorithm SHA256).Hash.ToLowerInvariant()
$itBrowserReview = [ordered] @{
    ...
    host = '127.0.0.1'; port = $Port; database_prefix = 'oblivion_ticket_work_browser_'
...
$itBrowserRoot = Join-Path $itBrowserTesting ('it-ticket-work-browser-' + $Token)
$itBrowserDatabase = 'oblivion_ticket_work_browser_' + $Token
...
if ($Mode -eq 'CreateAndStart') {
    ...
    if (Test-Path -LiteralPath $itBootstrapStartedPath) {
        ...
        if ($itBootstrapStarted.token -ne $Token -or $itBootstrapStarted.script -ine (Join-Path $PSScriptRoot 'ticket-workflow-browser-bootstrap.php')) { throw 'Bootstrap ownership record differs.' }
...
try {
    ...
    if ($Mode -eq 'CreateAndStart') {
        $itBootstrapArgument = '"' + (Join-Path $PSScriptRoot 'ticket-workflow-browser-bootstrap.php') + '"'
        ...
        Write-ItBrowserNewJson (Join-Path $itBrowserRoot 'bootstrap-started.json') ([ordered] @{
            ...
            executable = $itBrowserPhp; script = (Join-Path $PSScriptRoot 'ticket-workflow-browser-bootstrap.php')
        })
        ...
        $itRouterArgument = '"' + (Join-Path $PSScriptRoot 'ticket-workflow-browser-router.php') + '"'
        ...
        Write-ItBrowserNewJson (Join-Path $itBrowserRoot 'server-started.json') ([ordered] @{
            ...
            executable = $itBrowserPhp; router = (Join-Path $PSScriptRoot 'ticket-workflow-browser-router.php'); url = $itBrowserUrl
        })
        ...
        if (Test-Path -LiteralPath $itStartedPath) {
            ...
            if ($itStarted.token -ne $Token -or $itStarted.router -ine (Join-Path $PSScriptRoot 'ticket-workflow-browser-router.php')) { throw 'Server ownership record differs.' }
        ...
        & $itBrowserPhp (Join-Path $PSScriptRoot 'ticket-workflow-browser-teardown.php') --drop-stopped-owned-schema
        ...
        Write-ItBrowserNewJson (Join-Path $PSScriptRoot ('ticket-workflow-browser-cleanup-' + $Token + '.json')) $itCleanupEvidence
