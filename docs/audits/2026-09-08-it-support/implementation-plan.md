...
1. **Service desk** — existing `/it`. User-confirmed 13 September scope: ticket overview, queues and request entry points. Technician rail: Overview, Tickets, Service catalogue, My requests. Provisioning has its own `/it/provisioning` page; Knowledge and Reports have their own pages. Preserve old tab deep links through redirects. Staff see the relevant My requests/catalogue views and Get help, with a normal link to their published guides.
...
**13 September user priority override:** deliver separate Provisioning, Knowledge and Reports pages, then modernise Problems/Changes/Major Incidents; Knowledge must cover structured systems, assets, software, vendors, relationships, runbooks, revisions and review ownership. Preserve the full programme scope and domain dependencies. Implement requested fixes before routine verification; batch the relevant tests/build/browser checks at the end, testing earlier only when a specific result is required to diagnose or decide a change.
...
- **User clarification, 13 September 2026 (new-session handover):** Every Knowledge document must have a complete, directly addressable full page. Make full-page reading and editing the primary document experience, bringing content, files, diagrams, related records, review ownership and revision history together. The current library-reader dialog alone does not meet this requirement. Preserve canonical article identity, permission checks, old links and the originating library filters/page when returning.
...
- **User clarification, 13 September 2026:** Knowledge must accept uploaded Word documents (`.doc`/`.docx`) and PDFs, and users must be able to open them from the workspace. Provide an access-checked Open/preview or explicit original-file open/download fallback, replacement versions and retained history. Unsupported/failed preview must not look like an empty or successfully opened document. Verify upload, open, replacement, history and direct-file denial; text-only article editing does not satisfy this requirement.
- **User clarification, 13 September 2026 (new-session handover):** Users must be able to create and edit diagrams within Knowledge, save them with a document, reopen them for editing and retain their revision history. Protected image uploads alone do not deliver in-workspace diagram creation. The implementation default is visual authoring with shapes, connectors and text, plus a readable published view, using the same draft/review/access boundary as the document. Full document pages, Word/PDF and diagrams are the next feature implementation slice; W23 vendors and W24 credentials follow as the next dedicated workstream. Implement meaningful feature slices first, then group relevant checks and rerun affected failures only.
...
- **User clarification, 13 September 2026 (parallel vendor session):** Vendor contracts and supporting commercial documents must be addable as protected files, with upload, opening/preview or original download, replacement versions and retained history. Contracts, pricing and commercial terms are visible only to Finance and explicitly permitted Management. The user confirmed Management INCLUDES organisation management AND site/house managers; preserve their approved-site/record scope. General vendor-directory access, IT access, credential reveal access or ordinary house-staff access must not grant commercial-document access. Enforce this at record/file endpoints and in metadata, relationship projections, search, exports and notifications, not only by hiding a tab. Finance/Management edit/upload rights must be separate from general vendor maintenance. Use dedicated restricted permissions and map the existing role catalogue carefully; finance.ap.view alone is insufficient because it is also granted to auditors. The user now requests a dedicated vendor/credential session running alongside Knowledge, superseding the earlier sequential handover; coordinate file ownership and serialize shared builds, database-heavy checks and browser runtimes in the same saved checkout.
