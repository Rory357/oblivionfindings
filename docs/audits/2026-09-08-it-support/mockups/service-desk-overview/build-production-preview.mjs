...
const result = await build({entryPoints:[path.join(dir,'production-entry.tsx')],absWorkingDir:repo,bundle:true,write:false,minify:true,format:'iife',jsx:'automatic',metafile:true,define:{'process.env.NODE_ENV':'"production"'},alias:{'@':path.join(repo,'resources/js'),'@inertiajs/react':path.join(dir,'inertia-mock.tsx')}});
...
await fs.writeFile(path.join(dir,'production-fragment.html'),fragment);
await fs.writeFile(path.join(dir,'production-preview.html'),`<!doctype html><html lang="en-NZ"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Service Desk Overview · Interactive mockup</title></head><body>${fragment}</body></html>`);
const evidence = [];
...
await fs.writeFile(path.join(dir,'production-source-snapshot.json'),JSON.stringify(evidence,null,2));
console.log(JSON.stringify({artifact:path.join(dir,'production-preview.html'),bytes:Buffer.byteLength(fragment),sourceFiles:inputs.length}));
