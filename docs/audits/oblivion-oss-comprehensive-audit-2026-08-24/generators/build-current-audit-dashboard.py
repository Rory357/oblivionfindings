...
assert sha256_file("evidence/benchmark/current-facet-neutral-comparison-wave-02.json") == "4dc66b8c7c11bd4d89e361d2f5b8bb8b91fda3cb1f05ed1c1ddc464dbf563b6e"
assert sha256_file("evidence/benchmark/current-facet-neutral-comparison-agent-register.json") == "d5e6587a1125ffb2456c2b8f20b26f5d602b7282bbe001055cedb6b48ca18633"
...
assert facet_refinement["run_id"] == "RUN-052"
assert facet_refinement_agents["run_id"] == "RUN-052"
assert facet_refinement["generated_at"] == facet_refinement_agents["generated_at"]
assert facet_refinement["stage_lineage"]["status"] == "NO_GO_AGENT_A_TO_B_SANITIZED_BEHAVIOUR_PACKET_MISSING"
assert facet_refinement["stage_lineage"]["agent_a_sanitized_behaviour_forwarded_to_agent_b"] is False
...
assert facet_counts["wave_facets"] == 24
assert facet_counts["candidate_locators"] == 12
assert facet_counts["bounded_no_candidates_not_final_no_match"] == 12
assert facet_counts["source_anchor_occurrences_validated"] == 155
assert facet_counts["source_anchor_files_validated"] == 84
assert facet_counts["comparison_dimension_ratings"] == 144
assert facet_counts["independent_no_go_verdicts"] == 6
...
assert facet_refinement["canonical_matrix_disposition"]["status"] == "UNCHANGED_LINEAGE_NO_GO_OVERLAY_ONLY"
assert facet_refinement["canonical_matrix_disposition"]["sha256"] == sha256_file("03-feature-to-benchmark-matrix.csv")
...
<body>
  ...
  <main>
    <div class="notice" role="status"><strong>No completion claim:</strong> RUN-030 freezes 340 current-source static canonical targets (300 H · 40 D · 0 M). RUN-034–038 materialize observer evidence for 95/95 prompt repositories and review all 16 blockers, yielding 88 complete observer-only and 7 partial. RUN-039–046 complete the first six-target comparison overlay with 0 formal edges. RUN-047–052 then refine those same targets into 24 exact facets, 155 pinned source anchors, and 144 comparison ratings. Independent adjudication records a lineage NO-GO because identity-stripped RUN-047 observations were not passed from Agent A to Agent B. The canonical matrix remains byte-identical with 0/340 mappings or final no-matches; runtime, rendered application-browser, executed-test, benchmark, ease, release, Pass, and audit-completion credit remain zero.</div>
    ...
    <div class="split"><section class="panel"><h2>Evidence waves represented</h2><p>RUN-001 through RUN-052 are represented by audit artifacts; none grants application runtime, browser, executed-test, benchmark, or completion credit.</p><ul class="list"><li>RUN-001–016: census, discovery, page/static visual, and benchmark-metadata foundations</li><li>RUN-017–022: frontline/platform identity adjudication and blocked-owner reconciliation</li><li>RUN-023–025: cross-scope and remaining-owner arbitration</li><li>RUN-026: report-catalog identity adjudication</li><li>RUN-027: medical-profile owner tie-break</li><li>RUN-028–029: denominator integration and independent red team</li><li>RUN-030: deterministic materialization of 362 Layer-A edges and 340 canonical targets</li><li>RUN-031–033: disjoint upstream observer-triage partitions covering 95 repositories and 98 occurrence-weighted entries</li><li>RUN-034: normalized observer-only triage integration with 79 complete and 16 partial records</li><li>RUN-035–037: disjoint official-source reviews of every partial observer record</li><li>RUN-038: deterministic blocker-resolution integration with 9 resolved observer-only, 7 retained partial, and zero downstream credit</li><li>RUN-039–045: six-target upstream packets, pinned current-source packets, scope red team, blind neutralization, clean comparison, 18-facet composite overlay, and independent adjudication</li><li>RUN-046: deterministic lineage and hash integration; 6 NO-GO, 0 formal edges, and unchanged 0/340 mapping credit</li><li>RUN-047–050: 24 upstream facets, separate normative packets, 155-anchor current-source packets, and 144 reconciled comparison ratings</li><li>RUN-051: independent mechanical and semantic review; 6 feature NO-GO and missing Agent A-to-B sanitized-behaviour derivation</li><li>RUN-052: deterministic integration; lineage NO-GO, 0 formal edges, and unchanged 0/340 mapping credit</li></ul></section><section class="panel"><h2>Execution credit</h2><p>Static, observer, and source-comparison evidence are not runtime evidence.</p><ul class="list"><li><span class="zero">0</span> framework route executions</li><li><span class="zero">0</span> current application tests</li><li><span class="zero">0</span> rendered current-build visual instances</li><li><span class="zero">0</span> current-build application browser routes</li><li><span class="zero">0</span> benchmark mappings promoted</li><li><span class="zero">0</span> completed Pass 1–8 modules</li></ul></section></div>
    ...
    <section class="panel"><h2>RUN-047–052 facet refinement evidence</h2><p>Twenty-four exact facets are retained for diagnosis. Independent adjudication records six feature NO-GO dispositions and zero formal edges because the prompt-required Agent A-to-B sanitized-behaviour derivation is missing.</p><ul class="list"><li><a href="evidence/benchmark/raw-run-047-upstream-facet-refinement-clinical-incident-wave-02.json">RUN-047 clinical and incident upstream facets</a></li><li><a href="evidence/benchmark/raw-run-047-upstream-facet-refinement-composites-wave-02.json">RUN-047 HR, medication, and finance upstream facets</a></li><li><a href="evidence/benchmark/current-upstream-facet-refinement-wave-02.json">RUN-047 integrated upstream refinement</a></li><li><a href="evidence/benchmark/current-upstream-facet-refinement-agent-register.json">RUN-047 upstream agent register</a></li><li><a href="evidence/benchmark/raw-run-048-blind-neutral-facet-requirements-wave-02.json">RUN-048 source-independent requirements</a></li><li><a href="evidence/benchmark/raw-run-049-current-source-facet-refinement-wave-02.json">RUN-049 pinned current-source facets</a></li><li><a href="evidence/benchmark/raw-run-050-clean-facet-comparison-clinical-incident-hr-wave-02.json">RUN-050 clinical, incident, and HR comparison</a></li><li><a href="evidence/benchmark/raw-run-050-clean-facet-comparison-medication-finance-wave-02.json">RUN-050 medication and finance comparison</a></li><li><a href="evidence/benchmark/raw-run-050-clean-facet-comparison-full-crosscheck-wave-02.json">RUN-050 full cross-check</a></li><li><a href="evidence/benchmark/raw-run-050-witness-authority-tiebreak-wave-02.json">RUN-050 witness-authority tie-break</a></li><li><a href="evidence/benchmark/raw-run-050-clean-facet-comparison-reconciled-wave-02.json">RUN-050 reconciled comparison</a></li><li><a href="evidence/benchmark/raw-run-051-independent-facet-adjudication-wave-02.json">RUN-051 independent adjudication</a></li><li><a href="evidence/benchmark/current-facet-neutral-comparison-wave-02.json">RUN-052 integrated facet overlay</a></li><li><a href="evidence/benchmark/current-facet-neutral-comparison-agent-register.json">RUN-052 facet agent register</a></li></ul></section>
...
dashboard = TEMPLATE.substitute(
    ...
    facet_wave_facets=facet_counts["wave_facets"],
    ...
    facet_anchors=facet_counts["source_anchor_occurrences_validated"],
    facet_anchor_files=facet_counts["source_anchor_files_validated"],
    facet_ratings=facet_counts["comparison_dimension_ratings"],
    facet_no_go=facet_counts["independent_no_go_verdicts"],
