  ...
  <main>
    ...
    <section id="checkpoint" class="panel">
      ...
      <div class="table-wrap"><table><thead><tr><th>Gate slice</th><th>Current denominator</th><th>Evidence boundary</th></tr></thead><tbody>
	    ...
	    <section class="panel"><h2>RUN-158–160 current adjudication checkpoint</h2><div class="table-wrap"><table><thead><tr><th>Run</th><th>Observed evidence</th><th>Boundary</th></tr></thead><tbody><tr><td>RUN-158 exact dashboard verification</td><td><strong>4/4 viewports · 50/50 visible checks · 10/10 navigation · 387/387 local resources</strong></td><td class="partial">exact superseded RUN-157 dashboard only · zero application credit</td></tr><tr><td>RUN-159/R MED-RBAC adjudication</td><td><strong>3 independent current-source ALREADY_FIXED reviews · $bounded_tests tests · $bounded_assertions assertions · exact receipt GO</strong></td><td class="partial">historical MED-RBAC identity retired from current provisional queue · no application change · no scope/atomicity inheritance · no final finding or completion credit</td></tr><tr><td>RUN-160 live reporting</td><td><strong>$finding_count current provisional P1 + $historical_fixed_count historical already-fixed · 12 retained identities</strong></td><td class="partial">Gate 4 and audit completion false · exact regenerated dashboard requires RUN-161 verification</td></tr></tbody></table></div></section>
    ...
    <section class="panel"><h2>Fresh RUN-161 audit-dashboard verification required</h2><p>The exact regenerated RUN-160 dashboard must be checked at 1440×900, 1280×800, 1024×768, and 390×844. The linked RUN-161 receipt must record page overflow, bounded mobile table scrolling, navigation, local links, anchors, duplicate authored IDs, console output, visible 664/307/357 ownership, 95 bridges, 118/389 queue accounting, 12 retained claim identities split into $finding_count current provisional P1 and $historical_fixed_count historical already-fixed, RUN-159's $bounded_tests tests / $bounded_assertions assertions, exact MED-RBAC-only retirement, no scope/atomicity inheritance, current $benchmark_mapped/340 benchmark mapping, $final_no_matches/340 final no-match/NCM, $benchmark_unresolved unresolved targets, one operating organisation across multiple Sites, Gate 4 open, and every non-bounded runtime, browser, final-finding, release, Pass, feature-completion, and audit-complete zero-credit boundary. It verifies the audit artifact only and grants no application-browser, responsive-application, visual, workflow, release, Pass, feature-completion, or audit-complete credit.</p><ul class="list"><li><a href="evidence/browser/current-audit-dashboard-verification-run-161-wave-28.json">RUN-161 responsive audit-dashboard verification receipt</a> (forward reference until materialized)</li></ul></section>
    ...
    <p class="footer">Generated deterministically from independently reviewed static, Git/source, bounded MED-RBAC runtime, and exact-artifact evidence through RUN-159R and reported in RUN-160. Exactly two matrix rows have static benchmark-mapping credit; RUN-159 establishes only the bounded already-fixed MED-RBAC disposition and $bounded_tests tests / $bounded_assertions assertions. All application-browser, full-suite/coverage, ease, release, Pass, final-finding, feature-completion, and audit-completion boundaries remain open. Audit artifacts only; no application remediation was required or performed.</p>
  </main>
</body>
</html>
""")
...
current_reporting_template_rewrites = [
    ...
    (
        ...
        "RUN-156/R record and independently review bounded two-checkpoint medication-governance Git/source provenance; RUN-157 reports that receipt; RUN-158 verifies that exact superseded dashboard; RUN-159/R retire only historical MED-RBAC after three unanimous current-source reviews, $bounded_tests tests / $bounded_assertions assertions, and an exact-artifact GO; RUN-160 reports the live $finding_count + $historical_fixed_count split. Current-source framework reachability beyond the bounded MED-RBAC slice, application browser, build, rendered visual, full-suite coverage, ease, release, Pass, final-finding, feature-completion, remote-currency, publication, and audit-completion remain open.",
    ),
    (
    ...
    (
        ...
        "RUN-157 materializes its historical reporting checkpoint; RUN-158 verifies that exact dashboard; RUN-159/R authorize MED-RBAC retirement after bounded source/runtime evidence; RUN-160 reconciles the live finding register without changing Fleet ownership, direct-exact review-queue, benchmark, or completion counts.",
    ),
    (
    ...
    (
        ...
        "<li>RUN-157: historical deterministic reporting checkpoint</li><li>RUN-158: exact RUN-157 dashboard verified at 4/4 viewports · 50/50 visible checks · 10/10 navigation · 387/387 local resources · zero application credit</li><li>RUN-159/R: three unanimous current-source ALREADY_FIXED reviews · $bounded_tests tests / $bounded_assertions assertions · exact corrected receipt GO · MED-RBAC-only retirement</li><li>RUN-160: live register reconciled to $finding_count current provisional P1 + $historical_fixed_count historical already-fixed · fresh RUN-161 dashboard verification required</li>",
    ),
    (
...
current_visible_boundaries = [
    ...
    "Fresh RUN-161 audit-dashboard verification required",
    "4f57ad4202df",
    "2/340 mappings",
