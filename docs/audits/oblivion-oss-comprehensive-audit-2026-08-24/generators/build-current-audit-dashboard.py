...
assert all(sha256_file(path) == digest for path, digest in reporting_materialization["inputs"].items())
assert all(sha256_file(path) == digest for path, digest in reporting_materialization["outputs"].items())
...
assert all(sha256_file(path) == digest for path, digest in reporting_review["hashes"].items())
...
assert formal_upstream["matrix_immutability"]["after_sha256"] == sha256_file("03-feature-to-benchmark-matrix.csv")
...
checkpoint_evidence = [
    ...
    ("RUN-073 reporting materialization evidence", "evidence/source/current-required-reporting-materialization-wave-05.json"),
    ("RUN-073 report 07 module findings", "07-module-findings.md"),
    ("RUN-073 report 08 cross-module journeys", "08-cross-module-journeys.md"),
    ("RUN-073 report 09 UI/UX/accessibility", "09-ui-ux-accessibility-visual-consistency.md"),
    ("RUN-073 report 10 architecture/data/integration/security", "10-architecture-data-integration-security.md"),
    ("RUN-073 report 11 prioritised roadmap", "11-prioritised-roadmap.md"),
    ("RUN-073 report 12 native-build register", "12-native-build-and-do-not-copy-register.md"),
    ("RUN-073 provisional findings contract", "findings.json"),
]
...
<body>
  ...
  <main>
    ...
    <div class="notice" role="status"><strong>RUN-071–073 current checkpoint:</strong> all 26 completion gates are reconciled. RUN-071's 9/18 and RUN-072's 11/18 are historical snapshots; RUN-073 has all 18 prompt-required files/directories present, including <span class="mono">evidence/</span> and excluding this generated dashboard. Presence is not completion. RUN-072 retains 300/300 source-bound H contracts with 0 validated tasks and every current/target measurement <span class="mono">NOT_MEASURED</span>. RUN-073 adds 8 independently source-reviewed journeys (44 handoffs: 27 PROVEN · 8 PARTIAL · 9 NOT_ESTABLISHED) and 13 architecture entity families, 17 concerns, 9 provisional claims, and 10 explicit unknowns. Prompt-grade journey execution, final/runtime architecture findings, mapping, final-no-match, NCM, runtime, browser, test, ease, Pass, and completion credit remain zero.</div>
    ...
    <section id="checkpoint" class="panel">
      <h2>RUN-071–073 completion-gate checkpoint</h2>
      <p>The 26 literal completion gates were reconciled before RUN-072 added source-bound usability and incident-chain evidence and RUN-073 materialized the remaining reporting paths, source journeys, and architecture synthesis. Static artifacts are not measured task, runtime, browser, mapping, Pass, final-finding, or completion evidence.</p>
    ...
    <section class="panel"><h2>Prior audit-dashboard verification</h2><p>RUN-070 and RUN-072 responsive verification are superseded for this changed RUN-073 HTML; no prior viewport, overflow, navigation, table, link, or console proof transfers.</p><ul class="list"><li><a href="evidence/browser/current-audit-dashboard-verification-formal-upstream-wave-03.json">Superseded RUN-070 verification</a></li><li><a href="evidence/browser/current-audit-dashboard-verification-run-072-wave-04.json">Superseded RUN-072 verification</a></li></ul></section>
    <section class="panel"><h2>Fresh RUN-073 audit-dashboard verification</h2><p>The final generated dashboard is checked at 1440×900, 1280×800, 1024×768, and 390×844. Page-level horizontal overflow is absent, wide tables retain bounded internal scrolling at mobile width, navigation remains locally scrollable, all local links resolve, and the browser console has zero warnings or errors. This verifies the audit artifact only and grants no application-browser, responsive, visual, workflow, finding, runtime, test, release, Pass, completion, or audit-complete credit.</p><ul class="list"><li><a href="evidence/browser/current-audit-dashboard-verification-run-073-wave-05.json">RUN-073 responsive audit-dashboard verification</a></li></ul></section>
    <section class="panel"><h2>RUN-071–073 evidence lineage</h2><p>Every new raw, sealed, generated, reviewed, and adjudicated artifact is linked with its current SHA-256. The task-script directory link carries the deterministic 300-file bundle hash.</p><ul class="list">$checkpoint_evidence_links</ul></section>
    ...
    <p class="footer">Generated deterministically from evidence integrated through RUN-073. Audit artifacts only; no application remediation is authorised.</p>
  </main>
</body>
</html>
""")
