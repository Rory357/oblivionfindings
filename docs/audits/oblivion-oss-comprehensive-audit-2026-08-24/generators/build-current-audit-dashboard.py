...
assert sha256_file("03-feature-to-benchmark-matrix.csv") == static_linkage_integration["matrix"]["updated_sha256"]
...
assert static_linkage_integration["matrix"]["updated_sha256"] == sha256_file("03-feature-to-benchmark-matrix.csv")
...
assert static_linkage_reporting["pins"]["updated_matrix_sha256"] == sha256_file("03-feature-to-benchmark-matrix.csv")
...
assert all(sha256_file(path) == digest for path, digest in static_linkage_reporting["inputs"].items())
...
assert all(sha256_file(path) == digest for path, digest in static_linkage_reporting["outputs"].items())
...
assert live_gap_ids["route_paths"] == static_linkage_integration["remaining_gaps"]["route_paths"]
assert live_gap_ids["route_names"] == static_linkage_integration["remaining_gaps"]["route_names"]
assert live_gap_ids["page_files"] == static_linkage_integration["remaining_gaps"]["page_files"]
assert live_gap_ids["backend_anchors"] == static_linkage_integration["remaining_gaps"]["backend_anchors"]
assert live_gap_ids["test_anchors"] == static_linkage_integration["remaining_gaps"]["test_anchors"]
assert live_both_gap_ids == static_linkage_integration["remaining_gaps"]["both_route_and_page"]
assert len(live_gap_ids["route_paths"]) == static_linkage_integration["counts"]["remaining_missing_route_paths"]
assert len(live_gap_ids["page_files"]) == static_linkage_integration["counts"]["remaining_missing_page_files"]
assert len(live_both_gap_ids) == static_linkage_integration["counts"]["remaining_missing_both_route_and_page"]
assert len(live_gap_ids["backend_anchors"]) == static_linkage_integration["counts"]["remaining_missing_backend_anchors"]
assert len(live_gap_ids["test_anchors"]) == static_linkage_integration["counts"]["remaining_missing_test_anchors"]
...
<body>
  ...
  <main>
    ...
    <section id="checkpoint" class="panel">
      <h2>RUN-071–076 completion-gate checkpoint</h2>
      <p>The 26 literal completion gates were reconciled before RUN-072 added source-bound usability and incident-chain evidence, RUN-073 materialized reporting paths and source synthesis, and RUN-074–076 independently reconstructed, reviewed, and integrated bounded feature-side static linkage. Static artifacts are not measured task, runtime, browser, mapping, Pass, final-finding, or completion evidence.</p>
    ...
    <section class="panel"><h2>Prior audit-dashboard verification</h2><p>RUN-070, RUN-072, and RUN-073 responsive verification are immutable history for superseded HTML; no prior viewport, overflow, navigation, table, link, or console proof transfers to RUN-076.</p><ul class="list"><li><a href="evidence/browser/current-audit-dashboard-verification-formal-upstream-wave-03.json">Superseded RUN-070 verification</a></li><li><a href="evidence/browser/current-audit-dashboard-verification-run-072-wave-04.json">Superseded RUN-072 verification</a></li><li><a href="evidence/browser/current-audit-dashboard-verification-run-073-wave-05.json">Superseded RUN-073 verification</a></li></ul></section>
    <section class="panel"><h2>Fresh RUN-076 audit-dashboard verification</h2><p>The final generated dashboard is checked at 1440×900, 1280×800, 1024×768, and 390×844. Page-level horizontal overflow is absent, wide tables retain bounded internal scrolling at mobile width, navigation remains locally scrollable, all local links resolve, and the browser console has zero warnings or errors. This verifies the audit artifact only and grants no application-browser, responsive, visual, workflow, finding, runtime, test, release, Pass, completion, or audit-complete credit.</p><ul class="list"><li><a href="evidence/browser/current-audit-dashboard-verification-run-076-wave-06.json">RUN-076 responsive audit-dashboard verification</a></li></ul></section>
    <section class="panel"><h2>RUN-071–076 evidence lineage</h2><p>Every current raw, generated, reviewed, and integrated artifact is linked with its SHA-256. RUN-073 receipts remain immutable history; the task-script directory link carries the historical RUN-072 300-file bundle hash.</p><ul class="list">$checkpoint_evidence_links</ul></section>
    ...
    <p class="footer">Generated deterministically from evidence integrated through RUN-076. Audit artifacts only; no application remediation is authorised.</p>
  </main>
</body>
</html>
""")
