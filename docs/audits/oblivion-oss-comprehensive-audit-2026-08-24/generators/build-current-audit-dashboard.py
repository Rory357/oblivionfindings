...
<body>
  ...
  <main>
    ...
    <section id="checkpoint" class="panel">
      <h2>RUN-071–157 completion-gate checkpoint</h2>
    ...
    <section class="panel"><h2>RUN-155–157 medication-governance source receipt</h2><p>RUN-155 verifies only the exact now-superseded RUN-154 audit-dashboard artifact: 4/4 required viewports, 38/38 visible boundary checks, 10/10 navigation targets, 379/379 unique local links, and zero console warnings, console errors, or page errors. None of that proof transfers to the RUN-157 dashboard or the application.</p><p>RUN-156 separates the 359-path historical first-parent merge payload at <span class="mono">cd5d34e6b8aa7e494808745041ec1dfa187dc101</span> (87 added · 272 modified) from effective application checkpoint <span class="mono">c5c0ad0903d2e2e2229d5d0090fc0a69a2206f0f</span>: 358 payload blobs remain unchanged and exactly <span class="mono">resources/js/pages/my-day/index.tsx</span> is superseded. The complete post-merge My Day delta is three modified paths; the three later commits through <span class="mono">86b232cb14967c63ff345ac5208ec6d4c379f24f</span> are audit-root-only and preserve the 12,784-entry non-audit manifest.</p><p>RUN-156R is committed at <span class="mono">$run156r_commit</span>. Three distinct coordinated review lanes—explicitly not blind or isolated, with the single <span class="mono">/root</span> record materializer disclosed—return GO with zero discrepancies and authorize reporting only.</p><ul class="list">$medication_record_items</ul><p>The three medication records remain reference-only provisional claims pinned to historical audited application commit <span class="mono">a0493442b9e392d324055c35bf25b69421dc2d35</span>. RUN-156/R performs no semantic adjudication, rebase, promotion, remediation, verification, closure, or finding/completion credit. Its origin statement is an <strong>unfetched local remote-tracking observation only</strong>: <span class="mono">origin/main</span> was locally observed at <span class="mono">20ad5cef0aacb3d055e685d2f8b7b583cb8d78f4</span>, with the sealed RUN-156 checkpoint 179 ahead and 0 behind; fetch, remote currency, publication, and push remain unverified.</p><p>RUN-157 reports only this bounded receipt class. The current $static_owner_records/$static_owner_routes/$static_owner_pages/$static_action_bridges Fleet owner/route/page/bridge counts, $queue_reviewed reviewed and $queue_pending pending queue rows, $queue_without_owner rows without ownership, $benchmark_mapped/340 mappings, $final_no_matches/340 final no-match/NCM, $benchmark_unresolved unresolved targets, and $finding_count provisional findings remain unchanged. Gate 4 and audit completion are false; the rebuilt dashboard requires fresh RUN-158 verification.</p></section>
    ...
    <section class="panel"><h2>Fresh RUN-158 audit-dashboard verification required</h2><p>The exact regenerated RUN-157 dashboard must be checked at 1440×900, 1280×800, 1024×768, and 390×844. The linked RUN-158 receipt must record page overflow, bounded mobile table scrolling, navigation, local links, anchors, duplicate authored IDs, console output, visible 664/307/357 ownership, 95 bridges, 118/389 queue accounting, all six Fleet provisional-not-final source observations, the three a049-pinned reference-only provisional medication records, exact RUN-156 two-checkpoint 359 = 358 + 1 accounting, complete three-path My Day delta, audit-root-only later lineage, unchanged 12,784-entry non-audit manifest, and coordinated non-blind/non-isolated RUN-156R disclosure, current $benchmark_mapped/340 benchmark mapping, $final_no_matches/340 final no-match/NCM, $benchmark_unresolved unresolved targets, exact RUN-145 matrix/register/receipt pins, one operating organisation across multiple Sites, Gate 4 open, and every medication-correctness, origin-currency, publication, and other non-mapping zero-credit boundary. It verifies the audit artifact only and grants no application-browser, responsive-application, visual, workflow, finding, runtime, test, release, Pass, feature-completion, or audit-complete credit.</p><ul class="list"><li><a href="evidence/browser/current-audit-dashboard-verification-run-158-wave-27.json">RUN-158 responsive audit-dashboard verification receipt</a> (forward reference until materialized)</li></ul></section>
    <section class="panel"><h2>RUN-071–157 evidence lineage</h2><p>Every current raw, generated, reviewed, and integrated RUN-077–157 source/reporting/benchmark artifact is linked with its exact SHA-256. Earlier receipts remain immutable history; the task-script directory link carries the historical RUN-072 300-file bundle hash.</p><ul class="list">$checkpoint_evidence_links</ul></section>
    ...
    <p class="footer">Generated deterministically from independently reviewed static and Git/source-receipt evidence through RUN-156R and reported in RUN-157. Exactly two matrix rows have static benchmark-mapping credit; all other application/runtime/browser/test/coverage/medication-correctness/origin-currency/publication/Pass/finding/completion boundaries remain unchanged. Audit artifacts only; no application remediation is authorised.</p>
  </main>
</body>
</html>
""")
...
medication_record_items = "".join(
    f'<li><strong>{html.escape(row["id"])}</strong> · canonical record <span class="mono">{html.escape(row["canonical_record_sha256"])}</span> · historical application pin <span class="mono">{html.escape(row["audited_application_commit"])}</span> · reference-only provisional · zero final-finding/completion credit</li>'
...
temporary_path = output_path.with_name(f".{output_path.name}.tmp-run157-dashboard")
