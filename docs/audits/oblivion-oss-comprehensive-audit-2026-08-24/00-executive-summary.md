...
- `03-feature-to-benchmark-matrix.csv`: 340-row canonical static identity matrix with P1 identity frozen only; retained gaps are 1 route path, 166 route names, 4 page files, 0 backend anchors, and 8 static test anchors. Benchmark, ease, executed-test, runtime, browser, release, P2–P8, and completion credit remain explicit and zero.
...
- `06-open-source-benchmark-register.csv`: byte-identical carry-forward register of 95 exact prompt-repository rows plus three historical extras, with prompt occurrence weights totalling 98, current official GitHub metadata, RUN-031 through RUN-034 observer-triage fields, and RUN-035 through RUN-038 blocker-review fields. RUN-070 records the later 9/95 project-record acceptance overlay separately; mapping and completion credit remain zero.
...
2. Continue from RUN-080's retained matrix gaps—1 route path, 166 route names, 4 page files, 0 backend anchors, and 8 static test anchors—and separately adjudicate the 3,003 explicit unmapped route-like rows and 393 page evidence gaps to exact canonical feature IDs. Then establish safe framework reachability and refresh the RUN-072 task locators without inheriting runtime, browser, test, mapping, or completion credit.
