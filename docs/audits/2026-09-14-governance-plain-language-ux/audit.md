...
- **Follow-ups done 16 September 2026:** committee meetings link to their committee risk view and report; the board monthly report counts current (open + accepted) risks with the register's severity bands and shows real last-change times; policies due to be confirmed again appear in My work (shared `GovernancePolicy::confirmationStateFor()` rule); the Dusk chair journey uses the Workflow tab. Still to do: a signed-in browser walkthrough.
