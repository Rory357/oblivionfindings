...
if(!preg_match('/^oblivion_gov_(review3_runtime|gemini_verify)_20260913_[0-9]+$/',$state['database']??''))throw new RuntimeException('Disposable database guard failed');
...
probe('records_ignores_explicit_document_capability_denial',function()use($h,$actors){
 $u=$actors['observer'];$p=App\Models\Permission::where('key','governance.documents.view')->firstOrFail();
 $u->permissionOverrides()->syncWithoutDetaching([$p->id=>['allowed'=>false]]);$u->unsetRelation('permissionOverrides');$u->unsetRelation('roles');
 $doc=M\GovernanceDocument::create(['title'=>'Synthetic document forbidden by capability','document_type'=>'report','file_path'=>'synthetic-restricted-report.pdf','uploaded_by'=>$actors['chair']->id,'version_number'=>1,'is_current'=>true]);
 $r=$h->actingAs($u)->get('/governance/records');$direct=$h->actingAs($u)->get('/governance/documents/'.$doc->id);
 return ['home_permission'=>$u->canDo('governance.view'),'documents_permission'=>$u->canDo('governance.documents.view'),'records_status'=>$r->status(),'title_disclosed'=>str_contains($r->getContent(),$doc->title),'direct_status'=>$direct->status()];
});
file_put_contents(__DIR__.'/records-probe-results.json',json_encode($out,JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES));echo json_encode($out,JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES);
