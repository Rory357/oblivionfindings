...
if(!$root || is_file(__DIR__.'/stop-runtime.flag') || !preg_match('/^oblivion_gov_review3_runtime_20260913_[0-9]+$/',$state['database']??'')){http_response_code(503);exit('Audit runtime unavailable');}
...
$overrides=['APP_ENV'=>'local','APP_DEBUG'=>'false','APP_CONFIG_CACHE'=>__DIR__.'/unused-config.php','APP_URL'=>'http://127.0.0.1:8783','DB_DATABASE'=>$state['database'],'SESSION_DRIVER'=>'file','SESSION_COOKIE'=>'gov_review3_20260913','SESSION_SECURE_COOKIE'=>'false','MAIL_MAILER'=>'array','QUEUE_CONNECTION'=>'sync','BROADCAST_CONNECTION'=>'null','CACHE_STORE'=>'array'];
...
$app->make(Illuminate\Foundation\Vite::class)->useBuildDirectory('build-gov-audit3-20260913')->useHotFile(__DIR__.'/unused-hot');
