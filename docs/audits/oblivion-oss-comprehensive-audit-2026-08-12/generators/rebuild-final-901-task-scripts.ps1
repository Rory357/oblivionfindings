...
$manifestPath = Join-Path $sourceDir 'working-capability-manifest-901.json'
...
$destination = [System.IO.Path]::GetFullPath((Join-Path $auditRoot 'task-scripts\final-901'))
$summaryPath = Join-Path $sourceDir 'final-901-task-script-generation-summary.json'
...
if ($targets.Count -ne 901 -or $humanTargets.Count -ne 788) {
...
if (($stableIds | Sort-Object -Unique).Count -ne 901) {
...
if (Test-Path -LiteralPath $destination) {
    ...
    if ($existingNames.Count -gt 0) {
        ...
        if ($existingNames.Count -ne $expectedNamesSorted.Count -or (Compare-Object -ReferenceObject $expectedNamesSorted -DifferenceObject $existingNames).Count -gt 0) {
            throw 'Task-script destination is non-empty but does not contain exactly the expected final-901 filename set. Refusing partial or stale overwrite.'
        }
    }
} else {
...
"@
    ...
    $scriptIndex.Add([pscustomobject][ordered]@{
        feature_id = $id
        file = "task-scripts/final-901/$filename"
...
$indexLines = @($scriptIndex | Sort-Object feature_id | ForEach-Object { "$($_.feature_id)|$($_.file)|$($_.sha256)" })
$indexText = $indexLines -join "`n"
$scorecardSha = Sha256-File $scorecardPath

$summary = [ordered]@{
    schema_version = '1.0'
    artifact = 'final-901-task-script-generation-summary'
    ...
    input = [ordered]@{
        manifest = 'working-capability-manifest-901.json'
    ...
    outputs = [ordered]@{
        directory = 'task-scripts/final-901'
    ...
    scripts = @($scriptIndex | Sort-Object feature_id)
}
