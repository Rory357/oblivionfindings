# FLEET-ROUTE-FLEET-TRIPS-PLAYBACK: Fleet Trips Playback
...
- Legacy family: `FLEET-ROUTE-FLEET-TRIPS-PLAYBACK`
...
Start at `fleet/trips/{trip}/playback` (`fleet.trips.playback`). Route existence is exact; visibility and access are unexecuted.
...
1. Enter through `GET|HEAD fleet/trips/{trip}/playback` (`fleet.trips.playback`); the route is exact, but menu visibility and runtime access were not executed.

## Source-applicable states and transitions

- **information presented** is applicable only to `Closure` / `ROUTE-0850` at `routes/*.php closure`; it is not runtime-observed.
...
- `GET|HEAD fleet/trips/{trip}/playback` — `fleet.trips.playback` — `Closure` — `routes/*.php closure:line unresolved` — middleware `web`
