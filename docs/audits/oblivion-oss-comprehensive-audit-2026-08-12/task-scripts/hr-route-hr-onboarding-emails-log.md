...
Start at `hr/onboarding/emails/log` (`hr.onboarding.emails.log`). Route existence is exact; visibility and access are unexecuted.
...
1. Enter through `GET|HEAD hr/onboarding/emails/log` (`hr.onboarding.emails.log`); the route is exact, but menu visibility and runtime access were not executed.

## Source-applicable states and transitions

- **information presented** is applicable only to `Closure` / `ROUTE-1572` at `routes/*.php closure`; it is not runtime-observed.
...
- `GET|HEAD hr/onboarding/emails/log` — `hr.onboarding.emails.log` — `Closure` — `routes/*.php closure:line unresolved` — middleware `web, auth, permission:hr.onboarding.view`
