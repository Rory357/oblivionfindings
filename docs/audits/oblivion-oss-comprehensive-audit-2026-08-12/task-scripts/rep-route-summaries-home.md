...
Start at `summaries` (`summaries.home`). Route existence is exact; visibility and access are unexecuted.
...
1. Enter through `GET|HEAD summaries` (`summaries.home`); the route is exact, but menu visibility and runtime access were not executed.

## Source-applicable states and transitions

- **information presented** is applicable only to `Closure` / `ROUTE-2945` at `routes/*.php closure`; it is not runtime-observed.
...
- `GET|HEAD summaries` — `summaries.home` — `Closure` — `routes/*.php closure:line unresolved` — middleware `web, auth`
