# HR-ROUTE-CAREERS-SHOW: Careers Show
...
- Legacy family: `HR-ROUTE-CAREERS-SHOW`
...
Start at `careers/{slug}` (`careers.show`). Route existence is exact; visibility and access are unexecuted.
...
1. Enter through `GET|HEAD careers/{slug}` (`careers.show`); the route is exact, but menu visibility and runtime access were not executed.

## Source-applicable states and transitions

- **information presented** is applicable only to `Closure` / `ROUTE-0089` at `routes/*.php closure`; it is not runtime-observed.
...
- `GET|HEAD careers/{slug}` — `careers.show` — `Closure` — `routes/*.php closure:line unresolved` — middleware `web`
