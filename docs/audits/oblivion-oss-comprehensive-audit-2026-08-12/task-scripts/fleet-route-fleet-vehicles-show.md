# FLEET-ROUTE-FLEET-VEHICLES-SHOW: Fleet Vehicles Show
...
- Legacy family: `FLEET-ROUTE-FLEET-VEHICLES-SHOW`
...
Start at `fleet/vehicles/{asset}` (`fleet.vehicles.show`). Route existence is exact; visibility and access are unexecuted.
...
1. Enter through `GET|HEAD fleet/vehicles/{asset}` (`fleet.vehicles.show`); the route is exact, but menu visibility and runtime access were not executed.

## Source-applicable states and transitions

- **information presented** is applicable only to `Closure` / `ROUTE-0851` at `routes/*.php closure`; it is not runtime-observed.
...
- `GET|HEAD fleet/vehicles/{asset}` — `fleet.vehicles.show` — `Closure` — `routes/*.php closure:line unresolved` — middleware `web`
