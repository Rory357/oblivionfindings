# PRIV-ROUTE-PRIVACY-POLICY: Privacy Policy
...
- Legacy family: `PRIV-ROUTE-PRIVACY-POLICY`
...
Start at `privacy-policy` (`privacy.policy`). Route existence is exact; visibility and access are unexecuted.
...
1. Enter through `GET|HEAD privacy-policy` (`privacy.policy`); the route is exact, but menu visibility and runtime access were not executed.

## Source-applicable states and transitions

- **information presented** is applicable only to `Closure` / `ROUTE-2293` at `routes/*.php closure`; it is not runtime-observed.
- Presentation-state applicability must be determined from exact signals in `resources/js/pages/privacy.tsx`; page presence alone is not loading/empty/error proof.
...
- `GET|HEAD privacy-policy` — `privacy.policy` — `Closure` — `routes/*.php closure:line unresolved` — middleware `web`

## Source anchors and limits

- Backend anchor: `routes/*.php closure`.
- Exact render/action page relationships: `resources/js/pages/privacy.tsx`.
