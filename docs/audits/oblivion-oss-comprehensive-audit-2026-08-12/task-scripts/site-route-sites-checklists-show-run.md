# SITE-ROUTE-SITES-CHECKLISTS-SHOW-RUN: Sites Checklists Showrun
...
- Legacy family: `SITE-ROUTE-SITES-CHECKLISTS-SHOW-RUN`
...
Start at `checklists/runs/{run}` (`sites.checklists.showRun`). Route existence is exact; visibility and access are unexecuted.
...
1. Enter through `GET|HEAD checklists/runs/{run}` (`sites.checklists.showRun`); the route is exact, but menu visibility and runtime access were not executed.

## Source-applicable states and transitions

- **information presented** is applicable only to `Closure` / `ROUTE-0115` at `routes/*.php closure`; it is not runtime-observed.
...
- `GET|HEAD checklists/runs/{run}` — `sites.checklists.showRun` — `Closure` — `routes/*.php closure:line unresolved` — middleware `web, auth, verified, permission:checklists.view`
