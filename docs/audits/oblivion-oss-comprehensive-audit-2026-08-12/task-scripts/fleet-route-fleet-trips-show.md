# FLEET-ROUTE-FLEET-TRIPS-SHOW: Fleet Trips Show
...
- Legacy family: `FLEET-ROUTE-FLEET-TRIPS-SHOW`
...
Start at `fleet/trips/{trip}` (`fleet.trips.show`). Route existence is exact; visibility and access are unexecuted.
...
1. Enter through `GET|HEAD fleet/trips/{trip}` (`fleet.trips.show`); the route is exact, but menu visibility and runtime access were not executed.

## Source-applicable states and transitions

- **information presented** is applicable only to `Closure` / `ROUTE-0847` at `routes/*.php closure`; it is not runtime-observed.
...
- `GET|HEAD fleet/trips/{trip}` — `fleet.trips.show` — `Closure` — `routes/*.php closure:line unresolved` — middleware `web`
