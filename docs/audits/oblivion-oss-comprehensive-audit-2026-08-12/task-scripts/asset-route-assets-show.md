...
Start at `assets/{asset}` (`assets.show`). Route existence is exact; visibility and access are unexecuted.
...
1. Enter through `GET|HEAD assets/{asset}` (`assets.show`); the route is exact, but menu visibility and runtime access were not executed.

## Source-applicable states and transitions

- **information presented** is applicable only to `Closure` / `ROUTE-0046` at `routes/*.php closure`; it is not runtime-observed.
...
- `GET|HEAD assets/{asset}` — `assets.show` — `Closure` — `routes/*.php closure:line unresolved` — middleware `web, auth`
