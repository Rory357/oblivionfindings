...
| D-05 | 15, 16 | Tasks 15–16: canonical uncapped seven-criterion handover scope separates individually required work from an exact frozen carry-forward summary; current snoozed, shift-member-owned/watched, SLA-risk, due-task, governance-change, and outgoing-lead-pinned work remain decision-relevant; incoming visibility is rechecked at prepare and accept; immutable prepared snapshots fail closed on malformed or fabricated state. Configurable stale recovery is owned by a dedicated Admin/Provider Manager/Coordinator permission, requires an audited reason, retains all scope/acceptance gates, and strictly validated snapshot data must match the persisted audit row. The E2E fixture creates a deterministic site-bounded fresh shift without touching unrelated records or active shifts. | Task 15 definitive backend: 110 tests, 718 assertions; isolated 40-alert full SLA query budget: 1 test, 4 assertions; frontend: 3 tests. Task 16 backend: 19 tests, 238 assertions; frontend: 4 tests; RBAC seed, TypeScript, targeted ESLint/Prettier/Pint/PHP syntax, client build 4,968 modules, SSR build 1,620 modules, diff integrity clean | — | — | Active |
| D-06 | 10 | — | — | — | — | Open |
