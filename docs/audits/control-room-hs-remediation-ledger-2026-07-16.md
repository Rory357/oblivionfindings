...
| Client production build | `npm run build` | Task 12 production client bundle passed with 4,966 transformed modules; final all-task rerun remains owned by Task 20 | 0 | 2026-07-16 | Pass |
| SSR production build | `npm run build:ssr` | Task 12 production client bundle passed with 4,966 transformed modules in 3m 29s and SSR passed with 1,618 modules in 41.33s; final all-task rerun remains owned by Task 20 | 0 | 2026-07-17 07:15 NZST | Pass |
...
| Universal Tasks and permission recovery | Task 10 checkpoint `d5708ead7`; Task 11 checkpoint (this commit) | Corrective-action display state, active/done buckets, incident-journey references, private server-side search terms, row/detail/CSV lifecycle labels, complete owner search, SQL-filtered exhaustive journey search, role-aware actions, exact filtered recovery, focus restoration, watcher integrity, and stable composite task identities are implemented; browser/live proof remains open | Active |
| Evidence continuity and closure gates | Task 12 checkpoint (this commit) | Typed immediate controls now persist and flow into serious Control Room-created incidents; linked evidence presentation and closure-gate work continue in Tasks 13–14 | Active |
...
- D-08 is Active, not deferred: Task 12 completes capture and canonical incident carry-forward; Task 13 still owns read-only linked Control Room evidence presentation in Incident and H&S, followed by production-built browser and live proof.
