...
- [x] **8. Cross-module** — DONE the high-value/low-risk set; deferred the churny/high-plumbing ones (per review recommendation):
      ...
      - ✅ **E2-backend** active-BSP auto-link mirrored into `storeEvent` (the data-integrity piece). ✅ **E2-frontend DONE** — respite page now opens the gold `RestraintEventWizard` (prescope-locked to the stay's client/site, `RespiteWorkspaceController@restraintPickers` feeds sites/staff/incidents/plans; `recordRestraint` field names match the wizard payload 1:1 and still auto-link the active BSP). Off-pattern `RestraintModal` retired.
      ...
      - ✅ **E4 DONE** restraint KPI tiles on the H&S dashboard (Lagging tab) + analytics explorer. Built `App\Services\HealthSafety\RestraintKpiService` (dedicated, read-only — zero churn on shared HsKpiService/HsAnalyticsService): `summary` (events/out-of-plan/injury/critical/unreviewed/active-BSP/review-due/no-active-BSP), `unreviewedWorklist`, `breakdowns` (by type/severity/plan-status). Wired additively into `HealthSafetyDashboardController@index`+`@analytics` via `rescue(…, $fallback, false)` (exact-shape fallback ⇒ fail-safe). Frontend `RestraintStrip` (6 deep-link tiles + unreviewed mini-list) + `RestraintBreakdowns` (stat tiles + 3 charts). Events site-scope by own `site_id`, plans via `client.site_id`. Every tile href verified against the register's tab match-arms.
      - ✅ **D3 DONE** post-hoc "Link incident" via a **dedicated** `RestraintController@linkIncident` endpoint (`POST /events/{event}/link-incident`, `permission:restraints.review|manage`, same-client integrity guard, nullable to unlink) — separate from `updateEvent` so it doesn't re-stamp the review audit. UI = detail-dialog "Link incident" pane. +4 feature tests (same-client / cross-client reject / unlink / requires-review-perm).
...
- [x] **9. Adversarial review (/workflows)** — ✅ DONE. `restraints-adversarial-review` workflow (20 agents, 6 dims→verify→synthesize):
      ...
      Deferred w/ rationale (all subsequently completed): E2-frontend, E4, D3, D2.
- [x] **10. All deferred items completed** (follow-up /loop): ✅ E2-frontend, ✅ E4, ✅ D3 (above), ✅ **D2** event-detail rail consolidation — reviewed and **accepted as built**: the rail's 5 entries are read-only governance views (live-computed blurbs) while record-changing workflows (Review, Link-incident) deliberately replace the body as panes rather than adding rail rows; a design-rationale comment now documents this so the section/pane split isn't mistaken for duplication. **28 tests green (140 assert); tsc 0; vite build clean; php -l clean.**
- [ ] **11. (user) deploy / Chrome-verify** on .com (code-only — NO new migrations, NO new permissions; RestraintKpiService only reads existing tables).
