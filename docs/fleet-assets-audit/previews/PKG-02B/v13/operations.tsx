...
export type Action = {
    ...
    validationFocus?: (v: Values) => string | undefined;
    validateStep?: (v: Values, step: number) => string;
    alternatives?: Values[];
    title: string;
    sections: Section[];
    values: Values;
...
export function useVehicleModel(
    ...
    const complianceProblems = complianceIssues(data.compliance, odo, TODAY);
    const complianceProblem = complianceProblems[0] ?? '';
    const bookingUseProblem =
        complianceProblem ||
        (lastCheckOutcome !== 'Passed'
            ? 'Review the vehicle condition check before vehicle use.'
            : !odo ||
                (scenario === 'stale' &&
              ...
              ? 'Review missing or stale readings and overdue service/check requirements before vehicle use.'
              : hold
            ...
            sections: [
                section(
                    ...
                    [
                        ...
                        f(
                            'files',
                            'Completion report / retest attachments',
                            'files',
                            false,
                        ),
                ...
                v.reviewer !== 'Operations Manager'
                    ...
                    : v.decision === 'Release for use' &&
                      ...
                      : v.decision === 'Release for use' &&
                        ...
                        : v.decision === 'Release for use' && complianceProblem
                          ? complianceProblem
                          : '',
            save: (v) =>
                patch(
    ...
    function compliance(id: string, initialStep = 0) {
        const c = data.compliance.find((x) => x.id === id)!;
        open({
            ...
            sections: [
                ...
                section(
                    ...
                    [
                        ...
                        f('evidence', 'Evidence reference', 'text', false),
                        f(
                            'files',
                        ...
                        ...(c.name === 'RUC'
                            ? [
                                  f(
                                      'low',
                                      'Licence start odometer',
                                      'number',
                                      false,
                                  ),
                                  f(
                                      'high',
                                      'Licence end odometer',
                                      'number',
                                      false,
                                  ),
                              ]
                        ...
                        ...field,
                        visibleWhen: (v: Values) =>
                            v.applies !== 'Not applicable',
                        requiredWhen: [
                            'evidence',
                            'due',
                            'low',
            ...
            validate: (v) =>
                c.name === 'RUC' &&
                v.applies === 'Applicable' &&
                ['Recorded', 'Passed'].includes(v.outcome) &&
                +v.high <= +v.low
                    ...
                    : v.applies === 'Applicable' &&
                      ...
                      : v.applies === 'Applicable' &&
                          ['Recorded', 'Passed', 'Failed'].includes(
                              v.outcome,
                          ) &&
                          !v.evidence?.trim()
                    ...
                    (d) => ({
                        ...
                        works:
                            ...
                            !d.works.some(
                                ...
                                : d.works,
                        restriction:
                            v.applies === 'Applicable' && v.outcome === 'Failed'
                                ? true
                                : d.restriction,
                        released:
                            v.applies === 'Applicable' && v.outcome === 'Failed'
    ...
    function followup(
        ...
        const document = r?.documentGroup
            ? data.documents.find((f) => f.id === r.source)
            : undefined;
        if (document && (!document.status || document.status === 'Current')) {
            recordTools.editDocument(document, true);
            return;
        }
        const sources = [
            ...data.documents
                .filter(
                    (f) =>
                        (!f.status ||
                            f.status === 'Current' ||
                            f.id === r?.source) &&
                        (!['reportonly', 'denied'].includes(scenario) ||
                            !f.finance),
                )
                .map((f) => ({
                    id: f.id,
    ...
    function booking(
        start = defaultBookingStart(),
        id?: string,
        block = false,
    ) {
        const b = data.bookings.find((x) => x.id === id);
        const approvalBlocked = Boolean(bookingUseProblem);
        const timeIssue = (v: Values) =>
            bookingTimeIssue(
                v.start,
                v.end,
                Boolean(b),
        ...
        open(
            {
                ...
                validationSection: (v) => (timeIssue(v) ? 0 : 2),
                validationFocus: (v) => timeIssue(v)?.field,
                validateStep: (v, step) =>
                    step === 0 ? (timeIssue(v)?.message ?? '') : '',
                extraCompletion: (v) =>
                    v.approvalRoute === 'Approval not required'
                        ? [
                              Boolean(
                ...
                note: block
                    ...
                    : bookingUseProblem
                      ? `${bookingUseProblem} This request remains pending until readiness is resolved.`
                      : canManage
        ...
        open({
            ...
            validate: (v) => {
                if (mode === 'approve' || mode === 'out') {
                    ...
                    if (mode === 'out' && +v.odo < odo)
                        return 'Checkout reading must not precede the selected reading.';
                    if (mode === 'out' && b.start > PREVIEW_NOW)
                        return 'Pickup is in the future. Checkout requires a current booking.';
                    return conflict(b.start, b.end, id);
                }
                return mode === 'return' && +v.odo < b.outKm
    ...
    data.compliance
        .filter((c) => c.applies === 'Applicable' && c.due)
        .forEach((c) =>
            event(
                c.id,
                `${c.name} due · reminder`,
...
function ActionWizard({
    ...
    const [values, setValues] = useState(a.values),
        [step, setStep] = useState(a.initialStep ?? 0),
    ...
    useEffect(() => {
        ...
        const field =
            document.getElementById(`op-${focusRequest.key}`) ??
            document.getElementById(`op-${focusRequest.key}-date`);
        field?.focus();
    }, [focusRequest, step]);
    const visibleFields = (fields: Field[]) =>
        fields
            ...
            .map((f) => ({
                ...f,
                required: f.requiredWhen?.(values) ?? f.required,
            }));
    const fields = visibleFields(a.sections.flatMap((s) => s.fields));
    ...
    const validate = (all: boolean) => {
        const scoped = all
            ? fields
            : visibleFields(a.sections[step]?.fields ?? []);
        ...
        if (bad) {
            setStep(
                a.sections.findIndex((s) =>
                    s.fields.some((f) => f.key === bad.key),
                ),
            );
            setError(`Review ${bad.label.toLowerCase()}.`);
            setFocusRequest({ key: bad.key });
            return false;
        }
        const e =
            a.validateStep?.(values, step) ||
            (all || step === a.sections.length - 1 ? a.validate?.(values) : '');
        if (e) {
            setError(e);
            setStep(a.validationSection?.(values) ?? 0);
            const focus = a.validationFocus?.(values);
            if (focus) setFocusRequest({ key: focus });
            return false;
        }
        setError('');
        return true;
                    ...
                    <div className="flow-stack">
                        ...
                        {step < a.sections.length ? (
                            <>
                                ...
                                <div
                                    className={`op-fields ${a.fieldLayout === 'paired' ? 'op-fields-paired' : ''}`}
                                >
                                    {visibleFields(a.sections[step].fields)
                                        .length === 0 && (
                                        <Notice title="No additional evidence required">
                                            This requirement is marked not
                                            applicable. Continue to review its
                                            recorded basis; earlier evidence is
                                            retained.
                                        </Notice>
                                    )}
                                    {visibleFields(a.sections[step].fields).map(
                                        (field) => (
                                            <OperationField
                                                key={field.key}
                                                field={field}
                ...
                <>
                    ...
                    {!f.required && value && (
                        <Button
                            variant="ghost"
                            size="sm"
                            aria-label={`Clear ${f.label.toLowerCase()}`}
                            onClick={() => onChange('')}
                        >
                            Clear date
                        </Button>
