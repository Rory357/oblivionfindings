...
export function RecordStudio({
    ...
    const source = () =>
        w.source.startsWith('CR-') && onAlert
            ? onAlert(w.source)
                ...
                <section className="studio-card">
                    ...
                    {tab === 'overview' ? (
                        <>
                            ...
                            {!!w.sourceChecks?.length && (
                                <div className="handoff-checks">
                                    <h3 className="text-section-title">
                                        Linked check evidence
                                    </h3>
                                    {w.sourceChecks.map((r) => (
                                        <details
                                            className="work-fold"
                                            key={r.id}
                                            open
                        ...
                        <>
                            ...
                            />
                            {(w.sourceChecks || []).map((r) => (
                                <div className="work-fold" key={r.id}>
                                    <h3>Source evidence · {r.id}</h3>
                                    {r.attachments?.map((f) => (
                                        <a
                                            className="inline-evidence"
                                            key={f.id}
                        ...
                        <>
                            ...
                            <div className="studio-inline mt-4">
                                {['WO-0188', 'WO-0268'].includes(w.id) && (
                                    <Button
                                        variant="outline"
                                        disabled={!m.canViewFinance}
                                        onClick={() =>
                                            m.setFinanceRecord(
                                                w.id === 'WO-0188'
                                                    ? 'BILL-DEMO-188'
...
export function BookingsStudio({ model: m }: { model: VehicleModel }) {
    return (
        <section className="studio-card">
            ...
            {m.bookingUseProblem && (
                <Notice title="Vehicle use needs review">
                    {m.bookingUseProblem} Requests can be recorded; confirmation
                    and checkout remain blocked.
                </Notice>
            )}
            {m.data.bookings.map((b) => {
                const active = !['Returned', 'Cancelled', 'Declined'].includes(
                        b.status,
                    ),
                    mode =
