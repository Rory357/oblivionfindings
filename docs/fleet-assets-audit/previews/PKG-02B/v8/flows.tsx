...
import { useChecklists, type CheckQuestion } from './checklist-library';
import { EvidenceField, readFiles, type EvidenceFile } from './evidence-field';
...
export function CheckFlow({
    ...
    const initial =
        templates.find((t) => t.id === initialTemplate) || templates[0];
    const [step, setStep] = useState(0),
        [template, setTemplate] = useState(initial.id),
        [observed, setObserved] = useState('2026-09-22T09:20'),
        [answers, setAnswers] = useState(initial.questions.map(() => '')),
    ...
    const current = templates.find((t) => t.id === template) || initial;
    const exampleQuestions = current.questions.map((q) => q.label);
    const missing =
        current.questions.some((q, i) => q.required && !answers[i]?.trim()) ||
        (current.evidence && !readFiles(attachments).length);
    const dirty =
        template !== initialTemplate ||
        observed !== '2026-09-22T09:20' ||
        answers.some(Boolean) ||
        Boolean(notes.trim()) ||
        readFiles(attachments).length > 0;
    const close = () => {
            ...
            const run: Run = {
                ...
                outcome:
                    ...
                    current.questions.some(
                        (q, i) =>
                            q.kind === 'condition' &&
                            (!answers[i] || answers[i] === 'Unable to assess'),
                    )
                        ? 'Needs assessment'
            ...
            <WizardShell
                ...
                pct={Math.round(
                    ([
                        ...
                        (2 +
                            current.questions.filter((q) => q.required).length +
                            Number(current.evidence))) *
                        100,
                )}
                onClose={close}
                footerStart={
                                ...
                                {exampleQuestions.map((q, i) => (
                                    <div className="field" key={q}>
                                        <label htmlFor={`answer-${i}`}>
                                            {q}{' '}
                                            {current.questions[i].required && (
                                                <span className="text-status-critical">
                                                    *
                                                </span>
                                            )}
                                        </label>
                                        {current.questions[i].kind ===
                                        'condition' ? (
                                            <select
                                                id={`answer-${i}`}
                                                value={answers[i] || ''}
                        ...
                        {step === 2 && (
                            <>
                                ...
                                >
                                    <ReviewRow
                                        label="Template"
                                        value={`${templates.find((t) => t.id === template)!.name} · ${current.version}`}
                                    />
                                    <ReviewRow
                                        label="Observed"
...
export function ReportFlow({
    ...
    workOptions: { id: string; name: string; detail: string }[];
    nextWorkId: string;
    run?: Run;
    onClose: () => void;
    onDone: (
    ...
    const submit = () => {
        ...
        setTimeout(() => {
            ...
            setSaved(!reportOnly && choice === 'link' ? link : nextWorkId);
        }, 700);
    };
    return (
        <>
                        ...
                        {step === 0 && (
                            <>
                                <CatalogPicker
                                    ...
                                    allowCreate={!reportOnly}
                                    catalogKey="vehicle-problem-types"
                                    value={title}
                                    onChange={setTitle}
                                    options={[
                                        'Engine warning',
...
export function UploadFlow({
                    ...
                    x.status === 'saved'
                        ...
                        : fail && attempt === 0 && n === i.length - 1
                          ...
                          : {
                                ...x,
                                status: 'saved',
                                id: `ATT-DEMO-${crypto.randomUUID().slice(0, 8)}`,
                            },
                ),
            );
            setAttempt((a) => a + 1);
    ...
    const finish = () => {
        const saved = items.filter((x) => x.status === 'saved');
        if (saved.length)
            onSaved?.(
                saved.map((x) => ({
                    id: x.id!,
    ...
    return (
        <>
            <Modal
                ...
                onClose={() => (pending ? setDiscard(true) : finish())}
                footer={
                    <>
                        <Button
                            variant="outline"
                            disabled={busy}
                            onClick={() =>
                                pending ? setDiscard(true) : finish()
                            }
                        >
                            Close
                        </Button>
