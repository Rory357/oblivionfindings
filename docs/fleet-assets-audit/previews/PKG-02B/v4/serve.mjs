...
const identity={version:'PKG-02B-v4',baseline:'5307692ec59be84f3503c06354419b7da95be805',worktree:'5b0a',model:'gpt-6-astra',effort:'xhigh',synthetic:true};
http.createServer(async(req,res)=>{
res.setHeader('Cache-Control','private, no-store');res.setHeader('X-PKG-Preview','PKG-02B-v4-5b0a');res.setHeader('X-Content-Type-Options','nosniff');
...
const pathname=new URL(req.url,'http://127.0.0.1:4339').pathname;
if(pathname==='/__preview'){res.setHeader('Content-Type','application/json');res.end(JSON.stringify(identity));return;}
const file=['/','/PKG-02B/v4/','/index.html'].includes(pathname)?'index.html':/^\/assets\/[a-zA-Z0-9_.-]+$/.test(pathname)?pathname.slice(1):null;
...
}).listen(4339,'127.0.0.1',()=>process.stdout.write('PKG-02B v4: http://127.0.0.1:4339/PKG-02B/v4/\n'));
