...
const identity={version:'PKG-02B-v12',baseline:'5307692ec59be84f3503c06354419b7da95be805',worktree:'5b0a',model:'gpt-6-astra',effort:'xhigh',synthetic:true};
const reportTiles=new Map();
http.createServer(async(req,res)=>{
res.setHeader('Cache-Control','private, no-store');res.setHeader('X-PKG-Preview','PKG-02B-v12-5b0a');res.setHeader('X-Content-Type-Options','nosniff');
...
const pathname=new URL(req.url,'http://127.0.0.1:4347').pathname;
...
if(tile){
 ...
 try{
  ...
  if(!bytes){const upstream=await fetch(`https://tile.openstreetmap.org/${z}/${x}/${y}.png`,{headers:{'User-Agent':'OblivionCare-LocalDesignPreview/1.0 (synthetic trip report)','Referer':'http://127.0.0.1:4347/'},signal:AbortSignal.timeout(12000)});if(!upstream.ok||!upstream.headers.get('content-type')?.includes('image/png'))throw new Error('Tile unavailable');bytes=Buffer.from(await upstream.arrayBuffer());if(reportTiles.size>=64)reportTiles.delete(reportTiles.keys().next().value);reportTiles.set(pathname,bytes);}
...
const file=['/','/PKG-02B/v12/','/index.html'].includes(pathname)?'index.html':/^\/assets\/[a-zA-Z0-9_.-]+$/.test(pathname)?pathname.slice(1):null;
...
}).listen(4347,'127.0.0.1',()=>process.stdout.write('PKG-02B v12: http://127.0.0.1:4347/PKG-02B/v12/\n'));
