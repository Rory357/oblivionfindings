...
import { documentReminders } from './document-workflow';
                    ...
                    fields: [
                        ...
                        <div className="collection-actions">
                            ...
                            <Button
                                ...
                                disabled={!m.canManage}
                                onClick={() =>
                                    m.uploadDocument(undefined, title)
                                }
                            >
                                Upload
                            </Button>
            ...
            {visible.length ? (
                <RecordCollection
                    ...
                    records={visible.map((f) => {
                        const renewal = documentReminders(m.data, f)[0];
                        ...
                        const open = () =>
                            m.detail(f.name, [
                                ...
                                [
                                    'Renewal reminder',
                                    renewal
                                        ? `${renewal.status} · ${dateLabel(renewal.at)} · ${renewal.owner}`
                                        : 'Not scheduled',
                                ],
                            ]);
                        const source = () =>
                            f.owner?.startsWith('WO-')
                                ? onWork(f.owner)
                                : f.owner?.startsWith('SCH')
                                  ? onNav('compliance', 'schedules')
                                  : f.owner?.startsWith('ODO')
                                    ? onNav('compliance', 'mileage')
                                    : onNav(
                                    ...
                                    {current && renewal && (
                                        <Button
                                            ...
                                            onClick={() =>
                                                onNav(
                                                    'calendar',
                                                    renewal.at.slice(0, 10),
                                                )
                                            }
                                        >
                                            <CalendarClock size={14} />{' '}
                                            {renewal.status} ·{' '}
                                            {dateLabel(renewal.at)}
                                        </Button>
                                    )}
                                    {f.replaces && (
