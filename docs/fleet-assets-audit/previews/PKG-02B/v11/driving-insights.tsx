...
import { confirmedDriver, eventKey, reviewedScore } from './driving-workflows';
import { ReviewWorkspace, SpeedSources } from './driving-workspace';
import type { VehicleModel } from './operations';
import { dateLabel } from './operations';
import { useSessionState } from './session-state';
import { drivingEventCount, journeys, speedOrigin } from './trip-data';
...
function AnalyticsBody({
    ...
    const [period, setPeriod] = useSessionState(
            'driving-period',
            'Last 7 days',
        ),
        [driver, setDriver] = useSessionState(
            'driving-driver',
            'All assignments',
        ),
        [eventType, setEventType] = useSessionState(
            'driving-event',
            'All driving events',
        ),
    ...
    const trips = journeys.filter(
        (t) =>
            ...
            (driver === 'All assignments' ||
                confirmedDriver(t, m.data.driving) === driver ||
                t.driver === driver),
    );
    const personalTrips = trips.filter(
        (t) =>
            confirmedDriver(t, m.data.driving) === driver &&
            t.events.every(
                (e, i) =>
                    !['driving', 'speed'].includes(e.kind) ||
                    ['Confirmed', 'Dismissed'].includes(
                        m.data.driving.reviews[eventKey(t, i)]?.outcome,
                    ),
            ),
    );
    const personalEnough =
        personalTrips.length >= m.data.driving.policy.trips &&
        personalTrips.reduce((n, t) => n + t.distance, 0) >=
            m.data.driving.policy.distance;
    const eligible = trips.filter(
        (t) =>
            tripScore(t) !== null &&
            (driver === 'All assignments' || personalTrips.includes(t)),
    );
    const distance = trips.reduce((n, t) => n + t.distance, 0),
        eligibleDistance = eligible.reduce((n, t) => n + t.distance, 0);
    const score =
        eligibleDistance && (driver === 'All assignments' || personalEnough)
            ? Math.round(
        ...
        <div className="telemetry-studio analytics-studio">
                            ...
                            <div>
                                ...
                                <p>
                                    {driver === 'All assignments'
                                        ...
                                        : `${personalTrips.length} reviewed, confirmed trips. Minimum ${m.data.driving.policy.trips} trips / ${m.data.driving.policy.distance} km; partial or split-driver trips excluded.`}
                                </p>
                                <Badge tone="info">
                                    Illustrative · SCORE-DEMO-v
                                    {m.data.driving.policy.version}
                                </Badge>
                                <button
                    ...
                    <div className="telemetry-two">
                        <section className="studio-card">
                            ...
                            {shownEvents.map((e, i) => (
                                <button
                                    ...
                                    key={e.trip.id + i}
                                    onClick={() => onTrip(e.trip.id, e.point)}
                                >
                        ...
                        <section className="studio-card analytics-quality">
                            ...
                            <Row
                                ...
                                value={`${Object.values(m.data.driving.assignments).filter((s) => s[0]?.point === 0).length} confirmed trip(s)`}
                            />
                            <Row
                                title="Coverage"
                            ...
                            <p className="muted">
                                ...
                                checkout / driver handover, at least{' '}
                                {m.data.driving.policy.trips} eligible trips and{' '}
                                {m.data.driving.policy.distance} km in the
                                period. These minimums and weights need
                            ...
                            <Button
                                ...
                                disabled={!m.canManage}
                                onClick={() =>
                                    m.driving.coach(trips[0] || journeys[1])
                                }
                            >
            ...
            {rules && (
                <Modal
                    ...
                    description={`SCORE-DEMO-v${m.data.driving.policy.version} · illustrative policy`}
                    icon={BarChart3}
                    size="standard"
                ...
                >
                    ...
                    <p className="score-formula">
                        100 − braking×{m.data.driving.policy.braking} −
                        acceleration×{m.data.driving.policy.acceleration} −
                        overspeed episodes×{m.data.driving.policy.speed}−
                        round(idle minutes×{m.data.driving.policy.idle})
                    </p>
                    <p>
                    ...
                    {eligible.map((t) => (
                        <Row
                            ...
                            value={`Reviewed score: ${tripScore(t)} · original ${t.braking} braking / ${t.acceleration} acceleration / ${t.overspeed ? 1 : 0} overspeed`}
                        />
                    ))}
                    ...
                    <p>
                        ...
                        km. Trips below {m.data.driving.policy.coverage}%
                        coverage do not contribute.
                    </p>
                    ...
                    <p>
                        ...
                        auditable correction and recalculation. Use Reviews &
                        coaching to record the decision.
                    </p>
                    <Row
...
export function DrivingInsights(props: Parameters<typeof AnalyticsBody>[0]) {
    const [workspace, setWorkspace] = useSessionState(
        'driving-workspace',
        'Analytics',
    );
    return (
        <div className="telemetry-studio">
            <nav className="workflow-switch" aria-label="Driving workspace">
                {['Analytics', 'Reviews & coaching', 'Speed limits'].map(
                    (x) => (
                        <Button
                            key={x}
                            variant={workspace === x ? 'default' : 'outline'}
                            aria-pressed={workspace === x}
                            onClick={() => setWorkspace(x)}
                        >
                            {x}
                        </Button>
                    ),
            ...
            {workspace === 'Analytics' ? (
                <AnalyticsBody {...props} />
            ) : workspace === 'Reviews & coaching' ? (
                <ReviewWorkspace model={props.model} onTrip={props.onTrip} />
            ) : (
                <SpeedSources model={props.model} onNav={props.onNav} />
            )}
        </div>
    );
}
