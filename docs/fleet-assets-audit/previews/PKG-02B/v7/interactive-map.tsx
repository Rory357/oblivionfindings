// Preview-local adapter of the canonical map. Adds coordinate context actions and resize handling.
...
export default function LeafletMap({
    ...
    function renderMarkers() {
        ...
        if (clustering && markers.length > 20) {
            ...
            markers.forEach((m) => {
                ...
                if (m.stats) {
                    const icons: Record<string, string> = {
                        Ignition: '<path d="m13 2-9 12h7l-1 8 10-12h-7z"/>',
                        Speed: '<path d="M4 18a9 9 0 1 1 16 0M12 13l4-4"/>',
                        Motion: '<path d="m21 3-6 18-4-8-8-4z"/>',
                        Voltage:
                            '<rect x="2" y="7" width="17" height="10" rx="2"/><path d="M22 10v4M6 12h9"/>',
                    };
                    leafletMarker.bindTooltip(
                        '<div class="vehicle-hover-card"><strong>' +
                            escapeMapHtml(m.title || 'Vehicle') +
                            '</strong><small>' +
                            escapeMapHtml(m.popup || '') +
                            '</small><div>' +
                            m.stats
                                .map(
                                    ([k, v]) =>
                                        '<section><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7">' +
                                        (icons[k] ||
                                ...
                                .join('') +
                            '</div><footer>Right-click vehicle for quick actions</footer></div>',
                        {
                            direction: 'auto',
                            offset: [20, 0],
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (!mapRef.current) {
                ...
                renderPolyline();
                if (autoFit && polyline?.length)
                    mapRef.current.fitBounds(
                        L.latLngBounds(polyline.map((p) => [p.lat, p.lng])),
    ...
    useEffect(() => {
        if (focus && mapRef.current && initRef.current)
            mapRef.current.panTo([focus.lat, focus.lng], { animate: false });
    }, [focus?.lat, focus?.lng, focus?.nonce]);
