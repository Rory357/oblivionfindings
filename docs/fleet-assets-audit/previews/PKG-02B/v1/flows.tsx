...
export const originalRun: Run = {
  ...
  notes:
    "A condition concern was recorded for assessment. The original evidence stays with this check.",
  outcome: "Failed",
};
const templates = [
  {
...
const exampleQuestions = ["Example observation A", "Example observation B"];
const locked = (
...
export function CheckFlow({
  ...
  const close = () => (saved ? onClose() : setDiscard(true));
  const advance = () => {
  ...
  return (
    <>
      <WizardShell
        ...
        stepIndex={step}
        onStepClick={(i) => i < step && !saving && setStep(i)}
        pct={step === 0 ? 20 : step === 1 ? 55 : 100}
        onClose={close}
        footerStart={
        ...
        footerEnd={
          <>
            ...
            <Button disabled={saving} onClick={step < 2 ? advance : submit}>
              {saving
                ? "Saving demo…"
                : step < 2
                  ? "Continue"
            ...
            <WizardSuccessPane
              ...
              blurb={
                <>
                  ...
                  <br />
                  Original answers and template {saved.version} are retained.
                  This does not release the vehicle.
                </>
              }
              actions={
                <>
      ...
      >
        <WizardStepPane>
          <div className="flow-stack">
            {locked}
            <Notice title="Synthetic checklist" tone="info">
              These two example observations demonstrate the interaction. They
              are not an approved safety procedure.
            </Notice>
            {error && (
            ...
            {step === 0 && (
              <>
                ...
                />
                {unconfigured && (
                  <Notice title="Approved rules unavailable" tone="warning">
                    Answers can be recorded for assessment. They cannot produce
                    a Passed or Ready result.
                  </Notice>
                )}
              </>
            )}
            {step === 1 && (
              <>
                <div className="section-intro">
                  ...
                  <Badge tone="info">
                    {template === "condition" ? "DEMO-3" : "DEMO-2"}
                  </Badge>
                </div>
                {exampleQuestions.map((q, i) => (
                  <div className="field" key={q}>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setAnswers((a) =>
                          a.map((v, n) => (i === n ? e.target.value : v)),
                        )
                      }
                    >
                ...
                <div className="field">
                  <label htmlFor="check-note">Observation notes</label>
                  <Textarea
                    id="check-note"
                    value={notes}
                ...
                <p className="muted">
                  Evidence from a failed check can be attached to the linked
                  Maintenance record. A new attachment does not change the
                  submitted answers.
                </p>
              </>
            )}
            {step === 2 && (
              <>
                ...
                >
                  ...
                  />
                  <ReviewRow label="Record" value="Kōwhai van · VH-014" />
                </ReviewCard>
                <ReviewCard
                  icon={MessageSquare}
                  title="Answers"
                ...
                >
                  {exampleQuestions.map((q, i) => (
                    <ReviewRow key={q} label={q} value={answers[i]} />
                  ))}
                  <ReviewRow label="Notes" value={notes} />
                </ReviewCard>
                <Notice title="Submission records the check">
                  Assessment and any restriction or release remain separate. The
                  submission never clears a restriction.
                </Notice>
              </>
            )}
          </div>
      ...
      {discard && (
        <CloseGuard onKeep={() => setDiscard(false)} onDiscard={onClose} />
      )}
    </>
  );
}
      ...
      <div className="split">
        <div className="flow-stack">
          <div className="section-intro">
            ...
            <span className="muted">Submission is not a safety release</span>
          </div>
          <ReviewCard icon={ClipboardCheck} title="Answers as submitted">
            {exampleQuestions.map((q, i) => (
              <ReviewRow key={q} label={q} value={run.answers[i]} />
            ))}
            <p className="mt-3">{run.notes || "No additional notes."}</p>
          </ReviewCard>
          <Notice title="Original evidence is preserved">
            Later templates or repair notes do not change these answers. A
            correction must be a separate attributable record.
          </Notice>
        </div>
        <div className="fact-list">
          <div>
            <small>Template version</small>
            <strong>{run.version} · fictional example</strong>
          </div>
          <div>
            <small>Observed</small>
            <strong>{localDateTimeLabel(run.observed)}</strong>
          </div>
          <div>
            <small>Submitted</small>
            <strong>{run.submitted} · Pacific/Auckland</strong>
          </div>
          <div>
            <small>Recorded by</small>
            <strong>Alex Morgan · demo staff</strong>
          </div>
          <div>
            <small>Vehicle</small>
            <strong>Kōwhai van · VH-014</strong>
          </div>
          <div>
            <small>Evidence reference</small>
            <strong>EV-DEMO-0182 · condition note</strong>
          </div>
        </div>
      </div>
    </Modal>
...
export function ReportFlow({
  ...
  const [step, setStep] = useState(0),
    ...
    [range, setRange] = useState<[string | null, string | null]>([null, null]),
  ...
  const next = () => {
    ...
    if (step === 1 && !reportOnly && choice === "link" && !link) {
      setError("Choose an existing work record or choose Create a new report.");
      return;
    }
    setError("");
    setStep(step + 1);
  ...
  return (
    <>
      <WizardShell
        ...
        onClose={() => (saved ? onClose() : setDiscard(true))}
        footerStart={
          <Button
            variant="ghost"
            disabled={saving}
        ...
        footerEnd={
          <>
            ...
            <Button disabled={saving} onClick={step < 2 ? next : submit}>
              {saving
                ? "Confirming demo…"
                : step < 2
                  ? "Continue"
            ...
            <WizardSuccessPane
              ...
              blurb={
                <>
                  ...
                  . The site's Coordinator owns the next assessment. No safety
                  release or financial approval has occurred.
                </>
              }
              actions={
          ...
          <div className="flow-stack">
            ...
            <div className="source-strip">
              ...
              <span>
                {run ? (
                  <>
                    Original check <strong>{run.id}</strong> · {run.version} ·
                    Answers unchanged
                  </>
                ) : (
            ...
            {error && (
              <Notice title="Report not confirmed" tone="critical">
                {error}
              </Notice>
            )}
            {step === 0 && (
              <>
                <div className="field">
                  <label htmlFor="report-title">What needs attention?</label>
                  <Input
                    id="report-title"
                    value={title}
                ...
                <div className="field">
                  <label>Estimated maintenance window · optional</label>
                  ...
                  <small className="muted">
                    An estimate is advisory. It does not reserve the vehicle or
                    book a provider.
                  </small>
                </div>
              </>
              ...
              <>
                {reportOnly ? (
                  <Notice title="Coordinator review required">
                    You can report the concern. Choosing or merging existing
                    work requires Maintenance manager access.
                  </Notice>
                ) : (
                  <>
                    <Notice title="Related work found" tone="warning">
                      Review the existing condition concern before creating
                      another job.
                    </Notice>
                    <label className="choice">
                      ...
                      />
                      <span>
                        <strong>Link this report to existing work</strong>
                        <small>
                          Keep the new report and original check as separate
                          evidence.
                        </small>
                      </span>
                    </label>
                    ...
                    <label className="choice">
                      ...
                      <span>
                        ...
                        <small>
                          Use when this is a separate concern. Coordinator
              ...
              <>
                ...
                >
                  ...
                  <ReviewRow
                    ...
                    value={
                      showDates ? `${range[0]} to ${range[1]}` : "Not known yet"
                    }
                  />
                  <ReviewRow
                ...
                <Notice
                  ...
                  tone="warning"
                >
                  Reporting or linking a concern cannot release the vehicle. The
                  example routing must be replaced by approved operational
                  assignments.
                </Notice>
              </>
            )}
...
export function UploadFlow({
  ...
  const upload = () => {
    setItems((i) =>
      i.map((x) => (x.status === "saved" ? x : { ...x, status: "uploading" })),
    );
    setTimeout(() => {
      setItems((i) =>
        i.map((x, n) =>
          x.status === "saved"
            ...
            : fail && attempt === 0 && n === i.length - 1
              ? { ...x, status: "failed" }
              : { ...x, status: "saved", id: `ATT-DEMO-${410 + n}` },
        ),
      );
      setAttempt((a) => a + 1);
    }, 750);
    ...
    <>
        ...
        footer={
          <>
            <Button
              variant="outline"
              disabled={busy}
              onClick={() => (pending ? setDiscard(true) : onClose())}
            >
              Close
            </Button>
      ...
      >
        <Notice title="Local demonstration">
          Files stay in this page's memory. Nothing is transmitted. Closing with
          unsaved files or reloading loses those selections.
        </Notice>
        ...
        {error && (
          <Notice title="File selection needs attention" tone="critical">
            {error}
          </Notice>
        )}
        <div className="flow-stack">
          {items.map((item, n) => (
            <div
              key={n}
              className={item.status === "saved" ? "saved-evidence" : undefined}
              role="group"
            ...
            >
              ...
              >
                ...
                {item.status === "saved" && (
                  <small className="muted">
                    Original selection retained as demonstration evidence. Saved
                    evidence cannot be removed here.
                  </small>
                )}
              </StagedFileCard>
            </div>
        ...
        {items.some((x) => x.status === "failed") && (
          <Notice title="Some evidence needs retry" tone="warning">
            Confirmed files are kept. Retry only the files that failed.
          </Notice>
        )}
      </Modal>
      {discard && (
