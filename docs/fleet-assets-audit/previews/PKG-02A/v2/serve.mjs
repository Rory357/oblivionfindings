...
const identity = { version: 'PKG-02A-v2', baseline: '2302ca33a95616e442a78e957ddce82d8db98669', worktree: '2b9f', model: 'gpt-6-astra', effort: 'xhigh', synthetic: true };
http.createServer(async (req, res) => {
  res.setHeader('Cache-Control', 'private, no-store, max-age=0');
  res.setHeader('X-PKG-Preview', 'PKG-02A-v2-2b9f');
  ...
  const pathname = new URL(req.url, 'http://127.0.0.1:4333').pathname;
  if (pathname === '/__preview') { res.setHeader('Content-Type', 'application/json'); res.end(JSON.stringify(identity));return; }
  const file = ['/', '/PKG-02A/v2/', '/index.html'].includes(pathname) ? 'index.html' : /^\/assets\/[a-zA-Z0-9_.-]+$/.test(pathname) ? pathname.slice(1) : null;
...
}).listen(4333, '127.0.0.1', () => process.stdout.write('PKG-02A v2 frozen preview: http://127.0.0.1:4333/PKG-02A/v2/\n'));
