...
const fullCan = { fleet: { viewAny: true }, assets: { viewAny: true, alertsView: true, telemetryView: true, trackersManage: true, geofencesManage: true }, hr: { assets: { view: true }, driver: { view: true } }, clients: { viewAny: true }, controlRoom: { viewAny: true }, securityDevices: { devicesView: true } };
export default defineConfig({
    ...
    plugins: [react(), tailwindcss(), { name: 'pkg-nav-read-only-fixture', configureServer(server) {
        server.middlewares.use(async (req, res, next) => {
            ...
            const can = role === 'assigned' ? { assets: { viewAssigned: true } } : role === 'staff' ? {} : role === 'assets' ? { assets: { viewAny: true } } : role === 'no-telemetry' ? { fleet: { viewAny: true } } : fullCan;
