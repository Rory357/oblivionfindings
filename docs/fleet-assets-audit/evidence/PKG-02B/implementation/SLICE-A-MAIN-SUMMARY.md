...
- ~~Maintenance › Checklists (`ChecklistController`) can still change a shared checklist with `fleet.maintenance.manage`.~~ Fixed on the daily-check branch: creating a checklist there (it's offered to every vehicle) now also needs `fleet.settings.manage`, and the page hides Create template otherwise.
- ~~Maintenance › Open work still gives work from a check later released as "no issue found" the check's warning tone.~~ Fixed on the daily-check branch (`VehicleWorkspacePresenter::workSources`).
...
3. **Daily checks: recorded, not gated** (decided by Stephan, 2026-09-24; follow-up branch `claude/reverent-burnell-a777f7`). Each check on the Daily checks page is now its own submitted record against the exact daily checklist version (submitted time, version, question snapshot, request key and fingerprint). Checking again adds a record; nothing is overwritten, and corrections are amendments. Daily checks appear under Recent checks as "No issue recorded" or "Issue recorded" (never Passed or Failed, which need an approved rule), and **never block bookings or a maintenance release**, whatever they record. An issue reaches Maintenance through Create or link maintenance, and the vehicle's header asks for review (without blocking) while its latest daily check records an issue. The daily checklist is recorded only from that page: the vehicle checklist library, check requirement, profile check and Maintenance checklist runner refuse it. Also fixed: "today" is the Auckland day (it reset at noon), a daily check saved after a retest no longer stops the release, and Maintenance's inspection page shows answers to checklist items without an id (answered by position, as the checklist runner records them).
