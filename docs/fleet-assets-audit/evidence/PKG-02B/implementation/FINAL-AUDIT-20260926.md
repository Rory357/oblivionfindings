...
### Main review and FINAL-01 correction, 27 September

Main reviewed exact candidate `81ed2ea57e66b291f09437a9ea80a88dd4448c6a` against published base `4ea64c547ed85a5b7504e59599db351f6eba7deb`. Its independent 176 frontend, 43 feature and 4 map tests passed (223 tests; overlapping earlier runs). Main traced privacy, command receipts/Undo, canonical Maintenance, Control Room and local maps, and sampled the compiled browser UI. Main requested one P2 correction, FINAL-01: two 40-second trips were exported as two one-minute cells while the cached total said one minute. The actual SUM formula recalculated to two minutes. A copy of Main's exact review is preserved with this packet.

The same Designer corrected FINAL-01 without changing PDF's existing whole-minute presentation: shared view data now carries original seconds, Excel uses fractional minutes throughout, and the SUM cache uses the aggregate seconds on the same basis. Two-decimal formatting changes display only. A workbook note explains why totals use unrounded values. No operating policy or new UI workflow was introduced.

The full affected trip/export test file passed **14 tests / 401 assertions** in 256.69s. New persisted-record/controller regressions cover `[40,40]`, `[20,20]` and `[29,31,61]` seconds, generated numeric cells/formulas/caches, display format and the unchanged PDF boundary. Independent synthetic XLSX probes were recalculated using `@oai/artifact-tool`; their sums match 80, 40 and 121 seconds within floating-point tolerance. The changed cell range was rendered and visually inspected. Native desktop Excel recalculation is not claimed. Pint and scoped diff whitespace checks passed. Earlier raw console evidence retains its original formatting; the all-evidence whitespace check is not represented as green.

A fresh native Chrome download, `pw-track-veh-trips-2026-09-23-to-2026-09-24 (3).xlsx` (257,523 bytes), contains four sheets, two embedded street maps, the duration format and the new note. The browser download-wait helper timed out after clicking; filesystem timestamp and independent workbook parsing confirm the actual download succeeded. No repeated click was used. This PHP-only correction leaves the previously verified production frontend bundle unchanged. Main's renewed disposition and publication verification remain pending until they actually occur.
