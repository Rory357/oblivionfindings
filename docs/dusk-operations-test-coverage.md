...
| operations shifts create redirects to the shifts index | `/operations/shifts/create` | Waits/sees `Shift`, path is `/operations/shifts` | Create page retired: the route serves JSON to the inline `CreateShiftDialog` and redirects HTML to `/operations/shifts?create=1`. Redirect/JSON behaviour also covered by `tests/Feature/ShiftControllerTest.php` + `ShiftLegacyRedirectTest.php`. |
