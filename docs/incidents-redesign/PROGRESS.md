...
### ◑ Step 7 — Corrective-actions migration (Option B, §6.6) — 7a DONE
**7a (DONE) — raise-from-incident:**
...
- 2026-06-17 — Step 7c: retire-inline-remediation migration (data-move legacy → H&S, then drop 5 columns) + model update. Verified with seeded data (migrate/rollback/re-migrate) + 5 tests. **⚠️ DESTRUCTIVE — flagged for review. BUILD COMPLETE (Steps 1–7).**
- 2026-06-17 — **Pre-merge verification sweep:** full regression — **IncidentControllerTest 132 + ControlRoom (incident+alert) 59 = 191 tests green** (750 assertions). Sweep caught + fixed one stale test (`test_index_filters_are_echoed_back` still asserted the removed status/reviewed filters → updated to tab/source). Confirmed no test seeds a ClientIncident with a dropped column. **Build verified; awaiting human review → merge → deploy → live verify.**
