...
| Shift create (data)     | Shared `CreateShiftDialog` via `use-create-shift-launcher` | `operations.shifts.create`                           | `@/routes/operations/shifts#create`        | `/operations/shifts/create` (JSON; HTML → `/operations/shifts?create=1`) |
| Shift store             | Shared `CreateShiftDialog` popup                      | `operations.shifts.store`                                 | `@/routes/operations/shifts#store`         | `/operations/shifts`                      |
...
| Shift edit data         | Shared `CreateShiftDialog` popup                      | `operations.shifts.editable`                              | `@/routes/operations/shifts#editable`      | `/operations/shifts/{shift}/editable`     |
| Shift update            | Shared `CreateShiftDialog` popup                      | `operations.shifts.update`                                | `@/routes/operations/shifts#update`        | `/operations/shifts/{shift}`              |
| Shift series store      | Shared `CreateShiftDialog` popup                      | `operations.shifts.series.store`                          | `@/routes/operations/shifts/series#store`  | `/operations/shifts/series`               |
...
- The standalone shift **create page was retired** (no more `create.tsx`). `operations.shifts.create` now returns the create form's data as JSON, consumed by the shared `CreateShiftDialog` through `resources/js/pages/operations/shifts/components/use-create-shift-launcher.tsx` (which also carries the coverage reservation token so gap closure still works). Non-JSON hits redirect to `/operations/shifts?create=1`, which opens the inline dialog. The `store` / `series.store` endpoints are unchanged — the dialog posts to them.
